...
mod tests {
    ...
    #[test]
    fn structural_validation_report_row_size_is_32() {
        assert_eq!(std::mem::size_of::<StructuralValidationReportGpu>(), 32);
    }

    #[test]
    fn structural_validation_report_row_alignment_is_4() {
        assert_eq!(std::mem::align_of::<StructuralValidationReportGpu>(), 4);
    }

    #[test]
    fn structural_validation_wgsl_source_contains_no_forbidden_semantic_terms() {
        scan_for_forbidden_validation_tokens(
            WGSL_STRUCTURAL_VALIDATION,
            "structural_validation.wgsl",
        );
    }

    #[test]
    fn structural_validation_gpu_reports_valid_packet() {
    ...
    #[test]
    fn structural_validation_gpu_detects_self_link_when_given_bad_rows() {
        let Some(ctx) = GpuContext::new_blocking().ok() else {
            eprintln!("skipping: no GPU");
            return;
        };
        ...
        let report =
            validate_structural_rows_on_gpu(&ctx.device, &ctx.queue, &rows).expect("validate");
        assert_eq!(report.self_link_count, 1);
    }

    #[test]
    fn structural_validation_gpu_handles_zero_link_packet() {
        let Some(ctx) = GpuContext::new_blocking().ok() else {
            eprintln!("skipping: no GPU");
            return;
        };
        ...
        let report =
            validate_structural_upload_on_gpu(&ctx.device, &ctx.queue, &buffers, &upload_report)
                .expect("validate");
        assert_eq!(report.location_count, 1);
        assert_eq!(report.link_count, 0);
        assert_eq!(report.invalid_link_endpoint_count, 0);
        assert_eq!(report.self_link_count, 0);
        assert_eq!(report.location_count, upload_report.location_count);
    }
}
