...
fn persistent_bench_hotspot_100k_summary_only() {
    ...
    let report =
        compare_persistent_bench_rich_with_harness(&harness, &scenario, DEFAULT_TICKS, false)
            .unwrap();
    assert_report_ok(&report);
    write_persistent_bench_reports(&report).expect("write persistent bench reports");
}
...
fn persistent_bench_records_mode_replays_small() {
    ...
    let harness = PersistentBenchHarness::new().unwrap();
    let pivot = harness
        .run_pivot_gpu_resident(&scenario, ticks, true, false)
        .unwrap();
    let replayed_gpu = replay_bench_records(
        &scenario.pools,
        &scenario.queues,
        &pivot.records,
        ticks,
    ...
    .unwrap();
    assert_eq!(replayed_gpu.0, pivot.final_pools);
    assert_eq!(replayed_gpu.1, pivot.final_queues);
}
...
fn persistent_bench_report_bundle() {
    ...
    write_persistent_bench_reports_bundle(&reports).expect("write persistent bench reports bundle");
}
