//! EML-EXP-PRIMITIVE-0 cost gate: driver-originated compiled resource effects
//! for the EXP primitive candidate versus its own pinned gadget encoding,
//! judged through the landed `ExactPrimitiveAdmissionDoor::verify_cost` key —
//! unweakened, no new cost dimension.
//!
//! Baseline: the ~20-node ordinary `ADD`/`MUL` Horner tree a designer can
//! author today (degree-5 Taylor `exp` over the steering working range) — 21
//! nodes, `LegacyFixed32`, executing on the canonical interpreter (the
//! sanctioned default for authored trees). Candidate: the guarded full-domain
//! `EXP` post program lowered by the SSA JIT to a straight-line block at
//! `CompactStack4`. Measurement source: `VK_KHR_pipeline_executable_properties`
//! on the certified adapter (the 5.7 census door).
...
/// The pinned gadget baseline: degree-5 Horner `exp` approximation as an
/// ordinary authored tree — `((((C5·x + C4)·x + C3)·x + C2)·x + C1)·x + C0`
/// with Taylor coefficients. 21 nodes, peak stack 2 → `LegacyFixed32`.
...
fn exp_candidate_post_program() -> Vec<EmlNodeGpu> {
    vec![
        target(),
        node(
...
unsafe fn measure_pipeline(
    ...
    for (executable_index, _) in executables.iter().enumerate() {
        ...
        for statistic in statistics {
            ...
            eprintln!("EML_EXP_COST_STAT {label} name={name:?} value={value}");
...
/// Local phase-boundary measurement on the certified adapter — the cost-gate
/// numbers pinned in the results doc come from this run.
#[test]
#[ignore = "local phase-boundary measurement: exact-primitive cost gate on the certified adapter"]
fn eml_exp_primitive_0_cost_gate_beats_the_pinned_gadget_baseline() {
    let baseline = registration(gadget_baseline_post_program());
    assert_eq!(
        baseline.resource_class(),
        EmlResourceClass::LegacyFixed32,
        "the 21-node gadget baseline exceeds CompactStack4 and rides Legacy32"
    );
    let candidate = registration(exp_candidate_post_program());
    assert_eq!(
        candidate.resource_class(),
        EmlResourceClass::CompactStack4,
        "the guarded EXP primitive call is a compact program"
    );
    // Baseline home: the canonical interpreter at its shipped Legacy32 shape
    // (the canonical shader's default EML_STACK_MAX = 32).
    let interpreter_source = include_str!("../../simthing-kernel/src/shaders/field_sweep.wgsl");
    // Candidate home: the SSA-JIT straight-line lowering.
    ...
    let app_name = CString::new("simthing-eml-exp-cost-evidence").unwrap();
    ...
    let jit = unsafe {
        measure_pipeline(
            ...
            pipeline_layout,
            &candidate_source,
            "jit-exp-primitive-compact4",
        )
    };
    eprintln!(
        "EML_EXP_COST baseline registers={} binary={} local={} | candidate registers={} binary={} local={}",
    ...
    ExactPrimitiveAdmissionDoor::verify_cost(evidence)
        .expect("EXP beats its own gadget encoding under the unweakened cost key");
