...
#[test]
fn field_policy_obs1_descriptor_admits_default_off_overlay_score() {
    let desc = obs0();
    assert_eq!(desc.id, FIELD_POLICY_OBS0_DESCRIPTOR_ID);
    assert!(desc.default_off);
    assert!(!desc.production_wiring);
    assert!(is_field_policy_obs0_overlay_score_descriptor(&desc));
    validate_kernel_descriptor_admission(&desc).expect("obs0 admits");
    println!(
        "field_policy_obs1_descriptor: id={FIELD_POLICY_OBS0_DESCRIPTOR_ID} label={FIELD_POLICY_OBS0_LABEL} default_off=true production_wiring=false"
    );
}

#[test]
fn field_policy_obs1_output_authority_is_correct() {
    let desc = obs0();
    let mag2 = desc
        .writes
        .iter()
    ...
    println!(
        "field_policy_obs1_authority: mag2_bits=Exact mag_bits=Exact score_bits=Approximate flags=Approximate"
    );
}

#[test]
fn field_policy_obs1_rejects_exact_score_for_f32_score_arithmetic() {
    let mut bad = field_policy_obs0_overlay_score_kernel_descriptor();
...
#[test]
fn field_policy_obs1_exact_mag_requires_q16_and_f_artifact() {
    let mut wrong_q = field_policy_obs0_overlay_score_kernel_descriptor();
    ...
    let mut no_mag2_contract = field_policy_obs0_overlay_score_kernel_descriptor();
    no_mag2_contract.mag2_source_contract = None;
    assert_admission_err(&no_mag2_contract, "Mag2SourceContract");

    let mut no_f = field_policy_obs0_overlay_score_kernel_descriptor();
    no_f.exact_sqrt_artifact = None;
    assert_admission_err(&no_f, "artifact-backed Candidate F");

    let mut bad_hash = field_policy_obs0_overlay_score_kernel_descriptor();
    ...
    let mut native_sqrt = field_policy_obs0_overlay_score_kernel_descriptor();
...
#[test]
fn field_policy_obs1_rejects_production_wiring_and_default_on() {
    let mut prod = field_policy_obs0_overlay_score_kernel_descriptor();
    prod.production_wiring = true;
    assert_admission_err(&prod, "production_wiring");

    let mut default_on = field_policy_obs0_overlay_score_kernel_descriptor();
    default_on.default_off = false;
    assert_admission_err(&default_on, "default_off");
}

#[test]
fn field_policy_obs1_builder_matches_landed_registry() {
    let built = field_policy_obs0_overlay_score_kernel_descriptor();
    let landed = obs0();
...
#[test]
fn field_policy_obs2_descriptor_admits_default_off_multilayer_score() {
    let desc = obs2();
    assert_eq!(desc.id, FIELD_POLICY_OBS2_DESCRIPTOR_ID);
    assert!(desc.default_off);
    assert!(!desc.production_wiring);
    ...
    println!(
        "field_policy_obs2_descriptor: id={FIELD_POLICY_OBS2_DESCRIPTOR_ID} label={FIELD_POLICY_OBS2_LABEL} layers={FIELD_POLICY_OBS2_LAYER_COUNT}"
    );
}

#[test]
fn field_policy_obs2_rejects_exact_score_for_f32_score_arithmetic() {
    let mut bad = field_policy_obs2_multilayer_overlay_score_kernel_descriptor();
    for out in &mut bad.writes {
        if out.name == "score_bits" {
            out.authority = OutputAuthority::ExactAuthoritative;
        }
...
#[test]
fn field_policy_obs3_descriptor_admits_fixed_point_score() {
    let desc = obs3();
    assert_eq!(desc.id, FIELD_POLICY_OBS3_DESCRIPTOR_ID);
    assert!(desc.default_off);
    assert!(!desc.production_wiring);
    assert!(is_field_policy_obs3_multilayer_fixed_score_descriptor(
        &desc
    ));
    ...
    println!(
        "field_policy_obs3_descriptor: id={FIELD_POLICY_OBS3_DESCRIPTOR_ID} label={FIELD_POLICY_OBS3_LABEL} score=ExactQ16WeightedSum obs2_f32=ApproximateDiagnostic"
    );
}

#[test]
fn field_policy_obs3_f32_score_exact_still_rejects() {
    let mut bad_obs2 = field_policy_obs2_multilayer_overlay_score_kernel_descriptor();
    ...
    let mut bad_obs3_score_bits = field_policy_obs3_multilayer_fixed_score_kernel_descriptor();
    ...
    let mut native_sqrt = field_policy_obs3_multilayer_fixed_score_kernel_descriptor();
    native_sqrt.native_math = NativeMathClass::ApproximateJitOnly;
    assert_admission_err(&native_sqrt, "approximate native math");

    let mut wrong_mag2 = field_policy_obs3_multilayer_fixed_score_kernel_descriptor();
    wrong_mag2.mag2_source_contract = None;
    assert_admission_err(&wrong_mag2, "Mag2SourceContract");
}

#[test]
fn field_policy_obs4_descriptor_admits_threshold_event() {
    let desc = obs4();
    assert_eq!(desc.id, FIELD_POLICY_OBS4_DESCRIPTOR_ID);
    assert!(desc.default_off);
    assert!(!desc.production_wiring);
    ...
    println!(
        "field_policy_obs4_descriptor: id={FIELD_POLICY_OBS4_DESCRIPTOR_ID} label={FIELD_POLICY_OBS4_LABEL} threshold=ExactQ16Threshold event=ExactDeterministicEventFlag"
    );
}

#[test]
fn field_policy_obs4_rejects_approximate_event_outputs() {
    let mut bad = field_policy_obs4_threshold_event_kernel_descriptor();
    for out in &mut bad.writes {
    ...
    assert_admission_err(
        &bad,
        "FIELD_POLICY threshold event requires exact-authoritative event_code_u32",
    );
}

#[test]
fn field_policy_event0_descriptor_admits_compaction() {
    let desc = event0();
    assert_eq!(desc.id, FIELD_POLICY_EVENT0_DESCRIPTOR_ID);
    assert!(desc.default_off);
    assert!(!desc.production_wiring);
    assert!(is_field_policy_event0_compaction_descriptor(&desc));
    validate_kernel_descriptor_admission(&desc).expect("event0 admits");
    println!(
        "field_policy_event0_descriptor: id={FIELD_POLICY_EVENT0_DESCRIPTOR_ID} membership=ExactAuthoritativeUnordered order=UnspecifiedAtomicOrder"
    );
}

#[test]
fn field_policy_event0_rejects_sqrt_artifact_binding() {
    let mut bad = field_policy_event0_compaction_kernel_descriptor();
...
#[test]
fn field_policy_pipe0_descriptor_admits_integrated_pipeline() {
    let desc = pipe0();
    assert_eq!(desc.id, FIELD_POLICY_PIPE0_DESCRIPTOR_ID);
    assert!(desc.default_off);
    assert!(!desc.production_wiring);
    ...
    println!(
        "field_policy_pipe0_descriptor: id={FIELD_POLICY_PIPE0_DESCRIPTOR_ID} membership=ExactAuthoritativeUnordered order=UnspecifiedAtomicOrder"
    );
}

#[test]
fn field_policy_pipe0_rejects_missing_event_record_output() {
    let mut bad = field_policy_pipe0_observer_event_pipeline_kernel_descriptor();
    bad.writes.retain(|out| out.name != "event_record");
    assert_admission_err(&bad, "exact-authoritative event_record");
}

#[test]
fn field_policy_event1_descriptor_admits_code_bucketing() {
    let desc = event1();
    assert_eq!(desc.id, FIELD_POLICY_EVENT1_DESCRIPTOR_ID);
    assert!(desc.default_off);
    assert!(!desc.production_wiring);
    ...
    println!(
        "field_policy_event1_descriptor: id={FIELD_POLICY_EVENT1_DESCRIPTOR_ID} membership=ExactAuthoritativeUnordered order=UnspecifiedAtomicOrder"
    );
}

#[test]
fn field_policy_event1_rejects_sqrt_artifact_binding() {
    let mut bad = field_policy_event1_code_bucketing_kernel_descriptor();
    bad.exact_sqrt_artifact = Some(exact_sqrt_f_artifact_descriptor());
    assert_admission_err(&bad, "must not bind sqrt artifact");
}

#[test]
fn field_policy_event2_descriptor_admits_bucket_reductions() {
    let desc = event2();
    assert_eq!(desc.id, FIELD_POLICY_EVENT2_DESCRIPTOR_ID);
    assert!(desc.default_off);
    assert!(!desc.production_wiring);
    ...
    println!(
        "field_policy_event2_descriptor: id={FIELD_POLICY_EVENT2_DESCRIPTOR_ID} reductions=order-invariant order=UnspecifiedAtomicOrder"
    );
}

#[test]
fn field_policy_event2_rejects_missing_sum_score_output() {
    let mut bad = field_policy_event2_bucket_reductions_kernel_descriptor();
    bad.writes.retain(|out| out.name != "sum_score");
    assert_admission_err(&bad, "exact-authoritative sum_score");
}

#[test]
fn field_policy_act0_descriptor_admits_numeric_proposals() {
    let desc = act0();
    assert_eq!(desc.id, FIELD_POLICY_ACT0_DESCRIPTOR_ID);
    assert!(desc.default_off);
    assert!(!desc.production_wiring);
    ...
    println!(
        "field_policy_act0_descriptor: id={FIELD_POLICY_ACT0_DESCRIPTOR_ID} membership=ExactAuthoritativeUnordered order=UnspecifiedAtomicOrder"
    );
}

#[test]
fn field_policy_act0_rejects_sqrt_artifact_binding() {
    let mut bad = field_policy_act0_numeric_proposals_kernel_descriptor();
    bad.exact_sqrt_artifact = Some(exact_sqrt_f_artifact_descriptor());
    assert_admission_err(&bad, "must not bind sqrt artifact");
}

#[test]
fn field_policy_act1_descriptor_admits_phase_e_proposal_consumer() {
    let desc = act1();
    assert_eq!(desc.id, FIELD_POLICY_ACT1_DESCRIPTOR_ID);
    assert!(desc.default_off);
    assert!(!desc.production_wiring);
    assert!(is_field_policy_act1_phase_e_proposal_consumer_descriptor(
        &desc
    ));
    validate_kernel_descriptor_admission(&desc).expect("act1 admits");
    println!(
        "field_policy_act1_descriptor: id={FIELD_POLICY_ACT1_DESCRIPTOR_ID} summary=OrderInvariantExact default_off=true"
    );
}

#[test]
fn field_policy_act1_rejects_missing_proposal_summary_output() {
    let mut bad = field_policy_act1_phase_e_proposal_consumer_kernel_descriptor();
    bad.writes.retain(|out| out.name != "proposal_summary");
    assert_admission_err(&bad, "exact-authoritative proposal_summary");
}

#[test]
fn field_policy_act2_descriptor_admits_proposal_admission_records() {
    let desc = act2();
    assert_eq!(desc.id, FIELD_POLICY_ACT2_DESCRIPTOR_ID);
    assert!(desc.default_off);
    assert!(!desc.production_wiring);
    assert!(is_field_policy_act2_proposal_admission_records_descriptor(
        &desc
    ));
    validate_kernel_descriptor_admission(&desc).expect("act2 admits");
    println!(
        "field_policy_act2_descriptor: id={FIELD_POLICY_ACT2_DESCRIPTOR_ID} admission=ExactAuthoritative default_off=true"
    );
}

#[test]
fn field_policy_act2_rejects_sqrt_artifact_binding() {
    let mut bad = field_policy_act2_proposal_admission_records_kernel_descriptor();
    bad.exact_sqrt_artifact = Some(exact_sqrt_f_artifact_descriptor());
    assert_admission_err(&bad, "must not bind sqrt artifact");
}

#[test]
fn field_policy_act3_descriptor_admits_economic_fixture_records() {
    let desc = act3();
    assert_eq!(desc.id, FIELD_POLICY_ACT3_DESCRIPTOR_ID);
    assert!(desc.default_off);
    assert!(!desc.production_wiring);
    ...
    println!(
        "field_policy_act3_descriptor: id={FIELD_POLICY_ACT3_DESCRIPTOR_ID} fixture=ExactAuthoritative default_off=true"
    );
}

#[test]
fn field_policy_act3_rejects_sqrt_artifact_binding() {
    let mut bad = field_policy_act3_economic_fixture_records_kernel_descriptor();
    bad.exact_sqrt_artifact = Some(exact_sqrt_f_artifact_descriptor());
    assert_admission_err(&bad, "must not bind sqrt artifact");
}

#[test]
fn field_policy_act4_does_not_add_runtime_descriptor() {
    ...
    println!("field_policy_act4_spec: corpus_only=true no_runtime_descriptor=true");
}
