...
#[test]
fn missing_property_rejects_with_transfer_and_side() {
    ...
    let spec = ResourceEconomySpec {
        transfers: vec![ResourceTransferSpec {
            id: "t_missing".into(),
            source: simthing_spec::PropertyKey::new("core", "missing"),
            source_role: SubFieldRole::Named("amount".into()),
            target: simthing_spec::PropertyKey::new("core", "store"),
    ...
    let err = compile_resource_economy_authoring_preview(&spec, &reg, &eml, false)
        .expect_err("missing source property rejected");
    assert_eq!(
        err,
        SpecError::UnknownResourceEconomySourceProperty {
            transfer: "t_missing".into(),
            namespace: "core".into(),
            name: "missing".into(),
        }
    );
...
#[test]
fn missing_role_rejects_with_context() {
    let mut reg = support::empty_registry();
    ...
    let spec = ResourceEconomySpec {
        transfers: vec![ResourceTransferSpec {
            ...
            target: simthing_spec::PropertyKey::new("core", "food"),
            target_role: SubFieldRole::Named("amount".into()),
            amount: 1.0,
            order_band: 0,
        }],
    ...
    match err {
        SpecError::InvalidResourceEconomyRole {
            ...
            assert!(context.contains("t_bad_role"));
            assert!(property.contains("core::food"));
            assert_eq!(role, "Named(missing)");
        }
...
#[test]
fn duplicate_ids_reject_clearly() {
    let mut reg = support::empty_registry();
    register_amount_property(&mut reg, "core", "a");
    register_amount_property(&mut reg, "core", "b");
    let eml = exact_eml_registry(&[]);
    let spec = ResourceEconomySpec {
        transfers: vec![
            amount_property("dup", "core", "a", 1.0, 0),
            amount_property("dup", "core", "b", 1.0, 0),
        ],
        ..Default::default()
    };

    let err = compile_resource_economy_authoring_preview(&spec, &reg, &eml, false)
        .expect_err("duplicate transfer id rejected");
    assert_eq!(
        err,
        SpecError::DuplicateResourceEconomyId { id: "dup".into() }
    );
}

#[test]
fn non_finite_transfer_amount_rejects() {
    let mut reg = support::empty_registry();
    register_amount_property(&mut reg, "core", "a");
    register_amount_property(&mut reg, "core", "b");
    let eml = exact_eml_registry(&[]);
    ...
    for bad in [f32::NAN, f32::INFINITY, f32::NEG_INFINITY] {
        let spec = ResourceEconomySpec {
            transfers: vec![amount_property("bad_amount", "core", "a", bad, 0)],
            ..Default::default()
        };
        let err = compile_resource_economy_authoring_preview(&spec, &reg, &eml, false)
            .expect_err("non-finite transfer amount rejected");
        assert_eq!(
            err,
            SpecError::InvalidTransferAmount {
                transfer: "bad_amount".into()
            }
        );
    }
}
...
#[test]
fn non_finite_recipe_unit_cost_rejects() {
    let mut reg = support::empty_registry();
    register_amount_property(&mut reg, "core", "food");
    ...
    let spec = ResourceEconomySpec {
        recipes: vec![ResourceRecipeSpec {
            id: "bad_recipe".into(),
            inputs: vec![simthing_spec::RecipeInputSpec {
                property: simthing_spec::PropertyKey::new("core", "food"),
                role: SubFieldRole::Named("amount".into()),
                unit_cost: f32::NAN,
            }],
            target: simthing_spec::PropertyKey::new("core", "out"),
            target_role: SubFieldRole::Named("amount".into()),
    ...
    let err = compile_resource_economy_authoring_preview(&spec, &reg, &eml, false)
        .expect_err("non-finite recipe unit_cost rejected");
    assert_eq!(
        err,
        SpecError::InvalidRecipeUnitCost {
            recipe: "bad_recipe".into()
        }
    );
}
