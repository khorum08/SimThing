...
fn loyalty_world(n_cohorts: usize) -> (DimensionRegistry, SlotAllocator, SimThing, SimPropertyId, u32) {
...
fn c2_flag_off_clears_stale_intent_accumulator() {
    ...
    let mut proto_legacy = BoundaryProtocol::new(
        proto.root.clone(),
        proto.registry.clone(),
        proto.allocator.clone(),
    );
    proto_legacy.flags.use_accumulator_intent = false;
    let mut state_legacy = WorldGpuState::new(
        GpuContext::new_blocking().expect("gpu"),
        &proto_legacy.registry,
        n_slots,
    );
    let pipelines_legacy = Pipelines::new(&state_legacy.ctx);
    let mut patcher_legacy = TransformPatcher::new(n_slots as usize);
    let mut coord_legacy = DispatchCoordinator::new(n_slots, n_dims, 8);
    let (tx_legacy, rx_legacy) = feeder_channel();
    coord_legacy.shadow.copy_from_slice(&coord.shadow);
    coord_legacy.upload_full_shadow(&state_legacy);
    proto_legacy.initial_gpu_sync(&coord_legacy, &mut state_legacy);
    tx_legacy
        .send(FeederWork::Patch(PatchTransform {
            target: cohort_id,
            delta: PropertyTransformDelta {
                property_id: pid,
                sub_field_deltas: vec![(SubFieldRole::Amount, TransformOp::Add(0.1))],
        ...
        .unwrap();
    let legacy = coord_legacy.tick(
        &rx_legacy,
        &mut patcher_legacy,
        &proto_legacy.registry,
        &proto_legacy.allocator,
        &pipelines_legacy,
        &mut state_legacy,
        1.0,
    );

    assert_eq!(stale_off.events.len(), legacy.events.len());
    assert_eq!(state.read_values(), state_legacy.read_values());
}
