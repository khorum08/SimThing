...
use simthing_gpu::{
    ...
    reset_legacy_exact_reduction_bucket_call_count, GpuContext, Pipelines, ReductionPlanMode,
    SlotAllocator, Topology, TopologyState, WorldGpuState,
};
...
fn setup_reduction_state(
    ...
    let descriptors = build_column_rule_descriptors(reg, n_dims);
    let plan = plan_reduction_orderband(&topo_state, &descriptors, state.n_dims, mode).unwrap();
    state
        .upload_reduction_soft_ops_with_bands(&plan.ops, plan.n_bands, exact_active)
        .unwrap();
...
fn run_accumulator_reduction(state: &mut WorldGpuState) {
    ...
    pipelines.run_c5_soft_reduction_passes(state, &mut session);
...
fn c6_mixed_soft_and_exact_columns_all_accumulator_matches_legacy() {
    ...
    for slot in 0..state.n_slots as usize {
        ...
        assert!(
            (legacy[i] - acc[i]).abs() < TOL,
            "soft col slot {slot}: legacy={} acc={}",
            legacy[i],
            acc[i]
        );
    }
...
fn c6_multi_depth_exact_and_weighted_dependencies_match_legacy() {
    ...
    assert!(max_abs_error(&legacy, &acc) < TOL);
}

#[test]
fn c6_full_reduction_path_does_not_dispatch_legacy_reduction() {
    let Some(_ctx) = try_gpu() else {
        eprintln!("skipping: no GPU");
        return;
    };
    ...
    let (reg, world, alloc) = cohort_tree_with_rule(ReductionRule::Sum, &[1.0, 2.0, 3.0]);
    let (mut state_c5, _, _) = setup_reduction_state(
        &reg,
        &world,
        &alloc,
    ...
    assert!(bridge_buckets > 0, "C-5 bridge should dispatch legacy exact buckets");

    let (mut state_c6, _, _) = setup_reduction_state(
        &reg,
        &world,
        &alloc,
        ReductionPlanMode::AllRules,
    ...
    reset_legacy_exact_reduction_bucket_call_count();
    run_accumulator_reduction(&mut state_c6);
    assert_eq!(
...
fn c6_production_path_no_cpu_mediated_reduction() {
    ...
    let (mut state, _, _) = setup_reduction_state(
        &reg,
        &world,
        &alloc,
        ReductionPlanMode::AllRules,
