...
//!
//! 6. **Velocity alerts** (AI-registered, Week 4+) — registered separately
//!    by the AI layer, not derived from SimProperty definitions.

use simthing_core::{
    DecayBehavior, DimensionRegistry, Direction, SimPropertyId, SimThing, SimThingId,
    SubFieldRole,
};
...
impl ThresholdBuilder {
    ...
    fn walk(
        ...
        if let Some(slot) = allocator.slot_of(node.id) {
            for (pid, _pval) in &node.properties {
                if !dim_reg.is_active(*pid) { continue; }
                ...
                Self::push_decay_thresholds(
                    node.id, *pid, slot, prop.decay.as_ref(), range.start, layout,
                    dim_reg, gpu_regs, cpu_reg,
                );
            }
        }
    ...
    fn push_decay_thresholds(
        ...
        let semantic = ThresholdSemantic::PropertyExpiry { sim_thing_id, property_id };
        match decay {
            Some(DecayBehavior::OnThreshold { threshold, direction }) => {
            ...
            Some(DecayBehavior::WhenProperty { other, threshold }) => {
                ...
                if !dim_reg.is_active(*other) { return; }
...
mod tests {
    ...
    fn event_kind_registry_indices_are_sequential() {
        ...
        let ek0 = r.push(ThresholdSemantic::PropertyExpiry { sim_thing_id: id, property_id: SimPropertyId(0) });
        let ek1 = r.push(ThresholdSemantic::PropertyExpiry { sim_thing_id: id, property_id: SimPropertyId(1) });
        assert_eq!(ek0, 0);
        assert_eq!(ek1, 1);
