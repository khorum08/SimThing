...
fn resolve_node(
    ...
    {
        let should_dissolve: Vec<bool> = node.overlays.iter().map(|overlay| {
            if let OverlayLifecycle::Transient { dissolution_conditions } = &overlay.lifecycle {
                dissolution_conditions.iter().all(|cond| {
                    evaluate_condition(cond, node, registry, values_shadow, base)
                })
            } else {
                false
            }
        ...
        for (i, overlay) in node.overlays.iter_mut().enumerate() {
            if let OverlayLifecycle::Transient { dissolution_conditions } = &mut overlay.lifecycle {
            ...
            if should_dissolve[i] { dissolved_indices.push(i); }
        }
    }
    ...
    for i in dissolved_indices.into_iter().rev() {
        ...
        if let (Some(base), Some(ref handler)) = (
            base,
            registry.property(overlay.transform.property_id).on_expire.as_ref(),
        ) {
...
fn evaluate_condition(
    ...
    match cond {
        ...
        DissolveCondition::PropertyReaches { property, sub_field, value } => {
            read_sub_field(node, registry, values_shadow, base, *property, sub_field)
                .map(|v| v >= *value)
                .unwrap_or(false)
        }
        DissolveCondition::PropertyBelow { property, sub_field, value } => {
            read_sub_field(node, registry, values_shadow, base, *property, sub_field)
                .map(|v| v < *value)
                .unwrap_or(false)
        }
...
fn apply_expire_effects(
    ...
    for effect in &handler.write_back {
        match effect {
            simthing_core::ExpireEffect::AddVelocity { property, sub_field, delta } => {
                if !registry.is_active(*property) { continue; }
                let range  = registry.column_range(*property);
...
mod tests {
    use super::*;
    use simthing_core::{
        DimensionRegistry, DissolveCondition, Overlay, OverlayId, OverlayKind,
        OverlayLifecycle, OverlaySource, PropertyTransformDelta, SimProperty, SimPropertyId,
    ...
    fn after_ticks_nonzero_decrements_and_survives() {
        ...
        if let OverlayLifecycle::Transient { dissolution_conditions } = &root.overlays[0].lifecycle {
