...
fn run_movement_feedback_application(ctx: &GpuContext) -> FrontierV2MovementFeedbackRun {
    ...
    let tick2 = run_single_live_tick(ctx, &skeleton, &tick2_config, 2, 0, Some(&shadow_after));
...
fn frontier_v2_2_no_simthing_sim_semantic_awareness() {
    ...
    println!("frontier_v2_2_sim: semantic_free=true fixture_id={FRONTIER_V2_2_FIXTURE_ID}");
}
...
fn frontier_v2_2_no_unauthorized_gpu_primitive() {
    let frontier_descriptor = landed_jit_kernel_descriptors()
        .into_iter()
        .find(|d| d.id.contains("frontier_v2_2") || d.id.contains("movement_feedback"));
    ...
    println!("frontier_v2_2_gpu: no_new_primitive=true fixture_id={FRONTIER_V2_2_FIXTURE_ID}");
}

#[test]
fn frontier_v2_2_no_implementer_self_acceptance() {
    let report_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../docs/tests/phase_m_frontier_v2_2_movement_feedback_application_results.md");
    ...
    println!("frontier_v2_2_no_self_accept: report_ok=true fixture_id={FRONTIER_V2_2_FIXTURE_ID}");
}
