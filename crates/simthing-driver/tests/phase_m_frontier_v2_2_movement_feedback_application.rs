//! FrontierV2-2 — Own-column movement feedback application (Tier-2, test-only).
//!
//! Extends FrontierV2-1 by applying movement FixtureCandidates to fixture-only own-column
//! shadow state and feeding updated source placement into the next tick's field route.
...
fn gpu_seeds(
    ...
    let ((row_a, col_a), (row_b, col_b)) = source_seed_placement(config, shadow);
    vec![
        FirstSliceSeed {
            row: row_a,
            col: col_a,
...
struct FrontierV2MovementFeedbackRun {
    summary: FrontierV2MovementFeedbackSummary,
...
fn run_single_live_tick(
    ...
    mapping_session
        .queue_seeds(&gpu_seeds(config, shadow))
        .expect("queue seeds");
    ...
    assert_gpu_field_matches_cpu_reference(&spec, &field_values, &seed_tuples(config, shadow));
...
fn run_movement_feedback_application(ctx: &GpuContext) -> FrontierV2MovementFeedbackRun {
    ...
    let shadow_after = apply_movement_to_own_column_shadow(
        &shadow_before,
        &tick1.movement,
    ...
    let tick2_config = apply_feedback_to_config(&tick1_config, &tick1.feedback);
    let tick2 = run_single_live_tick(
        ctx,
        &skeleton,
        &tick2_config,
        2,
    ...
    let movement_feedback_delta_hash = hash_movement_feedback_delta(
        shadow_before,
        shadow_after,
        tick1.mapping_hash,
        tick2.mapping_hash,
...
#[test]
fn frontier_v2_2_happy_path_movement_feedback_application_runs() {
    with_gpu(|ctx| {
        let (skeleton, _) = smoke_fixture();
        let run = run_movement_feedback_application(ctx);
        ...
        assert_ne!(run.shadow_before, run.shadow_after);
        assert!(!skeleton.enabled_by_default);

        println!(
            "frontier_v2_2_happy: fixture_id={FRONTIER_V2_2_FIXTURE_ID} fp={} shadow_before={:?} shadow_after={:?}",
...
#[test]
fn frontier_v2_2_own_column_movement_changes_position() {
    with_gpu(|ctx| {
        let run = run_movement_feedback_application(ctx);
        assert_ne!(run.shadow_before, run.shadow_after);
        assert_eq!(
        ...
        let expected_row = clamp_grid_coord(
            run.shadow_before.row as i32 + run.tick1.movement.delta_row,
            frontier_v1_1_fixture_config().grid_size,
        );
        let expected_col = clamp_grid_coord(
            run.shadow_before.col as i32 + run.tick1.movement.delta_col,
            frontier_v1_1_fixture_config().grid_size,
        );
        assert_eq!(run.shadow_after.row, expected_row);
        assert_eq!(run.shadow_after.col, expected_col);

        println!(
            "frontier_v2_2_shadow: before=({}, {}) after=({}, {}) classification=OwnColumnShadowWrite",
            run.shadow_before.row,
            run.shadow_before.col,
            run.shadow_after.row,
            run.shadow_after.col,
        );
    });
}
...
fn frontier_v2_2_rejects_cross_entity_movement_write() {
    ...
    println!(
        "frontier_v2_2_cross_entity: rejected=true fixture_id={FRONTIER_V2_2_FIXTURE_ID}"
    );
}
...
fn frontier_v2_2_movement_feedback_affects_next_tick() {
    with_gpu(|ctx| {
        ...
        println!(
            "frontier_v2_2_feedback: tick1_map={} tick2_map={} tick1_prop={} tick2_prop={}",
            run.tick1.mapping_hash,
            run.tick2.mapping_hash,
            run.tick1.proposal_dispatch_hash,
            run.tick2.proposal_dispatch_hash,
        );
    });
}

#[test]
fn frontier_v2_2_cpu_oracle_parity() {
    with_gpu(|ctx| {
        let (skeleton, base_config) = smoke_fixture();
        let run = run_movement_feedback_application(ctx);
        ...
        let tick_configs = [
            ...
            (
                ...
                Some(run.shadow_after),
            ),
        ];

        for (tick_index, config, shadow) in tick_configs {
            let tick = match tick_index {
                0 => &run.tick0,
                1 => &run.tick1,
                _ => &run.tick2,
            };
            ...
            let ((row_a, col_a), _) = source_seed_placement(&config, shadow_ref);
            if tick_index == 2 {
                assert_eq!(row_a, run.shadow_after.row);
                assert_eq!(col_a, run.shadow_after.col);
            }
        ...
        let expected_after = apply_movement_to_own_column_shadow(
            &run.shadow_before,
            &run.tick1.movement,
            base_config.grid_size,
            1,
        ...
        assert_eq!(run.shadow_after, expected_after);

        println!(
            "frontier_v2_2_parity: fp={} delta={}",
            run.summary.combined_hex(),
            run.summary.movement_feedback_delta_hash,
        );
    });
}

#[test]
fn frontier_v2_2_replay_reproducibility() {
    with_gpu(|ctx| {
        let run_a = run_movement_feedback_application(ctx);
        let run_b = run_movement_feedback_application(ctx);
        ...
        assert_eq!(run_a.shadow_before, run_b.shadow_before);
        assert_eq!(run_a.shadow_after, run_b.shadow_after);
        assert_eq!(run_a.summary.combined_hex(), run_b.summary.combined_hex());
        assert_eq!(
            run_a.summary.combined_hex(),
            FRONTIER_V2_2_MOVEMENT_FEEDBACK_FINGERPRINT
        );

        println!(
            "frontier_v2_2_replay: fp={} fixture_id={FRONTIER_V2_2_FIXTURE_ID}",
            run_a.summary.combined_hex()
        );
    });
}
...
#[test]
fn frontier_v2_2_defaults_remain_disabled() {
    ...
    println!("frontier_v2_2_defaults: disabled=true fixture_id={FRONTIER_V2_2_FIXTURE_ID}");
}

#[test]
fn frontier_v2_2_resource_route_stays_allocator_only() {
    ...
    let _ = config;
    println!(
        "frontier_v2_2_allocator: rejects=true fixture_id={FRONTIER_V2_2_FIXTURE_ID}"
    );
}
...
fn frontier_v2_2_deferred_features_reject() {
    ...
    println!(
        "frontier_v2_2_deferred: rejects=true fixture_id={FRONTIER_V2_2_FIXTURE_ID}"
    );
}
...
fn frontier_v2_2_no_simthing_sim_semantic_awareness() {
    ...
    println!(
        "frontier_v2_2_sim: semantic_free=true fixture_id={FRONTIER_V2_2_FIXTURE_ID}"
    );
}

#[test]
fn frontier_v2_2_no_unauthorized_gpu_primitive() {
    let frontier_descriptor = landed_jit_kernel_descriptors().into_iter().find(|d| {
        d.id.contains("frontier_v2_2") || d.id.contains("movement_feedback")
    });
    ...
    if wgsl_dir.is_dir() {
        for entry in std::fs::read_dir(&wgsl_dir).expect("read wgsl dir") {
            ...
            let name = path
                ...
                .to_ascii_lowercase();
            assert!(
                !name.contains("frontier_v2_2"),
                "no FrontierV2-2 WGSL: {}",
                path.display()
            );
        }
    }
    println!(
        "frontier_v2_2_gpu: no_new_primitive=true fixture_id={FRONTIER_V2_2_FIXTURE_ID}"
    );
}

#[test]
fn frontier_v2_2_no_implementer_self_acceptance() {
    ...
    let report = std::fs::read_to_string(&report_path).expect("V2-2 results report must exist");
    let forbidden = [
        "Phase M closed",
        "Phase E closed",
        "M/E closed",
    ...
    println!(
        "frontier_v2_2_no_self_accept: report_ok=true fixture_id={FRONTIER_V2_2_FIXTURE_ID}"
    );
}
