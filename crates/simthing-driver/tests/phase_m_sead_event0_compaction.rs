//! SEAD-EVENT-0 — GPU-resident event compaction from threshold event rows (Tier-2, test-only).
//!
...
use simthing_spec::{
    is_sead_event0_compaction_descriptor, landed_jit_kernel_descriptors,
    validate_kernel_descriptor_admission, EventCompactionMembershipAuthority,
    EventCompactionOrderAuthority, MappingExecutionProfile, SEAD_EVENT0_DESCRIPTOR_ID,
};
...
fn run_compaction(
    ...
    let input_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("sead_event0_inputs"),
        contents: bytemuck::cast_slice(&inputs),
        usage: wgpu::BufferUsages::STORAGE,
    });
    let meta_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("sead_event0_meta"),
        contents: &[0u8; 8],
        usage: wgpu::BufferUsages::STORAGE
    ...
    let records_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("sead_event0_records"),
    ...
    let params_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("sead_event0_params"),
        contents: bytemuck::bytes_of(&params),
        usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
    });

    let bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("sead_event0_bg"),
        layout: &bgl,
        entries: &[
            wgpu::BindGroupEntry {
                binding: 0,
    ...
    for _ in 0..repeat_dispatches {
        ...
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("sead_event0_enc"),
        });
        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("sead_event0_pass"),
    ...
    let meta_staging = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("sead_event0_meta_readback"),
        size: 8,
    ...
    let records_staging = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("sead_event0_records_readback"),
    ...
    let mut enc2 = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("sead_event0_readback_enc"),
    });
...
#[test]
fn sead_event0_wgsl_semantic_free() {
    ...
    println!("sead_event0_wgsl: semantic_free=true ordering={ORDERING_CLASS}");
}

#[test]
fn sead_event0_compaction_edge_rows() {
    with_gpu(|ctx| {
        for (rows, capacity, label) in edge_compaction_cases() {
            ...
            println!(
                "sead_event0_edge[{label}]: inputs={} nonzero={} capacity={capacity} event_count={} written={written} overflow={} ordering={ORDERING_CLASS}",
                rows.len(),
                expected.len(),
                outcome.event_count,
                outcome.overflow
...
#[test]
fn sead_event0_compaction_dense_corpus() {
    with_gpu(|ctx| {
        ...
        println!(
            "sead_event0_dense: rows={} nonzero={} event_count={} membership=exact_unordered ordering={ORDERING_CLASS}",
            rows.len(),
            expected.len(),
            outcome.event_count
        );
...
#[test]
fn sead_event0_obs4_to_compaction_smoke() {
    with_gpu(|ctx| {
        ...
        assert!(membership_exact(&expected, &outcome.records));
        println!(
            "sead_event0_obs4_smoke: input_rows={N} nonzero={} compact_count={} overflow={} membership=exact_unordered",
            expected.len(),
            outcome.event_count,
            outcome.overflow
        );
...
#[test]
fn sead_event0_perf_34k_compaction() {
    with_gpu(|ctx| {
        const N: usize = 34_000;
        for density_pct in [0u32, 1, 10, 50, 100] {
            ...
            println!(
                "sead_event0_perf_density_{density_pct}: rows={N} density_pct={density_pct} elapsed_ms={ms:.3} per_row_us={per_row_us:.4} event_count={} per_event_us={per_event_us:.4} capacity={capacity} overflow={}",
                outcome.event_count,
                outcome.overflow
            );
        }
...
#[test]
fn sead_event0_perf_34k_warm_repeated_dispatch() {
    with_gpu(|ctx| {
        const N: usize = 34_000;
        const REPEAT: u32 = 32;
...
#[test]
fn sead_event0_no_default_runtime_wiring() {
    assert_eq!(
    ...
    let desc = landed_jit_kernel_descriptors()
        .into_iter()
        .find(|d| d.id == SEAD_EVENT0_DESCRIPTOR_ID)
    ...
    println!("sead_event0_wiring: descriptor=landed no_cpu_planner no_bridge");
}
