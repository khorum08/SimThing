//! Phase M Economy + SEAD Product Fixture V1 — boundary economy signal into first-slice commitment.
//!
//! Option A product/acceptance fixture: test-level orchestration only. Discrete economy resolves
//! treasury at boundary; test maps stress to EML weight profiles; SEAD commitment emerges through
...
#[test]
fn economy_sead_fixtures_admit_and_preview() {
...
#[test]
fn surplus_economy_produces_no_sead_commitment() {
    with_gpu(|ctx| {
        ...
        let threshold = preview
            ...
            .expect("commitment")
            .threshold;
        let sead = run_sead_commitment_with_economy_weights(ctx, &preview, &economy);

        assert!(sead.threat.is_finite() && sead.threat > 0.0);
        assert!(sead.urgency.is_finite());
        assert!(
            sead.urgency < threshold,
            "surplus/low-stress urgency should stay below threshold: {} >= {}",
            sead.urgency,
            threshold
        );
        assert!(sead.report.threshold_events.is_empty());
        assert_eq!(sead.report.mapping.reduction_stencil_readbacks, 0);
        println!(
            "surplus treasury={} stress={} weights={:?} urgency={} threshold={} sead_events={}",
            sead.treasury,
            economy_sead_product_fixture::economy_stress_signal(sead.treasury),
            sead.eml_weights,
            sead.urgency,
            threshold,
            sead.report.threshold_events.len()
        );
    });
}

#[test]
fn deficit_economy_produces_one_sead_commitment() {
    with_gpu(|ctx| {
        ...
        assert!(sead.urgency.is_finite());
        assert!(
            sead.urgency > commitment.threshold,
            "deficit/high-stress urgency should cross threshold: {} <= {}",
            sead.urgency,
            commitment.threshold
        );
        assert_eq!(sead.report.threshold_events.len(), 1);
        let event = sead.report.threshold_events[0];
        assert_eq!(event.event_kind, SEAD_COMMITMENT_EVENT_KIND);
        ...
        assert!((event.value - sead.urgency).abs() < 0.001);
        assert_eq!(sead.report.mapping.reduction_stencil_readbacks, 0);
        println!(
            "deficit treasury={} stress={} weights={:?} urgency={} threshold={} sead_events={}",
            sead.treasury,
            economy_sead_product_fixture::economy_stress_signal(sead.treasury),
            sead.eml_weights,
            sead.urgency,
...
#[test]
fn economy_sead_product_fixture_is_deterministic() {
    with_gpu(|ctx| {
        let preview = compile_commitment_scenario();

        let run_surplus = || {
            let economy = run_surplus_economy_boundary_day();
            run_sead_commitment_with_economy_weights(ctx, &preview, &economy)
        };
...
#[test]
fn economy_sead_product_fixture_posture_preserved() {
    let sim_sources = [
