//! FIELD_POLICY-ACT-4 — Authorable validation corpus for ACT-3 Economic V1 fixture records (Tier-2, test-only).
//!
...
use simthing_spec::{
    ...
    MappingExecutionProfile, FIELD_POLICY_ACT3_DESCRIPTOR_ID,
};

pub const FIELD_POLICY_ACT4_CORPUS_ID: &str =
    "field_policy_act4_economic_fixture_validation_corpus_v1";
pub const FIELD_POLICY_ACT4_CORPUS_ROW_COUNT: usize = 18;
pub const FIELD_POLICY_ACT4_CORPUS_FINGERPRINT: &str = "2e1f2b2a4ff3f65e";
...
fn field_policy_act4_corpus_row_coverage() {
    ...
    println!(
        "field_policy_act4_coverage: rows={} corpus_id={FIELD_POLICY_ACT4_CORPUS_ID}",
        labels.len()
    );
}

#[test]
fn field_policy_act4_corpus_stable_fingerprint() {
    let rows = authoring_corpus();
    let fp = corpus_fingerprint(rows);
    assert_eq!(rows.len(), FIELD_POLICY_ACT4_CORPUS_ROW_COUNT);
    assert_eq!(fp, FIELD_POLICY_ACT4_CORPUS_FINGERPRINT);
    println!(
        "field_policy_act4_fingerprint: id={FIELD_POLICY_ACT4_CORPUS_ID} rows={} fp={fp}",
        rows.len()
    );
}

#[test]
fn field_policy_act4_act3_substrate_crosscheck() {
    let table = default_mapping_table();
    let mapping_n = mapping_count();
    ...
    println!(
        "field_policy_act4_act3_crosscheck: record_code={} priority={} tier={} corpus_id={FIELD_POLICY_ACT4_CORPUS_ID}",
        got.record_code, got.priority, got.tier,
    );
}

#[test]
fn field_policy_act4_no_new_runtime_wiring() {
    assert_eq!(
    ...
    println!(
        "field_policy_act4_wiring: corpus_only=true act3_substrate={FIELD_POLICY_ACT3_DESCRIPTOR_ID} corpus_id={FIELD_POLICY_ACT4_CORPUS_ID}"
    );
}

#[test]
fn field_policy_act4_no_new_wgsl_or_gpu_primitive() {
    let corpus_only = true;
    assert!(corpus_only);
    println!("field_policy_act4_gpu: new_wgsl=false new_descriptor=false corpus_id={FIELD_POLICY_ACT4_CORPUS_ID}");
}
