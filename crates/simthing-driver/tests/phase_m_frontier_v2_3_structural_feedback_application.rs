//! FrontierV2-3 — Structural BoundaryRequest feedback application (Tier-2, test-only).
//!
...
pub const FRONTIER_V2_3_STRUCTURAL_FEEDBACK_FINGERPRINT: &str = "0ad0e0d7c80316ee";
...
fn gpu_seeds(config: &FrontierV1FixtureConfig) -> Vec<FirstSliceSeed> {
    let ((row_a, col_a), (row_b, col_b)) = source_seed_placement(config, None);
    vec![
        FirstSliceSeed {
            row: row_a,
            col: col_a,
...
fn expected_evolved_structural(
...
struct FrontierV2StructuralFeedbackRun {
    summary: FrontierV2StructuralFeedbackSummary,
...
fn run_structural_feedback_application(ctx: &GpuContext) -> FrontierV2StructuralFeedbackRun {
    ...
    let shadow_before = empty_boundary_request_shadow(0);
    ...
    let summary = FrontierV2StructuralFeedbackSummary {
        ...
        shadow_before_hash: hash_boundary_request_shadow(shadow_before),
        shadow_after_hash: hash_boundary_request_shadow(shadow_after),
        tick2_structural_feedback_code: structural_feedback_code,
        ...
        structural_feedback_delta_hash,
        overflow_flags,
    ...
    FrontierV2StructuralFeedbackRun {
        summary,
        tick0,
        tick1,
        tick2,
...
#[test]
fn frontier_v2_3_happy_path_structural_feedback_application_runs() {
    with_gpu(|ctx| {
        let (skeleton, _) = smoke_fixture();
        let run = run_structural_feedback_application(ctx);
        ...
        assert!(run.shadow_after.applied);
        assert!(!skeleton.enabled_by_default);

        println!(
            "frontier_v2_3_happy: fixture_id={FRONTIER_V2_3_FIXTURE_ID} fp={} S1 boundary={} feedback_code={}",
            run.summary.combined_hex(),
            run.shadow_after.boundary_request_code,
...
#[test]
fn frontier_v2_3_boundary_request_shadow_accepts_valid_route() {
    with_gpu(|ctx| {
        let run = run_structural_feedback_application(ctx);
        assert_eq!(
            run.shadow_after.route_code,
        ...
        println!(
            "frontier_v2_3_valid_route: route_code={} applied={} fixture_id={FRONTIER_V2_3_FIXTURE_ID}",
            run.shadow_after.route_code,
            run.shadow_after.applied,
        );
    });
}
...
fn frontier_v2_3_structural_feedback_affects_next_tick() {
    with_gpu(|ctx| {
        ...
        println!(
            "frontier_v2_3_feedback: tick1_map={} tick2_map={} tick1_prop={} tick2_prop={}",
...
#[test]
fn frontier_v2_3_cpu_oracle_parity() {
    with_gpu(|ctx| {
        let (skeleton, base_config) = smoke_fixture();
        let run = run_structural_feedback_application(ctx);

        let tick1_config = apply_feedback_to_config(&base_config, &run.tick0.feedback);
        let tick2_base = apply_feedback_to_config(&tick1_config, &run.tick1.feedback);
        let tick2_config =
            apply_structural_feedback_to_config(&tick2_base, run.structural_feedback_code);

        for (tick, config) in [
            (&run.tick0, base_config),
        ...
        assert_eq!(
            run.structural_feedback_code,
            derive_next_tick_structural_feedback_code(&run.shadow_after)
        );

        println!(
            "frontier_v2_3_parity: fp={} delta={}",
            run.summary.combined_hex(),
            run.summary.structural_feedback_delta_hash,
        );
    });
}

#[test]
fn frontier_v2_3_replay_reproducibility() {
    with_gpu(|ctx| {
        let run_a = run_structural_feedback_application(ctx);
        let run_b = run_structural_feedback_application(ctx);
        ...
        assert_eq!(
            ...
            FRONTIER_V2_3_STRUCTURAL_FEEDBACK_FINGERPRINT
        );

        println!(
            "frontier_v2_3_replay: fp={} fixture_id={FRONTIER_V2_3_FIXTURE_ID}",
            run_a.summary.combined_hex()
        );
    });
}
...
fn frontier_v2_3_defaults_remain_disabled() {
    ...
    println!("frontier_v2_3_defaults: disabled=true fixture_id={FRONTIER_V2_3_FIXTURE_ID}");
}

#[test]
fn frontier_v2_3_resource_route_stays_allocator_only() {
    ...
    if wgsl_dir.is_dir() {
        for entry in std::fs::read_dir(&wgsl_dir).expect("read wgsl dir") {
            ...
            let name = path
                ...
                .to_ascii_lowercase();
            assert!(
                !name.contains("frontier_v2_3"),
                "no FrontierV2-3 WGSL: {}",
                path.display()
            );
        }
    }
