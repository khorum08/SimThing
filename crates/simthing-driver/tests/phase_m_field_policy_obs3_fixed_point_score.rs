//! FIELD_POLICY-OBS-3 — Fixed-point aggregate score for multi-layer observer overlay (Tier-2, test-only).
//!
...
use simthing_spec::{
    ...
    is_field_policy_obs3_multilayer_fixed_score_descriptor, landed_jit_kernel_descriptors,
    validate_kernel_descriptor_admission, MappingExecutionProfile, FIELD_POLICY_OBS2_DESCRIPTOR_ID,
    FIELD_POLICY_OBS3_DESCRIPTOR_ID, FIELD_POLICY_OBS3_LAYER_COUNT, MAG2_Q16_SCALE,
...
const LAYER_COUNT: usize = FIELD_POLICY_OBS3_LAYER_COUNT as usize;
...
fn run_multilayer_batch_repeated(
    ...
    let module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("field_policy_obs3_multilayer_fixed"),
        source: wgpu::ShaderSource::Wgsl(wgsl.into()),
    });
    let bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("field_policy_obs3_bgl"),
    ...
    let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        label: Some("field_policy_obs3_pipeline"),
        layout: Some(
            &device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("field_policy_obs3_pl"),
    ...
    let storage = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("field_policy_obs3_values"),
    ...
    let bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("field_policy_obs3_bg"),
        layout: &bgl,
    ...
    for _ in 0..repeat_dispatches {
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("field_policy_obs3_enc"),
        });
        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("field_policy_obs3_pass"),
...
#[test]
fn field_policy_obs3_fixed_score_edge_rows() {
    with_gpu(|ctx| {
        ...
        println!(
            "field_policy_obs3_edge: cases={} mag2_exact={mag2_exact} mag_max_ulp={mag_max_ulp} score_exact={score_exact}/{} overflow={overflow} worst={worst:?}",
            cases.len(),
            cases.len()
        );
...
#[test]
fn field_policy_obs3_fixed_score_dense_corpus() {
    with_gpu(|ctx| {
        ...
        println!(
            "field_policy_obs3_dense: rows={} layer_mag_slots={layer_mag_slots} mag_exact={mag_exact} mag_max_ulp={mag_max_ulp} score_exact={score_exact} overflow={overflow} score_authority=ExactQ16WeightedSum",
            rows.len()
        );
...
#[test]
fn field_policy_obs3_perf_34k_fixed_score() {
    with_gpu(|ctx| {
        ...
        println!(
            "field_policy_obs3_perf_34k: rows={N} layers={LAYER_COUNT} dispatches=1 includes_readback=true elapsed_ms={ms:.3} per_row_us={per_row_us:.4} spot_mag_max_ulp={spot_mag_ulp} spot_score_exact={spot_score_exact}/512 overflow={overflow} score_authority=ExactQ16WeightedSum"
        );
    });
}

#[test]
fn field_policy_obs3_perf_34k_warm_repeated_dispatch() {
    with_gpu(|ctx| {
        const N: usize = 34_000;
        const REPEAT: u32 = 32;
        let rows = mobile_multilayer_rows(N);
        ...
        println!(
            "field_policy_obs3_warm_34k: rows={N} layers={LAYER_COUNT} dispatches={REPEAT} includes_readback=true total_ms={total_ms:.3} per_dispatch_ms={per_dispatch_ms:.3} per_row_us={per_row_us:.4} per_layer_mag_us={per_layer_mag_us:.4} spot_mag_max_ulp={spot_mag_ulp} score_authority=ExactQ16WeightedSum"
        );
        println!(
...
#[test]
fn field_policy_obs3_score_authority_fixed_point() {
    ...
    let obs3 = landed_jit_kernel_descriptors()
        .into_iter()
        .find(|d| d.id == FIELD_POLICY_OBS3_DESCRIPTOR_ID)
    ...
    println!("field_policy_obs3_score_authority: score_fixed=ExactQ16WeightedSum obs2_score_bits=ApproximateDiagnosticF32");
}

#[test]
fn field_policy_obs3_no_default_runtime_wiring() {
    assert_eq!(
