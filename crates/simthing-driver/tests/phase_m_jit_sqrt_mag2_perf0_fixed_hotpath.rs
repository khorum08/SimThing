...
#[test]
fn sqrt_mag2_perf0_readback_baseline_34k() {
    with_gpu(|ctx| {
        let pairs = mobile_simthing_fixed_pairs(N34K);
        let bytes = pairs.len() as u64 * ROW_STRIDE as u64 * 4;
        let outcome = run_bench(ctx, &pairs, BenchPath::ReadbackBaseline, 1, true);
        print_timing("sqrt_mag2_perf0_readback_baseline_34k", N34K, &outcome);
        println!("sqrt_mag2_perf0_readback_baseline_bytes: {bytes}");
    });
}

#[test]
fn sqrt_mag2_perf0_fixed_mag2_only_34k() {
    with_gpu(|ctx| {
        let pairs = mobile_simthing_fixed_pairs(N34K);
        let outcome = run_bench(ctx, &pairs, BenchPath::Mag2OnlyQ16, 1, true);
        let mut max_int_err = 0u64;
        for ((sum, mag2, _), (dx, dy)) in outcome.outputs.iter().zip(pairs.iter()).take(512) {
            let cpu_sum = cpu_mag2_sum(*dx, *dy);
            assert_eq!(*sum, cpu_sum);
            assert_eq!(*mag2, cpu_mag2_bits_q16(*dx, *dy));
            max_int_err = max_int_err.max(sum.abs_diff(cpu_sum));
        }
        assert_eq!(max_int_err, 0);
        print_timing("sqrt_mag2_perf0_fixed_mag2_only_34k", N34K, &outcome);
        println!("sqrt_mag2_perf0_mag2_only_max_int_error=0");
    });
}

#[test]
fn sqrt_mag2_perf0_f_sqrt_only_34k() {
    with_gpu(|ctx| {
        let pairs = mobile_simthing_fixed_pairs(N34K);
        let outcome = run_bench(ctx, &pairs, BenchPath::FSqrtOnly, 1, true);
        let mut max_ulp = 0u32;
        for ((_, mag2, mag), (dx, dy)) in outcome.outputs.iter().zip(pairs.iter()).take(512) {
            let cpu_mag2 = cpu_mag2_bits_q16(*dx, *dy);
            assert_eq!(*mag2, cpu_mag2);
            max_ulp = max_ulp.max(ulp_distance(*mag, cpu_mag_bits(*mag2)));
        }
        assert_eq!(max_ulp, 0);
        print_timing("sqrt_mag2_perf0_f_sqrt_only_34k", N34K, &outcome);
        println!("sqrt_mag2_perf0_f_only_max_ulp={max_ulp}");
    });
}

#[test]
fn sqrt_mag2_perf0_fixed_mag2_plus_f_34k() {
    with_gpu(|ctx| {
        let pairs = mobile_simthing_fixed_pairs(N34K);
        let outcome = run_bench(ctx, &pairs, BenchPath::CombinedQ16, 1, true);
        let mut max_ulp = 0u32;
        for ((sum, mag2, mag), (dx, dy)) in outcome.outputs.iter().zip(pairs.iter()).take(512) {
            assert_eq!(*sum, cpu_mag2_sum(*dx, *dy));
            assert_eq!(*mag2, cpu_mag2_bits_q16(*dx, *dy));
            max_ulp = max_ulp.max(ulp_distance(*mag, cpu_mag_bits(*mag2)));
        }
        assert_eq!(max_ulp, 0);
        print_timing("sqrt_mag2_perf0_fixed_mag2_plus_f_34k", N34K, &outcome);
        println!("sqrt_mag2_perf0_combined_max_ulp={max_ulp}");
    });
}

#[test]
fn sqrt_mag2_perf0_scaled_smoke() {
    with_gpu(|ctx| {
        for n in [10_000usize, 34_000, 100_000] {
...
#[test]
fn sqrt_mag2_perf0_no_readback_dispatch_proxy_34k() {
    with_gpu(|ctx| {
        const REPEAT: u32 = 32;
        let pairs = mobile_simthing_fixed_pairs(N34K);
        let outcome = run_bench(ctx, &pairs, BenchPath::CombinedQ16, REPEAT, false);
        let per_dispatch_ms = outcome.elapsed_ms / REPEAT as f64;
        let per_entity_us = per_dispatch_ms * 1000.0 / N34K as f64;
        println!(
            "sqrt_mag2_perf0_no_readback_proxy: rows={N34K} repeat={REPEAT} dispatches={} includes_readback=false total_ms={:.3} per_dispatch_ms={per_dispatch_ms:.3} per_entity_us={per_entity_us:.4}",
            outcome.dispatches, outcome.elapsed_ms
        );
    });
}

#[test]
fn sqrt_mag2_perf0_candidate_a_q12_dense_corpus() {
    with_gpu(|ctx| {
        let rows = dense_fixed_pairs_q12();
        let outcome = run_bench(ctx, &rows, BenchPath::CombinedQ12, 1, true);
        let mut exact_mag2 = 0usize;
        let mut exact_mag = 0usize;
        for ((sum, mag2, mag), (dx, dy)) in outcome.outputs.iter().zip(rows.iter()) {
            let cpu_sum = cpu_mag2_sum(*dx, *dy);
            let cpu_mag2 = cpu_mag2_bits_q12(*dx, *dy);
            assert_eq!(*sum, cpu_sum);
            assert_eq!(*mag2, cpu_mag2);
            exact_mag2 += 1;
            if ulp_distance(*mag, cpu_mag_bits(*mag2)) == 0 {
                exact_mag += 1;
            }
        }
        let mut q12_vs_q16_mag2_diff = 0usize;
        for &dx in gradient_samples().iter() {
            for &dy in gradient_samples().iter() {
                let q16 = (f32_to_q16(dx), f32_to_q16(dy));
                let q12 = (f32_to_q12(dx), f32_to_q12(dy));
                if cpu_mag2_bits_q12(q12.0, q12.1) != cpu_mag2_bits_q16(q16.0, q16.1) {
                    q12_vs_q16_mag2_diff += 1;
                }
            }
        }
        let outcome_34k = run_bench(
            ctx,
            &mobile_simthing_fixed_pairs_q12(N34K),
            BenchPath::CombinedQ12,
            1,
            true,
        );
...
#[test]
fn sqrt_mag2_perf0_candidate_c_split_kernels_34k() {
    with_gpu(|ctx| {
        let pairs = mobile_simthing_fixed_pairs(N34K);
        let outcome = run_bench(ctx, &pairs, BenchPath::SplitMag2ThenSqrt, 1, true);
        let mut max_ulp = 0u32;
        for ((sum, mag2, mag), (dx, dy)) in outcome.outputs.iter().zip(pairs.iter()).take(512) {
            assert_eq!(*sum, cpu_mag2_sum(*dx, *dy));
            assert_eq!(*mag2, cpu_mag2_bits_q16(*dx, *dy));
        ...
        assert_eq!(max_ulp, 0);
        print_timing("sqrt_mag2_perf0_candidate_c_split_34k", N34K, &outcome);
    });
}

#[test]
fn sqrt_mag2_perf0_correctness_edge_and_dense_q16() {
    with_gpu(|ctx| {
        let edge: Vec<(i32, i32)> = vec![
            (0, 0),
            (f32_to_q16(3.0), f32_to_q16(4.0)),
            (f32_to_q16(16.0), f32_to_q16(16.0)),
        ];
        let dense = dense_fixed_pairs_q16();
        for (label, rows) in [("edge", edge), ("dense", dense)] {
            let outcome = run_bench(ctx, &rows, BenchPath::CombinedQ16, 1, true);
            for ((sum, mag2, mag), (dx, dy)) in outcome.outputs.iter().zip(rows.iter()) {
                assert_eq!(*sum, cpu_mag2_sum(*dx, *dy));
                assert_eq!(*mag2, cpu_mag2_bits_q16(*dx, *dy));
