//! FIELD_POLICY-ACT-2 — Fixture-local proposal admission records from ACT-1 numeric summaries (Tier-2, test-only).
//!
//! Consumes proposal_summary; emits fixture-local admission records under fixed integer thresholds.
...
use simthing_spec::{
    is_field_policy_act2_proposal_admission_records_descriptor, landed_jit_kernel_descriptors,
    validate_kernel_descriptor_admission, MappingExecutionProfile,
    PhaseEFixtureProposalAdmissionAuthority, FIELD_POLICY_ACT1_ADMITTED_TABLE_SIZE,
    FIELD_POLICY_ACT2_DESCRIPTOR_ID, FIELD_POLICY_EVENT1_CODE_COUNT,
};
...
fn edge_admission_cases() -> Vec<(ProposalSummary, AdmissionRulesGpu, &'static str)> {
    let base_rules = AdmissionRulesGpu {
        admission_code: 5001,
        min_accepted: 2,
        min_max_score: 100,
        max_invalid: 1,
    ...
    let mut below_count = zero;
    below_count.accepted_count = 1;
    below_count.max_score = 200;
    let mut at_count = below_count;
    at_count.accepted_count = 2;
    let mut above_count = at_count;
    above_count.accepted_count = 5;
    let mut below_score = at_count;
    below_score.max_score = 99;
    let mut at_score = at_count;
    at_score.max_score = 100;
    let mut above_score = at_count;
    above_score.max_score = 5000;
    let mut invalid_ok = at_count;
    invalid_ok.invalid_count = 1;
    let mut invalid_high = at_count;
    invalid_high.invalid_count = 2;
    let mut input_ovf = at_count;
    input_ovf.flags = FLAG_INPUT_OVERFLOW;
    let mut sum_ovf = at_count;
    sum_ovf.flags = FLAG_SUM_OVERFLOW;
    let mut neg_score = at_count;
    neg_score.max_score = -50;
    let mut large_score = at_count;
    large_score.max_score = 2_000_000;
    vec![
        (zero, base_rules, "zero_accepted"),
        (below_count, base_rules, "accepted_below"),
        (at_count, base_rules, "accepted_equal"),
        (above_count, base_rules, "accepted_above"),
        (below_score, base_rules, "max_score_below"),
        (at_score, base_rules, "max_score_equal"),
        (above_score, base_rules, "max_score_above"),
        (invalid_ok, base_rules, "invalid_below_max"),
        (invalid_high, base_rules, "invalid_above_max"),
        (input_ovf, base_rules, "input_overflow"),
        (sum_ovf, base_rules, "summary_overflow"),
        (neg_score, base_rules, "negative_max_score"),
        (large_score, base_rules, "large_positive_score"),
    ]
}

fn dense_summaries() -> Vec<ProposalSummary> {
    (0..64)
        .map(|idx| ProposalSummary {
            accepted_count: 1 + idx % 5,
            ignored_count: idx % 3,
            invalid_count: idx % 4,
            summary_lo: (idx * 37) as u32,
            summary_hi: 0,
            max_score: (idx as i32).wrapping_mul(101),
            flags: if idx % 11 == 0 { FLAG_SUM_OVERFLOW } else { 0 },
        })
        .collect()
}
...
#[test]
fn field_policy_act2_wgsl_semantic_free() {
    let wgsl = emit_admit_wgsl();
    ...
    assert!(wgsl.contains("admit_pass"));
    assert!(wgsl.contains("admission_record"));
    assert!(!wgsl.contains("scheduler"));
    println!("field_policy_act2_wgsl: semantic_free=true ordering={ORDERING_CLASS}");
}

#[test]
fn field_policy_act2_admission_edge_rows() {
    with_gpu(|ctx| {
        for (summary, rules, label) in edge_admission_cases() {
            let outcome = run_admit_gpu(ctx, summary, &rules, 1);
            let exp = cpu_admit(summary, &rules);
            assert!(admission_eq(outcome.admission, exp), "{label}");
            println!(
                "field_policy_act2_edge[{label}]: code={} flags={} admitted={} ordering={ORDERING_CLASS}",
                outcome.admission.admission_code,
                outcome.admission.flags,
                outcome.admission.flags & FLAG_ADM_ADMITTED != 0,
            );
        }
    });
}
...
#[test]
fn field_policy_act2_dense_admission_corpus() {
    let rules = default_admission_rules();
    with_gpu(|ctx| {
        for (idx, summary) in dense_summaries().into_iter().enumerate() {
            let outcome = run_admit_gpu(ctx, summary, &rules, 1);
            let exp = cpu_admit(summary, &rules);
            assert!(admission_eq(outcome.admission, exp), "dense[{idx}]");
        }
        println!(
            "field_policy_act2_dense: rows={} ordering={ORDERING_CLASS}",
            64
        );
    });
}
...
#[test]
fn field_policy_act2_act1_to_admission_smoke() {
    ...
    with_gpu(|ctx| {
        ...
        let outcome = run_bucket_reduce_propose_consume_gpu(
            ctx, &compact, cap, &rules, prop_cap, &admitted, admitted_n, &adm_rules, 1,
        );
        verify_chain_admission(
            &outcome, &compact, cap, &rules, prop_cap, &admitted, admitted_n, &adm_rules,
        );
        println!(
            "field_policy_act2_act1_smoke: proposal_count={} accepted={} admission_code={} flags={} dispatches=5 ordering={ORDERING_CLASS}",
...
#[test]
fn field_policy_act2_full_chain_admission_smoke() {
    let rules = rules_for_smoke();
    let admitted = default_admitted_table();
    let admitted_n = admitted_count();
    let adm_rules = smoke_admission_rules();
    with_gpu(|ctx| {
        ...
        let (_, counts) = cpu_bucket_from_compact(&compact, cap);
        verify_chain_admission(
            &outcome, &compact, cap, &rules, prop_cap, &admitted, admitted_n, &adm_rules,
        );
        println!(
            "field_policy_act2_full_chain: compact={} event_count={} bucket_counts={counts:?} proposal_count={} accepted={} admission_code={} flags={} overflow={} ordering={ORDERING_CLASS}",
            compact.len(),
            compact.len(),
            outcome.proposal_count,
            outcome.summary.accepted_count,
...
#[test]
fn field_policy_act2_perf_34k_admission_records() {
    let rules = rules_for_smoke();
    let admitted = default_admitted_table();
    let admitted_n = admitted_count();
    let adm_rules = smoke_admission_rules();
    with_gpu(|ctx| {
        ...
        println!(
            "field_policy_act2_34k: dispatches=5 elapsed_ms={elapsed_ms:.3} readback=true event_count={N} proposal_count={} accepted={} admission_code={} overflow={} per_record_us={per_record_us:.4} ordering={ORDERING_CLASS}",
            outcome.proposal_count,
            outcome.summary.accepted_count,
            outcome.admission.admission_code,
            outcome.proposal_overflow,
...
#[test]
fn field_policy_act2_perf_34k_warm_repeated_dispatch() {
    let rules = rules_for_smoke();
    let admitted = default_admitted_table();
    let admitted_n = admitted_count();
    let adm_rules = smoke_admission_rules();
    with_gpu(|ctx| {
        ...
        let compact = balanced_12_records(N);
        let outcome = run_bucket_reduce_propose_consume_gpu(
            ctx, &compact, CAP, &rules, PROP_CAP, &admitted, admitted_n, &adm_rules, REPEATS,
        );
        ...
        println!(
            "field_policy_act2_34k_warm: repeats={REPEATS} total_ms={total_ms:.3} per_pipeline_ms={per_pipeline_ms:.4} per_record_us={per_record_us:.4} admission_code={} flags={} overflow={} ordering={ORDERING_CLASS}",
            outcome.admission.admission_code,
            outcome.admission.flags,
            outcome.proposal_overflow,
        );
...
#[test]
fn field_policy_act2_no_default_runtime_wiring() {
    assert_eq!(
    ...
    let _ = PhaseEFixtureProposalAdmissionAuthority::ExactAuthoritative;
    println!(
        "field_policy_act2_wiring: default_off=true descriptor={FIELD_POLICY_ACT2_DESCRIPTOR_ID}"
    );
}
