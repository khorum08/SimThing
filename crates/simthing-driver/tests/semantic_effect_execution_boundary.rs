//! SEMANTIC-EFFECT-EXECUTION-BOUNDARY-0 — semantic execution boundary driver proofs.
...
use simthing_driver::{
    ...
    compile_runtime_tick_shell_plan, compile_semantic_effect_execution_boundary_plan,
    compile_semantic_local_effects_plan,
};
use simthing_spec::{
    serialize_scenario_authority, RuntimeTickId, SemanticEffectExecutionKind,
...
#[test]
fn semantic_execution_boundary_compile_composes_recursive_semantic_source_plan() {
    let spec = build_owner_silo_disburse_down_scoped_spec();
    let plan = compile_semantic_effect_execution_boundary_plan(
        &spec,
        TICK_ONE,
    ...
    assert!(
        plan.semantic_recursive_source_plan
...
#[test]
fn semantic_execution_boundary_compile_recursive_mode_produces_execution_records() {
    let spec = build_owner_silo_disburse_down_scoped_spec();
    let plan = compile_semantic_effect_execution_boundary_plan(
        &spec,
        TICK_ONE,
    ...
    assert!(plan.execution_report.execution_record_count > 0);
    assert!(plan.execution_report.executed_amount_total > 0);
    assert!(!plan.execution_report.execution_records.is_empty());
    assert!(plan.semantic_execution_boundary_proven);
    assert!(plan
        .execution_report
        .execution_records
        .iter()
...
#[test]
fn semantic_execution_boundary_compile_does_not_change_default_runtime_rf_tick_plan() {
    let spec = build_owner_silo_disburse_down_scoped_spec();
    let before = compile_runtime_rf_tick_plan(&spec).expect("before");
    let _plan = compile_semantic_effect_execution_boundary_plan(
        &spec,
        TICK_ONE,
        SemanticEffectExecutionSourceMode::RecursiveSemanticLocalEffectsSelectable,
        REPLAY_THREE,
...
#[test]
fn semantic_execution_boundary_compile_does_not_change_default_tick_shell_plan() {
    let spec = build_owner_silo_disburse_down_scoped_spec();
...
#[test]
fn semantic_execution_boundary_compile_does_not_change_default_local_effect_application_plan() {
    let spec = build_owner_silo_disburse_down_scoped_spec();
    let before =
...
#[test]
fn semantic_execution_boundary_compile_does_not_change_default_semantic_local_effects_plan() {
    let spec = build_owner_silo_disburse_down_scoped_spec();
    let before =
...
#[test]
fn semantic_execution_boundary_compile_defers_participant_property_mutation() {
    let spec = build_owner_silo_disburse_down_scoped_spec();
    let plan = compile_semantic_effect_execution_boundary_plan(
        &spec,
        TICK_ONE,
...
#[test]
fn semantic_execution_boundary_compile_reports_no_new_gpu_primitive_required() {
    let spec = build_owner_silo_disburse_down_scoped_spec();
    let plan = compile_semantic_effect_execution_boundary_plan(
        &spec,
        TICK_ONE,
...
#[test]
fn semantic_execution_boundary_compile_reports_no_fused_recursive_rf_kernel() {
    let spec = build_owner_silo_disburse_down_scoped_spec();
    let plan = compile_semantic_effect_execution_boundary_plan(
        &spec,
        TICK_ONE,
...
#[test]
fn semantic_execution_boundary_compile_preserves_scenario_authority() {
    let spec = build_owner_silo_disburse_down_scoped_spec();
