//! SEAD-ACT-1 — Default-off Phase E fixture consumer for ACT-0 numeric proposal records (Tier-2, test-only).
//!
...
#[test]
fn sead_act1_wgsl_semantic_free() {
    ...
    println!("sead_act1_wgsl: semantic_free=true ordering={ORDERING_CLASS}");
}

#[test]
fn sead_act1_consumer_edge_rows() {
    ...
    with_gpu(|ctx| {
        for (count, overflow, props, cap, label) in edge_consumer_cases() {
            ...
            println!(
                "sead_act1_edge[{label}]: accepted={} ignored={} invalid={} flags={} ordering={ORDERING_CLASS}",
...
#[test]
fn sead_act1_dense_consumer_corpus() {
    let admitted = default_admitted_table();
    let admitted_n = admitted_count();
    with_gpu(|ctx| {
        ...
        println!(
            "sead_act1_dense: accepted={} invalid={} max={} ordering={ORDERING_CLASS}",
            outcome.summary.accepted_count,
            outcome.summary.invalid_count,
...
#[test]
fn sead_act1_act0_to_phase_e_consumer_smoke() {
...
#[test]
fn sead_act1_full_chain_phase_e_consumer_smoke() {
    let rules = rules_for_smoke();
    let admitted = default_admitted_table();
    let admitted_n = admitted_count();
    with_gpu(|ctx| {
...
#[test]
fn sead_act1_perf_34k_phase_e_consumer() {
    let rules = rules_for_smoke();
    let admitted = default_admitted_table();
    let admitted_n = admitted_count();
    with_gpu(|ctx| {
...
#[test]
fn sead_act1_perf_34k_warm_repeated_dispatch() {
    let rules = rules_for_smoke();
    let admitted = default_admitted_table();
    let admitted_n = admitted_count();
    with_gpu(|ctx| {
        ...
        println!(
            "sead_act1_34k_warm: repeats={REPEATS} total_ms={total_ms:.3} per_pipeline_ms={per_pipeline_ms:.4} per_record_us={per_record_us:.4} accepted={} overflow={} ordering={ORDERING_CLASS}",
            outcome.summary.accepted_count,
            outcome.proposal_overflow,
        );
    });
