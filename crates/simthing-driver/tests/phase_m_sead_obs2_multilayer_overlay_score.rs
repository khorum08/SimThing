//! SEAD-OBS-2 — Multi-layer GPU-resident observer overlay score probe (Tier-2, test-only).
//!
...
const LAYER_COUNT: usize = SEAD_OBS2_LAYER_COUNT as usize;
...
fn run_multilayer_batch_repeated(
    ...
    let module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("sead_obs2_multilayer"),
        source: wgpu::ShaderSource::Wgsl(wgsl.into()),
    });
    let bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("sead_obs2_bgl"),
    ...
    for _ in 0..repeat_dispatches {
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("sead_obs2_enc"),
        });
        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("sead_obs2_pass"),
...
#[test]
fn sead_obs2_wgsl_semantic_free() {
    ...
    println!(
        "sead_obs2_wgsl: semantic_free=true layers={LAYER_COUNT} F_hash={SQRT_F_ARTIFACT_HASH}"
    );
}

#[test]
fn sead_obs2_dense_multilayer_correctness() {
    with_gpu(|ctx| {
...
#[test]
fn sead_obs2_perf_34k_multilayer_overlay_score() {
    with_gpu(|ctx| {
        ...
        println!(
            "sead_obs2_perf_34k: rows={N} layers={LAYER_COUNT} dispatches=1 includes_readback=true elapsed_ms={ms:.3} per_row_us={per_row_us:.4} layer_mags={layer_mags} spot_mag_max_ulp={spot_mag_ulp} score_authority=ApproximateDiagnosticF32"
        );
    });
}

#[test]
fn sead_obs2_perf_34k_warm_repeated_dispatch() {
    with_gpu(|ctx| {
        const N: usize = 34_000;
        const REPEAT: u32 = 32;
        let rows = mobile_multilayer_rows(N);
        ...
        println!("sead_obs2_warm_compare: SEAD-OBS-1 warm ~0.129 ms/dispatch single-layer");
    });
}

#[test]
fn sead_obs2_score_authority_is_approximate() {
    let wgsl = emit_multilayer_wgsl(1);
    ...
    println!("sead_obs2_score_authority: ApproximateDiagnosticF32 per_layer_mag=Exact");
}

#[test]
fn sead_obs2_no_default_runtime_wiring() {
    assert_eq!(
    ...
    println!("sead_obs2_wiring: descriptor=landed no_scheduler_no_bridge");
}
