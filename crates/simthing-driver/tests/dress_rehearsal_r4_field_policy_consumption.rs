...
fn r4_opening_status_matches_track() {
    ...
    assert!(track.contains("R4 — FIELD_POLICY field-consumption + exact sqrt (EC2)"));
...
fn r4_consumes_r1_r2_r3_contracts() {
    let admitted = report();
    assert_eq!(admitted.id, DRESS_REHEARSAL_R4_FIELD_POLICY_CONSUMPTION_ID);
    assert!(DRESS_REHEARSAL_R4_FIELD_POLICY_CONSUMPTION_STATUS_PASS.contains("IMPLEMENTED / PASS"));
...
fn r4_opt_in_default_off() {
    let disabled = run_dress_rehearsal_r4_field_policy_consumption(
        &DressRehearsalR4Input::default_simsession(),
    );
    ...
    let rejected = run_dress_rehearsal_r4_field_policy_consumption(&default_on);
