//! SEAD-OBS-4 — GPU-resident threshold event emission from exact observer scores (Tier-2, test-only).
//!
...
const LAYER_COUNT: usize = SEAD_OBS4_LAYER_COUNT as usize;
...
fn run_threshold_batch(
    ...
    let module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("sead_obs4_threshold_event"),
        source: wgpu::ShaderSource::Wgsl(wgsl.into()),
    });
    let bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("sead_obs4_bgl"),
    ...
    for _ in 0..repeat_dispatches {
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("sead_obs4_enc"),
        });
        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("sead_obs4_pass"),
...
#[test]
fn sead_obs4_wgsl_semantic_free() {
    ...
    println!("sead_obs4_wgsl: semantic_free=true F_hash={SQRT_F_ARTIFACT_HASH}");
}

#[test]
fn sead_obs4_threshold_edge_rows() {
    with_gpu(|ctx| {
...
#[test]
fn sead_obs4_threshold_dense_corpus() {
    with_gpu(|ctx| {
        ...
        let (score_exact, state_exact, event_exact, overflow, events_up) =
            verify_outputs(&outputs, &rows);
        println!(
            "sead_obs4_dense: rows={} score_exact={score_exact} state_exact={state_exact} event_exact={event_exact} overflow={overflow} events_up={events_up}",
            rows.len()
        );
...
#[test]
fn sead_obs4_perf_34k_threshold_event() {
    with_gpu(|ctx| {
        ...
        println!(
            "sead_obs4_perf_34k: rows={N} dispatches=1 includes_readback=true elapsed_ms={ms:.3} per_row_us={per_row_us:.4} spot_score_exact={score_exact}/512 spot_state_exact={state_exact}/512 spot_event_exact={event_exact}/512 overflow={overflow} total_events={total_events} events_up_sample={events_up}"
        );
    });
}

#[test]
fn sead_obs4_perf_34k_warm_repeated_dispatch() {
    with_gpu(|ctx| {
        const N: usize = 34_000;
        const REPEAT: u32 = 32;
        let rows = mobile_threshold_rows(N);
        ...
        println!("sead_obs4_warm_compare: SEAD-OBS-3 warm ~0.278 ms/dispatch fixed score");
    });
}

#[test]
fn sead_obs4_event_authority_is_exact_deterministic() {
    let desc = landed_jit_kernel_descriptors()
        .into_iter()
        .find(|d| d.id == SEAD_OBS4_DESCRIPTOR_ID)
...
#[test]
fn sead_obs4_no_default_runtime_wiring() {
    assert_eq!(
    ...
    println!("sead_obs4_wiring: descriptor=landed no_scheduler_no_bridge");
}
