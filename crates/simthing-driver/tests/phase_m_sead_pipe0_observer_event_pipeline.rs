//! SEAD-PIPE-0 — integrated GPU observer threshold event + compaction pipeline (Tier-2, test-only).
//!
...
#[test]
fn sead_pipe0_wgsl_semantic_free() {
    let wgsl = emit_pipeline_wgsl();
    ...
    println!(
        "sead_pipe0_wgsl: semantic_free=true F_hash={SQRT_F_ARTIFACT_HASH} ordering={ORDERING_CLASS}"
    );
}

#[test]
fn sead_pipe0_edge_rows() {
    with_gpu(|ctx| {
        for (rows, capacity, label) in edge_pipeline_cases() {
            ...
            println!(
                "sead_pipe0_edge[{label}]: rows={} capacity={capacity} event_count={} overflow={} score_exact={score_exact}/{} state_exact={state_exact}/{} event_exact={event_exact}/{} membership_ok={membership} ordering={ORDERING_CLASS}",
                rows.len(),
                outcome.event_count,
                outcome.overflow,
                rows.len(),
...
fn sead_pipe0_dense_corpus() {
    with_gpu(|ctx| {
        ...
        let (score_exact, state_exact, event_exact) =
            verify_threshold_rows(&outcome.event_rows, &rows);
        println!(
            "sead_pipe0_dense: rows={} event_count={} overflow={} score_exact={score_exact} state_exact={state_exact} event_exact={event_exact} membership={}/{} ordering={ORDERING_CLASS}",
            rows.len(),
            outcome.event_count,
            outcome.overflow,
            expected.len(),
...
#[test]
fn sead_pipe0_perf_34k_integrated_pipeline() {
    with_gpu(|ctx| {
        ...
        println!(
            "sead_pipe0_34k_integrated: rows={N} dispatches={} readback=true elapsed_ms={elapsed_ms:.3} per_row_us={per_row_us:.4} event_count={} overflow={} sample_score={score_exact}/512 sample_state={state_exact}/512 sample_event={event_exact}/512 membership={}/{} ordering={ORDERING_CLASS}",
...
#[test]
fn sead_pipe0_perf_34k_capacity_variants() {
    with_gpu(|ctx| {
        const N: usize = 34_000;
        let rows = mobile_observer_rows(N);
        let expected_rows = cpu_event_rows(&rows);
        ...
        for (capacity, label) in capacities {
            ...
            println!(
                "sead_pipe0_34k_cap[{label}]: capacity={capacity} event_count={} written={} overflow={} membership_ok={membership}",
                outcome.event_count,
                outcome.records.len(),
                outcome.overflow,
            );
...
#[test]
fn sead_pipe0_perf_34k_warm_repeated_dispatch() {
    with_gpu(|ctx| {
        const N: usize = 34_000;
        ...
        println!(
            "sead_pipe0_34k_warm: repeats={REPEATS} total_ms={total_ms:.3} per_pipeline_ms={per_pipeline_ms:.4} per_row_us={per_row_us:.4} dispatches={} event_count={} overflow={} membership={}/{} ordering={ORDERING_CLASS}",
            outcome.dispatch_count,
            outcome.event_count,
            outcome.overflow,
            expected.len(),
...
fn sead_pipe0_no_default_runtime_wiring() {
    ...
    println!(
        "sead_pipe0_wiring: default_off=true production_wiring=false descriptor={SEAD_PIPE0_DESCRIPTOR_ID}"
    );
}
