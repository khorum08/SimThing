//! FrontierV1-5 — Single-tick live GPU field agent route run toward FrontierV2 (Tier-2, test-only).
//!
//! Executes one live GPU-resident integrated field agent chain inside default-off FrontierV1:
...
pub const FRONTIER_V1_LIVE_FIELD_AGENT_REPLAY_FINGERPRINT: &str = "1653b84847be2dd2";
...
struct FrontierV1LiveFieldAgentRun {
    summary: FrontierV1LiveFieldAgentSummary,
    feedback: FrontierV1LiveFieldAgentFeedbackCandidate,
    cpu_oracle: FrontierV1LiveFieldAgentOracleOutput,
    threat: f32,
    urgency: f32,
...
fn run_frontier_v1_live_field_agent(ctx: &GpuContext) -> FrontierV1LiveFieldAgentRun {
    ...
    let (buckets, counts) =
        field_policy_v1_live_pipeline::cpu_bucket_from_compact(&compact, bucket_cap);
    ...
    let feedback_hash = hash_live_field_agent_feedback_candidate(feedback);
    let route_replay = build_route_replay_summary(&config, &skeleton);
    let route_hash = hash_route_replay_detail(route_replay);

    let summary = FrontierV1LiveFieldAgentSummary {
        mapping_summary_hash: mapping_hash,
        resource_flow_summary_hash: rf_hash,
        field_agent_summary_hash: field_agent_hash,
        feedback_candidate_hash: feedback_hash,
        route_summary_hash: route_hash,
        overflow_flags,
        mapping_status: FrontierV1LiveFieldAgentFieldStatus::GpuVerified,
        resource_flow_status: FrontierV1LiveFieldAgentFieldStatus::GpuVerified,
        field_agent_resource_route_status: FrontierV1LiveFieldAgentFieldStatus::GpuVerified,
        feedback_candidate_status: FrontierV1LiveFieldAgentFieldStatus::FixtureOnly,
        full_field_policy_pipe_status: FrontierV1LiveFieldAgentFieldStatus::GpuVerified,
        structural_route_status: FrontierV1LiveFieldAgentFieldStatus::ReplayAccepted,
        movement_route_status: FrontierV1LiveFieldAgentFieldStatus::ReplayAccepted,
        frontier_v2_status: FrontierV2Status::NotImplemented,
    };

    let cpu_oracle = cpu_live_field_agent_oracle(
        &skeleton,
        &config,
        0,
        0,
...
#[test]
fn frontier_v1_5_happy_path_live_field_agent_resource_route_runs() {
    with_gpu(|ctx| {
        let (skeleton, _) = smoke_fixture();
        let run = run_frontier_v1_live_field_agent(ctx);

        assert!(validate_frontier_v1_admission(&skeleton).accepted);
        assert_eq!(
            run.summary.mapping_status,
            FrontierV1LiveFieldAgentFieldStatus::GpuVerified
        );
        assert_eq!(
        ...
        assert_eq!(
            run.summary.field_agent_resource_route_status,
            FrontierV1LiveFieldAgentFieldStatus::GpuVerified
        );
        ...
        assert_eq!(
            ...
            FrontierV1LiveFieldAgentFieldStatus::FixtureOnly
        );
        assert_eq!(
        ...
        println!(
            "frontier_v1_5_happy: fixture_id={FRONTIER_V1_LIVE_FIELD_AGENT_FIXTURE_ID} fp={} threat={} urgency={} events={} proposals={}",
...
#[test]
fn frontier_v1_5_live_field_agent_cpu_oracle_parity() {
    with_gpu(|ctx| {
        let run = run_frontier_v1_live_field_agent(ctx);
...
#[test]
fn frontier_v1_5_live_field_agent_replay_reproducibility() {
    with_gpu(|ctx| {
        let run_a = run_frontier_v1_live_field_agent(ctx);
        let run_b = run_frontier_v1_live_field_agent(ctx);
        ...
        assert_eq!(
            ...
            FRONTIER_V1_LIVE_FIELD_AGENT_REPLAY_FINGERPRINT
        );

        println!(
            "frontier_v1_5_replay: fp={} fixture_id={FRONTIER_V1_LIVE_FIELD_AGENT_FIXTURE_ID}",
            run_a.summary.combined_hex()
        );
    });
}
...
fn frontier_v1_5_defaults_remain_disabled() {
    ...
    println!(
        "frontier_v1_5_defaults: disabled=true fixture_id={FRONTIER_V1_LIVE_FIELD_AGENT_FIXTURE_ID}"
    );
}
...
fn frontier_v1_5_rejects_resource_route_bypass() {
    ...
    let _ = config;
    println!(
        "frontier_v1_5_bypass: rejects=true fixture_id={FRONTIER_V1_LIVE_FIELD_AGENT_FIXTURE_ID}"
    );
}
...
fn frontier_v1_5_coupling_rejects_non_frontier_profile() {
    ...
    let _ = config;
    println!(
        "frontier_v1_5_coupling: frontier_only=true fixture_id={FRONTIER_V1_LIVE_FIELD_AGENT_FIXTURE_ID}"
    );
}
...
fn frontier_v1_5_deferred_features_reject() {
    ...
    println!(
        "frontier_v1_5_deferred: rejects=true fixture_id={FRONTIER_V1_LIVE_FIELD_AGENT_FIXTURE_ID}"
    );
}
...
fn frontier_v1_5_no_simthing_sim_semantic_awareness() {
    ...
    println!(
        "frontier_v1_5_sim: semantic_free=true fixture_id={FRONTIER_V1_LIVE_FIELD_AGENT_FIXTURE_ID}"
    );
}
...
fn frontier_v1_5_no_unauthorized_gpu_primitive() {
    ...
    println!(
        "frontier_v1_5_gpu: no_new_primitive=true fixture_id={FRONTIER_V1_LIVE_FIELD_AGENT_FIXTURE_ID}"
    );
}
...
fn frontier_v1_5_no_implementer_self_acceptance() {
    let report_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../docs/tests/phase_m_frontier_v1_5_live_field_agent_route_results.md");
    ...
    println!(
        "frontier_v1_5_no_self_accept: report_ok=true fixture_id={FRONTIER_V1_LIVE_FIELD_AGENT_FIXTURE_ID}"
    );
}
