//! SEAD-ACT-0 — GPU-resident numeric action proposals from EVENT-2 bucket reductions (Tier-2, test-only).
//!
...
#[test]
fn sead_act0_wgsl_semantic_free() {
    let wgsl = format!(
        "{}\n{}\n{}",
        emit_proposal_wgsl(),
        emit_reduction_wgsl(),
    ...
    println!("sead_act0_wgsl: semantic_free=true ordering={ORDERING_CLASS}");
}

#[test]
fn sead_act0_proposal_edge_rows() {
...
#[test]
fn sead_act0_dense_proposal_corpus() {
    ...
    with_gpu(|ctx| {
        ...
        println!(
            "sead_act0_dense: count={} overflow={} membership=exact ordering={ORDERING_CLASS}",
...
#[test]
fn sead_act0_event2_to_proposal_smoke() {
    ...
    with_gpu(|ctx| {
        ...
        assert!(membership_exact(&outcome.proposals, &exp_props));
        println!(
            "sead_act0_event2_smoke: records={} proposal_count={} overflow={} dispatches=3 ordering={ORDERING_CLASS}",
            compact.len(),
            outcome.proposal_count,
            outcome.proposal_overflow,
        );
...
#[test]
fn sead_act0_pipe_to_proposal_smoke() {
    let rules = rules_for_smoke();
    with_gpu(|ctx| {
        ...
        println!(
            "sead_act0_pipe_smoke: compact={} event_count={} bucket_counts={counts:?} proposal_count={} overflow={} ordering={ORDERING_CLASS}",
            compact.len(),
            compact.len(),
            outcome.proposal_count,
            outcome.proposal_overflow,
...
#[test]
fn sead_act0_perf_34k_numeric_proposals() {
    let rules = rules_for_smoke();
    with_gpu(|ctx| {
        ...
        assert_eq!(outcome.proposal_overflow, exp_ovf);
        println!(
            "sead_act0_34k: dispatches=3 elapsed_ms={elapsed_ms:.3} readback=true event_count={N} proposal_count={} overflow={} per_record_us={per_record_us:.4} ordering={ORDERING_CLASS}",
            outcome.proposal_count,
            outcome.proposal_overflow,
        );
    });
...
#[test]
fn sead_act0_perf_34k_warm_repeated_dispatch() {
    let rules = rules_for_smoke();
    with_gpu(|ctx| {
        const N: usize = 34_000;
        const CAP: u32 = 20_000;
        ...
        println!(
            "sead_act0_34k_warm: repeats={REPEATS} total_ms={total_ms:.3} per_pipeline_ms={per_pipeline_ms:.4} per_record_us={per_record_us:.4} proposal_count={} overflow={} ordering={ORDERING_CLASS}",
            outcome.proposal_count,
            outcome.proposal_overflow,
        );
    });
...
fn sead_act0_no_default_runtime_wiring() {
    ...
    assert!(is_sead_act0_numeric_proposals_descriptor(&desc));
