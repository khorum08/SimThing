...
#[test]
fn control_0080_1_rejects_direct_terran_move() {
    let rejected = with_commands(vec![Control0081Command::DirectTerranMove]);
    assert!(!rejected.admitted);
    assert!(rejected
        .rejected_commands
        .iter()
        .any(|cmd| cmd.diagnostic == "direct_terran_move_rejected"));
...
#[test]
fn control_0080_1_rejects_direct_pirate_move() {
    let rejected = with_commands(vec![Control0081Command::DirectPirateMove]);
    assert!(!rejected.admitted);
    assert!(rejected
        .rejected_commands
        .iter()
...
#[test]
fn control_0080_1_rejects_external_boundary_request() {
    let rejected = with_commands(vec![Control0081Command::ExternalBoundaryRequest]);
    assert!(!rejected.admitted);
    assert!(rejected
        .rejected_commands
        .iter()
...
#[test]
fn control_0080_1_rejects_player_command_loop() {
    let mut input = Control0081AdmissionInput::explicit_opt_in();
    input.surface.player_command_loop = true;
    let rejected = admit_control_0080_1(&input);
    assert!(!rejected.admitted);
    assert!(rejected.diagnostics.contains(&"player_command_loop"));
}

#[test]
fn control_0080_1_rejects_ui_framework() {
    let mut input = Control0081AdmissionInput::explicit_opt_in();
...
#[test]
fn control_0080_1_rejects_realtime_loop() {
    let mut input = Control0081AdmissionInput::explicit_opt_in();
...
#[test]
fn control_0080_1_rejects_global_default_schedule() {
    let mut input = Control0081AdmissionInput::explicit_opt_in();
...
#[test]
fn control_0080_1_rejects_semantic_or_raw_wgsl() {
...
#[test]
fn control_0080_1_rejects_new_shader_or_gpu_kernel() {
...
#[test]
fn control_0080_1_rejects_hard_currency_markets_trade_aibudget() {
...
#[test]
fn control_0080_1_rejects_nested_resource_flow() {
...
#[test]
fn control_0080_1_rejects_clausething_dependency() {
