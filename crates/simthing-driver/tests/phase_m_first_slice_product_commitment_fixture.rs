...
#[test]
fn product_commitment_invalid_specs_reject() {
    let base = commitment_spec();

    let mut nan_threshold = base.clone();
    nan_threshold.commitment.as_mut().unwrap().threshold = f32::NAN;
    assert_region_field_err(&nan_threshold, "commitment threshold must be finite");

    let mut infinite_threshold = base.clone();
    infinite_threshold.commitment.as_mut().unwrap().threshold = f32::INFINITY;
    assert_region_field_err(&infinite_threshold, "commitment threshold must be finite");
    ...
    let mut missing_parent_formula = base.clone();
    missing_parent_formula.parent_formula = None;
    assert_region_field_err(
        &missing_parent_formula,
        "commitment requires parent_formula field_urgency",
    );

    let mut wrong_parent_formula = base.clone();
    ...
    wrong_parent_slot.commitment.as_mut().unwrap().parent_slot = 99;
    assert_region_field_err(
        &wrong_parent_slot,
    ...
    wrong_col.commitment.as_mut().unwrap().urgency_col = 5;
    assert_region_field_err(&wrong_col, "commitment urgency_col must be 4");

    let wrong_direction =
        COMMITMENT_FIXTURE_RON.replace("direction: Upward", "direction: Downward");
    assert!(deserialize_region_field_ron(&wrong_direction).is_err());

    let mut atlas_request = base;
