...
#[test]
fn gradient_follow_0080_2_rejects_multi_step_pathfinding() {
    let mut input = GradientFollow0082Input::explicit_opt_in();
    input.forbidden.multi_step_pathfinding = true;
    let rejected = run_gradient_follow_0080_2(&input);
    assert!(!rejected.admitted);
    assert!(rejected.diagnostics.contains(&"multi_step_pathfinding"));
}

#[test]
fn gradient_follow_0080_2_rejects_forbidden_flags() {
    for mutate in [
        |i: &mut GradientFollow0082Input| i.forbidden.direct_movement_command = true,
        |i: &mut GradientFollow0082Input| i.forbidden.external_boundary_request = true,
        |i: &mut GradientFollow0082Input| i.surface.global_default_schedule = true,
        |i: &mut GradientFollow0082Input| i.forbidden.semantic_or_raw_wgsl = true,
        |i: &mut GradientFollow0082Input| i.forbidden.reopen_closed_0080_1_ladder = true,
    ] {
        let mut input = GradientFollow0082Input::explicit_opt_in();
        mutate(&mut input);
        let rejected = run_gradient_follow_0080_2(&input);
...
#[test]
fn gradient_follow_0080_2_rejects_unbounded_decay() {
    let mut input = GradientFollow0082Input::explicit_opt_in();
    input.decay_weights.base_retain_num = 10;
...
#[test]
fn gradient_follow_0080_2_rejects_start_node_out_of_range() {
    let mut input = GradientFollow0082Input::explicit_opt_in();
    input.start_node = 99;
    let rejected = run_gradient_follow_0080_2(&input);
    assert!(!rejected.admitted);
    assert!(rejected.diagnostics.contains(&"start_node_out_of_range"));
}
