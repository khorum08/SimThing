//! FIELD_POLICY V1 live GPU pipeline helpers (PIPE-0 + ACT-2 chain) for FrontierV1-5 fixtures (test-only).
//!
//! Extracted from `phase_m_field_policy_pipe0_observer_event_pipeline` and
//! `phase_m_field_policy_act2_proposal_admission_records`. No new semantic WGSL or descriptors.
...
use simthing_spec::{
    FIELD_POLICY_ACT1_ADMITTED_TABLE_SIZE, FIELD_POLICY_EVENT1_CODE_COUNT,
    FIELD_POLICY_PIPE0_LAYER_COUNT, MAG2_Q16_SCALE,
};
