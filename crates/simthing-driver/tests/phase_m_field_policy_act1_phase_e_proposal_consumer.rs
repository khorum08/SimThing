//! FIELD_POLICY-ACT-1 — Default-off Phase E fixture consumer for ACT-0 numeric proposal records (Tier-2, test-only).
//!
//! Consumes ACT-0 proposal buffers; emits numeric proposal_summary under fixed integer admission rules.
...
use simthing_spec::{
    is_field_policy_act1_phase_e_proposal_consumer_descriptor, landed_jit_kernel_descriptors,
    ...
    FIELD_POLICY_EVENT1_CODE_COUNT,
};
...
fn run_bucket_reduce_propose_consume_gpu(
    ...
    FullChainOutcome {
        ...
        proposal_count: meta[0],
        proposal_overflow: meta[1],
        summary: decode_summary(&sum_vec),
        elapsed,
    }
}
...
fn edge_consumer_cases() -> Vec<(u32, u32, Vec<ProposalRecord>, u32, &'static str)> {
    vec![
        (0, 0, vec![], 64, "zero_proposals"),
        (
            1,
            0,
            vec![ProposalRecord {
...
#[test]
fn field_policy_act1_wgsl_semantic_free() {
    let wgsl = emit_consume_wgsl();
    ...
    assert!(wgsl.contains("consume_pass"));
    assert!(wgsl.contains("proposal_summary"));
    assert!(!wgsl.contains("scheduler"));
    println!("field_policy_act1_wgsl: semantic_free=true ordering={ORDERING_CLASS}");
}

#[test]
fn field_policy_act1_consumer_edge_rows() {
    ...
    with_gpu(|ctx| {
        for (count, overflow, props, cap, label) in edge_consumer_cases() {
            ...
            println!(
                "field_policy_act1_edge[{label}]: accepted={} ignored={} invalid={} flags={} ordering={ORDERING_CLASS}",
                outcome.summary.accepted_count,
                outcome.summary.ignored_count,
                outcome.summary.invalid_count,
                outcome.summary.flags,
            );
        }
    });
}
...
#[test]
fn field_policy_act1_dense_consumer_corpus() {
    let admitted = default_admitted_table();
    let admitted_n = admitted_count();
    with_gpu(|ctx| {
        let (count, overflow, props) = dense_proposals();
        let outcome = run_consume_gpu(ctx, count, overflow, &props, 256, &admitted, admitted_n, 1);
        let exp = cpu_consume(count, overflow, &props, 256, &admitted, admitted_n);
        assert!(summary_eq(outcome.summary, exp));
        println!(
            "field_policy_act1_dense: accepted={} invalid={} max={} ordering={ORDERING_CLASS}",
            outcome.summary.accepted_count,
            outcome.summary.invalid_count,
...
#[test]
fn field_policy_act1_act0_to_phase_e_consumer_smoke() {
    ...
    with_gpu(|ctx| {
        ...
        println!(
            "field_policy_act1_act0_smoke: proposal_count={} accepted={} invalid={} overflow={} dispatches=4 ordering={ORDERING_CLASS}",
...
#[test]
fn field_policy_act1_full_chain_phase_e_consumer_smoke() {
    let rules = rules_for_smoke();
    let admitted = default_admitted_table();
    let admitted_n = admitted_count();
    with_gpu(|ctx| {
        ...
        println!(
            "field_policy_act1_full_chain: compact={} event_count={} bucket_counts={counts:?} proposal_count={} accepted={} overflow={} ordering={ORDERING_CLASS}",
            compact.len(),
            compact.len(),
            outcome.proposal_count,
            outcome.summary.accepted_count,
...
fn field_policy_act1_no_default_runtime_wiring() {
    ...
    let desc = landed_jit_kernel_descriptors()
        .into_iter()
        .find(|d| d.id == FIELD_POLICY_ACT1_DESCRIPTOR_ID)
        .expect("act1 descriptor");
    ...
    let _ = PhaseEProposalConsumerInputAuthority::ExactAuthoritativeUnordered;
    let _ = PhaseEProposalSummaryOrderAuthority::OrderInvariantExact;
    println!(
        "field_policy_act1_wiring: default_off=true descriptor={FIELD_POLICY_ACT1_DESCRIPTOR_ID}"
    );
}
