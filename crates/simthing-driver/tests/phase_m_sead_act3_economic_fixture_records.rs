//! SEAD-ACT-3 — Economic V1-style fixture substrate records from ACT-2 admission records (Tier-2, test-only).
//!
...
use simthing_spec::{
    ...
    PhaseEEconomicFixtureRecordAuthority, SEAD_ACT3_DESCRIPTOR_ID, SEAD_ACT1_ADMITTED_TABLE_SIZE,
...
fn run_fixture_gpu(
    ...
    let module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("sead_act3_fixture"),
        source: wgpu::ShaderSource::Wgsl(emit_fixture_wgsl().into()),
    });
    let bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("sead_act3_fixture_bgl"),
        entries: &[
            storage_ro(0),
            storage_rw(1),
            storage_ro(2),
    ...
    let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        label: Some("sead_act3_fixture"),
        layout: Some(&device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("sead_act3_fixture_pl"),
            bind_group_layouts: &[&bgl],
            push_constant_ranges: &[],
        })),
        module: &module,
...
fn run_bucket_reduce_propose_consume_gpu(
    ...
    enc2.copy_buffer_to_buffer(&fixture_buf, 0, &fixture_staging, 0, (FIXTURE_STRIDE * 4) as u64);
    queue.submit(Some(enc2.finish()));
...
#[test]
fn sead_act3_wgsl_semantic_free() {
    ...
    println!("sead_act3_wgsl: semantic_free=true ordering={ORDERING_CLASS}");
}

#[test]
fn sead_act3_fixture_record_edge_rows() {
    let table = default_mapping_table();
    ...
    with_gpu(|ctx| {
        for (admission, label) in edge_fixture_cases() {
            ...
            println!(
                "sead_act3_edge[{label}]: record_code={} flags={} emitted={} ordering={ORDERING_CLASS}",
...
#[test]
fn sead_act3_dense_fixture_record_corpus() {
    let table = default_mapping_table();
    let mapping_n = mapping_count();
    with_gpu(|ctx| {
        ...
        println!(
            "sead_act3_dense: rows={} ordering={ORDERING_CLASS}",
            64
        );
    });
}
...
#[test]
fn sead_act3_act2_to_fixture_record_smoke() {
    ...
    with_gpu(|ctx| {
        ...
        println!(
            "sead_act3_act2_smoke: admission_code={} admission_flags={} record_code={} record_flags={} priority={} tier={} dispatches=6 ordering={ORDERING_CLASS}",
...
#[test]
fn sead_act3_full_chain_fixture_record_smoke() {
    ...
    with_gpu(|ctx| {
        ...
        println!(
            "sead_act3_full_chain: compact={} event_count={} bucket_counts={counts:?} proposal_count={} accepted={} admission_code={} record_code={} record_flags={} overflow={} ordering={ORDERING_CLASS}",
            compact.len(),
            compact.len(),
...
#[test]
fn sead_act3_perf_34k_fixture_records() {
    ...
    with_gpu(|ctx| {
        ...
        println!(
            "sead_act3_34k: dispatches=6 elapsed_ms={elapsed_ms:.3} readback=true event_count={N} proposal_count={} accepted={} admission_code={} record_code={} overflow={} per_record_us={per_record_us:.4} ordering={ORDERING_CLASS}",
            outcome.proposal_count,
            outcome.summary.accepted_count,
            outcome.admission.admission_code,
            outcome.fixture.record_code,
...
#[test]
fn sead_act3_perf_34k_warm_repeated_dispatch() {
    let records = balanced_12_records(N);
    let rules = rules_for_smoke();
    let admitted = default_admitted_table();
    let admitted_n = admitted_count();
    ...
    with_gpu(|ctx| {
        ...
        println!(
            "sead_act3_34k_warm: repeats={REPEATS} total_ms={total_ms:.3} per_pipeline_ms={per_pipeline_ms:.4} per_record_us={per_record_us:.4} record_code={} overflow={} ordering={ORDERING_CLASS}",
            outcome.fixture.record_code,
            outcome.admission.flags & (FLAG_ADM_INPUT_OVF | FLAG_ADM_SUM_OVF),
        );
    });
...
#[test]
fn sead_act3_no_default_runtime_wiring() {
    ...
    println!("sead_act3_wiring: default_off=true descriptor={SEAD_ACT3_DESCRIPTOR_ID}");
}
