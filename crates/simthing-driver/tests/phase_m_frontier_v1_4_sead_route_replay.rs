//! FrontierV1-4 — Integrated SEAD V1 route replay acceptance (Tier-2, test-only).
//!
...
pub const FRONTIER_V1_SEAD_ROUTE_REPLAY_FINGERPRINT: &str = "4382ec7ef93c9174";
...
struct FrontierV1SeadRouteRun {
    ...
    sead_replay: FrontierV1SeadReplaySummary,
    sead_consumed: sead_v1_route_replay::SeadV1ConsumptionReport,
}

fn run_frontier_v1_sead_route_replay(
    ctx: &GpuContext,
    skeleton: &FrontierV1ScenarioSkeleton,
    config: &FrontierV1FixtureConfig,
) -> FrontierV1SeadRouteRun {
    ...
    let sead_replay = build_sead_replay_summary(&cpu_output);
...
#[test]
fn frontier_v1_4_happy_path_sead_route_replay_runs() {
    with_gpu(|ctx| {
        ...
        println!(
            "frontier_v1_4_happy: fixture_id={FRONTIER_V1_SEAD_ROUTE_FIXTURE_ID} fp={} routes=r/e/m={}/{}/{}",
...
fn frontier_v1_4_route_replay_cpu_oracle_parity() {
    with_gpu(|ctx| {
        ...
        println!(
            "frontier_v1_4_parity: route=replay_accepted sead=replay_accepted fp={}",
            run.summary.combined_hex()
        );
    });
}
...
fn frontier_v1_4_route_replay_reproducibility() {
    with_gpu(|ctx| {
        let (skeleton, config) = smoke_fixture();
        let run_a = run_frontier_v1_sead_route_replay(ctx, &skeleton, &config);
        let run_b = run_frontier_v1_sead_route_replay(ctx, &skeleton, &config);

        assert_eq!(run_a.summary, run_b.summary);
        assert_eq!(run_a.route_replay, run_b.route_replay);
        assert_eq!(run_a.sead_replay, run_b.sead_replay);
        assert_eq!(run_a.summary.combined_hex(), run_b.summary.combined_hex());
        assert_eq!(
            run_a.summary.combined_hex(),
            FRONTIER_V1_SEAD_ROUTE_REPLAY_FINGERPRINT
        );

        println!(
            "frontier_v1_4_replay: fp={} fixture_id={FRONTIER_V1_SEAD_ROUTE_FIXTURE_ID}",
            run_a.summary.combined_hex()
        );
    });
}
...
#[test]
fn frontier_v1_4_rejects_route_bypasses() {
    let (skeleton, config) = smoke_fixture();
    ...
    let mut structural = skeleton;
    structural.sead.structural_via_threshold_emit = false;
    assert_eq!(
        classify_proposal_route(ProposalKind::StructuralCommit, &structural),
        ProposalRoute::Rejected
    );

    let mut movement = skeleton;
    movement.sead.movement_own_columns_only = false;
    assert_eq!(
        classify_proposal_route(ProposalKind::Movement, &movement),
        ProposalRoute::Rejected
    );
    ...
    let _ = config;
    println!("frontier_v1_4_bypass: rejects=true fixture_id={FRONTIER_V1_SEAD_ROUTE_FIXTURE_ID}");
}

#[test]
fn frontier_v1_4_coupling_rejects_non_frontier_profile() {
...
fn frontier_v1_4_deferred_features_reject() {
    ...
    println!("frontier_v1_4_deferred: rejects=true fixture_id={FRONTIER_V1_SEAD_ROUTE_FIXTURE_ID}");
}
