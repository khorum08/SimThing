//! SEAD-EVENT-1 — GPU-resident event-code bucketing from compact event records (Tier-2, test-only).
//!
...
fn run_bucketing(
    ...
    let overflow_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("sead_event1_overflow"),
        contents: &[0u8; CODE_COUNT * 4],
        usage: wgpu::BufferUsages::STORAGE
            | wgpu::BufferUsages::COPY_SRC
            | wgpu::BufferUsages::COPY_DST,
    ...
    let bucket_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("sead_event1_buckets"),
    ...
    let params_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("sead_event1_params"),
        contents: bytemuck::bytes_of(&params),
        usage: wgpu::BufferUsages::UNIFORM,
    });

    let bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("sead_event1_bg"),
        layout: &bgl,
        entries: &[
            wgpu::BindGroupEntry {
                binding: 0,
    ...
    for _ in 0..repeat_dispatches {
        ...
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("sead_event1_enc"),
        });
        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("sead_event1_pass"),
...
fn staging_buf(device: &wgpu::Device, size: u64) -> wgpu::Buffer {
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("sead_event1_staging"),
        size,
...
fn run_integrated_three_pass(
    ...
    let module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("sead_event1_integrated"),
    ...
    let bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("sead_event1_integrated_bgl"),
        entries: &entries,
    });
    let pl = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        label: Some("sead_event1_integrated_pl"),
        bind_group_layouts: &[&bgl],
        push_constant_ranges: &[],
    });
...
#[test]
fn sead_event1_wgsl_semantic_free() {
    ...
    println!("sead_event1_wgsl: semantic_free=true ordering={ORDERING_CLASS}");
}

#[test]
fn sead_event1_bucket_edge_rows() {
    with_gpu(|ctx| {
        for (records, capacity, label) in edge_bucket_cases() {
            ...
            println!(
                "sead_event1_edge[{label}]: records={} capacity={capacity} counts={:?} overflow={:?} invalid={} membership_ok={ok} ordering={ORDERING_CLASS}",
                records.len(),
...
#[test]
fn sead_event1_bucket_dense_corpus() {
    with_gpu(|ctx| {
        ...
        println!(
            "sead_event1_dense: records={} invalid={} counts={:?} ordering={ORDERING_CLASS}",
            records.len(),
            outcome.invalid_code_count,
            &outcome.bucket_counts,
        );
...
#[test]
fn sead_event1_pipe0_to_bucket_smoke() {
    with_gpu(|ctx| {
        const N: usize = 512;
...
#[test]
fn sead_event1_perf_34k_bucketing() {
    with_gpu(|ctx| {
        ...
        for label in [
            ...
            println!(
                "sead_event1_34k[{label}]: elapsed_ms={elapsed_ms:.3} per_record_us={per_record_us:.4} per_bucket_us={per_bucket_us:.4} counts={:?} overflow={:?} invalid={} capacity={CAP}",
                &outcome.bucket_counts,
                &outcome.bucket_overflow,
                outcome.invalid_code_count,
            );
...
#[test]
fn sead_event1_perf_34k_warm_repeated_dispatch() {
    with_gpu(|ctx| {
        const N: usize = 34_000;
        ...
        println!(
            "sead_event1_34k_warm: repeats={REPEATS} total_ms={total_ms:.3} per_dispatch_ms={per_dispatch_ms:.4} per_record_us={per_record_us:.4} counts={:?} overflow={:?} invalid={} ordering={ORDERING_CLASS}",
            &outcome.bucket_counts,
            &outcome.bucket_overflow,
            outcome.invalid_code_count,
        );
