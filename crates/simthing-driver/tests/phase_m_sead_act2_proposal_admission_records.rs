//! SEAD-ACT-2 — Fixture-local proposal admission records from ACT-1 numeric summaries (Tier-2, test-only).
//!
...
use simthing_spec::{
    is_sead_act2_proposal_admission_records_descriptor, landed_jit_kernel_descriptors,
    validate_kernel_descriptor_admission, MappingExecutionProfile,
    PhaseEFixtureProposalAdmissionAuthority, SEAD_ACT2_DESCRIPTOR_ID, SEAD_ACT1_ADMITTED_TABLE_SIZE,
...
const FORBIDDEN_SEMANTIC_TERMS: &[&str] = &[
    ...
    "factory", "decision", "allocate", "production",
];
...
fn run_bucket_reduce_propose_consume_gpu(
    ...
    enc2.copy_buffer_to_buffer(&summary_buf, 0, &sum_staging, 0, (SUMMARY_STRIDE * 4) as u64);
    enc2.copy_buffer_to_buffer(&admission_buf, 0, &admit_staging, 0, (ADMIT_STRIDE * 4) as u64);
    queue.submit(Some(enc2.finish()));
...
#[test]
fn sead_act2_wgsl_semantic_free() {
    ...
    println!("sead_act2_wgsl: semantic_free=true ordering={ORDERING_CLASS}");
}

#[test]
fn sead_act2_admission_edge_rows() {
    with_gpu(|ctx| {
        for (summary, rules, label) in edge_admission_cases() {
            ...
            println!(
                "sead_act2_edge[{label}]: code={} flags={} admitted={} ordering={ORDERING_CLASS}",
...
#[test]
fn sead_act2_dense_admission_corpus() {
    ...
    with_gpu(|ctx| {
        ...
        println!(
            "sead_act2_dense: rows={} ordering={ORDERING_CLASS}",
            64
        );
    });
}
...
#[test]
fn sead_act2_act1_to_admission_smoke() {
    ...
    with_gpu(|ctx| {
        ...
        println!(
            "sead_act2_act1_smoke: proposal_count={} accepted={} admission_code={} flags={} dispatches=5 ordering={ORDERING_CLASS}",
...
#[test]
fn sead_act2_full_chain_admission_smoke() {
    let rules = rules_for_smoke();
    let admitted = default_admitted_table();
    let admitted_n = admitted_count();
    let adm_rules = smoke_admission_rules();
    with_gpu(|ctx| {
        ...
        println!(
            "sead_act2_full_chain: compact={} event_count={} bucket_counts={counts:?} proposal_count={} accepted={} admission_code={} flags={} overflow={} ordering={ORDERING_CLASS}",
            compact.len(),
            compact.len(),
            outcome.proposal_count,
            outcome.summary.accepted_count,
...
#[test]
fn sead_act2_perf_34k_admission_records() {
    let rules = rules_for_smoke();
    let admitted = default_admitted_table();
    let admitted_n = admitted_count();
    let adm_rules = smoke_admission_rules();
    with_gpu(|ctx| {
        ...
        println!(
            "sead_act2_34k: dispatches=5 elapsed_ms={elapsed_ms:.3} readback=true event_count={N} proposal_count={} accepted={} admission_code={} overflow={} per_record_us={per_record_us:.4} ordering={ORDERING_CLASS}",
            outcome.proposal_count,
            outcome.summary.accepted_count,
            outcome.admission.admission_code,
            outcome.proposal_overflow,
...
#[test]
fn sead_act2_perf_34k_warm_repeated_dispatch() {
    let rules = rules_for_smoke();
    let admitted = default_admitted_table();
    let admitted_n = admitted_count();
    let adm_rules = smoke_admission_rules();
    with_gpu(|ctx| {
        ...
        println!(
            "sead_act2_34k_warm: repeats={REPEATS} total_ms={total_ms:.3} per_pipeline_ms={per_pipeline_ms:.4} per_record_us={per_record_us:.4} admission_code={} flags={} overflow={} ordering={ORDERING_CLASS}",
            outcome.admission.admission_code,
            outcome.admission.flags,
            outcome.proposal_overflow,
        );
...
#[test]
fn sead_act2_no_default_runtime_wiring() {
    ...
    println!("sead_act2_wiring: default_off=true descriptor={SEAD_ACT2_DESCRIPTOR_ID}");
}
