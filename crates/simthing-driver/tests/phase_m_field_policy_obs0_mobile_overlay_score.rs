//! FIELD_POLICY-OBS-0 — GPU-resident mobile observer overlay score probe (Tier-2, test-only).
//!
...
use simthing_spec::{
    fnv1a64_hex, is_exact_mag2_fixed_descriptor, is_field_policy_obs0_overlay_score_descriptor,
    landed_jit_kernel_descriptors, validate_kernel_descriptor_admission, MappingExecutionProfile,
    FIELD_POLICY_OBS0_DESCRIPTOR_ID, MAG2_FIXED_DESCRIPTOR_ID, MAG2_Q16_COMPONENT_MAX,
...
fn run_overlay_score_batch_repeated(
    ...
    let module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("field_policy_obs0_overlay_score"),
        source: wgpu::ShaderSource::Wgsl(wgsl.into()),
    });
    let bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("field_policy_obs0_bgl"),
    ...
    let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        label: Some("field_policy_obs0_pipeline"),
        layout: Some(
            &device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("field_policy_obs0_pl"),
    ...
    let storage = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("field_policy_obs0_values"),
    ...
    let bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("field_policy_obs0_bg"),
        layout: &bgl,
    ...
    for _ in 0..repeat_dispatches {
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("field_policy_obs0_enc"),
        });
        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("field_policy_obs0_pass"),
...
#[test]
fn field_policy_obs0_wgsl_semantic_free() {
    ...
    println!(
        "field_policy_obs0_wgsl: semantic_free=true F_hash={SQRT_F_ARTIFACT_HASH} mag_path=fixed_q16_plus_F"
    );
}
...
fn field_policy_obs0_exact_magnitude_spot_check() {
    with_gpu(|ctx| {
        ...
        println!(
            "field_policy_obs0_spot: rows=512 mag2_exact={mag2_exact}/512 mag_max_ulp={mag_max_ulp} overflow=0"
        );
...
#[test]
fn field_policy_obs0_dense_overlay_corpus() {
    with_gpu(|ctx| {
        ...
        println!(
            "field_policy_obs0_dense: tested={} mag2_exact={mag2_exact} mag_max_ulp={mag_max_ulp} score_max_ulp={score_max_ulp} overflow={overflow}",
            rows.len()
        );
...
#[test]
fn field_policy_obs0_score_authority_matches_arithmetic() {
...
#[test]
fn field_policy_obs0_no_default_runtime_wiring() {
    assert_eq!(
    ...
    let obs = landed_jit_kernel_descriptors()
        .into_iter()
        .find(|d| d.id == FIELD_POLICY_OBS0_DESCRIPTOR_ID)
        .expect("field_policy obs0 descriptor");
    assert!(is_field_policy_obs0_overlay_score_descriptor(&obs));
...
#[test]
fn field_policy_obs1_no_default_runtime_wiring() {
    assert_eq!(
        MappingExecutionProfile::default(),
        MappingExecutionProfile::Disabled
    );
    ...
    println!("field_policy_obs1_wiring: descriptor=landed no_scheduler_no_bridge");
}

#[test]
fn field_policy_obs0_edge_rows_correctness() {
    with_gpu(|ctx| {
        ...
        println!(
            "field_policy_obs0_edge: tested={} mag_max_ulp=0",
            rows.len()
        );
    });
}
