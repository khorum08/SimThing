//! SEAD-EVENT-2 — GPU-resident per-bucket reductions from event-code buckets (Tier-2, test-only).
//!
...
use simthing_spec::{
    is_sead_event2_bucket_reductions_descriptor, landed_jit_kernel_descriptors,
    validate_kernel_descriptor_admission, EventBucketReductionInputAuthority,
    EventBucketReductionOrderAuthority, MappingExecutionProfile, SEAD_EVENT1_CODE_COUNT,
...
#[test]
fn sead_event2_wgsl_semantic_free() {
    ...
    println!("sead_event2_wgsl: semantic_free=true ordering={ORDERING_CLASS}");
}

#[test]
fn sead_event2_reduction_edge_rows() {
    with_gpu(|ctx| {
        for (buckets, counts, cap, label) in edge_reduction_cases() {
            ...
            println!(
                "sead_event2_edge[{label}]: counts={counts:?} ok={ok} flags={:?} ordering={ORDERING_CLASS}",
...
#[test]
fn sead_event2_reduction_dense_corpus() {
    with_gpu(|ctx| {
...
#[test]
fn sead_event2_event1_to_reductions_smoke() {
    with_gpu(|ctx| {
        ...
        println!(
            "sead_event2_event1_smoke: records={} counts={counts:?} ordering={ORDERING_CLASS}",
            compact.len()
        );
    });
}
...
#[test]
fn sead_event2_pipe0_to_bucket_reductions_smoke() {
    with_gpu(|ctx| {
        ...
        println!(
            "sead_event2_pipe0_smoke: compact={} event_count={} counts={counts:?} ordering={ORDERING_CLASS}",
            compact.len(),
            compact.len(),
        );
    });
...
#[test]
fn sead_event2_perf_34k_bucket_reductions() {
    with_gpu(|ctx| {
...
#[test]
fn sead_event2_perf_34k_warm_repeated_dispatch() {
    with_gpu(|ctx| {
        const N: usize = 34_000;
        const CAP: u32 = 20_000;
...
fn sead_event2_no_default_runtime_wiring() {
    ...
    println!("sead_event2_wiring: default_off=true descriptor={SEAD_EVENT2_DESCRIPTOR_ID}");
}
