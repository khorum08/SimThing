    ...
    for field in 0..DELTA_FIELDS {
        ops.push(AccumulatorOp {
            source: SourceSpec::SlotValue {
                slot: SlotIndex::new(layout.staging_field_slot(row, field)),
...
#[cfg(test)]
mod fast_membership_sentinel_tests {
    use super::{apply_plan_oracle, build_membership_plan, MembershipAction, SlotState};
    use crate::dress_rehearsal_r6c_integrated_run::{R1bStructuralEvent, R1bStructuralEventKind};
    use crate::runtime_0080_0_r1c_b::Runtime0080R1cBAllocationRow;

    #[test]
    fn r1c_fast_membership_delta_applies_to_one_slot() {
        let fleet_slot_count = 2;
        let initial = vec![
            SlotState {
                cell: 10,
                owner: 1,
        ...
        let allocation_rows = vec![Runtime0080R1cBAllocationRow {
            tick: 2,
            request_event_index: 0,
            requested_owner: 3,
            requested_source_cell: 30,
            requested_ships: 1,
            allocated_slot: Some(2),
            allocation_success: true,
            allocation_failure_reason: "",
            gpu_selected_slot_value: Some(2),
        }];

        let plan = build_membership_plan(fleet_slot_count, &events, &allocation_rows);
        assert!(plan.len() <= 8, "bounded delta table on tiny fixture");
        assert!(plan
            .iter()
            .any(|delta| delta.action == MembershipAction::MoveOut));
        assert!(plan
            .iter()
            .any(|delta| delta.action == MembershipAction::MoveIn));
        assert!(plan
            .iter()
            .any(|delta| delta.action == MembershipAction::BirthIn));

        let expected = apply_plan_oracle(&initial, &plan);
        assert_eq!(expected[0].cell, 11);
        assert_eq!(expected[0].owner, 1);
        assert!(expected[0].active);
        assert_eq!(expected[2].cell, 30);
        assert_eq!(expected[2].owner, 3);
        assert!(expected[2].active);
    }
}
