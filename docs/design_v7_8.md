...
**AO-WGSL-0** is an authorized named gate for generic AccumulatorOp WGSL, **ACCEPTED
(AO-WGSL-0-ACCEPT, 2026-05-30)** as a generic GPU performance option (default-off OrderBand fast path
for compatible AO plans; unsupported shapes fall back). This does not alter the semantic-WGSL
guardrail: designer/spec admission still rejects raw or semantic WGSL.
...
- **Status:** **C-0/C-1/C-2 ACCEPTED — map batching CLOSED at the designer surface (C-2-ACCEPT-0,
  2026-05-30).** C-0 (real packed-atlas GPU path, algebraic G=0, full-tile protocol-oracle parity,
  `GpuVerifiedApproximate`) + C-1 (2000-star budget envelope) + **C-2 (bounded algebraic-G=0 atlas
  admission relaxation)** are accepted
  ([`tests/phase_m_c2_acceptance_review_results.md`](tests/phase_m_c2_acceptance_review_results.md);
  two compile breaks remediated inline during review). `request_atlas_batching` now admits **only**
  bounded algebraic-G=0, homogeneous-square, protocol-oracle-backed specs that fit the active
  `V78AtlasVramBudget` with multiplier reporting; everything else (physical gutter, active mask,
  source identity, production runtime, default-on) stays rejected. `MappingExecutionProfile` default
  stays `Disabled`. The atlas **production runtime / sparse-residency scheduler** (C-1's noted need at
  true 2000-star scale) is a **separate later gate**, not open.
