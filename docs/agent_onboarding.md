...
**A cold agent's first action is to orient, and it carries the receipt.** Orientation is generated from live
harness state, so it is always current; a receipt proves the agent oriented against *this* state, not a stale
memory. Everything below is that rule, specialized.
...
- **Cold start:** its first shell command is
  `bash scripts/ci/orient.sh --role=coding` — prints its role-keyed contract and an `ORIENT-RECEIPT`.
- **You hand it:** the rung's production handoff (from the orchestrator or DA) + "orient first, carry the receipt."
...
|---|---|---|---|
| Coding (Grok/Cursor) | `bash scripts/ci/orient.sh --role=coding` | PROBATION / proof-present | nothing |
...
**When to update this file:** only when a *tier's cold-start ritual* changes (a new entrypoint command or a
new agent tier). Per-rung governance is not here — it lives in the generated orientation digest, which cannot
drift. This file is a stable operator manual, not a changelog.
