...
| K4 | `OC-K-EML-OPCODE-GATE-0` | Opcode/combine admission hard-gate at registration (no semantic ops); "gadget tree first" stays anchor prose — the gate types only the door. | **GRADUATED / PR pending** — closed EvalEML opcode vocabulary; generic primitive Tier-2 parity gate; semantic op rejected; gadget toolkit pathway; evidence [oc_k_eml_opcode_gate_0_results.md](tests/oc_k_eml_opcode_gate_0_results.md). **Next:** closeout C after DA merge. | T2/Owner |
