...
> **What this is.** The single authoritative map of the `0.0.8.4.6` CI doctrinal-screening layer (Track A,
> CLOSED): what each file is, how the screening logic works, the **strict rigor** required to change the scan
> and allow/block lists, and the **triage agent's** role in keeping the surface narrow. It exists so the layer
> is **auditable** (anyone can read exactly what is screened and why), **maintainable** (there is one correct,
> low-risk way to change it), and **ready for Track C** (introspection reads this surface; onboarding steers
> agents by it).
>
...
## 6. Track C readiness — introspection and onboarding build on this surface

Track C (the "carrot", deferred-but-scoped) pulls this same surface *forward* into the agent's context and inner
loop. It needs **no new source of truth** — it consumes the artifacts described here:

- **Onboarding — the sanctioned-surface digest.** `allow/*.txt` is the seed for a compact, always-current
  "these are the only kernel doors you may call, here is their shape, here is what's forbidden and why" digest,
  injected into a kernel-touching agent's context. Because the onboarding heuristic keeps `allow/*.txt` accurate,
  the digest is **accurate for free** — the same discipline that keeps CI honest keeps the agent's context honest.
  This is the low-context-agent prosthetic: a tiny, current statement of the valid surface so a small window
  can't drift.
- **Introspection — machine-readable throughout.** The `DOCTRINE-SCAN-VERDICT:` footer, the `triage_log.tsv`
  telemetry, and the closed-set `allow/*.txt` files are all greppable/parseable data an introspection layer reads
  to answer "what is screened, what's the fire-rate per scan, which scans are retirement candidates, how wide is
  the sanctioned surface." No dashboard is required — the data *is* the interface.
- **Inner-loop self-scan.** The same `doctrine_scan.sh` + `cargo check` an agent can run per-edit locally is the
  generation-time complement to the CI gate — the conformant path becomes the path of least resistance, lowering
  INSPECT volume at the source and lightening triage.

**The through-line:** one artifact set — `scans.tsv` + `allow/*.txt` + the triage log — serves three pipeline
positions (context digest **before**, inner-loop self-scan **during**, CI gate **after**). Keeping it narrow,
data-driven, and honestly maintained is what makes all three trustworthy. Every change to this surface is a
diff-auditable data edit reviewed against the rigor in §4 and the triage discipline in §5.
