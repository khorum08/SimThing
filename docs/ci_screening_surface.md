...
|---|---|---|
| `test_inventory.tsv` | inventory ledger | one row per surviving test: `crate \| file \| test_name \| kind \| class \| superseding_boundary \| verdict \| note \| promotion_target \| birth_track \| dsu_survivals`. Every KEEP row must name an `until-closeout:*` class or another admitted promotion target, and NO row is exempt from explicit closeout disposition; `dsu_survivals` counts renewals, and a 4th demands promotion evaluation |
| `test_residue_classes.tsv` | data list | the closed set of `until-closeout:*` classes (`oracle-parity`, `golden-byte`, `seal-proof`, `determinism`, `behavior-regression`, `escaped-bug`, `doc-named-invariant`, `stead-required`, `dependency-floor`). **These name a residue KIND, not immunity (Owner mandate 2026-07-30: no proof is ever permanent).** Track closeout is the expiry event: ordinary rows are dispositioned then, with no `closed_at + GRACE_DAYS` wall-clock runway; only the inherently durable `compile_fail` / `trybuild` proof kinds retain their standing kind exception |
...
| `test_inventory_drift_check.sh` | drift gate | the `TEST-INVENTORY-DRIFT` stock gate body: inventory must match discovered tests and every KEEP row must be owned; unledgered runnable tests FAIL. `until-closeout:dependency-floor` rows are exempt from the stale-drift check only |
