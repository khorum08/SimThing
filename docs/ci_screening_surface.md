...
Track D note: owner-deep full batteries remain quarantined artillery. Smoke PASS is mechanics-only and not seal-proof. Seal-residue rungs still require targeted profile/probe proof. Do not decide weekly/master-only owner-deep cadence until `TEST-PARE-INVENTORY-0` and `TEST-PARE-AUDIT-1` data are DA-cleared. Track D inventory/audit rungs are classification and planning only; per-crate paring rungs do the actual deletions or collapses.
