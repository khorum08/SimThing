...
Track D note: owner-deep full batteries remain quarantined artillery. Smoke PASS is mechanics-only and not seal-proof. Seal-residue rungs still require targeted profile/probe proof. `DA-RULING: ADMISSION-BOUNDARY-COLLAPSE` supersedes the old same-family audit ledger: deletion authority is now `scripts/ci/test_pare_boundaries.tsv` plus `scripts/ci/test_pare_boundary_rows.tsv`, checked by `scripts/ci/test_pare_boundary_check.sh`. The burden of proof is on retention for admission-adjacent, hygiene-theater, and usecase-superseded rows. Until material reduction lands, weekly scheduled sentinel means sentinel-core only, and full quarantined battery remains workflow_dispatch-only. Do not implement scheduled workflow changes from Track D without an explicit cadence rung.
