...
| **A running guard is not ceremony; asserting a result is** | "Harness ceremony" (§0.6.5) means a harness that produces artifacts *in place of* actually running. Its opposite — a **running guard** that executes and enforces (§0.6.6: type / hard-error / **running guard**) — is real enforcement, *not* ceremony. The **CI doctrine-scan** (self-testing, data-driven, designed to retire) is such a guard. The rule "the specified consumer must actually run" **cuts both ways**: it equally forbids *asserting, summarizing, or fabricating* a check / scan / self-test result you did not run — that is ceremony in its worst form (a claim standing in for execution). A guard scan being a §1.2 "promotion target" means **promote the invariant to a type and retire the scan in the *same* PR** — never skip, ignore, or delete a scan while its invariant is still only a scan. **Citing §0.6.5, D8, or "no governance artifacts" to argue the CI screen is theater, or to skip running it, is the non-conformant route this rule forecloses.** |
