...
| Mobile observer overlay score authority follows arithmetic | Exact Q16.16 mag/F magnitude may be exact-authoritative, but weighted overlay scores are exact only if their weighting arithmetic is fixed-point/pinned; f32 score arithmetic (e.g. SEAD-OBS-0 `score = bias + weight * mag`) is diagnostic/probe-only and does not authorize CPU-side planner or production bridge wiring |
| Mobile observer overlay score descriptors pin score authority | Overlay score descriptors (`m_jit_sead_obs0_overlay_score`, `m_jit_sead_obs2_multilayer_overlay_score`, `m_jit_sead_obs3_multilayer_fixed_score`, `m_jit_sead_obs4_threshold_event`) may expose exact mag2/mag under Q16.16 + artifact-backed F, but f32 weighted scores remain `ApproximateDiagnostic` and may not feed exact-authoritative state without a separate fixed-point/pinned score contract |
| Multi-layer observer score descriptors pin per-layer exactness | Multi-layer observer score descriptors may expose exact per-layer magnitudes under the Q16.16 + artifact-backed F contract, but aggregate weighted scores remain `ApproximateDiagnostic` unless their accumulation arithmetic is fixed-point/pinned |
| Exact observer aggregate scores require pinned fixed-point accumulation | Exact observer aggregate scores require a pinned fixed-point accumulation contract (`ExactQ16WeightedSum`). f32 weighted sums remain `ApproximateDiagnostic` even when their per-layer magnitudes are exact |
| GPU threshold event outputs require pinned fixed-point operands | GPU threshold event outputs may be exact-authoritative only when their input score and threshold/hysteresis operands are fixed-point/pinned; threshold events do not authorize CPU-side planner, urgency traversal, or production bridge wiring |
| Compacted GPU event buffers pin count and unordered membership | Compacted GPU event buffers may claim exact event count and membership only under a declared capacity/overflow contract. Atomic compaction does not imply deterministic event ordering unless ordering is explicitly proven |
| Integrated GPU observer-event pipelines pin count and unordered membership | Integrated GPU observer-event pipelines may claim exact event count and unordered membership only under fixed score/event contracts and declared compaction capacity. Pipeline integration does not authorize CPU-side planner, urgency traversal, commitment emission, scheduler/cache, or production bridge wiring |
| GPU event-code buckets pin per-code counts and unordered membership | GPU event-code buckets may claim exact per-code counts and unordered membership only under a declared capacity/overflow contract. Atomic bucketing does not imply deterministic record ordering and does not authorize CPU-side planner, urgency traversal, commitment emission, scheduler/cache, or production bridge wiring |
| GPU event-bucket reductions pin order-invariant numeric summaries | GPU event-bucket reductions may claim exact numeric summaries only for order-invariant reductions under declared overflow and empty-bucket contracts. Reductions over unordered buckets do not imply deterministic record ordering or authorize CPU-side planner, urgency traversal, commitment emission, scheduler/cache, or production bridge wiring |
| SEAD numeric proposal records are semantic-free substrate | SEAD numeric proposal records may be exact-authoritative only under fixed integer rule and capacity contracts. Proposal records do not authorize semantic action interpretation, CPU-side planning, urgency traversal, commitment emission, scheduler/cache, default wiring, or production economy bridge wiring |
| Phase E-style SEAD proposal consumers summarize numeric inputs only | Phase E-style SEAD proposal consumers may summarize numeric proposal records only under fixed integer and overflow contracts. They do not authorize semantic action interpretation, CPU-side planning, urgency traversal, commitment emission, scheduler/cache, default wiring, simthing-sim awareness, or production economy bridge wiring |
| Fixture-local SEAD proposal admission records are rule-bound substrate | Fixture-local SEAD proposal admission records may be exact-authoritative only under fixed integer rule and overflow contracts. They do not authorize semantic action interpretation, CPU-side planning, urgency traversal, commitment emission, scheduler/cache, default wiring, simthing-sim awareness, or production economy bridge wiring |
| Economic V1-style SEAD fixture records are mapping-bound substrate | Economic V1-style SEAD fixture records may be exact-authoritative only under fixed integer mapping and overflow contracts. They do not authorize semantic action interpretation, CPU-side planning, urgency traversal, commitment emission, scheduler/cache, default wiring, simthing-sim awareness, or production economy bridge wiring |
| SEAD ACT-3 fixture validation corpora are authoring evidence only | Authorable validation corpora over ACT-3 numeric fixture records may verify expected outputs under fixed integer oracle contracts only. They do not authorize runtime implementation, semantic action interpretation, CPU-side planning, scheduler/cache, default wiring, simthing-sim awareness, production economy bridge wiring, or Resource Flow default-on behavior |
| SEAD self-AI proposals route through accepted substrates only | SEAD self-AI proposals route only through accepted substrates: Resource Flow allocator for resource dispatch, Threshold+EmitEvent→BoundaryRequest for structural commitments, or the SimThing's own columns for movement. The SEAD ladder is closed at Proposal Pipeline V1; further stages require a separately named scenario. simthing-sim remains semantic-free |
