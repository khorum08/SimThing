...
| Exact Euclidean magnitude is F-routed | Exact Euclidean magnitude may be exact-authoritative only when its sqrt component is routed through the artifact-backed Candidate F descriptor (`m_jit_mag_f_exact`); diagnostic `mag2` and raw/native `sqrt` remain non-exact |
