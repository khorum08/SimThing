...
| Mobile observer overlay score descriptors pin score authority | Overlay score descriptors (`m_jit_sead_obs0_overlay_score`) may expose exact mag2/mag under Q16.16 + artifact-backed F, but f32 weighted scores remain `ApproximateDiagnostic` and may not feed exact-authoritative state without a separate fixed-point/pinned score contract |
