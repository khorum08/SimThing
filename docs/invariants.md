...
| Rule | Enforced by |
|---|---|
| Arena participation is DERIVED at admission (typed, bounded, inspectable) | Participation derives from admitted resource properties + the parent edge at session build (the graduated P1 law; supersedes the old per-participant authored-selector rule); the derivation is sealed and inspectable, ambiguity REFUSES typed, and declared upper bounds still cap every arena. "Explicit admission" survives as the sealed derivation — never as a separately authored selector per participant |
...
| Rule | Enforced by |
|---|---|
| First-slice mapping gate — HISTORICAL/COMPLETED; scope limited to the legacy RegionCell preview path | The original restriction ("no production mapping runtime without first-slice gating; fields test/opt-in-driven only") is DISCHARGED: the current Triad/`FieldSweepRegistration` core path is ordinary production, DEFAULT-DERIVED at admission per the row above, and is NOT gated by this rule. Any residual gating applies only to the legacy/diagnostic RegionCell preview wiring, which remains an explicitly optional facility behind its own gate |
...
| Mapping is bounded and derived, sparse when inert | Applicable Triad/field behavior is DEFAULT-DERIVED at admission from admitted map-spec data with explicit authored opt-out (the graduated P5 law; supersedes the old blanket default-off rule); absent an admitted datum the mechanism is sparse/inert at zero cost — no separate enabling step exists for the core mechanism. Diagnostic previews and explicitly optional facilities may remain default-off. Every field still declares hard caps (grid bounds, horizon cap, source cap) |
...
| CPU boundary consumes, never recomputes | At the boundary the CPU consumes resolved summaries/events/metadata only; it must not recompute economy/threat/urgency, must not emit commitments via CPU planner logic, and must not scan dense RegionCell grids by default. Discrete boundary banking uses the opt-in `ResourceEconomySpec` substrate; Resource Flow is the ORDINARY continuous substrate — participation derives at admission from admitted resource data (`ResidentRequired` is the ordinary market posture; sparse/inert when no admitted datum exists) — and is not the discrete-banking answer |
