...
>
> **Next horizon:** `R1c-a` (`RESIDENT-REENROLL-0`, resident free-list mark-only / no compaction as the smaller step toward structural
> decision authority); then resident scatter/compact for membership + cohort table behind the §11 / free-list-scatter stop-lines; then multi-atlas batching + M-4A masking
