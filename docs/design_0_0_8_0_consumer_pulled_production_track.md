...
| `DEFAULT-SCHEDULE-0080-1` | `SCENARIO-0080-1` schedule / movement | **OPEN WITH NARROWING — scenario-scoped Nested Starmap schedule/movement gate; no implementation** — deterministic opt-in/default-off step driver that consumes `PRODUCTION-PATH-0080-1` and turns its read-only SEAD composite-gap terms into live movement via `Threshold + EmitEvent → BoundaryRequest` routed through the existing mobility/transfer substrate (Terran + Pirate ships); preserves identity + owner overlays, updates membership without reparenting; **not a global default schedule; no observation/control/demo; no direct move; no new substrate**. Spec: [`production_paths/default_schedule_0080_1_opening_spec.md`](production_paths/default_schedule_0080_1_opening_spec.md); review: [`tests/phase_default_schedule_0080_1_opening_review_results.md`](tests/phase_default_schedule_0080_1_opening_review_results.md). |
