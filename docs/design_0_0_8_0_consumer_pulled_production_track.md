...
| `DEMO-0080-1` | `SCENARIO-0080-1` headless demo/export packaging | **OPEN WITH NARROWING — headless Nested Starmap demo/export packaging; no implementation** — deterministic opt-in/default-off **library helper** that applies a canonical bounded `CONTROL-0080-1` command batch and runs the existing `control → schedule → observation/export` path, emitting the existing deterministic transcript/export + a compact demo report (atlas residency, faction-index ECON, owner-overlay/up-aggregation, SEAD movement trace, Terran/Pirate movement rows, command transcript, replay checksum). Packaging of the completed slice; no new behavior. **CLI decision: `No CLI binary`.** UI, real-time loop, player command loop, direct movement control, and global default schedule remain CLOSED. Spec: [`gameplay/demo_0080_1_opening_spec.md`](gameplay/demo_0080_1_opening_spec.md); review: [`tests/phase_demo_0080_1_opening_review_results.md`](tests/phase_demo_0080_1_opening_review_results.md). |
