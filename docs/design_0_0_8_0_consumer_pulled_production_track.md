...
>
> **Next horizon:** class-by-class GPU structural-decision boundary after R1c-f — sibling rungs for `DamageDelta` (reduce), move `StepOpportunity` threshold (`MoveRequest`), `LocalBirthRequest`, `FusionRequest`, and remaining journal classes until umbrella `structural_decisions_gpu_emitted` flips true. M-4A / multi-atlas remains parked.
