...
>
> **Next horizon — no new rung until a consumer is chosen.** Do not continue R1c substrate rungs and do not unpark M-4A unless a chosen consumer proves it required. The next consumer is selected only after deciding among: A. class-by-class GPU decision conversion for remaining structural event classes (`R2a`–`R2d`); B. richer emergence scenario over the runtime; C. multi-theater / M-4A sparse residency; D. multi-faction economy; E. system→planet recursion.
