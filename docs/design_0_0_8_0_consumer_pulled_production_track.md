...
> R1–R6/R6B remain single-galactic-tier, opt-in/default-off, CPU-oracle primary (GPU-shaped row ops). **R7 REOPENED (2026-06-04): the R7 docs-only closeout was premature — the ladder's intended culmination (spec §8.1: observe which emergent behaviors actually appear over a run) was never given an implementation rung. A new rung `R6C — Integrated multi-tick run` is inserted before R7 to assemble the rungs into one ticking session with feedback and actually run it; R7 re-closes only after R6C.** Mechanism-chain proof (R1–R6B) stands; the integrated emergence run is the outstanding deliverable.
> SEAD consumption remains deferred to R4. M-4A
> sparse-residency scheduler and REENROLL remain parked.
>
...
| **R6C — Integrated multi-tick run (the ladder's culmination)** — *OPEN 2026-06-04 ([opening spec](scenarios/scenario_0080_2_r6c_integrated_run_opening_spec.md))* | assemble R1→R6B into **one mutable session-state** and **tick it ~100 times with feedback**: each tick runs disruption (R1) → economy/blockade/divert (R2) → overlays (R3) → SEAD field read (R4) → movement/REENROLL + fission (R5) → combat on **actually co-located** fleets (R6) → production/reinforcement/fusion (R6B), then **writes the mutated world (positions, ship counts, stockpiles, disruption) forward to the next tick**. Field made **non-degenerate** (multi-hotspot; R4 tie-breaker dropped or proven dominated). Emits a per-tick trace + the §8.1 emergence checklist. opt-in/default-off; CPU-oracle primary; **no** default SimSession wiring, **no** CPU planner, **no** new invariant | **the integrated emergence run** — what every prior rung was building toward; closes the §8.1 observation gap | R1–R6B rungs; mobility runtime tick composition; FrontierV2 closed-loop feedback pattern |
| **R7 — CLOSE + closeout integrity + report** — *REOPENED 2026-06-04 (prior docs-only closeout was premature — see [closeout report, reopened](tests/scenario_0080_2_r7_closeout_report.md)); blocked on R6C* | design-authority closeout + human/layman report. Mechanism-chain proof (R1–R6B) stands; **R7 re-closes only after R6C produces the integrated-run trace**, at which point it reports which §8.1 behaviors actually emerged (replacing the interim "not yet emerged" finding with run evidence). Prior numeric-only reconciliations and the R4 tie-breaker / GPU-posture wording carry forward | the **closeout-integrity** meta-opportunity; the **proof+emergence narrative** | R6C run trace |
