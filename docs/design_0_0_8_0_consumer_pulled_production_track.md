...
> ACCEPTED (2026-06-02). It deliberately opens two previously-parked substrate gates as docs/design only:
> `ATLAS-0080-0` (atlas production runtime / sparse-residency nested mapping) and `ECON-SCALE-0080-0`
> (multi-faction faction-index ECON scaling, pirate as full economy faction). Opt-in/default-off; no
> implementation yet. Packet: [`scenarios/scenario_0080_1_admission_packet.md`](scenarios/scenario_0080_1_admission_packet.md);
...
| `ATLAS-0080-0` | Atlas production runtime / sparse-residency nested mapping | **OPEN — docs/design gate; no implementation** — opened by `SCENARIO-0080-1` as the named multi-theater consumer (the *first-slice gating* the invariant contemplates); opt-in sparse residency + nested theater descent/ascent; residency is a strict value no-op (I8 parity); **no default session pass-graph wiring**. Spec: [`production_paths/atlas_0080_0_opening_spec.md`](production_paths/atlas_0080_0_opening_spec.md). |
