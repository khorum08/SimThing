...
>
> **Next consumer selected (`RUNTIME-0080-0-CLOSED-NEXT-CONSUMER-0`, 2026-06-06): C — M-4A / multi-atlas sparse residency.** Design authority recommended B (richer emergence); the project selected C. Opened as a **capability/infrastructure track, not a gameplay consumer** — no gameplay overclaim until a named gameplay consumer uses it. Opening spec: [`production_paths/runtime_0080_m4a_opening_spec.md`](production_paths/runtime_0080_m4a_opening_spec.md).
>
> **`RUNTIME-0080-M4A-OPEN-0` — OPEN (capability track):** prove the closed single-theater 100-tick GPU-forward runtime runs across N ≥ 2 resident theaters under one sparse-residency dispatch. First slice `RUNTIME-0080-M4A-0` reuses the closed R2 loop as the per-theater unit and adds the scheduler + sparse-residency accounting; acceptance is per-theater parity + sparse residency (inactive theaters ~0 dispatch) + cross-theater isolation, with no CPU planner and no new semantic GPU op. Consumes prior art (`ATLAS-BATCH-0` batch allocation, `atlas_0080_0` scenario-scoped sparse residency) rather than redoing it. Parked: gameplay emergence (`SCENARIO-0080-3`), multi-faction economy, system→planet recursion, default session wiring, invariant edits, scenario reopen, class-by-class structural conversion. The other unselected candidates (A DamageDelta conversion, B richer emergence, D multi-faction economy, E system→planet recursion) remain available but **not** opened — one track at a time.
