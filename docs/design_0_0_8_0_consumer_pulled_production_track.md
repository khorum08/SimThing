...
>
> **Next consumer (`RUNTIME-0080-RR-OPEN-0`, OPEN): recursive galaxy→system→planet-surface rehearsal.** Build what the dress-rehearsal spec actually specifies, with **no tier collapse**: nested galaxy→system(10×10)→planet→surface(10×10) residency (generalize `atlas_0080_0`) + pop-cohort/factory **planet-surface labor economy** (reduce-up/disburse-down, §0.2) + the proven R2 galactic combat loop, ticked 100 ticks vs a recursive CPU oracle. Rung ladder RR-0 (recursive world + oracle) → RR-1 (nested residency) → RR-2 (surface economy on GPU) → RR-3 (recursive reduce-up/disburse-down) → RR-4 (integrated recursive 100-tick rehearsal — the horizon). Every rung carries a Scope Ledger; no hygiene theater. Opening spec: [`production_paths/runtime_0080_recursive_rehearsal_opening.md`](production_paths/runtime_0080_recursive_rehearsal_opening.md). Parked: multi-faction economy generality, default session wiring, semantic WGSL, pinned-number changes, scenario reopen.
