...
|---|---|---|---|---|---|
| 0.1 | `RF-COLUMN-ADMISSION-GATE-0` | Phase 0: `ColumnIndex` constructor taxonomy, NON-BREAKING (fenced raw door); retarget `COLUMN-INDEX-MINT` to choke-point tokens; mechanize DA-gated exclusion edits. | Scan proves every remaining `::new` inside the fence; corpus green. | DA-reserve · Frontier — Codex 5.6/Fable | TODO |
| 0.2 | `EXECUTION-STATUS-TAXONOMY-0` | Phase 0: `executed / oracle / rehearsal / compile-plan` classification as data; board-surfaced. | Every driver/kernel execution surface classified; board digest renders it. | Std — Grok | TODO |
