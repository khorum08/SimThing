...
- **P5 — Intrinsic field influence: semiring-generic (Owner PALMA elevation, 2026-07-26).**
  Field propagation over the lattice is ONE sweep mechanism parameterized by semiring, with
  three named instances: **STEAD** (accumulate-style superposed falloff — the heatmaps),
  **PALMA** (select-style `(min,+)` relaxation — least-cost potentials `D` from an impedance
  column `W`; lineage: the PALMA tropical-algebra paper, algebra borrowed, ARM implementation
  explicitly not), and **Boolean reachability** (`(OR,AND)`, riding the P0(e) domain rider).
  ...
  semiring tag at library speed, zero kernel change. `W`/`D` are ORDINARY property columns
  (homogeneous lanes, nothing special) and therefore **born STEAD anchors** per P0(e): banding
  a `D`-field yields corridor-opened/severed/chokepoint events as sealed band crossings for
  free — PALMA outputs are STEAD inputs, recursively. Fields feed **Gu-Yang gradient descent
  and PALMA borders** through the Structural Execution Convergence Contract (stead §10) —
  existing ops, never bespoke kernels; movers are field samplers, never planner clients; `D`
  is eventually-consistent (path-length generations — Wei P1 finite information speed), never
  an instant-query service (CPU oracle retained for tooling/proof only).
  **The PALMA pathing door (whitelist law):** any quantitative pathing between nodes derives
  ...
  door, operationalizing §7's "no CPU map planner." Bespoke graph-search in production crates
