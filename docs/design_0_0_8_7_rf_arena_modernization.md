...
| 5.2 | `WRITE-DOOR-BAND-DELTA-0` | Phase 5: fulcrum 2 — in-pass band-crossing derivation; structural ops refuse to encode without anchor remaps. | **PROBATION / proof-present / DA-review-pending** — sealed `BandCrossingDelta` minted from fused threshold emissions + registration sidecar (Anchored filter); typed `AnchorRemapSection` fail-closed before `sync_gpu_buffers`; GPU/CPU oracle matrix + remap negatives + census green | Std — Grok | **PROBATION** |
