...
| 5.8 | `GUYANG-COMPARATIVE-PROJECTIONS-0` | Phase 5: dominance/margin/contest/border-band/chokepoint projections; default-derived at ≥2 emitter classes; deterministic tie-break; consumes generic field-sweep outputs. | **PROBATION / proof-present / DA-review-pending** — DA-amended border = winner-identity change; stall = gross minus abs(net) on generic EML; default-derived Anchored mint; unmodified TP theater front+chokepoint with controls; CPU/GPU parity (RTX 4080/Vulkan). Driver-only consumer; no allowlist widen. See results doc. | Std — Grok | **PROBATION** |
