...
| 6.3 | `CONTINUOUS-POSTURE-SOAK-0` | Phase 6: continuous posture (batched generations) + N-generation soak under forced lag. | Soak exit-proof per the phase row; paced posture unchanged (scenario-neutral posture regression green). | Std — Grok | TODO |
...
| 8.1 | `CONTENTION-CONSERVATION-JUDGE-0` | Phase 8: RF-1-style conservation judge over CONTESTED FLOWS (oracle-first), **independent of the authored resolution rule**. Combat is a DOMAIN word; the mechanism is N claimants against bounded supply. | Judge red/green proven on seeded violations against SCENARIO-NEUTRAL synthetic contention fixtures. **The R6 dress rehearsals are PURGE-SCHEDULED and may NOT be an oracle basis.** | Std — Grok | TODO |
| 8.2 | `CONTENTION-ARENA-EXECUTED-0` | Phase 8: promote contention resolution to the EXECUTED RF arena — claims reduce up, the authored resolution rule allocates, results disburse down. **The rule is AUTHORED DATA, never a kind or enum branch**: proportional (cooperative), priority-ordered (queueing), price-clearing (2.3 order-weight already does this), attrition (adversarial — this is what was called combat). NOT ALL CONTENTION IS ADVERSARIAL and the engine must not assume it is. | Executed path judged green; **emergence falsifier: changing ONLY the authored resolution rule changes the outcome with no code change**; prior rehearsals re-anchor as scenario-neutral oracles. | DA-reserve · Frontier — Codex 5.6/Fable | TODO |
...
| Item | State |
|---|---|
| Active open rung | `COMPARATIVE-DEFAULT-BIRTH-0` |
