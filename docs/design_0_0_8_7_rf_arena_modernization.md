...
| 1.3 | `ARENA-PARTICIPANT-DEPRECATION-0` | Phase 1: eliminate the synthetic participation wrapper, its scaffold, and reserved-gap pools; admitted SimThings host flow properties on their own rows and carry resource-parent membership in the registry. | **IMPLEMENTED / pending DA** — wrapper kind/module absent; consumers resolve own-row slots; sparse child sets lower to one ordered `INPUT_LIST` Sum writer per target/band through existing op machinery; live-GPU sparse and dynamic-fission referees prove exact economics, RF-1, stable resync, and replay; elimination ledger complete. | DA-reserve · Frontier — Codex 5.6/Fable | **EXIT-PROOF GREEN / pending DA** — `HD-RECEIPT: 28d2e8e276f2`; workspace build, live-GPU driver battery, RF-1, successful canonical TP baseline/final fingerprint, orientation, and doc budget PASS; doctrine hard failures 0; economics diffs EMPTY; current harness clearance `DA-RESERVE(class-envelope-violation)`; evidence: `docs/tests/arena_participant_elimination_ledger.tsv` |
