...
| 4.1 | `ROW-SLOT-OBJECT-SEMANTICS-0` | Phase 4: parent-slotting + child-row allocation as object semantics through the role pathway. | **PROBATION / proof-present / DA-review-pending** — typed object-issued residency is the sole production structural row door; allocator-derived layouts match the legacy canonical + uneven fixtures exactly; install/AddChild/fission-clone/reparent/tombstone and sidecar rejection referees green; structural file splits landed with `mixed_ruled=0`. | DA-reserve · Frontier — Codex 5.6/Fable | **PROBATION / DA-review-pending** |
...
| Item | State |
|---|---|
| Active open rung | `PLAN-STRUCT-TYPING-0` |
