...
| 5.8 | `GUYANG-COMPARATIVE-PROJECTIONS-0` | Phase 5: dominance/margin/contest/border-band/chokepoint projections; default-derived at ≥2 emitter classes; deterministic tie-break; consumes generic field-sweep outputs. | **STOP / DA-residue (Remand 1+1A)** — driver-only consumer over generic field-sweep (kernel/GPU/allowlist reverted). Exact top1−top2 ⟂ sign-flip border; generic Gu-Yang stall column underdetermined; install-path default-derived Anchored birth + unmodified TP witness not discharged. See `docs/tests/guyang_comparative_projections_0_results.md`. | Std — Grok | **STOP / DA-residue** |
