...
| 2.1 | `OVERLAY-EFFECT-HOST-ADMISSION-0` | Phase 2: close the effect-target v0 trap — host placement + `overlay_hosts` admission-checked; misdelivery = spanned error. | **PROBATION / proof-present / DA-review-pending** — missing-property fixture hard-errors with overlay id + resolved host + property key + source span; correctly-hosted twin admits with `overlay_hosts == affects == placement` and transforms; existing full `simthing-driver` live-GPU battery green unmodified. | DA-reserve · Frontier — Codex 5.6/Fable | **PROBATION / proof-present / DA-review-pending** — `HD-RECEIPT: dbace3440530`; driver battery green; workspace/doctrine gates recorded in relay. |
...
| Item | State |
|---|---|
| Active open rung | `CAPABILITY-PREREQ-DAG-ADMISSION-0` |
