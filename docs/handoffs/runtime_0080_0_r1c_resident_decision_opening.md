...
## 2. The non-negotiable anti-drift contract (applies to R1c and every later rung)
...
3. **The journal is the only cross-tier structural hand-off.** Bounded boundary maintenance consumes journal rows and applies them; it must never re-derive decisions from value columns.
4. **Equivalence is a test, not a hope.** Keep an assertion that the structural authority equals the CPU oracle every tick (R1b proves this via exact per-tick journal parity). Any rung that adds structural residency must keep an equivalent per-tick structural-parity gate so drift is caught at the tick it starts, not 44 ticks later.
...
## 3. The "do not starve the GPU on the CPU" contract

The drift temptation came from coupling the GPU tick to the CPU boundary pass. Forbidden. The pipeline must be **decoupled**:
...
3. **CPU boundary maintenance runs at the boundary cadence** (save/pause/stable-state point), draining accumulated journal rows in bounded batches. It is a consumer, never a gate on GPU progress.
...
5. **Backpressure, not stalls.** If the CPU consumer falls behind, the journal grows (or applies a bounded ring with a documented high-water mark) — the GPU does not wait. Document the chosen policy.

## 4. R1c scope (what flips the PARTIAL to PASS)
...
This sits **behind** the §11 / free-list-scatter stop-lines and the M-4A / multi-atlas gate. Do not pull those forward.

## 5. Stop conditions (return to Opus)
