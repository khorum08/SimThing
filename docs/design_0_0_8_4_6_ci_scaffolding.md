...
| D2g | `TEST-PARE-CPU-SAFE-BOUNDARY-SWEEP-0` | Delete remaining CPU-safe fossil residue (table shells, hygiene cleanup) and disposition all live CPU-safe collapse rows. | **PROBATION** | 13 empty spec admission shells deleted; scenario_ingestion helper cleanup; 380 boundary rows dispositioned; 365 blocked (GPU/compile/source/kernel-sim); inventory unchanged at 5,675; profile `test-pare-cpu-safe-boundary-sweep`. |
