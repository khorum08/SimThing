...
| D2d | `TEST-PARE-TIER2-CPU-ADMISSION-COLLAPSE-0` | CPU-safe Tier 2 admission-boundary collapse over `simthing-clausething` and `simthing-mapgenerator`. | **PROBATION** | 86 CPU-safe boundary mappings considered (82 live + 4 historical PARED); 33 live admission-enumeration tests deleted; 8 representatives retained; live inventory 5,706 rows; targeted Doctrine Exec profile is exact CPU-side binaries only and excludes driver/gpu/mapeditor/tools/workshop/kernel/sim plus Bevy/WGPU/desktop proof surfaces. |
