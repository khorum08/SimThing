...
| D2r | `TEST-PARE-PROTECTED-REPRESENTATIVE-RESTORE-0` | Restore Fable-required protected representatives for SoftAggregateGuard boundary-firing and fission capability-OVERLAY inheritance. | **HOLD - DA/Owner review required** | `docs/tests/test_repair_or_replace_true_oracle_binaries_0_results.md`; adds two behavior-regression representatives with `catches:` notes, records +2 inventory/audit/boundary rows, and makes no product logic, gate-state, scanner, allowlist, kernel, or spec edits. |
