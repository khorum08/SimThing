...
| D2c | `TEST-CONSOLIDATE-CLASSIFIER-FAMILIES-0` | Cross-crate hygiene-theater classifier-input consolidation under Fable consolidation-exit. | **PROBATION** | All 134 live `CONSOLIDATE_TO_TABLE` rows processed (`B-T4-HYGIENE-THEATER-CONSOLIDATION` only); 132 independent integration tests retired into 7 crate table representatives preserving classifier-input labels; 2 driver source-level rows `PROMOTION_REQUIRED`; live inventory 5,745 rows; `test-consolidate-classifier-families` Doctrine Exec profile runs seven exact table binaries only. |
