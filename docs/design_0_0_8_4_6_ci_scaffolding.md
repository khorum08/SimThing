...
| D2i | `TEST-PARE-SRC-UNIT-FOSSIL-RESIDUE-0` | Delete src cfg-test admission/usecase fossil residue across protected crates under DA-approved temporary src scope. | **PROBATION** | 88 src unit rows deleted (mapeditor 42, kernel 15, core 9, sim 9, driver 6, gpu 6, feeder 1); inventory 5,583; profile `test-pare-src-unit-fossil-residue`; `cargo check -p` proof only. |
