...
| D2d | `TEST-PARE-TIER2-CPU-ADMISSION-COLLAPSE-0` | CPU-safe Tier 2 admission-boundary collapse over `simthing-clausething` and `simthing-mapgenerator`. | **PROBATION / merge-reviewable after docs 0R** | 33 clausething Tier 2 tests deleted, 8 representatives retained, mapgenerator rows blocked for checker-law follow-up, final targeted CPU-only proof PASS. |
