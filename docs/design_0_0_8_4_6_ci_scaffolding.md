...
|---|---|---|---|---|
| D0–D2z | *(collapsed)* `TEST-PARE-*` incremental boundary-paring era | Inventory/ledger construction (`TEST-PARE-INVENTORY-0`, `TEST-PARE-STANDARD-DA-0`, `TEST-ADMISSION-REGIME-0`), per-crate/per-boundary collapse waves, protected-class audit + residue deletion, GHA proof-seal + compile-floor gates, Studio/typeface owner-deep review, and the mapgenerator promotion wave. | **COLLAPSED — SUPERSEDED BY `TEST-NECESSITY-SWEEP-0`** | ~30 merged rungs (#1086–#1121) drove inventory 6,301 → 4,070 under the retired one-representative-per-boundary premise. All gates they built (`test_inventory_check.sh`, `test_pare_boundary_check.sh`, `test_inventory_drift_check.sh`, `doctrine_exec_gha_proof_seal.sh`, `tests-compile-floor-non-bevy`) remain live and are inherited by the sweep and closeout. Provenance: git history + `current_evidence_index.md` + graduation-log blockquotes below. |
...
| D-RESUME | `CI-SCAFFOLDING-RESUME-AFTER-TRACK-D-0` | Post-closeout CI scaffolding resume map + Track-D-era CI artifact audit (profiles, scripts, TSVs). | **PROBATION / ORCHESTRATION REVIEW** | Docs-only. Verifies #1122/#1123/#1124 closure on live `master`; confirms Rustified lifecycle + closure-certificate rule; classifies `test-pare-*` Doctrine Exec profiles as executable-stale; audits CI-layer residue (`test_edit_scope*`, `test_pare_audit.tsv`, boundary machinery rename debt). Follow-ons: `CI-LIFECYCLE-RESIDUE-DELETE-0`, `CI-PROOF-PROFILE-TAXONOMY-0`. Evidence: `docs/tests/ci_scaffolding_resume_after_track_d_0_results.md`. |
...
- **Boundary ownership:** `test_pare_boundary_check.sh` requires every KEEP to name its owning boundary + tier;
