...
| 1b | `CI-A-ALLOWLIST-SCANS-0` | **The rigor upgrade — closed-set allowlist scans (RELIABLE), so a clean result is trusted without DA re-verification.** Three allowlist files in `scripts/ci/allow/`, each a reviewed sanctioned set; *anything outside the set FAILs* (catches novel/subtle holes a blocklist misses): **(a) sealed-type producers** — every `pub fn … -> (Self\|ThresholdEvent\|EmissionRecord\|ThresholdEmission\|PlacedParticipant\|ResolvedWriteAuthority\|…)` in the authority crates must be on the sanctioned-door allowlist (`read_*`/`dispatch_*`/`apply_*`/`cpu_oracle_*`); **(b) authoritative buffer handles** — every `pub` `&Buffer`/`Buffer` field/accessor in `simthing-kernel` must be `pub(crate)` *or* on the inert-utility allowlist; **(c) kernel public surface** — the kernel's exported `pub` items are a closed reviewed set; a new public item not on the list FAILs. | Cursor/Grok | each allowlist scan FAILs when a fixture adds an unsanctioned producer/handle/surface item; adding a *legit* new door requires an explicit allowlist edit (deliberate, greppable in the diff); the doc-hidden-minter fixture FAILs without any blocklist pattern for it. |
