...
| D2s | `TESTS-COMPILE-FLOOR-NON-BEVY-0` | Standing non-owner-deep GHA `cargo check -p <crate> --tests` floor for GHA-safe crates. | **PROBATION / DA REVIEW after 0R** | `docs/tests/tests_compile_floor_non_bevy_0_results.md`; profile `tests-compile-floor-non-bevy` includes only five non-desktop crates; hardened `doctrine_exec_profile_lint.sh` forbids ALSA, libudev/udev, X/Xvfb, Wayland, xkbcommon, xcb, EGL/GLX, Mesa/Vulkan, Bevy, winit/wininit, wgpu, mapeditor, typeface, apt-get, and desktop/GPU setup tokens plus blocked crates in all `-p`/`--package` forms in `owner_deep=false` executable profile commands; gate-state — not orchestrator-mergeable. |
