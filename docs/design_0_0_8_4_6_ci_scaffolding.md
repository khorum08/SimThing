...
| F | `CI-A-CLOSEOUT-0` | Record the **DA-equivalence contract** (agents treat PASS as DA-ran; FAIL as DA-HOLD; INSPECT → DA) **and the retirement contract** (promoting an invariant to a type boundary in any future rung *must* delete its now-redundant scan in the same PR — scans shrink as types grow) in this doc + a one-line pointer in the handoff template so handoffs cite "CI doctrine-scan = the automated DA scan layer" **and carry a mandatory `seal-residue-risk` declaration** (does this rung touch a sanctioned door?). | Opus/Owner (DA) | OPEN | both contracts recorded; reliability legend published; handoff spine updated; track CLOSED. |
