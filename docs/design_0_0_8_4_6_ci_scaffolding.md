...
| D2w | `TEST-PARE-STUDIO-ADMISSION-SINGLETON-BOUNDARY-0` | Focused boundary ruling for two Studio/mapeditor admission singleton rows escalated by #1115. | **PROBATION / DA REVIEW** | `docs/tests/test_pare_studio_admission_singleton_boundary_0_results.md`; classification-first; no forbidden mapeditor/Studio execution; both rows ruled `KEEP_SELECTED_REPRESENTATIVE`; any source deletion requires owner-local survivor proof; deletion waves must never rewrite preserved representatives. |
