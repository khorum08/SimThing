...
| D2n | `TEST-PARE-PROTECTED-PARE-DELETE-0` | Physically delete the unprotected false-member PARE rows produced by the protected-residue ledger cut. | **DONE — DA-APPROVED** (2026-07-03; DA-verified head + gates + SHA-bound targeted exec; merged #1104 at `be1bd0761ef9`) | `docs/tests/test_pare_protected_pare_delete_0_results.md`; #1103 merged at `f32a0a93db068f95cf5201860063951568cfdec0`; 98 FALSE_MEMBER-to-PARE rows physically deleted, 0 blocked, inventory 5,332 -> 5,234; TRUE_MEMBER, NEEDS_PROMOTION, dependency-floor, STEAD, seal, golden, doc-named, SimThing-Kernel, and CI Scaffolding rows untouched; profile `test-pare-protected-pare-delete` is cargo-check-floor only. |
| D2o | `TEST-PARE-CONSERVATIVE-SURVIVOR-DELETE-0` | DA-executed review + deletion of the 112 conservative promotion-review survivors (owner §17 authority). | **DA-EXECUTED** (2026-07-03) | 87 fossils deleted + 2 emptied files removed (inventory 5,234 → 5,147); 25 keeps reclassified to permanent residue with judgment `catches:` notes (sanctioned-door contracts, invariants-named representatives, STEAD/PALMA field-not-route, atlas default-off, determinism); all 112 promotion-review rows retired; #1104 scope rows re-sealed + this rung's temporary rows added; `determinism` class-enum gap fixed; **MAJOR FINDING recorded: sim c-suite TRUE_MEMBER parity binaries are non-compiling on master (pre-existing seal-adoption drift; inert gates §0.6.6) — named follow-on `TEST-REPAIR-OR-REPLACE-TRUE-ORACLE-BINARIES-0`**; evidence `docs/tests/test_pare_conservative_survivor_delete_0_results.md` + 114-row review table. |
| D2p | `TEST-REPAIR-OR-REPLACE-TRUE-ORACLE-BINARIES-0` | Retire dead TRUE_MEMBER oracle/parity fossil binaries under corrected non-kernel, non-admission scope. | **DONE — DA-APPROVED (2026-07-03, corrected 0I scope; DA deep review — see graduation log; residue = named follow-on rungs)** | `docs/tests/test_repair_or_replace_true_oracle_binaries_0_results.md`; INTERVENTION 0I applied after scope review: CI/gate-state profile/scope edits reverted, admission-substrate deletions restored/deferred, simthing-kernel untouched, and deletion retained only across allowed non-kernel/non-admission surfaces. Inventory now records 5,147 -> 4,252 rows after restoring 30 admission rows; 895 allowed-surface rows remain deleted. #1106 was later DA-split/merged; Fable ruling requires D2r representative restore before the protected-representative surface is settled. No new profiles, no new replacement binaries, no scanner/allowlist edits. |
| D2q | `TEST-PARE-CLAUSETHING-STUDIO-DEAD-BINARIES-0` | Delete Clausething/Studio fossil tests that #1106/0I deferred too conservatively while preserving protected corpora. | **DONE — DA-APPROVED (2026-07-03; orchestrator-cleared per §1A(B), DA spot-audited via gates + tree)** | This wave deliberately protects Admission Substrate, SimThing-Kernel, and CI Scaffolding corpora while deleting redundant Clausething/Studio fossils superseded by the 0.0.8.4.x regimes. It deletes 3 non-live Clausething files / 9 rows, keeps current oracle/parity and doc-named Clausething enforcement rows, records Studio/typeface DA deferral, and makes no new profile, scope-row, scanner, allowlist, kernel, spec, or CI-gate edits. |
| D2r | `TEST-PARE-PROTECTED-REPRESENTATIVE-RESTORE-0` | Restore Fable-required protected representatives for SoftAggregateGuard boundary-firing and fission capability-OVERLAY inheritance. | **DONE - merged #1111** | `docs/tests/test_repair_or_replace_true_oracle_binaries_0_results.md`; two behavior-regression representatives with `catches:` notes; +2 inventory rows; merge commit `c49abf044950132da76e365357c7f6320ca498ed`; no product logic or gate-state edits. |
| D2s | `TESTS-COMPILE-FLOOR-NON-BEVY-0` | Standing non-owner-deep GHA `cargo check -p <crate> --tests` floor for GHA-safe crates. | **DONE — DA-APPROVED (2026-07-03, deep review: guard perturbed with two DA-chosen probes and bit both times; five-crate floor re-run green locally; surface-truth INSPECT triaged GREEN — GHA cargo-public-api tooling gap, local run on identical head PASSES baseline; D2s follow-up closed by D2y)** | `docs/tests/tests_compile_floor_non_bevy_0_results.md`; profile `tests-compile-floor-non-bevy` includes only five non-desktop crates; hardened `doctrine_exec_profile_lint.sh` forbids ALSA, libudev/udev, X/Xvfb, Wayland, xkbcommon, xcb, EGL/GLX, Mesa/Vulkan, Bevy, winit/wininit, wgpu, mapeditor, typeface, apt-get, and desktop/GPU setup tokens plus blocked crates in all `-p`/`--package` forms in `owner_deep=false` executable profile commands; gate-state — not orchestrator-mergeable. **0R2 (owner-found, DA-executed 2026-07-03):** the lint scanned only `tests`/`doc_tests` but the engine also executes the **`crate_checks`** column — four merged closed-wave profiles carried blocked crates (`simthing-driver`/`gpu`/`mapeditor`) there; the lint now scans `crate_checks`, the four profiles are scrubbed, a negative-control prove case guards the column, and `ci_screening_surface.md` §10 lands the owner ruling *deletion never licenses a desktop/GPU probe; the owner's local run is authoritative for the delete decision*. Evidence: `docs/tests/tests_compile_floor_non_bevy_0r2_results.md`. |
| D2t | `TEST-PARE-LINGERING-OWNER-DEEP-DEAD-TESTS-0` | Delete lingering owner-deep legacy-default fossil rows without Linux desktop/GPU probing. | **DONE — ORCHESTRATOR-CLEAR / merged #1114 (`59d9b61cbf`)** | `docs/tests/test_pare_lingering_owner_deep_dead_tests_0_results.md`; deletes 5 `simthing-driver` usecase-superseded legacy-default test functions while retaining same-file behavior representatives and spec-side coverage; inventory 4,245 -> 4,240; no protected corpus, SimThing-Kernel, CI profile/gate, scanner, allowlist, workflow, or `.github` edits; no driver/GPU/Studio/tools Linux probe. |
| D2u | `TEST-PARE-STUDIO-TYPEFACE-OWNER-DEEP-0` | Review Studio/typeface owner-deep residue under the Section 10 no-Linux-desktop-probe deletion rule. | **DONE — INTERIM REVIEW CONSUMED BY D2v + D2w** | Merged as an accurate interim review-only rung with zero webchat-executable deletions. **DA finding (2026-07-03):** split 845 unclassified AUDIT rows into closed-ladder fossil DELETE candidates vs live Studio behavior KEEP. Follow-ons now closed: `TEST-PARE-TYPEFACE-LADDER-CLOSED-FOSSIL-DELETE-0` (D2v, DA-APPROVED) and `TEST-PARE-STUDIO-ADMISSION-SINGLETON-BOUNDARY-0` (D2w, DA-APPROVED, #1117). Star-nameplates visual sign-off remains separate product rung. |
| D2v | `TEST-PARE-TYPEFACE-LADDER-CLOSED-FOSSIL-DELETE-0` | Delete closed TYPEFACE-LADDER LR* fossil rows in `simthing-tools` after row-level classification. | **DONE — DA-APPROVED (2026-07-03, deep review + DA repair)** | 170 closed-ladder fossils deleted (inventory 4240→4070); 5 whole-file deletions verified removed; 10 never-pare representatives verified (5 golden-byte + 5 admission). **DA repair:** the survivor compile floor for the edited crate — which §10.3 reserves to the owner's local run and Codex correctly could not run on GHA — found `typeface_lr6.rs` **non-compiling**: its two survivors had been silently *rewritten* to `read_fonts::FontRef` (not a dependency) instead of master's `msdf_font::ttf_parser::Face`. DA reconstructed `typeface_lr6.rs` from master (17 fossils deleted, both survivors restored **master-verbatim**, compiles green on the owner's machine); the other 9 files' representatives confirmed master-verbatim. **Wave-integrity lesson recorded:** a deletion wave deletes fossil fns ONLY and must never rewrite a preserved representative. Manifest: `docs/tests/typeface_ladder_closed_fossil_manifest.tsv`. |
| D2w | `TEST-PARE-STUDIO-ADMISSION-SINGLETON-BOUNDARY-0` | Focused boundary ruling for two Studio/mapeditor admission singleton rows escalated by #1115. | **DONE — DA-APPROVED (2026-07-03, executive DA review; merged #1117 @ `d32168aee0`)** | Both rows `KEEP_SELECTED_REPRESENTATIVE`; 0R synced inventory/audit ledgers to `selected-admission-representative / KEEP`. **DA verification against the tree:** the already-merged `test_pare_boundary_rows.tsv` names each row the *sole* KEEP for its boundary (`B-T2-…PARSER_SPAN_ADMISSION` with 11 collapse variants; `B-T2-…MISSING_OR_UNKNOWN_REFERENCE_ADMISSION` with 8) — the sync makes inventory/audit agree with that authority, not the reverse; both source tests confirmed live admission (`StudioConfigError::Deserialize`; `UnsupportedGridcellRole` deferral), not obsolete behavior; two promotion targets well-formed (29→31); no source edits; no forbidden mapeditor/Studio/GPU execution; Doctrine Scan PASS, Doctrine Exec correctly not scheduled. |
| D2x | `TRACK-D-POST-DA-STATUS-SYNC-0` | Sync Track D post-DA statuses after #1114–#1117 closure. | **DONE — ORCHESTRATOR-CLEAR / merged #1118** | Docs-only closure hygiene; aligns design ledger and current evidence index; no source, CI, inventory, audit, boundary, or promotion-ledger edits. |
| D2y | `SURFACE-TRUTH-INSPECT-REASON-SPLIT-0` | Split surface-truth INSPECT reporting into match/divergence/tooling-gap. | **DONE — DA-APPROVED (2026-07-03, deep review; closes the D2s follow-up)** | CI proof-semantics rung; no source edits; preserves PASS/INSPECT authority while making triage reason machine-readable (`SURFACE-TRUTH-REASON: match\|divergence\|tooling-gap`, unknown→INSPECT). **0R:** synthetic probes quarantined behind `DOCTRINE_SURFACE_TRUTH_SYNTHETIC_ALLOWED=1` (default 0), gated *before* the `cargo-public-api` check; offline reason-split proof no longer depends on the tool; production behavior unchanged. **DA anti-fabrication perturbation:** with the env guard unset, `synthetic-divergence` does **not** forge an INSPECT — it falls through to the real check (verified: unset→PASS/match, set→INSPECT/divergence); the env var is set only in the test harness, no workflow/profile/exec path sets it, and enabling it would require a reserved-class `.github/` edit; probe-path change is a benign grep-pattern update; reason-test 11/11 PASS incl. exec taxonomy mapping. Evidence: `docs/tests/surface_truth_inspect_reason_split_0_results.md`. |
