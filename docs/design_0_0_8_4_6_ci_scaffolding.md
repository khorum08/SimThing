...
| D2c | `TEST-CONSOLIDATE-CLASSIFIER-FAMILIES-0` | Cross-crate hygiene-theater classifier-input consolidation under Fable consolidation-exit. | **0R / merge-reviewable** | 132 independent integration tests retired; one CPU-only `simthing-spec` table (`table::track-d::hygiene_theater_cases`) preserves all case labels; 0R rejected Bevy/Linux desktop bootstrap proof; `test-consolidate-classifier-families` profile runs one CPU-side exact binary only; live inventory 5,739 rows. |
