...
| D-PROFILE-TAXONOMY | `CI-PROOF-PROFILE-TAXONOMY-0` | Retire stale Track-D `test-pare-*` Doctrine Exec profiles; guard executable profile table. | **PROBATION / DA-OWNER REVIEW** | Retired 11 Track-D deletion profiles; retained 6 current proof profiles; added `FORBIDDEN-TRACK-D-PROFILE` lint + `--prove-no-track-d-deletion-profiles`; deleted uninvoked `protected_class_audit.py`. Executable profiles must be current proof surfaces — not historical deletion batteries. Closure-certificate `cargo test --workspace --all-targets` remains outside the profile table. Evidence: `docs/tests/ci_proof_profile_taxonomy_0_results.md`. |
