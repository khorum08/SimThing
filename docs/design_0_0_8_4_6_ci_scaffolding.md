...
| 1 | `CI-A-SCAN-DEFS-0` | **The auditable data home.** `scripts/ci/scans.tsv` (one line per scan: `id \| severity(RELIABLE/HEURISTIC) \| target-glob \| pattern \| exclude \| doctrine-ref`) + `scripts/ci/allow/*.txt` (allowlists, one entry per line) + `scripts/ci/README.md` (layout + how to add/edit a scan or allowlist entry). Seed it with §5. | Cursor/Grok | OPEN | data files exist; README documents add/edit; a new scan = one new `scans.tsv` line; a new sanctioned door = one new `allow/*.txt` line. No heuristic lives in shell. |
...
| 4 | `CI-A-WORKFLOW-0` | `.github/workflows/doctrine-scan.yml` — `ubuntu-latest`, no toolchain: checkout → run self-test → run PR scan → publish the report (job summary + uploaded artifact the DA reads). Hard FAIL fails the check; INSPECT annotates. | Cursor/Grok | OPEN | a known-bad PR fails the check; a clean PR passes; an INSPECT-only PR passes with the flag visible in the report. |
| F | `CI-A-CLOSEOUT-0` | Record the **DA-equivalence contract** (agents treat PASS as DA-ran; FAIL as DA-HOLD; INSPECT → DA) in this doc + a one-line pointer in the handoff template so handoffs cite "CI doctrine-scan = the automated DA scan layer." | Opus/Owner (DA) | OPEN | contract recorded; reliability legend published; track CLOSED. |
