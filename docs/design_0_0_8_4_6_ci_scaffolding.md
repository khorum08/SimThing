...
| D2u | `TEST-PARE-STUDIO-TYPEFACE-OWNER-DEEP-0` | Review Studio/typeface owner-deep residue under the Section 10 no-Linux-desktop-probe deletion rule. | **HOLD / DA REVIEW** | `docs/tests/test_pare_studio_typeface_owner_deep_0_results.md`; review-only implementation with zero deletions because the current boundary ledger leaves no live Studio/typeface DELETE row. `simthing-tools` typeface rows and `simthing-mapeditor` LR8 rows are retained as behavior/golden/admission representatives; active Terran-Pirate row remains `B-T7-ACTIVE-TP-LIVE-RUNG / NEVER_PARE`; stale LR7 audit-collapse notes are superseded by boundary rows. Risk class: owner-deep deletion wave / Studio-typeface residue. |
