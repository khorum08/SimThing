...
| D2s | `TESTS-COMPILE-FLOOR-NON-BEVY-0` | Standing non-owner-deep GHA `cargo check -p <crate> --tests` floor for GHA-safe crates. | **PROBATION / DA REVIEW** | `docs/tests/tests_compile_floor_non_bevy_0_results.md`; profile `tests-compile-floor-non-bevy`; five GHA-verified included crates (driver deferred: `alsa-sys` on ubuntu); blocked/deferred documented; prevents dead test-binary reform after #1106/#1111; gate-state — not orchestrator-mergeable. |
