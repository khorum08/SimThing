...
| D2t | `TEST-PARE-LINGERING-OWNER-DEEP-DEAD-TESTS-0` | Delete lingering owner-deep legacy-default fossil rows without Linux desktop/GPU probing. | **PROBATION / ORCHESTRATOR REVIEW** | `docs/tests/test_pare_lingering_owner_deep_dead_tests_0_results.md`; deletes 5 `simthing-driver` usecase-superseded legacy-default test functions while retaining same-file behavior representatives and spec-side coverage; inventory 4,245 -> 4,240; no protected corpus, SimThing-Kernel, CI profile/gate, scanner, allowlist, workflow, or `.github` edits; no driver/GPU/Studio/tools Linux probe. |
| D2u | `TEST-PARE-STUDIO-TYPEFACE-OWNER-DEEP-0` | Review Studio/typeface owner-deep residue under the Section 10 no-Linux-desktop-probe deletion rule. | **INTERIM MERGED — DELETION WAVE OPEN (DA finding 2026-07-03)** | Merged as an accurate interim; escalations accepted (2 Studio-admission singletons → focused ruling; star-nameplates → owner visual sign-off; LR7 stale notes superseded). **DA finding:** "zero deletions" was correct only *for a webchat agent* (§10.3 reserves the Bevy/desktop delete decision to the owner's local run) but conflated the **845 unclassified AUDIT rows** with must-keep. Split: **175 closed-TYPEFACE-LADDER `LR*` AUDIT rows** = fossil DELETE candidates; **618 `simthing-mapeditor` + remaining tools rows** = live Studio behavior (active 0.0.8.5 consumer, no kernel representative) = KEEP. Follow-on: `TEST-PARE-TYPEFACE-LADDER-CLOSED-FOSSIL-DELETE-0`. Risk class: owner-deep deletion wave. |
