...
| D-BIRTH-TRIPWIRE | `CI-LIFECYCLE-BIRTH-TRACK-TRIPWIRE-0` | Mechanize the test **lifecycle** (default-delete-at-track-close) as a residue-as-tripwire: `birth_track` schema field + `test_lifecycle_expiry_check.sh`. Track-A/ledger-layer, **not** §3B (this is text/data analysis, not executable proof). | **OPEN — DA-authored (2026-07-04); implement BEFORE 0.0.8.5 births new tests** | **Rationale:** the regime's *form* is CI-enforced (class + catches-note + boundary ownership + drift) but its *lifecycle* is not — there is no `birth_track` field, so nothing surfaces a test when its birth ladder closes. That gap is why paring had to be one 3,478-test manual sweep instead of continuous cleanup. **DoD:** (1) add `birth_track` (origin-ladder id) column to `test_inventory.tsv` schema; every new KEEP row records its birth ladder; existing rows backfill to their known track or `pre-lifecycle`. (2) `scripts/ci/test_lifecycle_expiry_check.sh` flags any row whose class is **non-durable** (behavior-regression — NOT oracle-parity/golden-byte/seal-proof/stead-required/determinism/dependency-floor) **AND** whose `birth_track` is closed **AND** which declares no `downstream-utility:` justification → **INSPECT** (surface for the DA Necessity-Test delete call; never an auto-delete — the *decision* stays DA judgment, the *surfacing* is mechanized). (3) machine-parseable footer `LIFECYCLE-EXPIRY-VERDICT: PASS\|INSPECT expired=N`; wire into the inventory-check chain. (4) fire on **track-closeout PRs** (the close event) + the **scheduled sentinel** (accumulated backlog); NOT whole-tree every-PR (would re-flag the backlog and drown triage — delta/schedule-scoped like HEURISTIC scans). (5) one perturbation prove-case: a synthetic non-durable row with a closed birth_track and no downstream-utility → INSPECT; a durable-class row or a justified row → clean. **Semantic residue (unchanged):** whether a `catches:`/`downstream-utility:` note is *true* stays DA judgment — the tripwire surfaces candidates, it does not adjudicate them (no semantic content scan; that would be §1A hill-climbing bait). **Optional later:** a thin `/lifecycle-expiry` ChatOps command riding the `CI-B-GH-TRIAGE-0` engine (§3B) — not part of this rung. Evidence: `docs/tests/ci_lifecycle_birth_track_tripwire_0_results.md`. |
