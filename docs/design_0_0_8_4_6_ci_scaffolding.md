...
| D2v | `TEST-PARE-TYPEFACE-LADDER-CLOSED-FOSSIL-DELETE-0` | Delete closed TYPEFACE-LADDER LR* fossil rows in `simthing-tools` after row-level classification. | **PROBATION / DA REVIEW** | `docs/tests/test_pare_typeface_ladder_closed_fossil_delete_0_results.md`; deletes closed-ladder capability-proof fossils; preserves golden-byte and selected admission representatives; no forbidden execution; DA-held. Manifest: `docs/tests/typeface_ladder_closed_fossil_manifest.tsv`. |
