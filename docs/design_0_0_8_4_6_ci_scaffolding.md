...
| D2u | `TEST-PARE-STUDIO-TYPEFACE-OWNER-DEEP-0` | Review Studio/typeface owner-deep residue under the Section 10 no-Linux-desktop-probe deletion rule. | **INTERIM MERGED — DELETION WAVE OPEN (DA finding 2026-07-03)** | Merged as an accurate interim; escalations accepted (2 Studio-admission singletons → focused ruling; star-nameplates → owner visual sign-off; LR7 stale notes superseded). **DA finding:** "zero deletions" was correct only *for a webchat agent* (§10.3 reserves the Bevy/desktop delete decision to the owner's local run) but conflated the **845 unclassified AUDIT rows** with must-keep; the wave's candidate manifest was not produced. Split: **175 closed-TYPEFACE-LADDER `LR*` AUDIT rows** = fossil DELETE candidates pending the owner's local run; **618 `simthing-mapeditor` + remaining tools rows** = live Studio behavior (active 0.0.8.5 consumer, no kernel representative) = KEEP. Follow-on: `TEST-PARE-TYPEFACE-LADDER-CLOSED-FOSSIL-MANIFEST-0`. Risk class: owner-deep deletion wave. |
| D2v | `TEST-PARE-TYPEFACE-LADDER-CLOSED-FOSSIL-MANIFEST-0` | Produce the DELETE-candidate manifest over the 175 closed TYPEFACE-LADDER `LR*` AUDIT rows (classification only, no execution) for the owner's §10.3 authoritative local run. | **OPEN** | Classify each closed-ladder `LR*` AUDIT row: DELETE-candidate (closed-rung capability-proof fossil) vs never-pare (golden-byte glyph byte-exactness / selected admission). No cargo/compile/probe of Bevy/desktop crates (§10). Deliver the manifest; the owner runs the local compile floor + deletes. |
