...
**Owner-local proof citation contract (`CI-B-CLOSEOUT-0`, 2026-07-04).** GPU/Bevy/Desktop proof remains
owner-local-only: GHA must not run those binaries or install desktop/GPU dependencies. But a fresh owner-local
`DOCTRINE-TESTS-VERDICT: PASS` report is **citable validation** for the owner-local-only proof class when a
GitHub-side check, Doctrine Exec report, webchat orchestrator, or DA review needs that proof — GPU, Bevy,
Studio/`simthing-mapeditor`, desktop/typeface, or real-adapter parity. To be citable, the report must match the
current PR `head_sha`, name the tested profile, carry `owner_local=true`, preserve the strict `DOCTRINE-TESTS-VERDICT`
footer, and emit the relevant PASS tripwire tag(s) (`COMPILE_FAIL_PROVEN` / `PARITY_BIT_EXACT` / `OWNER_LOCAL_PASS`).
`INSPECT` is never validation; stale or `head_sha`-mismatched local reports are rejected. Citing owner-local PASS
proof does **not** license GHA-side execution of GPU/Bevy/Desktop probes — it only recognizes a fresh owner-local
green report as evidence for a class GitHub is structurally unable or forbidden to execute. GitHub-side CPU proof
(Doctrine Exec, §3B) remains the nonblocking, citable path for CPU proof classes; the two lanes never merge.
...
| 1 | `CI-B-LOCAL-HARNESS-0` | `scripts/ci/doctrine_tests.sh` (local) — **narrowed to the GPU/Bevy-exclusive residue (owner-ratified 2026-07-02)**: real-adapter parity/terminal proofs and Studio/`simthing-mapeditor` legs only. Everything CPU-side moves to `CI-B-GH-CPU-0`. Batch invocations, never spawn-per-fixture (§3C simplicity debt). Emits the §1 report. | Cursor/Grok | **DONE — DA-APPROVED (2026-07-04, deep review)** | **DA verification (DA-run):** scope clean; single narrow `owner-local-gpu-bevy` profile resolving from live inventory (not a hardcoded transcript); `--prove-report` PASS incl. malformed-footer + GHA-overlap guard (local commands absent from `doctrine_exec_profiles.tsv`); under `GITHUB_ACTIONS=true` emits `owner_local:true`+**INSPECT**, never a silent PASS (correct posture — real GPU/Bevy proofs are owner-invoked on seal-residue rungs); gates green. |
| 2 | `CI-B-TRIPWIRE-TAGS-0` | Executable-specific tripwires in `doctrine_tests.sh`: owner-local reports emit explicit `--- tripwire-tags ---` (`GPU_SKIPPED`, `BEVY_SKIPPED`, `DESKTOP_SKIPPED`, `OWNER_PREREQ_MISSING`, `PLAN_ONLY`, `GITHUB_ACTIONS_REFUSAL`, `NO_LIVE_LOCAL_PROOF_TARGET`, `FLAKY`, `PERF_VARIANCE` → INSPECT; `COMPILE_FAIL_PROVEN`, `PARITY_BIT_EXACT`, `OWNER_LOCAL_PASS` → PASS); strict `DOCTRINE-TESTS-VERDICT` footer preserved; `--prove-report` enforces tag contract; no GHA owner-local execution. | Cursor/Grok | **DONE — DA-APPROVED (2026-07-04, deep review; merged #1132 @ `d1b75c57`)** | **DA verification (DA-run):** central invariant confirmed on a real no-GPU host — `--plan` and live `--profile owner-local-gpu-bevy` both return INSPECT with explicit skip tags, never a silent PASS; strict footer intact; `--prove-report` PASS incl. fake-PASS-without-tags and malformed-footer bites; GHA proof-seal + no-track-d-deletion prove PASS; scope clean (4 files). Non-blocking refinements recorded under `CI-B-CLOSEOUT-0`. Evidence: `docs/tests/ci_b_tripwire_tags_0_results.md`. |
| F | `CI-B-CLOSEOUT-0` | Doctrine/status closeout of Track B: record the owner-local executable-proof contract and the **owner-local PASS citation rule** (§3 below), status-sync #1129/#1132, and record the #1132 non-blocking refinements. No new machinery; no workflow/product/cargo scope. | Opus/Owner (DA) | **DONE — DA-CLOSED (2026-07-04, graduated #1133)** | **DA graduation:** all 12 acceptance gates met; 4 docs only (no scripts/workflows/product/cargo); gates green (`doctrine_scan`/`gen_digest --check`/profile-lint/GHA-proof-seal/no-track-d-prove) + live CI `doctrine-scan` PASS. executable-proof contract + citation rule recorded; #1129/#1132 DONE/DA-APPROVED; refinements ledgered. Track B local executable-proof ladder CLOSED. Evidence: `docs/tests/ci_b_closeout_0_results.md`. |
...
## 4. Track D — test-corpus paring (CLOSED 2026-07-04): admission ladder applied to tests

> **Track D is CLOSED** (`TRACK-D-CLOSEOUT-0`, #1123). It ran as a corpus-maintenance track after
> `CI-B-WEBCHAT-PR1R`, collapsed the corpus from 6,301 to **731 rows** via `TEST-NECESSITY-SWEEP-0` (#1122),
> and landed the **Rustified Test Lifecycle** regime (§4.1) as standing law. This section is retained as the
> governing doctrine for *how tests live and die* — the Necessity Test (below) and the lifecycle regime (§4.1)
> are current law; the rung table below is collapsed to historical provenance. Do not reopen Track D or resume
> the D0–D2z sequence.

Fable High / DA ruling `DA-RULING: ADMISSION-BOUNDARY-COLLAPSE` retired the old source-file-family ledger to
legacy context only; deletion authority became boundary-keyed, with tests hanging off the proof boundary that
owns the invariant — and the Necessity Test then drove the floor to zero. That authority did its work and is
now spent: the sweep is complete. What remains live is the classification vocabulary (boundary tiers below) and
the lifecycle regime that keeps the pared corpus from re-accreting.

The burden of proof is on retention for admission-adjacent, hygiene-theater, and usecase-superseded rows. Historical examples are not inherently valuable. A test whose invariant is owned by a type/seal boundary, admission hard-error, doctrine scan, compile-fail proof, oracle, golden artifact, or required invariant is retired, collapsed, or consolidated according to the owning boundary tier. Rejection-class enumeration is kabuki: the production admission hard-error on a live path *is* the coverage, so the floor is zero — not "one representative." **The Necessity Test (2026-07-03, DA on owner mandate): the floor is ZERO, not one.** A test is admissible only if it catches a regression that neither the compiler / a type boundary, a production admission hard-error on a live path, nor an existing integration/canonical path already catches. If deleting it cannot break production and it is not a downstream dependency or required for canonical function, DELETE it. *"One representative per boundary" was a **catastrophic compromise of the Rustification mission** — a pre-substrate fossil premise (tests = coverage) admitted into the kernel-substrate era, which guaranteed the corpus could never shrink below (number of boundaries) regardless of redundancy and stalled the paring at diminishing returns. It is retired.* The DA that authored ADMISSION-BOUNDARY-COLLAPSE set the admission-tier count at "one" instead of following its own tier-1 "zero for type-enforced" logic to its conclusion; the Necessity Test completes that logic.
...
Proof route for operators (post-closure; material reduction has landed — corpus is 731 rows):

- Owner-deep full batteries are quarantined artillery, not routine proof.
- Full-workspace `cargo test --workspace --all-targets` is **not** a scheduled or routine battery. It is the one-time **PR-ladder closure certificate** (§4.1 / §3): run once to certify a ladder close, or to satisfy a deferred DA review — never wired into a default/scheduled/comment-triggered profile.
...
- Routine PR proof stays sparse and targeted: doctrine scan, profile lint, GHA proof-seal, the inventory/drift/boundary gates where relevant, and `cargo check` floors for touched crates (plus cheap CPU representatives only when profile-safe).
...
Terminal-proof protection (standing under the §4.1 lifecycle regime; formerly the Track-D "draconian rule"):
never delete `compile_fail`, trybuild, seal-proof, oracle-parity, golden-byte, STEAD-required, invariant-required,
doc-named invariant coverage, `custom_layout_ethics_axis`, escaped-bug regressions, or active-track live rung
suites — these are the `TIER7` terminal proof classes, and each survives only by carrying a specific `catches:`
note. They are pared only if a later DA-cleared rung proves an explicit stronger superseding boundary (which
promotes the invariant into a `simthing-kernel` type/seal or EML opcode-stack and deletes the test).

Boundary tiers:

- `TIER1_TYPE_SEAL`: illegal state is uncompilable; runtime duplicates are DELETE candidates with zero runtime representatives.
- `TIER2_ADMISSION_HARD_ERROR`: **zero** tests when the production admission hard-error runs on a live/canonical path (the production check IS the coverage; a per-boundary test is a redundant witness — Necessity Test). A test survives here **only** if it catches a regression neither the compiler, the production hard-error, nor an existing integration path would catch (e.g. genuine parser/format behavior a type cannot absorb). **"One representative per boundary" is RETIRED as a fossil premise** (it assumed a boundary needs a test for coverage — false after the kernel admission substrate). Enumeration variants are still kabuki; the corrected floor is *delete unless the Necessity Test is passed*, not *keep one*.
...
> **Table collapsed at Track D closeout (2026-07-04, executive DA on owner mandate).** The incremental
> boundary-paring era (rungs **D0–D2z**, PRs #1086–#1121) is **superseded and collapsed into the single
> `TEST-NECESSITY-SWEEP-0` pass** below. That era operated under the now-**retired** *"one representative per
> boundary"* premise, which floored the corpus at (number of boundaries) and stalled paring at diminishing
> returns (it only reached 6,301 → 4,070 across ~30 rungs). Per-rung provenance — merge commits, DA graduation
> logs, and evidence — is preserved in git history, in `docs/tests/current_evidence_index.md`, and in the two
> DA graduation-log blockquotes retained below. Do not re-open the collapsed rungs; the Necessity Test is the
> sole governing law and the sweep is its single terminal application.

| Rung | ID | Scope | State | DoD |
|---|---|---|---|---|
| D0–D2z | *(collapsed)* `TEST-PARE-*` incremental boundary-paring era | Inventory/ledger construction (`TEST-PARE-INVENTORY-0`, `TEST-PARE-STANDARD-DA-0`, `TEST-ADMISSION-REGIME-0`), per-crate/per-boundary collapse waves, protected-class audit + residue deletion, GHA proof-seal + compile-floor gates, Studio/typeface owner-deep review, and the mapgenerator promotion wave. | **COLLAPSED — SUPERSEDED BY `TEST-NECESSITY-SWEEP-0`** | ~30 merged rungs (#1086–#1121) drove inventory 6,301 → 4,070 under the retired one-representative-per-boundary premise. Lifecycle gates inherited by sweep/closeout: `test_inventory_check.sh`, `test_lifecycle_boundary_check.sh`, `test_inventory_drift_check.sh`, `doctrine_exec_gha_proof_seal.sh`, `tests-compile-floor-non-bevy`. Provenance: git history + `current_evidence_index.md` + graduation-log blockquotes below. |
| D-SWEEP | `TEST-NECESSITY-SWEEP-0` | One-rung, default-**DELETE** application of the Necessity Test across the whole corpus. Per-boundary floor is **zero**. | **DONE — DA-APPROVED (2026-07-04, executive DA; merged #1122 @ `3ef232506f`)** | Deleted **3,478** tests; inventory **4,070 → 731** = 592 explicit keep-set survivors + 137 ledger-only `cfg_test_mod` marker rows + 2 non-runnable `dependency-floor` helpers; promotion backlog 25 → 0. **Binding proof independently re-run by the DA: `cargo check --workspace --all-targets` PASS (zero errors, all targets, clean tree at head).** 0R flipped default to DELETE; 0R1 reconciled docs; **0R2 build-integrity repair** (orphaned doc-comment/attribute cleanup in `install.rs`/`spec_session.rs`/mobility fixture; two dependency-floor helpers restored — `disburse_down_fixture.rs`, `mapgen_pr8_scheduled_concurrency.rs` — both non-runnable and imported by surviving tests; `ct_3b_4a_gpu_projection.rs` survivor aligned to live production API; `scenario_io` test-fixture `Default`). DA verified: production logic untouched (all src "deletions" are inline `#[cfg(test)] mod tests` removals), `dependency-floor` residue class + drift exception are narrow and keyed to `permanent-residue:dependency-floor` (stale-only; the `missing`/unledgered-runnable-test check is untouched, so no deleted runnable test can hide), and the full inventory-drift gate passes in CI on the head. Evidence: `docs/tests/test_necessity_sweep_0_results.md` + `test_necessity_sweep_0_manifest.tsv`. |
| D-CLOSE | `TRACK-D-CLOSEOUT-0` | Close Track D: collapse the rung table, land the **Rustified Test Lifecycle** regime (§4.1), and update constitution/design/handoff/CI-surface docs. Authorize the **one-time full-workspace test** as the PR-ladder closure gate. | **DONE — DA-CLOSED (2026-07-04)** | This rung. Track D is **CLOSED**. The corpus is at 731 rows; the lifecycle regime (§4.1) makes default-delete-at-track-close standing law so the corpus can never re-propagate. **Closure gate:** a single `cargo test --workspace --all-targets` run is sanctioned **once** to certify a PR-ladder closure (or when a DA review would otherwise be deferred to it) — it is a closure certificate, not routine proof; routine proof stays the sparse targeted profiles + owner-local floor. |
| D-RESUME | `CI-SCAFFOLDING-RESUME-AFTER-TRACK-D-0` | Post-closeout CI scaffolding resume map + Track-D-era CI artifact audit (profiles, scripts, TSVs). | **DONE — merged #1125** | Docs-only. Verifies #1122/#1123/#1124 closure; classifies `test-pare-*` Doctrine Exec profiles as executable-stale; audits CI-layer residue. Evidence: `docs/tests/ci_scaffolding_resume_after_track_d_0_results.md`. |
| D-LIFECYCLE-DELETE | `CI-LIFECYCLE-RESIDUE-DELETE-0` | Delete/reframe Track-D-era CI residue; preserve live Rustified lifecycle enforcement. | **DONE — DA-APPROVED / merged #1126 @ `86205e7d`** | Gate-state. Deleted `test_edit_scope*` + `test_pare_audit.tsv`; renamed boundary machinery to `test_lifecycle_*`; pruned boundary rows to 731. Evidence: `docs/tests/ci_lifecycle_residue_delete_0_results.md`. |
| D-PROFILE-TAXONOMY | `CI-PROOF-PROFILE-TAXONOMY-0` | Retire stale Track-D `test-pare-*` Doctrine Exec profiles; guard executable profile table. | **DONE — DA-APPROVED (2026-07-04, gate-state deep review)** | Retired 11 Track-D deletion profiles; retained 6 current proof profiles; added `FORBIDDEN-TRACK-D-PROFILE` lint + `--prove-no-track-d-deletion-profiles`; deleted uninvoked `protected_class_audit.py`. **DA verification (against the tree):** the 6 retained profiles are all current proof surfaces (seal-residue `--doc` proofs, active-0.0.8.5 TP tests, compile floor, timeout proof, owner-deep artillery) — none references a deleted test binary; `owner-deep-full-cpu-quarantined` stays `profile_class=owner-deep` (not default/comment-triggered); no full-workspace closure proof normalized into the table. **Guard proven live + biting:** DA-perturbed with a clean `test-pare-*`/`test-deletion-*` row → `PROFILE-LINT: FAIL FORBIDDEN-TRACK-D-PROFILE`, restored → PASS; the guard runs on every PR because the default `ci-b-webchat-smoke` profile invokes `doctrine_exec_profile_lint.sh`. `protected_class_audit.py` deletion leaves no live reference. **Minor non-blocking observation:** a malformed profile row (missing trailing newline / short column count) is silently skipped by the lint — pre-existing robustness gap, optional follow-up. Evidence: `docs/tests/ci_proof_profile_taxonomy_0_results.md`. |
| D-BIRTH-TRIPWIRE | `CI-LIFECYCLE-BIRTH-TRACK-TRIPWIRE-0` | Mechanize the test **lifecycle** (default-delete-at-track-close) as a residue-as-tripwire: `birth_track` + `dsu_survivals` schema fields + `test_lifecycle_expiry_check.sh`. Track-A/ledger-layer, **not** §3B (text/data analysis, not executable proof). | **DONE — DA-APPROVED (2026-07-04, deep review, R3)** | **DA verification (DA-run against the tree):** `--prove` PASS incl. the cross-track false-green meta-proof; the 8 Necessity-Test survivor guards **immune** (scheduled-scan grep empty; footer `survivor-set expired: 0` — R2 defect-1 fix holds, no legitimate keep threatened); Defect 2 fixed (`has_structured_dsu` requires a parseable `<consumer>`; bare marker does not immunize; immunized rows emit a visible `justified-closed (audit)` list); Defect 3 rising-cost lease present (`dsu_survivals` + DA-tunable `test_lifecycle_dsu_tiers.tsv`, advisory 1–2 / rejustify 3–4 / presumed-stale ≥5 delete-or-promote); R3 fixed (`--closure-gate` global not birth_track-filtered, proven by the dsu=1+dsu=5 cross-track case → INSPECT audit=2 max=5). The 137 expired = `cfg_test_mod::tests` pre-lifecycle module markers, genuine non-durable backlog surfaced INSPECT on the *scheduled* path only (never every-PR `--schema`), the tripwire's next paring target, not a blocker. Gate battery green. **Rationale:** lifecycle *form* is CI-enforced (class + catches-note + boundary ownership + drift) but needed mechanized birth-track expiry surfacing. **DoD (implemented #1131):** (1) `birth_track` origin-ladder id on every inventory row; backfill known track or `pre-lifecycle`. (2) `dsu_survivals` integer lease counter on every row; schema-fails empty/non-int/negative. (3) `test_lifecycle_expiry_check.sh` modes: `--schema`, `--scheduled`, `--track-closeout <track_id>`, `--closure-gate <track_id>`, `--prove`. (4) Footer `LIFECYCLE-EXPIRY-VERDICT: PASS\|INSPECT\|FAIL expired=N audit=N max_dsu_survivals=N mode=...`. (5) Expiry flags non-durable, non-live-anchor rows on closed tracks; live-anchor Necessity-Test survivors (8 guards citing `invariants.md` / escaped-bug anchors) immune. (6) Structured `downstream-utility: <consumer>` only immunizes expiry; bare marker does not; immunized rows emit `justified-closed (audit): N` with row identity. (7) Rising-cost DSU lease via `test_lifecycle_dsu_tiers.tsv`: advisory `1–2`, rejustify `3–4`, presumed-stale `>=5` delete-or-promote pressure; sanctioned exit is promotion, not perpetual renewal. (8) `--closure-gate <track_id>` re-surfaces **all** structured downstream-utility rows across inventory (closure event context only; not birth_track-filtered), sorted by `dsu_survivals` desc. (9) Inventory-check wires `--schema` only — not scheduled scan every PR. (10) No auto-deletion; no semantic note-truth scan. **Optional later:** `/lifecycle-expiry` ChatOps (§3B) — not part of this rung. Evidence: `docs/tests/ci_lifecycle_birth_track_tripwire_0_results.md`. |
| DF | `TEST-PARE-CADENCE-DF` | Post-closure cadence for the pared corpus. | **FUTURE** | With material reduction landed (731 rows), the surviving suite may graduate to scheduled sentinel + ladder-boundary run; full battery remains workflow_dispatch/owner-local. Any *new* Track (e.g. 0.0.8.5 product rungs) births its tests under the §4.1 lifecycle regime; `CI-LIFECYCLE-BIRTH-TRACK-TRIPWIRE-0` makes that lifecycle self-surfacing at track close. |

> **DA graduation log (executive DA, 2026-07-03 — D2l `TEST-PARE-PROTECTED-CLASS-AUDIT-0`): DONE / DA-APPROVED
> with one recorded qualification.** Verified against the tree: the review table reconciles exactly
> (582 TRUE_MEMBER + 112 NEEDS_PROMOTION + 98 FALSE_MEMBER + 33 NECESSARY_CITED_DEPENDENCY + 4 LEDGER_DEFECT
> = 829 audited protected rows; 4,250 OUT_OF_SCOPE behavior-AUDIT rows confirmed non-shields; 5,079 table
> rows); the judgment-note rule proves green. Every queue the audit produced was consumed and validated
> downstream under DA verification: the 98 FALSE_MEMBER rows deleted via #1103/#1104 (DA-verified, SHA-bound
> targeted exec); the 112 NEEDS_PROMOTION rows were the input to the D2o DA review, which confirmed the
> classification sound *as an audit verdict* — the audit correctly declined to make the DA's call and refused
> to let promotion review become a shield; the 4 LEDGER_DEFECT markers routed to module-marker expansion; the
> coverage maps were the decisive instrument for D2o's disjointness proof. **Qualification (the one
> audit-quality gap, recorded, not re-run):** TRUE_MEMBER was certified by name/body inspection without a
> compile check — D2o proved several TRUE-certified binaries did not compile on master (inert gates, §0.6.6).
> The error direction was safe (over-protection; no deletion relied on TRUE liveness), and the correction is
> D2p — which has since retired 124 dead binaries / 925 rows and landed the tests-compile floor, so textual
> membership and liveness can no longer diverge silently. Re-running the audit would have duplicated D2p;
> graduation with the qualification is the anti-ceremony resolution.
...
> `test_inventory_check.sh`, and `test_lifecycle_boundary_check.sh`.

## 4.1 The Rustified Test Lifecycle (standing law, adopted at Track D closeout 2026-07-04)

Track D proved a structural failure mode: **tests propagate**. A corpus of 6,301 accreted because every PR
ladder minted per-step witness tests that outlived the step, and nothing scheduled their death. The Necessity
Test cut the corpus to 731; this regime makes sure it can **never re-accrete**. The mental model is
Rustification applied to the test corpus itself: a test is not a permanent asset, it is a **scoped borrow**
with a lifetime that ends at its birth track's closure unless it earns a longer one.

**The default lifetime rule.** Every test is born owned by the PR ladder / track that creates it and is
**assumed DELETED at that track's closure**. Birth-PR witness tests, scaffold checks, step-by-step "does this
compile / did I wire it" proofs, and enumeration variants have a lifetime that **ends with the track**. No
action is needed to delete them — closure deletes them. Keeping one requires a positive, recorded justification;
deletion is free.

**The three lifetime-extension classes (the only ways a test outlives its birth ladder).** A test earns a
lifetime beyond its birth track only if it is one of:

1. **Canonical heuristic / notion carrier.** It establishes or guards a *canonical* invariant or heuristic with
   genuine downstream future utility (not "this boundary exists" — that is the retired premise). **But if a test
   truly encodes a canonical notion, that notion is mis-housed as a test:** it should be promoted to live where
   canonical truth lives — a **`simthing-kernel` type / seal** (illegal state made uncompilable) or an **EML
   opcode-stack** construct (the notion executed as production behavior). Promotion **deletes** the test and
   **drops** inventory; a canonical notion that stays a test is a promotion-debt, tracked as `TIER6` and expected
   to climb, never a permanent shield.
2. **Terminal proof class (`TIER7_NEVER_PARE`).** `compile_fail`/trybuild, CPU-oracle-parity, golden-byte /
   bit-exact determinism, seal-proof fixtures, doc-named invariants, STEAD-required §8 suites, escaped-bug
   regressions, and active-track live rung suites *while that track is open*. These carry a `catches:` note
   naming the specific regression only they catch.
3. **Dependency-floor helper (`permanent-residue:dependency-floor`).** A non-runnable helper/fixture module
   imported (or `include_str!`-ed) by a surviving test. It has **no `#[test]`** of its own; it lives only to keep
   a survivor compiling. Enumerated narrowly; exempt from the stale-drift check but **not** from the
   unledgered-runnable-test check.

**Everything else dies at track close.** A test that (a) cannot break production if deleted, and (b) is neither a
downstream dependency nor required for canonical function, is by definition redundant with the compiler, a
production admission hard-error on a live path, or an existing canonical/integration path — the Necessity Test's
three catchers. It is DELETE. This is not a judgment call to be relitigated per row; it is the default.

**Enforcement (already live, inherited from Track D gates).**

- **Birth accounting:** `TEST-BUDGET` (§ D-LAW) flags a delta PR that adds more than three `#[test]` fns to one
  file without table-driven form — the first line of defense against re-propagation at birth.
- **Ledger truth:** `test_inventory_check.sh` requires every row to carry a class; `test_inventory_drift_check.sh`
  fails on any unledgered runnable `#[test]` (`missing`) and on stale rows (except the narrow
  `permanent-residue:dependency-floor` exception). A test cannot exist un-accounted, and a survivor cannot linger
  without a named lifetime-extension class.
- **Boundary ownership:** `test_lifecycle_boundary_check.sh` requires every survivor to name its owning boundary + tier;
  `TIER6` rows are a rustification queue, not a shield.
- **Closure certificate:** at a track/PR-ladder closure the sanctioned **one-time** `cargo test --workspace
  --all-targets` may be run to certify the survivor set compiles-and-passes as a whole (or to satisfy a deferred
  DA review). It is a certificate, not routine proof; routine proof stays the sparse targeted profiles + the
  owner-local floor (§10.3).

**Standing instruction for every future track (incl. 0.0.8.5 product rungs).** Births its tests under this
regime: assume each dies at track close; to keep one, either promote its notion into a `simthing-kernel`
type/seal or an EML opcode-stack construct (deleting the test), or classify it into a `TIER7` terminal class with
a `catches:` note, or ledger it as a dependency-floor helper. There is no fourth way to keep a test, and
"it might be useful later" is not one of the three.
