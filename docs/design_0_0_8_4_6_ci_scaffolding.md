...
| D2y | `SURFACE-TRUTH-INSPECT-REASON-SPLIT-0` | Split surface-truth INSPECT reporting into match/divergence/tooling-gap. | **PROBATION / DA REVIEW** | CI proof-semantics rung; no source edits; preserves PASS/INSPECT authority while making triage reason machine-readable. **0R:** synthetic probes quarantined behind `DOCTRINE_SURFACE_TRUTH_SYNTHETIC_ALLOWED=1`; offline reason-split proof no longer depends on `cargo-public-api`; production surface-truth behavior unchanged. |
