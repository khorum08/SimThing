...
| D2n | `TEST-PARE-PROTECTED-PARE-DELETE-0` | Physically delete the unprotected false-member PARE rows produced by the protected-residue ledger cut. | **PROBATION** | `docs/tests/test_pare_protected_pare_delete_0_results.md`; #1103 merged at `f32a0a93db068f95cf5201860063951568cfdec0`; 98 FALSE_MEMBER-to-PARE rows physically deleted, 0 blocked, inventory 5,332 -> 5,234; TRUE_MEMBER, NEEDS_PROMOTION, dependency-floor, STEAD, seal, golden, doc-named, SimThing-Kernel, and CI Scaffolding rows untouched; profile `test-pare-protected-pare-delete` is cargo-check-floor only. |
