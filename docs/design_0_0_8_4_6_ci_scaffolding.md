...
| D2p | `TEST-REPAIR-OR-REPLACE-TRUE-ORACLE-BINARIES-0` | Retire dead TRUE_MEMBER oracle/parity fossil binaries and add a standing test-binary compile floor. | **PROBATION** | `docs/tests/test_repair_or_replace_true_oracle_binaries_0_results.md`; D2o closed-scope rows re-sealed; iterative `cargo check -p <crate> --tests` discovery retired 124 non-compiling fossil test binaries / 925 rows (inventory 5,147 -> 4,222); coverage rows pointing at deleted owners pruned; local owner proof passed the nine-crate compile floor; targeted profile `test-repair-or-replace-true-oracle-binaries` plus bounded `sentinel-core` carry the desktop-safe GHA compile floor excluding `simthing-driver`'s Bevy/ALSA dev-dep path; targeted Doctrine Exec PASS run `28661354065`; no scanner/allowlist edits and no product logic. |
