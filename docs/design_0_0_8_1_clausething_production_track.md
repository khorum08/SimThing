...
| CT-2c category economy | **IMPLEMENTED / PASS** (2026-06-11; categories are translator-only hydration/admission metadata that compile away; economic key family `(category)_(resource)_(produces\|upkeep\|cost)_(add\|mult)` uses longest-match + hard diagnostics; Daily Economy ClauseScript canonical-matches the RON original; no spec widening, no new AccumulatorRole, Resource Flow remains explicit opt-in/default-off) | [`docs/tests/ct_2c_impl_results.md`](tests/ct_2c_impl_results.md) |
