...
| **CT-3b + CT-4a** | T1 over accepted substrate **[FRONTIER]** | **Headline vertical (reoriented ORIENT-0):** ClauseScript hydration → admitted SimThing RF/accumulator data → bounded arena pressure → GPU-resident movement-front / suppression-disruption heatmap (`RegionFieldSpec` / stencil propagation + reduce-up + threshold/EML interpretation), default-off — **not** a standalone movement/combat/economy engine | ClauseScript-authored headline scenario proves RF-fed heatmap vs CPU oracle; `simthing-sim` map-free and ClauseScript-blind; memo [`clausething/ct_3b_4a_movement_front_heatmap_memo.md`](clausething/ct_3b_4a_movement_front_heatmap_memo.md) |
...
| CT-2c category economy | **IMPLEMENTED / PASS** (2026-06-11; CT-2c-REMEDIAL-2 consumes RF-BASE-INTRINSIC-0: unit-template literal `produces` / `upkeep` lower to `BaseFlowObligationSpec`, canonical baseline + install/session/GPU oracle proof without side-channel `contributions` injection) | [`docs/tests/ct_2c_impl_results.md`](tests/ct_2c_impl_results.md) |
| CT-3b+4a headline vertical | **DESIGN MEMO / READY FOR REVIEW** (2026-06-11; CT-3b+4a-ORIENT-0 reorients headline vertical around Resource Flow arena pressure → movement-front / suppression-disruption heatmap; JIT EML/WGSL allowed later for bounded arena arithmetic only — no implementation; CT-2c RF floor accepted) | [`docs/clausething/ct_3b_4a_movement_front_heatmap_memo.md`](clausething/ct_3b_4a_movement_front_heatmap_memo.md) |
