...
| CT-3b+4a headline vertical | **DESIGN MEMO ACCEPTED WITH AMENDMENTS** (2026-06-11; ORIENT-0 reorientation **ratified**: Resource Flow arena pressure → movement-front / suppression-disruption heatmap over the accepted Movement-Front substrate, **plus the mandatory CT-4a leg** — `ai_will_do` → Layer-3 EML over reduced parent columns → `Threshold`+`EmitEvent` commitment; JIT EML/WGSL allowed for bounded GPU-resident arena/heatmap arithmetic per the product lift, incl. CT-RF-EML-RATE-0; implementation may open against the amended memo) | [`docs/clausething/ct_3b_4a_movement_front_heatmap_memo.md`](clausething/ct_3b_4a_movement_front_heatmap_memo.md) |
