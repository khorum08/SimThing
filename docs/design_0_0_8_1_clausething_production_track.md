...
**0.0.8.6 MapGeneratorCLI addendum (2026-06-14, PR10).** **0.0.8.2.5 MapGen remains closed.** PR10 proves a generated MapGeneratorCLI scenario can parse, lower, admit/install, and produce GPU-resident compact evidence using existing generic SimThing/MapGen GPU paths. Integration harness `mapgenerator_cli_pr10_gpu_compact_evidence` generates a tiny deterministic `static_galaxy_scenario` (static shape, hyperlanes, partition/cluster bridge couplings, nebula feedstock, initializer buckets) and runs the closed MapGen PR10-style GPU compact-evidence battery on a real adapter. No new grammar, no lowerer widening, no new `SimThingKind`, no new GPU kernels, no semantic WGSL, no route/path/predecessor/movement planner, no full-field readback, no Euclidean authority, and no FIELD-MOVIE-DATASET-0 work. **Zero** `crates/simthing-clausething/src/` (and other closed runtime crate) changes — test/docs/harness-only. **PASS pending DA review.** See [`mapgenerator_cli_pr10_gpu_compact_evidence_results.md`](tests/mapgenerator_cli_pr10_gpu_compact_evidence_results.md) (PROBATION). **PR11 next:** scale-envelope proof / 1000-star generated map stress.
