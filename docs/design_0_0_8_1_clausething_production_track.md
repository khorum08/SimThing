...
| CT-2c category economy | **PARTIAL / REMEDIAL BLOCKED** (2026-06-11; PR #590 landed category decoder, diagnostics, translator-only category metadata, Daily Economy canonical parity, and Resource Flow admission shape, but remedial review found no existing install-consumed base intrinsic-flow obligation surface for literal `produces` / `upkeep`; `OverlaySpec::Add` rejected as a base-rate workaround because it collapses intrinsic obligations into modifier semantics) | [`docs/tests/ct_2c_impl_results.md`](tests/ct_2c_impl_results.md) |
