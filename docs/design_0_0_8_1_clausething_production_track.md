...
**0.0.8.6 MapGeneratorCLI addendum (2026-06-14, PR7).** **0.0.8.2.5 MapGen remains closed.** MapGeneratorCLI PR7 adds bounded producer-side partition/cluster assignment and bridge endpoint selection represented **only** as `static_galaxy_scenario` `add_hyperlane` declarations. Generated output parses and lowers through existing closed MapGen lattice/link surfaces without front-end widening. No new grammar, no route/path/predecessor/movement/border/frontline semantics, field operators, RF, Movement-Front, PALMA, runtime, GPU, simthing-sim, new `SimThingKind`, Euclidean authority, or FIELD-MOVIE-DATASET-0 work. **PASS pending DA review.** See [`mapgenerator_cli_pr7_partition_bridge_results.md`](tests/mapgenerator_cli_pr7_partition_bridge_results.md) (PROBATION). **PR8 next:** registry completion / remaining vanilla shape descriptors / single-source executable strategy dispatch, unless DA reorders.
