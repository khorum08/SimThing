...
**0.0.8.6 MapGeneratorCLI addendum (2026-06-14, PR6b).** **0.0.8.2.5 MapGen remains closed.** MapGeneratorCLI PR6b adds bounded producer-side wormhole-pair and gateway special-route endpoint selection represented **only** as `static_galaxy_scenario` `add_hyperlane` declarations that lower through existing closed `mapgen_links` surfaces as bounded lane couplings. No new grammar, no front-end widening, no route/path/predecessor/movement/border/frontline semantics, field operators, RF, Movement-Front, PALMA, runtime, GPU, simthing-sim, new `SimThingKind`, Euclidean authority in output, or FIELD-MOVIE-DATASET-0 export. **PASS pending DA review.** See [`mapgenerator_cli_special_routes_results.md`](tests/mapgenerator_cli_special_routes_results.md) (PROBATION). **PR7 next:** partition/bridge structural producer + clustering — no route/path/predecessor semantics and no GPU.
