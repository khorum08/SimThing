...
**0.0.8.6 MapGeneratorCLI addendum (2026-06-14, PR6R).** **0.0.8.2.5 MapGen remains closed.** Remedial PR before PR6b/PR7: corrects PR6 production-doc evidence status and adds fail-closed validation for public `HyperlaneOptions` (`InvalidEdgeCounts`, `InvalidFanoutCap`, `UnsatisfiedMinEdgeCount`). **Zero** closed `src/` changes. No route/path/predecessor/movement/border/frontline semantics, field operators, RF, Movement-Front, PALMA, runtime, GPU, simthing-sim, or FIELD-MOVIE-DATASET-0 export. **PASS pending DA review.** See [`mapgenerator_cli_pr6r_hardening_results.md`](tests/mapgenerator_cli_pr6r_hardening_results.md) (PROBATION).
