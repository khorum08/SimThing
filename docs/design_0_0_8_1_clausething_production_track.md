...
**0.0.8.6 MapGeneratorCLI addendum (2026-06-13, PR9).** **0.0.8.2.5 MapGen remains closed.** PR9 adds bounded producer-side nebula field declarations, initializer bucket emission, and inert metadata passthrough/deferred metadata notes. Generated output uses only already-accepted `static_galaxy_scenario` and closed `nebula = { name radius }` surfaces plus existing Movement-Front `RegionFieldSpec` lowering — parses through the neutral-AST parser and lowers through closed MapGen lattice/field surfaces without front-end widening. No new grammar, no route/path/predecessor/movement/border/frontline semantics, no runtime/GPU/simthing-sim, no new `SimThingKind`, no semantic WGSL, no field execution, no Euclidean authority, and no FIELD-MOVIE-DATASET-0 work. Inert gameplay metadata (`num_empires`, crisis levers, etc.) is captured in producer dry-run reports only until a closed surface admits it. **PASS pending DA review.** See [`mapgenerator_cli_pr9_field_operator_results.md`](tests/mapgenerator_cli_pr9_field_operator_results.md) (PROBATION). **PR10 next:** generated scenario admit/install + GPU compact evidence on a real adapter.
