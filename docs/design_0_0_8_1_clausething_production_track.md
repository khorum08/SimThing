...
**0.0.8.6 MapGeneratorCLI addendum (2026-06-14, PR8).** **0.0.8.2.5 MapGen remains closed.** MapGeneratorCLI PR8 completes remaining vanilla shape registry descriptors and single-sources executable strategy dispatch/names via `ShapeStrategyEntry` rows in `strategies/registry.rs`. Adding shapes now touches registry entries and tests, not producer core or closed lowerer. Procedural `static`/`arbitrary_static` mode-gate asymmetry fixed (procedural mode rejects both without explicit cells). No topology, field operators, RF, Movement-Front, PALMA, runtime, GPU, simthing-sim, new `SimThingKind`, route/path/predecessor/movement/border/frontline semantics, Euclidean authority, or FIELD-MOVIE-DATASET-0 work. **PASS pending DA review.** See [`mapgenerator_cli_pr8_shape_registry_results.md`](tests/mapgenerator_cli_pr8_shape_registry_results.md) (PROBATION). **PR9 next:** nebula / field_operator declarative producer — no GPU/runtime.
