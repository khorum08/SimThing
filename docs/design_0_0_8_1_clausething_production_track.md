...
| CT-3b+4a headline vertical | **IMPLEMENTED / PASS** (2026-06-11; Line 3 session-loop integration closes the rung: `SimSession::run()` dispatches RF arena bands + the admitted mapping chain — on-device scatter → stencil → reduce → ai_will_do EML → GPU commitment scan — every tick under the explicit `SparseRegionFieldV1` profile, default-off, zero runtime readback; commitment crossings journaled per tick. Same-day 0B closures: CT-RF-EML-RATE-0, `value:` lowering, GPU projection w/ Named-column gadget hook. One named deferral: authored commitment-effect payloads for in-loop consequence application — the event→BoundaryRequest application itself is 0A-proven. PALMA untouched) | [`docs/tests/ct_3b_4a_impl_0a_results.md`](tests/ct_3b_4a_impl_0a_results.md) + amended memo [`docs/clausething/ct_3b_4a_movement_front_heatmap_memo.md`](clausething/ct_3b_4a_movement_front_heatmap_memo.md) |
