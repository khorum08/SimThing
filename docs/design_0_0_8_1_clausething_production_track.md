...
**0.0.8.6 MapGeneratorCLI addendum (2026-06-15, PR11).** **0.0.8.2.5 MapGen remains closed.** PR11 proves the MapGeneratorCLI scale envelope for 1000-star producer generation and records which lowered/admitted/GPU paths are proven under existing closed caps. Producer hardening: u64 lattice capacity fail-closed, occupancy free-placeable list (no per-insert full lattice scan), bounded pair enumeration (`PRODUCER_PAIR_CANDIDATE_CAP` = 65536), spatial hyperlane candidate windows. Integration proof: 1000-star elliptical scenario emits, parses, lowers lattice (1000 gridcells), exposes bounded hyperlane feedstock; RF/admit/install at 1000 stars **blocked** by closed lowerer caps (no widening). Full 1000-star admission/install is deferred pending DA-authorized 0.0.8.2+ closed-lowerer capacity amendment. PR10 real-adapter GPU compact evidence harness remains live for the tiny fixture. No new grammar, no lowerer widening, no new `SimThingKind`, no semantic WGSL, no new GPU kernels, no route/path/predecessor/movement planner, no full-field readback, no Euclidean authority, and no FIELD-MOVIE-DATASET-0 work. **PASS pending DA review.** See [`mapgenerator_cli_pr11_scale_envelope_results.md`](tests/mapgenerator_cli_pr11_scale_envelope_results.md) (PROBATION). **PR12 next:** docs-only closeout + UI handoff.
