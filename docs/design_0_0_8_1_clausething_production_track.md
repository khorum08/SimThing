...
| CT-3b+4a headline vertical | **PARTIAL — SUBSTRATE 0A PASS** (2026-06-11, CT-3b+4a-IMPL-0A; the headline spine runs end-to-end from ClauseScript on GPU: folded RF obligations → admitted `ArenaPressureBindingSpec` projection (bit-exact seed) → bounded stencil heatmap → reduce → **mandatory `ai_will_do` field_urgency EML with authored weights → authored threshold crossing → `BoundaryRequest` commitment via `apply_structural_mutations`**, CPU-oracle parity at every seam + pressure-dependence counterfactual. **CT-RF-EML-RATE-0 IMPLEMENTED same day**: gated produces/upkeep → per-tick `EvalEML` effective-rate band at OrderBand 0 with arena bands shifted +1; gate computed in-tree (`CMP_GE`, exact-class); rising/falling edges + no-compounding proven bit-exact on GPU; `value:` trees reduced to mechanical lowering onto the existing band. `value:` trees + the GPU-side projection (indexed-scatter WGSL, bit-identical vs CPU oracle, Named-column gadget hook) landed same day. Remaining 0B blocker: session-loop integration only) | [`docs/tests/ct_3b_4a_impl_0a_results.md`](tests/ct_3b_4a_impl_0a_results.md) + amended memo [`docs/clausething/ct_3b_4a_movement_front_heatmap_memo.md`](clausething/ct_3b_4a_movement_front_heatmap_memo.md) |
