...
**0.0.8.6 MapGeneratorCLI addendum (2026-06-14, PR5).** **0.0.8.2.5 MapGen remains closed.** MapGeneratorCLI PR5 proves PR4 `static_galaxy_scenario` text lowers through `parse_mapgen_neutral_document` → `generate_mapgen_lattice_hierarchy` on the **amended** closed lowerer. It adds only integration tests plus `simthing-mapgenerator` as a **dev-dependency** on `simthing-clausething`; **zero** `crates/simthing-clausething/src/` changes. PR5 required the prior child-id amendment (#680); the lowerer fix is not carried in the producer PR. No topology, links, field operators, RF, Movement-Front, PALMA, runtime, GPU, simthing-sim, new `SimThingKind`, pathfinding/movement/route/predecessor/border/frontline semantics, Euclidean authority in output, or FIELD-MOVIE-DATASET-0 export. **PASS pending DA review (#679).** See [`mapgenerator_cli_pr5_lowering_results.md`](tests/mapgenerator_cli_pr5_lowering_results.md) (PROBATION). **PR6 next:** bounded hyperlane / `add_hyperlane` emission — no route/path/predecessor semantics and no GPU.
