...
- **Opened `RUNTIME-0080-0-R1a-REMEDIAL-0`:** make the GPU the actual Tier-A transition authority and **unify** next-tick authority onto the production overlay/intent/threshold substrate (`WorldGpuState` + `Pipelines` Pass 0–7 + `OverlayDelta`/`IntentDelta`/`ThresholdEvent`) instead of a private journal. Register the R6C Tier-A transforms (already GPU-measured in GPU-MEASURE-0080-0) as `AccumulatorOp`s/overlays so the GPU produces `state_N+1`; player/AI overlays + SEAD threshold acts + the resident transition ride one substrate; Tier-B stays bounded CPU boundary maintenance via `ThresholdEvent → BoundaryRequest`.
