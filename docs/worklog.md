...
- **Seated PALMA-style** in [`docs/design_0_0_8_1_border_hack_track.md`](design_0_0_8_1_border_hack_track.md) with pinned v1 math (σ=clamp(u/u_sat), constant χ, Δt·χ ≤ 1/4 CFL bound, register-transient C, `SaturatingFlux{u_sat,chi}` spec shape) and a four-rung ladder: BH-0 operator+oracle (T2, frontier), BH-1 border readout column → reduce/threshold consumption (T1), BH-2 (1−C_u)→PALMA W impedance gradient-valley coupling (deferred, named-consumer), BH-3 ClauseThing authoring (deferred). Borders stay field readouts — no border service, objects, or segmentation pass; gradient descent remains the decision arena, C_u only shapes the field it descends.

# 2026-06-12 - CT-3B+4A LINE 3R: GPU commitment crossing state hardened

- **Remediated the fixture-scan seam in the production loop:** `run_mapping_step` now calls the production `tick_with_commitment_spec` (new) instead of the `*_fixture` scan, and the threshold previous-values baseline is no longer zeroed per scan — it is zeroed once at first scan, then snapshotted on-device after every scan (`AccumulatorOpSession::copy_values_to_previous`, one buffer copy, no readback). The Pass-7 kernel therefore edge-detects: held above-threshold urgency journals exactly one upward crossing; falling below and re-rising fires a fresh one. Decision stays GPU-side; compact event readback remains the only boundary surface.
- **Proof:** in-loop 3-boundary run with sustained urgency → exactly one journaled crossing (effect once, alarm transformed, unchanged); new standalone edge test: rise/hold/hold = 1/0/0 events, pressure-free decay emits nothing and drops below threshold, re-seed fires exactly once more. Disabled-profile and stripped-binding guards re-proven. Track closure reaffirmed.

# 2026-06-12 - CT TRACK CLOSED: authored commitment effects close the last deferral

- **The last open item is implemented:** `FirstSliceCommitmentSpec.effect` (`CommitmentEffectSpec`: target, property, deltas, closed lifecycle set, once-latch default) authors the structural consequence of an ai_will_do commitment crossing. ClauseScript: `effect { target targets_property amount_add }` inside the `urgency` block. At a boundary with new journaled crossings, the session builds the authored overlay and submits `BoundaryRequest::AttachOverlay` through the ordinary feeder channel — drained and applied by the existing boundary machinery, fast-path suppressed for that boundary, journal watermark prevents latch re-fires. Effect property seeded on the host at install (the overlay-compile contract).
- **GPU proof:** across a 3-boundary run with crossings every tick the effect applies exactly once; one `Custom("mapping_commitment")` overlay on the acting farmer; its Permanent transform raises the authored alarm column on subsequent ticks. The full authored loop is closed: ClauseScript economy → RF pressure → heatmap → ai_will_do → threshold → authored structural consequence on the world tree.
- **Track ledger: CLOSED (2026-06-12)** — CT-0a…CT-3b+4a all IMPLEMENTED / PASS, no open deferrals; §6 backlog items remain consumer-pulled future widening, not debts; the `modifiers.log` round-trip remains the bar for any future corpus-wide decoder claim (none made). `simthing-sim` ends the track ClauseThing-blind.

# 2026-06-11 - CT-3B+4A LINE 3: session-loop integration closes the headline rung — IMPLEMENTED / PASS

- **The production loop now runs the whole vertical.** `SimSession::run()` (and `record_to_path`) dispatch, per tick: RF arena bands (incl. the OrderBand-0 effective-rate EML band — previously only ever harness-driven) when `use_accumulator_resource_flow` is on, then the admitted mapping chain under `SessionMappingState` — on-device indexed scatter (session values → stencil input), GPU seed-then-zero, bounded stencil, Layer-2 reduce, ai_will_do EML, GPU commitment scan. **Zero value readback on the runtime path**; commitment crossings journaled per tick (`SimSession.mapping_commitments`) with `RunSummary` counters (`resource_flow_band_dispatches`, `mapping_ticks`, `mapping_commitment_events`).
- **Opt-in discipline test-proven:** `open_from_spec` installs mapping state iff the game mode authored `SparseRegionFieldV1` + exactly one region field + pressure binding + ai_will_do weights + commitment threshold; Disabled profile wires nothing; a half-authored configuration (binding stripped) is a hard open error naming the missing surface. `ct_3b_4a_session_loop.rs` (GPU): 3-boundary run dispatches both legs every tick and journals authored event-kind-7 crossings.
- **CT-3b+4a → IMPLEMENTED / PASS** with one named deferral: authored commitment-effect payloads for in-loop consequence application (the event→`BoundaryRequest` application itself is 0A-proven; the journal is the boundary-consumable surface until a rung authors commitment effects). **PALMA deliberately untouched** — Line 3 needed no traversal utility; min-plus remains seated as a generic GPU utility for a future consumer.

# 2026-06-11 - CT-3B+4A 0B push: value: trees on the rate band + GPU-resident pressure projection with the gadget hook

- **`value:` lowering (PR #598):** `script_value` formulas (base + ordered add/mult/floor_at/ceil_at over literals and live property-column reads) hydrate from `value:NAME` references at both rate consumption points — gated magnitudes and always-on dynamic terms (`GatedRateSpec.trigger` now optional) — and lower to formula subtrees on the CT-RF-EML-RATE-0 band. GPU proof bit-exact incl. the `ceil_at` clamp and live input reads. Flat formulas only; recursion rejected.
- **GPU projection (this PR):** generic indexed gather-scatter WGSL (`IndexedScatterOp`: one dispatch, host-validated bounds, duplicate-dst rejection, CPU oracle) moves the session values buffer into the stencil input buffer **on-device**; the mapping runtime gained a GPU-seed path running the one-shot seed-then-zero sequence with zero host value writes (and a fixed flag-clobber where `tick()` re-derived `seeds_applied_this_tick` from host seeds only). **Bit-identical** vs the 0A CPU projection through heatmap → ai_will_do → commitment (threat/urgency/events all `to_bits`-equal).
- **The SEAD composition contract:** `PressureSourceSpec::Named { sub_field }` projects any named flow-property column — session EvalEML/gadget ops write columns, columns become heatmap feedstock, the heatmap reduces into ai_will_do. Richness rides the one EML interpreter (Anchor B); the new WGSL is pure bounded data movement. Remaining 0B blocker: session-loop integration only.

# 2026-06-11 - CT-RF-EML-RATE-0: gated rates land as the per-tick EvalEML effective-rate band — ClauseScript conditionals reach the GPU

- **The 0A rung's largest blocker is closed:** `gated { trigger { property at_least } <key> = N }` inside ClauseScript `produces`/`upkeep` lowers to `GatedRateSpec` → install resolves participant slots + base/intrinsic/trigger global columns → the sync builds one `ExactDeterministic` EvalEML tree per (participant, pair) — gate computed **in-tree** via `CMP_GE` over the trigger column — and registers the ops at **OrderBand 0 with every arena reduce/allocate band shifted +1**: rate-before-reduce is structural. Formula: `intrinsic = (folded_base + Σadd×gate) × (1 + Σmult×gate)`; gated terms compose after the CT-2c static fold; the base column is written once at install and immutable.
- **Proof bit-exact on GPU** (`ct_rf_eml_rate.rs`): gate off = folded base 10.5; rising edge = 14.5; **held gate does not compound**; falling edge returns exactly to base — per-tick recomputation from base makes both edges exact by construction, and per-tick overlays on rate columns remain impossible. New surfaces: `ResourceFlowSpec.gated_rates` + admission (≤4 terms/target for the ≤32-node budget), `rate_base` sub-field on gated pairs, `trigger_property` dialect, driver `gated_rates` module, `SpecSessionState.resolved_gated_rates`, sync band-shift. `value:` trees now reduce to mechanical lowering onto this band (hard-errored, extension named). Report addendum: [`docs/tests/ct_3b_4a_impl_0a_results.md`](tests/ct_3b_4a_impl_0a_results.md).

# 2026-06-11 - CT-3B+4A-IMPL-0A: the RF-fed movement-front heatmap spine runs from ClauseScript — including the mandatory ai_will_do commitment

- **The SEAD-horizon spine is live end-to-end on GPU, authored in ClauseScript:** folded RF base obligations (CT-2c floor, 6 × 1.75 = 10.5) → new admitted `ArenaPressureBindingSpec` projects the participant's resolved `IntrinsicFlow` into region-field seeds (bit-exact provenance assert, no side-channel) → bounded `StructuredFieldStencilOp` heatmap (seed-then-zero, horizon-capped) → Layer-2 Sum reduce → **`ai_will_do` `field_urgency` EvalEML with spec-visible authored weights → authored threshold crossing (event_kind 7) → `BoundaryRequest::AttachOverlay` commitment applied through `apply_structural_mutations` onto the acting SimThing**. CPU recurrence oracle parity at the heatmap/urgency seams; counterfactual run proves the commitment depends on arena pressure. Report: [`docs/tests/ct_3b_4a_impl_0a_results.md`](tests/ct_3b_4a_impl_0a_results.md).
- **New surfaces (all consumer-pulled, opt-in):** `RegionFieldSpec.pressure_binding` + admission (bounds/duplicates/named-arena hard errors), `RegionFieldFormulaBindingSpec.weight_pressure/weight_resource` (ai_will_do as authoring data), driver `project_arena_pressure_seeds` (boundary-time consumption of resolved state), ClauseScript `region_field`/`mapping` dialect with mechanically derived slot/column layout. No new kernels needed — the lifted-WGSL allowance was exercised through the existing stencil/EML/threshold kernels. No sqrt anywhere (rule not triggered).
- **Honest 0A ledger:** PARTIAL — exact 0B blockers recorded: CT-RF-EML-RATE-0 (base/gate EvalEML band inside the RF execution plan), GPU-side projection copy, session-loop integration. Also fixed pre-existing `resource_flow_opt_in.rs` compile breakage from #592 (missing `base_obligations` literals).

# 2026-06-11 - CT-2C-REMEDIAL-3 + ORIENT-0 acceptance: executive review of the Codex/Cursor frontier-rung runs

- **CT-2c REMEDIAL-2 PASS ruled VOID (§0.6) and remediated in the same review.** Codex's modifier half was dead code: `_mult`/`_add` overlays were emitted into `GameModeSpec.overlays`, which install explicitly defers — no modifier ever changed any flow in any test; the mechanism was also wrong if wired (per-tick `Multiply` on a rate column compounds; stacking would be multiplicative vs the documented additive-in-effect). Categories were a flat table with the *required* `depth` parsed and discarded; no parent relation existed for the §6 inheritance asymmetry to act on; `value:` was absent; implementation jumped its own memo's acceptance gate.
- **Remediation (frontier, design authority):** parent-chain `category_map` (depth used, down-only validated, cycles rejected); `_mult` sweeps the category ancestor chain and `_add` applies leaf-only, **folded at hydration** into effective `BaseFlowObligationSpec` rates — `effective = (base + Σadd) × (1 + Σmult)`, additive-in-effect bit-exactly, deterministic order; dead modifiers and negative effective rates are hard errors; dead overlay emission ripped out; GPU oracle now proves folded rates flow install → reduce → allocate with bit parity (10.5 → 2.625/7.875 at 1:3 weights). Driver `intrinsic_flow_offset` fixed to route through `PropertyLayout::offset_of` (binding one-home invariant; enumeration position was width-1-coincidental). Report: [`docs/tests/ct_2c_impl_results.md`](tests/ct_2c_impl_results.md).
- **Conditionals get a real path instead of a deferral to a nonexistent rung:** ticket **CT-RF-EML-RATE-0** cut into §6 — a per-tick `EvalEML` effective-rate band over explicit base/gate columns ordered before the arena reduce bands (GPU-resident bounded arena arithmetic under the product WGSL/JIT-EML lift), carrying trigger-gated produces/upkeep (rising and falling edges), `value:` formula trees, and triggered rate modifiers. Named consumer: the CT-3b+4a implementation rung.
- **CT-3b+4a ORIENT-0 memo ACCEPTED WITH AMENDMENTS:** the Resource Flow arena-pressure reorientation is ratified in the §5 ladder row (recorded, not silent); §12's demotion of `ai_will_do` to "optional" is reversed — the CT-4a Layer-3 EML → threshold-commitment leg is mandatory in the implementation exit; CT-RF-EML-RATE-0 added to the allowed GPU computations.
- CT-2c design memo retroactively accepted with amendments (hydration fold supersedes runtime overlays for rate-typed targets; `value:` narrowing recorded as Deviation; lab `modifiers.log` round-trip remains a standing evidence obligation). CT-2a (Codex, non-frontier, oracle-backed) stands.

# 2026-06-11 - CT-1C: ClauseScript tradition set runs on the capability substrate — "designer writes Clausewitz, SimThing runs it"

- **The headline proof landed:** an original ClauseScript tradition set (`adaptability`: adopt → recycling → finisher) hydrates onto the existing `capability_tree_v1` machinery **verbatim** — `tradition_tree`/`category`/`tradition`/`possible{has_tradition}`/`modifier` → `CapabilityTreeSpec`/categories/entries/prereqs/Owner-targeted `Permanent` effects — installs per-faction (`AllOfKind`), and runs through a real GPU session. Zero substrate changes; hydration only. Report: [`docs/tests/ct_1c_impl_results.md`](tests/ct_1c_impl_results.md).
- **Prereq DAG → threshold ordering proven on GPU:** recycling's threshold fires first with adopt unresearched → waits in `OnPrereqMet` (nothing activates); adopt's threshold fires → adopt activates and the sweep activates recycling **in dependency order in the same boundary**; the finisher stays `Suspended` with its prereq met but threshold unfired (threshold AND prereqs, never OR). Both activated payloads transform the **owner faction's** `ct1c::potency` on subsequent ticks. Parity: hydrated pack canonically identical to the hand-authored RON baseline.
- **Doc corrected against source (house rule):** `capability_tree_v1.md` §4–§7 authored prereq `category` as bare names; the builder requires `namespace::name` (`parse_category_ref`, `UnknownPrereqCategory` on bare). All seven example occurrences fixed — found because the doc-faithful form failed install in this rung's first test run.
- **Test-hygiene fix in both CT GPU smokes:** `open_from_spec` errors no longer masquerade as "no GPU — skip" (one masked exactly this install error); GPU availability is probed with a bare `SimSession::open`, install errors fail loudly.
- **SCOPE-MEMO §8 prediction held:** install-owner targeting sufficed; no SPEC-SCOPE ticket pulled. Next: CT-2a (intrinsic flows, Codex-eligible) or CT-2c per ladder order.

# 2026-06-11 - SQRT-REPIN-0: Candidate F artifact hash re-pinned to LF-canonical bytes; exhaustive proof renewed

- **Root cause of the standing "pre-existing unrelated" failure found:** `sqrt_promote0_f_artifact_hash_guard` (red on this host since CT-0a, dismissed in three rung reports) was failing because the pinned Candidate F hash `e2e9e27601ee2e13` was computed over a **CRLF working-tree checkout** (5964 bytes — the archived SQRT-EXACT-4F proof log's own byte count confirms) while the repo's canonical committed artifact is LF (5855 bytes, hash `59ab4b2892e3c690`). The guard protecting exact-sqrt authority failed on every canonical checkout and passed only on line-ending-mangled ones — exactly backwards. Surfaced by product after watching native-sqrt bit-exact failures on another track and asking whether Candidate F was sufficiently discoverable.
- **Fix:** `SQRT_F_ARTIFACT_HASH` re-pinned to the LF-canonical `59ab4b2892e3c690` (artifact text character-identical, single commit in its history, never changed); `.gitattributes` (landed separately) makes the hash host-stable. Under the strict reading of "any F artifact change requires renewed exhaustive proof," the **full-domain exhaustive sweep was re-run over the canonical artifact**: 2,139,095,040 finite non-negative f32 values, `max_ulp=0`, `flush_count=0`, all bits exact (~66.6 s GPU). Addendum in [`docs/tests/phase_m_jit_sqrt_exact5f_exhaustive_sweep_results.md`](tests/phase_m_jit_sqrt_exact5f_exhaustive_sweep_results.md).
- **Guards now green everywhere:** spec `jit_exact_sqrt_artifact_admission` (10) + `field_policy_obs0_overlay_score_admission` (29); driver field-policy obs0/obs2/obs3/obs4/pipe0/event1 + candidate battery (35) — all previously failing their `include_str!` hash assert on LF checkouts. Active normative docs re-pinned (`workshop/sqrt_candidates.md` SQRT-REPIN-0 update block + ladder diagram, `mapping_current_guidance.md`, 0.0.8.0 ledger R4 chain); historical reports/archives untouched as records of their runs.
- **CT-1b cross-check (product question):** CT-1b itself contains no sqrt/magnitude anywhere (threshold compares + Add/Multiply overlays; no WGSL touched) — nothing was coded around native sqrt and nothing was raised off the GPU, so no lowering was owed. The native-sqrt guidance gap remains real for sqrt-adjacent tracks: any GPU sqrt claiming exactness must route through `m_jit_sqrt_f_exact` (invariants, JIT Kernel Registry row); native WGSL sqrt is `ApproximateJitOnly` and cannot close exact-authority tests.

# 2026-06-11 - CT-1B: recalc stress measurement — every-tick model CONFIRMED

- **The rung's question answered with measurement:** SimThing's re-evaluate-everything-every-tick model vs Paradox's cached/invalidated modifiers, stressed with a generated `triggered_modifier` corpus hydrated through ClauseThing (N = 16/64/256; each tm = own potential property + own payload property). **Confirmed, not retired:** steady-state armed corpus costs **zero CPU boundary work** (B3 fast path — pay-on-fire only); tick cost grows **sub-linearly** (15.5× columns → ~1.6× µs/tick); and at N=256 the armed corpus is **cheaper than the same column count of active permanent modifiers** (~536–591 vs ~679–696 µs/tick, reproduced ×2) because suspended overlays are parked and threshold evaluation undercuts overlay application. Counts exactly linear (N thresholds + N suspended overlays + 2N properties), RON baseline canonically identical at corpus scale. Report: [`docs/tests/ct_1b_impl_results.md`](tests/ct_1b_impl_results.md).
- **Lowering landed (same-scope only, per accepted SCOPE-MEMO §8):** `triggered_modifier { potential modifier }` → `Suspended{when_activated}` standalone overlay + `EventSpec` threshold trigger (`Current`, Rising) + `ActivateOverlayRef` effect. GPU smoke proves the full consumer path: hydrated corpus → install → Pass 7 threshold → boundary handler → exactly the matching overlay flips `Suspended → Permanent`.
- **§6 EffectSpec-widening first batch:** `EffectSpec::ActivateOverlayRef { target, overlay_ref }` (authored overlay activation by id — runtime `OverlayId` cannot be authored); driver install resolves refs against the standalone-overlay map with hard errors for dangling refs, cross-pack id collisions, and **ambiguous multi-owner refs (rejected pending SPEC-SCOPE-1, not approximated)**. `simthing-sim`/GPU/WGSL untouched.
- **Substrate behavior recorded:** threshold events are per-tick readback consumed only on boundary ticks — mid-day crossings don't reach the scripted-event handler (smoke uses tpd=1); flagged for whichever rung first needs multi-tick days with scripted events.

# 2026-06-10 - CT-TRACK-OPEN: ClauseThing production track authorized and opened (0.0.8.1)

- **Product authorization:** the parked L3 ClauseThing front-end is OPEN as the **`CT-` track** — [`docs/design_0_0_8_1_clausething_production_track.md`](design_0_0_8_1_clausething_production_track.md). Consumer: the Stellaris/Clausewitz-engine grand-strategy audience. Crate: **`simthing-clausething`** (spec option A, renamed).
- **Sequencing determination (binding, §2 of the track):** parser-first. `simthing-spec` needs **widening, not rebuilding** (gap inventory led by `ScopeRef {Current, Slot(u32)}` and three-variant `EffectSpec`); stages 1–4 touch zero spec surface; speculative enrichment is the named anti-pattern; "flawless" attaches to the **lossless Stage-4 raw model** + the **CT-1a bit-identity instrument** (ClauseScript entity ≡ hand-authored RON, tree bit-exact); one design-first exception gated as **SCOPE-MEMO** (post-CT-0d corpus evidence, pre-cross-scope rungs).
- **Ladder:** CT-0a (vendor jomini text path + license accounting) → CT-0b (lossless raw model) → CT-0c (expansion passes) → CT-0d (scope extraction + frequency report) → CT-1a (bit-identical entity) → SCOPE-MEMO → CT-1b (recalc stress measurement) → CT-1c (capability tree) → CT-2a/2c (economy) → CT-3b+4a (headline Movement-Front vertical). Standing consumer-pulled spec-widening backlog in track §6.
- **Jomini internalization:** vendor the **text path only** into `crates/simthing-clausething/src/jomini/` (binary/envelope/melting excluded permanently); MIT accounting — verbatim LICENSE retained, per-file attribution headers with vendored commit, `THIRD_PARTY_LICENSES.md` at repo root.
- **Fixture/licensing boundary (binding):** Paradox-generated material (`script_documentation/*.log`, game files) stays lab-only in `C:\Users\mvorm\Clauser\`, never committed; repo fixtures are original ClauseScript; jomini's own MIT fixtures may vendor.
- **Deviation record:** charter's exporter-first ruling deviated by product (importer first); export-aware metadata hooks deferred as documented inert `export_meta` placeholders from CT-0b; no corpus-export claims in this track.
- **Docs:** canonical reconciled spec copied to [`docs/clausething/ClauseThing_Spec.md`](clausething/ClauseThing_Spec.md) (banned acronym scrubbed to Movement-Front/FIELD_POLICY, status header rewritten to AUTHORIZED, v7.7 citations marked historical, §9 ladder superseded by the track); ClauseScript textbook + Paradox logs + jomini clone intentionally remain in the Clauser lab. Pointer updates: 0.0.8.0 ledger banner UPDATE, constitution §3 status row → AUTHORIZED/OPEN, §5 stop-condition list amended, §5.1 closing paragraph rewritten.
- **Hardness register added (track §6.1) after product audit question:** Grok's per-section transpiler-difficulty verdicts were only partially reflected; a full extraction pass over the textbook's implication sections (§3.9, §4, §5.5, §6.3–6.4, §7.7, §8.5, §9.4, §10.10, §11.4, §14.3, §16.7, §17.8, §18.2–3) produced a four-class register (A mechanical / B complicated / C difficult / D rejected-or-deferred) with every item pinned to a rung, backlog ticket, or permanent exclusion. **Genuinely missing items surfaced and added to §6:** control-flow lowering (`if`/`else_if`/`else`, `switch`; `while` bounded-or-reject policy), the **effect-ordering contract** (textbook §6.3 sequential observability vs batch boundary application — T2), and multi-polity higher-order governance (deferred, no rung). SCOPE-MEMO row widened to name transitive scope forms and `event_target:` references. Register carries a maintenance rule: ungraded constructs discovered by a rung update the register in the same PR.
- **Agent gating added (product directive):** conditional-compilation rungs flagged **[FRONTIER]** — implemented only by Claude Opus (max effort) or Claude Fable 5 (high effort): **CT-0c** (expansion ordering), **CT-1b** (triggered_modifier lifecycle + stress measurement), **CT-1c** (gate compilation / capability tree), **CT-2c** (`value:` SELECT lowering + category inheritance), **CT-3b+4a** (ai_will_do formula vertical); SCOPE-MEMO marked design-authority (Opus); backlog items **control-flow lowering** and **effect-ordering contract** frontier-gated. Mechanical rungs (CT-0a/0b/0d, CT-1a, CT-2a) remain Codex/Cursor-eligible. Rule recorded in §5 legend + §9: a frontier flag never downgrades. Mirrors the AccumulatorOp-v2 precedent of naming Opus-gated PRs explicitly rather than implying them.
- **Modifier/classifier challenge absorbed (product flag, §7 + §10.5 full read):** the decisive finding — **the generated modifier-key namespace is a grammar, not a dictionary**: `generate_mult/add_modifiers` synthesizes keys compositionally as `(category, resource, flow-kind, op)`, so the 3.8 MB `modifiers.log` long tail collapses into one decoder (new §6 classifier-engine item, frontier-gated, lands with CT-2c; CT-2c row widened). Binding semantics recorded: **inheritance asymmetry** (`_mult` sweeps category children via OrderBands, `_add` leaf-only — Paradox's anti-cascade rule matches sweep-vs-leaf reduction natively), **granularity leakage guard** (`modifier_category` → admission validation), `triggered_produces_modifier` as gated family generation. New §6 item: **timed modifiers & `has_modifier` reads** (days→tick overlay expiry; `remove_modifier`→`SuspendOverlay`; overlay-active readable state as a design choice). Register updated: Class A gains native provenance (overlays carry identity — "where did this bonus come from" is free); Class B gains the key-grammar trio + expiry mapping.
- **Provenance caveat + textbook relocation (product directive):** track §6.1 gains a binding **secondary-source provenance caveat** — the textbook is an AI-authored synthesis of community reverse-engineering of a closed-source engine; all its semantic claims (key grammar, inheritance asymmetry, stacking, granularity, expansion ordering) are provisional until verified against the lab primary sources; CT-2c's decoder is accepted only after round-tripping `modifiers.log` itself; failed claims are corrected in the register and noted as textbook errata; unverifiable corners fall to model-exactly-or-reject. The ClauseScript textbook was **moved** (not copied — one canonical copy) from the Clauser lab into [`docs/clausething/ClauseThing.md`](clausething/ClauseThing.md) so implementation agents can consult it when design authority is rate-limited; read-order §10 and the spec header updated. Paradox logs and the jomini clone remain lab-only.
- **Drift-risk adjudication (product question):** keeping the textbook in-repo, **armored** rather than exiled. Risk identified as distinct from the original spec-distraction mode (defused by the authorized track): **authority confusion about the runtime model** — the book authoritatively describes the foreign engine's lazy invalidation, sequential imperative effects, and CPU evaluation, i.e. the documented CPU-regression drift pattern in textbook form. Exile rejected because it breaks the rate-limit lookup use case and merely relocates the hazard (agents can read the Clauser lab too), while the book's value is proven (it produced the modifier findings). Armor applied: binding **REFERENCE-NOT-SPECIFICATION preamble** prepended to the textbook itself (subordination to track/core-design/invariants; drift warning naming the engine-model failure mode; provenance caveat; scope-from-ladder-only rule) + matching guard sentence in track §9. Reversible with one `move_file` if drift is observed anyway.
- **Pre-merge differential scan (product request, token-economic):** read the ~600 unread mechanism-body lines (skipping the ~700 already ingested, the out-of-scope savegame/binary chapters, and jomini-owned §2 syntax) plus a construct-inventory grep. **Two new hard items:** dynamic identifiers (`set_flag = @root` runtime-constructed names vs admission-time column registration — new §6 item, reject-or-bound) and `value:` recursion vs the 32-node cap (compile-time expansion under declared depth cap). **New caveat ruling:** where the textbook reports source semantics as implementation-defined (script-value evaluation order/caching), fidelity is not chased — ClauseThing defines its own deterministic order at admission. **Confirmations/one-liners absorbed:** §10.9's cycle resolution ("previous-tick snapshots") = the engine's native ping-pong discipline, independently derived; stockpile-banded `ai_budget` = threshold-gated weight overlays w/ native normalization; controller-vs-owner = second relation column; decisions = actor-initiated events; leaders/governors = subtree-scoped overlay SimThings; RMW variable effects bound to effect-ordering contract; on_action multi-registration = list-registry collect; §17.4 cross-faction reads (`relative_power`, opinion) → pairwise relation columns, unscoped (Class C). Register and §6 updated in place.
- **Primary-source log scan (product request; rate-limit reading session):** copied `effects.log`/`triggers.log`/`scopes.log`/`modifiers.log` + the `02_Dynamic` wiki doc to the container and measured rather than ingested. **Verdict: the script is what we expect** — all four logs are machine-parseable in exactly the shapes CT-0d/CT-2c assume. **Measured:** 90 scopes w/ Supported/Output rules; 1,015 effects; 1,041 triggers; ~28%/~20% iterator forms; 41,016 modifier keys — `shipsize_*` grammar ~69% (28,179), economic grammar ~16% (6,426), hand-defined ~15%. **Two textbook claims corrected by primary sources (errata block added to the textbook preamble + track §6.1):** #1 `modifier_category` is a soft tag w/ legal broadcast-down (not a leakage preventer) → granularity rule re-lowered as direction-only validation + ancestor-sweep; #2 grammar attribution → family of grammars, CT-2c implements the economic decoder only. **Upgrades:** `while` verified natively bounded (`limit` + `count=[N|Variable]` forms) → bounded-compile is fidelity; **`break`** discovered (sequential short-circuit) → added to control-flow item, binds to effect-ordering contract; longest-match category resolution noted (underscore-ambiguous segments). **Textbook omissions found:** DLC staged-progress systems (situations, archaeology, first contact, astral rifts) = progress sub-field + stage thresholds + approach overlays, no new substrate; recorded in §6.1 measured-vocabulary block. True corpus conditional density needs game `common/` files — flagged as a CT-1b fixture input.

# 2026-06-10 - CORE-DESIGN-KNEE-GUIDANCE: deferred Pareto-knee toolkit recorded as §7.3 guidance

- **Product question answered for the record:** the Pareto knee is **not** an opcode, gadget, or any implemented artifact in any crate — it exists only as horizon paper (charter §1.4, adjudicated 2026-06-09). The product's prior deferral of cascade-detection methods **stands**; charter ruling "opens no consumer of its own" stands. What was observed is **expressiveness, not presence**: over *measured* sensitivity columns the MCF is plain ratio/max algebra — an ordinary EML gadget tree under the existing admission contract — a theorem of the opcode set's generality (the same slot-in pattern as Wei/Odrzywolek/LeWM), neither planted nor accidental.
- **Guidance added (Tier-2 by-addition, product-mandated):** `simthing_core_design.md` §7.3 "Trade-off geometry over the front — the Pareto-knee toolkit (deferred; guidance only)" + snee reference appended. Records: status (deferred, unimplemented), the concept (knee = least maximal change; knee event = front kink; cascades nucleate at kinks; owner-entity fission §2.1 = the intra-faction knee), why no new substrate is needed (objectives/actions exist; sensitivities measured via paired counterfactual replay or dither — analytic derivatives unavailable in clamped/nonsmooth dynamics; MCF gadget + ordinary Threshold/EmitEvent), the three Movement-Front consumer uses (distance-from-knee as compiled risk tolerance; cascade early warning paired with velocity columns; certification-grade label generation with learned estimators ApproximateDiagnostic forever), and the actual gate (the unbuilt **sensitivity-production harness**, consumer-gated). Banned legacy acronym avoided throughout — Movement-Front naming only. Deferral preserved: guidance, not authorization.

# 2026-06-10 - CORE-DESIGN-POLICY-CAPTURE: owner-entity fission promoted into the permanent paradigm doc

- **Product-mandated Tier-2 by-addition amendment:** `simthing_core_design.md` gains **§2.1 "Owner-entity fission — policy capture, succession, and civil war"** plus drift-litmus **#12** (no rebellion/civil-war/coup system, flag, or special entity — the answer is influence flow + aggregate threshold + owner-entity fission with intensity-vector column partition). Motivation: the mechanism was discoverable only by deep retrieval across `capability_tree_v1.md`, the E-11B workshop findings, and chat history — invisible to a small-context agent reading the mandatory set. It now sits peer-level with the mapping and resource-flow concepts.
- **Content recorded:** influence as an ordinary leaf→root→owner flow (foreign-sponsored capture native via the root round-trip — lobbying/regulatory capture/fifth columns are one flow with different sources); trigger as an `AggregateAlertRegistration`-class threshold (civil war = values crossing thresholds, never flags); fission of the **owner entity** via existing `FissionTemplate` + `clone_capability_children`, assets partitioned by per-asset alignment-intensity as a **mass owner-column flip** (no spatial reparenting — avoids the E-11B arena-re-enrollment gap by construction); influence-velocity columns pre-announce the burst for predictive slot pre-allocation; per-arena `FissionPolicy` governs membership re-resolution; **subversion** named as the fourth strategic vector; endogenous reflexivity (the rules as contested objects inside the simulation) made explicit. Provenance paragraph pins the three source workshops so the findings are never re-excavated from archives.
- **Propagated to the LeWM horizon files:** charter §1.3 gains the **endogenous-reflexivity** bullet (regime-shift episodes — policy layer as state, not constant — as corpus product and OOD-benchmark material); charter §1.4 gains the **intra-faction knee** (owner-entity fission as the internal cooperation–defection knee event; influence-velocity columns as Tier-1 leading indicator); charter §1.5 adds **subversion as the league's fourth action verb** with a visor subversion-preference axis, pairing the two knee events (betrayal external / civil war internal). Case file §4 gains the **semantic-port generalization**: the importer slot is domain-agnostic (history, market intelligence, advertising as interchangeable calibration plugs; commercial data denser by orders of magnitude; Clausewitz-class game complexity-equivalent at substrate level); market-skin reflexivity hazard endogenized via core design §2.1, making the game corpus the source of ground-truthed rule-capture/regime-shift trajectories; aggregate-not-pixel boundary binds doubly for any market skin.

# 2026-06-10 - LEWM-CORPORA-CASE: case file landed; importer pivoted to ClauseThing, cliodynamics back-burnered

- **Retroactive record:** [`docs/simthing_lewm_corpora_case.md`](simthing_lewm_corpora_case.md) was written 2026-06-09 (session ended before its worklog entry): the horizon case file capturing the JEPA scaling-instrumentation thesis (SimThing ground-truth columns as the missing external coordinate system; three controlled scaling axes), the empirical-window audit (study ill-posed until SIGReg Nov 2025; AMI Labs cannot build the instrument by thesis; robotics integrators structurally elsewhere; Apple SALT circling; 12–24-month window), the Movement-Front/Wei prior-art audit (MIT NCA paper, Causal-JEPA, Agent World Model circling the composition; nobody assembling it; P3 = world performs the relevance partition, P2 = small law big world), and the formal H1/H0 cross-paradigm scaling hypothesis with the constants-vs-exponents primacy correction and render-entropy mechanism axis.
- **Product decision (2026-06-10):** the importer slot proceeds as the **ClauseThing transpiler** — named potential consumer: the Stellaris/Clausewitz-engine grand-strategy audience (players/modders). The **cliodynamics/cliometrics transpiler is back-burnered** to the case file §4 footnote (compressed record preserved: SDT-ODE→engine mapping, literature-supplied acceptance tests, second-anchor epistemics, aggregate-not-pixel boundary); revivable cheaply since both instantiations share the CLAUSE-SPEC lowering pipeline. Back-burnering is itself consumer-pulled: the gamer consumer is named and real, the historical-science consumer is not yet.
- **Edits:** case file rewritten in full (header amendment, new §4, §5 corpus line, §6 disposition, references annotated); charter `CLAUSETHING-IMPORTER-0` entry gains the product note. Scaling instrument proceeds on the internal anchor only; sequencing (exporter first) unchanged; nothing opened.
- **§2 addendum (2026-06-10, verified via search):** DeepMind/Fenris event recorded — CCP self-buyback from Pearl Abyss ($120M cash+crypto, rebrand to Fenris Creations, 2026-05-01); Google DeepMind minority stake ("millions") + research partnership running an **offline EVE Online on a local server** for controlled model evaluation (stated focus: long-horizon planning, memory, continual learning; Hassabis: games as "perfect training ground"). Adjudicated as the strongest third-party validation of the dual-mission asset class to date — but **an interactive evaluation arena, not a metrology lab** (no bit-exact replay, no do-operator pairs, no complexity dialing, no causal ground truth: EVE's causal layer is out-of-band — the server logs the kill, never the con). Unoccupied slot survives narrowed; planning-horizon window tightened to ~12 months (DeepMind holds motive, adjacent asset, and both Genie-generative and MuZero-latent lineages); EVE's public MER aggregate series flagged as a genre-native second-anchor candidate for the §4-footnote program.

# 2026-06-09 - FIELD-WORLD-MODEL-HORIZON: adjudicate Codex LeWM/dataset/ClauseThing handoff

- **Adjudicated horizon charter written:** [`docs/workshop/field_world_model_horizon.md`](workshop/field_world_model_horizon.md). Governing correction: SimThing's dual mission made explicit — videogame engine **and** GPU-resident simulation ontology engine producing interactive training corpora; the field-movie corpus is a product line, not instrumentation.
- **Key rulings:** (1) Codex's "safe" path (ClauseThing with inert export hooks, no exporter) rejected as the 0.0.7.9 build-ahead-of-consumer failure mode — constitutional order is narrow `FIELD-MOVIE-DATASET-0` proving the schema first, then export-aware `CLAUSETHING-IMPORTER-0`; (2) two-AIs clarification — FIELD_POLICY + personality overlays IS faction AI v1 and waits on no ML; the LeWM-class module is an external advisory imagination layer; (3) vocabulary compiles to existing mechanisms (visor = personality overlays; actions = weight columns/threshold biases; policy compiler = admission in reverse); (4) added the missed superpowers — interventional/counterfactual episode pairs from determinism + overlay-as-intervention, replay-log-pinned manifests as provenance backbone, Movement-Front cell schema as free probe labels, corpus-as-eval-suite, ClauseThing as curriculum language.
- **LeWM paper claims verified** against arXiv 2603.19312 v2 abstract (15M params, two-loss end-to-end JEPA, probing, surprise; Maes/Le Lidec/Scieur/LeCun/Balestriero).
- **No track opened.** All consumers named-only; authorization remains a product decision.
- **Charter §1.5 added (interactive training + self-play league):** product's 50/50 offline-interactive LeWM training paradigm with a generational self-play league adjudicated FEASIBLE AND DOUBLY USEFUL with corrections: offline half carries logged FIELD_POLICY behavior-policy actions (never action-free); 50/50 is a curriculum endpoint, not a start (offline-first, ramp interactive; requires FIELD-OPTIMIZER-0 machinery — no ladder reordering); actions are parameterized pressure primitives, never freeform heatmaps (planning dimensionality + admissible authority). League split: visors propagate by tournament selection, one shared world model per generation trains by SGD on pooled data (AlphaStar-league checkpoints + exploiters vs meta-cycling; non-stationarity stated honestly). Diplomacy ruled behavioral-not-linguistic (discrete diplomatic action vocabulary; betrayal = §1.4 knee event in cooperation-defection). League doubles as the corpus diversity engine (auto-curriculum, Elo-stamped episodes) and adversarial simulator fuzzer. Consumer name reserved, not opened: FIELD-LEAGUE-0.
- **Charter §1.4 added (Pareto-knee events / nonlinear decision cascades):** adjudicated the snee approach (Giovannelli/Raimundo/Vicente, arXiv:2501.16993) against the engine. Ruling: their weight simplex IS the policy-action space (weight columns + threshold biases); two-tier detection — Tier 1 engine-native (counterfactual pairs = measured Jacobian columns; MCF = EML ratio/max gadget tree; knee event = Threshold+EmitEvent crossing, FIELD_POLICY-compliant) is certification authority + label generator; Tier 2 LeWM-amortized (autograd action-sensitivity, rollout disagreement, surprise spike) is ApproximateDiagnostic forever. Snee's smoothness assumptions don't hold in clamped/thresholded dynamics — favoring the measured/learned route exactly where cascades nucleate (front kinks). Visor closure: risk tolerance = distance-from-knee preference. Probe targets added to LEWM-PROBE-0; no new consumer. Charter rewritten in full in the same pass to repair an encoding corruption from the original write.
- **Core design amended (product-approved, Tier-2 by-addition):** `simthing_core_design.md` §1 gains "The substrate, not the game, is the product" — the dual mission (game engine + GPU-resident simulation ontology engine producing field-movie corpora) recorded as *identity, not authorization*, grounded in the v4-era semantic-free guardrails; candidate technologies deliberately kept out of the permanent layer (pointer to the horizon charter). Constitution deliberately untouched — the core design is already in the §0.5 fixed harness, so restating in §0 would reintroduce the duplication DOCS-TRIM-0 removed.
...
- **Archived superseded design docs** to `docs/archive/superseded_design/`: `design_v7.md`, `design_v7_6/7/8.md`, `design_v7_8_production_track.md`, `design_v7_9_*.md`, `design_0_0_8_0.md`. Repointed the two content-gated test `include_str!` refs (`phase_m_boundary_cadence_doctrine`, `phase_m_jit_grad0_spatial_observer`) and one core doc-comment from `design_0_0_8_0.md` → `design_0_0_8_1.md`; preserved all content-gated strings.
- **Draconian docs/tests archive:** moved 251 superseded test-result/scratch docs to `docs/archive/superseded_tests/`; kept only the code-referenced set (RR-4 closure, R1A next-tick, R2 baseline/profiling, frontier v1.5/v2.0–v2.4, jit-sqrt sweep). Archived `docs/handoffs`, `docs/reviews`, `docs/scenarios`, `docs/gameplay`, `docs/production_paths` (openings for closed/parked tracks) wholesale under `docs/archive/`. Archived `nvidia_fp_determinism_test.md`.
- **Build-safe:** `cargo check --workspace --tests` green; `cargo fmt --all -- --check` clean; `phase_m_boundary_cadence_doctrine` 7/7 (reads `design_0_0_8_1.md`). Pre-existing GPU-pinned-fingerprint failure `frontier_v2_0_replay_reproducibility` confirmed present on pristine HEAD (unrelated to cleanup). Kept production track, `invariants.md`, and active reference docs in place.
...
- **Trigger:** the RUNTIME-0080-0 R6C/R2 100-tick rehearsal silently flattened the specified recursive structure (galaxy 20×20 → 13 star-systems each 10×10 → planet → 10×10 surface → pop-cohort + factory children → labor economy) into a single galactic tier, and closed PASS without recording it. Multi-theater residency and the planet-surface economy were never built — which is why M-4A/recursion stayed theoretical.
- **Compliance applied:** Deviation Record + Specified-vs-Implemented Scope Ledger attached to `docs/tests/runtime_0080_0_r2_stable_100_tick_rehearsal_results.md`. RUNTIME-0080-0 remains CLOSED **only as a flat galactic-tier runtime proof**, not as the recursive spec.
- **Superseded:** `RUNTIME-0080-M4A-OPEN-0` (mis-scoped parallel theaters).
- **Opened:** `RUNTIME-0080-RR-OPEN-0` — recursive galaxy→system→planet-surface rehearsal. Spec: [`docs/production_paths/runtime_0080_recursive_rehearsal_opening.md`](production_paths/runtime_0080_recursive_rehearsal_opening.md). Ladder RR-0→RR-4; nested residency (generalize atlas_0080_0) + planet-surface pop/factory/labor economy + proven R2 galactic loop, 100 ticks vs recursive CPU oracle, no flattening, Scope Ledger per rung. M-4A sparse residency consumed here as the nested residency mechanism.
...
- **Anti-drift contract hardened:** the CPU shadow remains complete, serializable, and pausable: Tier-A values, membership, positions, birth/removal flags, fusion lineage, slot allocation, and structural hashes are captured in a deterministic snapshot, encoded, reloaded, and checked for continue-stable equality. No structural state is reconstructed from value-only Tier-A projection.
- **Backpressure posture recorded:** the GPU value loop may run ahead; the bounded CPU boundary journal drain is not a hot-path gate; per-tick decision readback remains forbidden; maximum unserialized lag is documented as one tick at the boundary save point.
- **Verdict:** PARTIAL (STOP-LINE) — resident REENROLL scatter, birth/removal authority, and fusion compaction remain behind free-list-scatter / compaction. Report: [`docs/tests/runtime_0080_0_r1c_resident_decision_results.md`](tests/runtime_0080_0_r1c_resident_decision_results.md).

# 2026-06-05 - RUNTIME-0080-0-R1b-DRIFT-FIX-0: self-consistent decision witness; full event-journal parity

- **Fixed structural drift:** R1b's first cut reconstructed the CPU boundary witness each tick from **partial** Tier-A GPU readback (value-only columns), silently losing structural state (births/removals/fusion/positions) and drifting from the oracle — the two GPU-decided combat ticks (44, 50) produced nothing. Replaced the per-tick GPU-readback reconstruction with `R1aBoundaryWitness::step_tick_capture_events`, a self-consistent CPU decision witness that carries its own structural state forward (R1a's proven non-drifting pattern). The GPU Tier-A value loop now runs resident in parallel and is no longer in the decision path, so the GPU is not starved waiting on the CPU.
- **Fixed non-finite journal field:** combat `DamageDelta` carries a negative delta; the journal bit-cast `f32::from_bits(-1i64 as u32)` = NaN, which the resident fill rejects (`NonFiniteValue`), so combat ticks failed to stage. Signed deltas are now stored as exact f32 values (ship counts << 2^24).
- **Verdict:** PARTIAL — but the journal substrate is now **complete and exactly oracle-matched**: all seven event kinds emit, `gpu_event_row_count_total == oracle_event_row_count_total`, per-tick canonical parity holds every tick, boundary pass consumes GPU rows without rerunning tick logic, R1a Tier-A + R6C checksum `1bba891c779190a4` preserved. The only remaining gap is `structural_decisions_gpu_emitted = false` (CPU decision witness still decides; resident REENROLL/scatter/compact = R1c).
- **Test gate:** `cargo test -p simthing-driver --test runtime_0080_0_r1b` → 26 passed; 0 failed. Regressions: R1a 35, R6C 22, R0 16 — all passed. Report: [`docs/tests/runtime_0080_0_r1b_resident_event_journal_results.md`](tests/runtime_0080_0_r1b_resident_event_journal_results.md). Anti-drift + GPU-non-starvation handoff: [`docs/handoffs/runtime_0080_0_r1c_resident_decision_opening.md`](handoffs/runtime_0080_0_r1c_resident_decision_opening.md).
...
- **Verdict:** PARTIAL — journal created, GPU writes/reads rows, boundary pass consumes rows without re-deriving movement/combat/production/blockade planners; aggregate oracle parity not yet earned (`DamageDelta`, `ZeroCohort`, `ShipCountDelta`, `LocalBirthRequest` missing from GPU totals). *(Superseded by the DRIFT-FIX-0 entry above, which earns full parity.)*
...
- **Finding (confirmed):** the ban was **stricter than the constitution**. `design_0_0_8_0.md` §2.3 already says *"There is no blanket WGSL ban … New generic WGSL is a Tier-2 gate, not a prohibition,"* and `docs/invariants.md` line 112 already admits generic, semantic-free shader extensions paired with CPU-oracle parity. The real guardrail is semantic-freeness + parity + meaning pinned at the spec/admission layer — not a per-rung prohibition.
- **Constitutional clarification (the one real ambiguity — EML interpreter opcodes):** `design_0_0_8_0.md` §2.4 now states the gadget-layer "no new opcode" rule does **not** ban extending the generic interpreter; `docs/invariants.md` gains the companion row *"Extending the generic substrate vocabulary is a Tier-2 gate, not a prohibition"* (generic, semantic-free opcode / `AccumulatorOp` combine fn / kernel admissible under §2.3 with bit-exact CPU-oracle parity, reusable by any SimThing, behaviour stays data; scenario-specific = semantic = banned).
- **Handoff amended (new §4a admission gate):** [`docs/handoffs/runtime_0080_0_r1a_remedial_opening.md`](handoffs/runtime_0080_0_r1a_remedial_opening.md) — adding a generic, semantic-free opcode/combine-fn/kernel is **permitted** for Outcome A (moved out of stop-lines into the §4a Tier-2 gate); semantic WGSL/opcodes stay banned; the §6 anti-faking protocol is unchanged (a new opcode that only moves a CPU-injected next-state still fails the negative control). Spec note: [`docs/production_paths/runtime_0080_0_r1_next_tick_authority_spec.md`](production_paths/runtime_0080_0_r1_next_tick_authority_spec.md) §14.1.

# 2026-06-05 - RUNTIME-0080-0-R1a-REMEDIAL-0: correct faked Tier-A authority + unify substrate (design authority)

- **Opus ruling (docs-only):** Audited `RUNTIME-0080-0-R1a-IMPL-0` and found the PASS **overclaimed**. The GPU does **not** compute the Tier-A transition: the CPU recomputes the full next-state trajectory (`build_tier_a_oracle_states`) and injects `state_N+1` into the GPU each tick (`write_slot_col_values` → `COL_JOURNAL_DELTA`); the GPU "tick" is three `Identity` copies + swap. `gpu_state_feeds_next_tick == true` holds only mechanically; the CPU remains the computational authority (the R0A gap in a costume). `inter_tick_tier_a_upload_count = 0` is inaccurate (the per-tick journal write is that upload); no test distinguishes GPU-computed from CPU-injected. **Downgraded R1a-IMPL-0 to IMPLEMENTED / PARTIAL (SCAFFOLD).**
- **Opened `RUNTIME-0080-0-R1a-REMEDIAL-0`:** make the GPU the actual Tier-A transition authority and **unify** next-tick authority onto the production overlay/intent/threshold substrate (`WorldGpuState` + `Pipelines` Pass 0–7 + `OverlayDelta`/`IntentDelta`/`ThresholdEvent`) instead of a private journal. Register the R6C Tier-A transforms (already GPU-measured in GPU-MEASURE-0080-0) as `AccumulatorOp`s/overlays so the GPU produces `state_N+1`; player/AI overlays + FIELD_POLICY threshold acts + the resident transition ride one substrate; Tier-B stays bounded CPU boundary maintenance via `ThresholdEvent → BoundaryRequest`.
- **Anti-faking oversight (binding):** acceptance requires independence (zero inter-tick CPU writes of Tier-A next-state after seed), a **negative control** (disabling the GPU transform must FAIL parity), measured counters (not hardcoded), earned per-column parity (GPU-vs-oracle of a GPU-produced value), and a source-shape guard (measured shapes, not `Identity` passthrough); the report must carry that evidence. A PASS reproducing the inject-and-copy pattern is rejected; a correct PARTIAL/BLOCKED naming the gap is acceptance.
- **Handoff:** [`docs/handoffs/runtime_0080_0_r1a_remedial_opening.md`](handoffs/runtime_0080_0_r1a_remedial_opening.md); spec §14: [`docs/production_paths/runtime_0080_0_r1_next_tick_authority_spec.md`](production_paths/runtime_0080_0_r1_next_tick_authority_spec.md). Report path (rewrite): `docs/tests/runtime_0080_0_r1a_next_tick_authority_results.md`. Recipient: Cursor / Codex5.5max. No `docs/invariants.md` edit.
...
# 2026-06-05 - RUNTIME-0080-0-R1-DESIGN-0: open GPU-resident next-tick authority substrate (design authority)

- **Opus ruling (docs-only):** Confirmed the R0A claim check verbatim — R0A is a persistent-GPU-session **mirror-dispatch** scheduler, not GPU-resident tick authority; CPU remains the tick driver. **Closed R0A as PARTIAL / informative** (correct honest outcome; no PR #531 change). **Opened `RUNTIME-0080-0-R1`** defining the substrate primitive **`GPU-STATE-AUTH-0`** — GPU-resident world state as the input authority for tick N+1 (`gpu_state_feeds_next_tick == true`).
- **Hybrid A+B scope:** R1 is the umbrella; first IMPL sub-rung **`R1a`** is narrowed to **Tier-A field columns** (disruption, location_status, stockpiles, construction_progress, per-cohort `num_ships` value, blockade/divert code, R4 magnitude) via a resident **double-buffer** swap — no new op/WGSL/atlas-batching/M-4A, glue only. **Tier-B** structural ops (REENROLL membership scatter, cohort birth/removal, fusion lineage) are applied by a **bounded CPU boundary-maintenance pass driven by a GPU-written event journal** (boundaryEvent dispatch) — GPU-decided / CPU-applied, not a CPU planner. **Tier-C** (multi-atlas, M-4A, recursion, multi-faction) deferred.
- **SimThing Maximality + parity directives:** any row/mask/reduce/disburse/threshold/emission-band transition is promoted toward resident execution; CPU shadow is **retained** as parity witness (not deleted — would break semantic-GPU parity), only authority moves to the resident buffer, reconciled at the **tick boundary** (the save/pause/stable-state point). Correct fallback is honest PARTIAL/BLOCKED + named substrate gap, never CPU orchestration wearing GPU language.
- **Spec:** [`docs/production_paths/runtime_0080_0_r1_next_tick_authority_spec.md`](production_paths/runtime_0080_0_r1_next_tick_authority_spec.md). Report path for impl: `docs/tests/runtime_0080_0_r1_next_tick_authority_results.md`. Recipient: Cursor / Codex5.5max. No `docs/invariants.md` edit.
...
- **Adapter measured:** NVIDIA GeForce RTX 4080 Laptop GPU. Stable report checksum `6b5340e84da289cb`; R6C checksum remains `1bba891c779190a4`.
- **Guardrails held:** no new semantic WGSL, no new `AccumulatorOp`, no invariant edit, no pinned-number change, no default SimSession wiring, no gameplay change, and no reopen of `SCENARIO-0080-2`. R6C whole-run execution deliberately keeps `GPU-conformant; GPU execution not yet measured`.
- **Verified:** GPU-MEASURE (11/0), ATLAS STORE-GPU (10/0), ATLAS STORE CPU (11/0), R6C (22/0), R6B (24/0), R6 (25/0), R4 (16/0), R2 (13/0), R1 (34/0), mobility REENROLL (16/0), mobility runtime composition (23/0), `cargo check --workspace` PASS with pre-existing warnings only. Nearest GPU-adjacent checks also passed: GPU-EXEC-0 (13/0), KERNEL-0 (16/0), KERNEL-6 chain (22/0), PACK-GPU (8/0), Candidate-F magnitude (12/0), fixed mag2 + F sqrt (7/0), exact sqrt candidate battery (35/0, 3 ignored exhaustive sweeps), structured-field stencil (30/0), accumulator bridge (3/0).

# 2026-06-04 - RUNTIME-0080-0-R0-IMPL-0: single-tier GPU-resident R6C runtime scheduler

- **Implemented RUNTIME-0080-0-R0** as an opt-in/default-off single-theater GPU-resident tick scheduler for the R6C 100-tick loop (`crates/simthing-driver/src/runtime_0080_0_r0.rs`, tests `runtime_0080_0_r0.rs`). World state persists in a GPU `AccumulatorOpSession` across ticks (upload-only between ticks; `inter_tick_world_readbacks=0`); per-tick R1/R2/R4/R6/R6B shapes dispatch on GPU via accepted AccumulatorOp / StructuredFieldStencil paths; CPU R6C oracle remains determinism reference.
- **Adapter:** NVIDIA GeForce RTX 4080 Laptop GPU. **CPU-oracle parity:** checksum `1bba891c779190a4` bit-exact; R4 f32 within `1.0e-4`. **R6C whole-run posture:** `R6C whole-run GPU-measured on RUNTIME-0080-0-R0`. Report: [`docs/tests/runtime_0080_0_r0_results.md`](tests/runtime_0080_0_r0_results.md); stable report checksum `04a4f921f33845ef`.
- **R6C hook:** `R6cGpuTickHook` + `execute_model_with_gpu_hook` in `dress_rehearsal_r6c_integrated_run.rs` for per-tick GPU scheduler integration without changing R6C computation.
- **Stop-lines held:** no `request_atlas_batching`, no M-4A masking-at-scale, no new semantic WGSL/op/invariant/pinned-number change, no `SCENARIO-0080-2` reopen, no default SimSession wiring.
- **Verified:** runtime_0080_0_r0 `20/0`; gpu_measure_0080_0 `11/0`; dress_rehearsal_r6c_integrated_run `22/0`; atlas_batch_0_store_gpu `10/0`; atlas_0080_0 `17/0`; `cargo check --workspace` PASS with pre-existing warnings only.

# 2026-06-04 - GPU-MEASURE-0080-0-COMPLETE-0: open RUNTIME-0080-0 production runtime track (design authority)

- **Ruling: C — open `RUNTIME-0080-0`, a production runtime tick scheduler over sparse-resident GPU shapes.** `GPU-MEASURE-0080-0` (PR #528) reviewed at report + test level: on the RTX 4080, R1/R2/R6/R6B are `GPU-measured (integer bit-exact)` and R4 is `GPU-measured (verified-approximate)` (Δ `3.0994415e-6` ≤ `1.0e-4`); the test harness asserts `selected_discrete_gpu` with a real adapter, so it is a genuine GPU run. R6C whole-run stays `GPU-conformant; GPU execution not yet measured`. No defect → no reopen.
- **Why C over B and A.** The only remaining GPU caveat — R6C's *whole-run* execution — is a **runtime-substrate gap, not a measurement gap**: measuring it honestly requires holding world state GPU-resident across ticks and dispatching the (already-measured) per-tick kernels from a scheduler. That is exactly the parked M-4A sparse-residency scheduler / Atlas production runtime. So candidate B collapses into the runtime track (its own stop-line said as much), and C **subsumes** B by making the GPU-resident R6C run R0's first consumer. Candidate A (richer emergence) is a recorded finding, not a bottleneck, and is cheaper/more meaningful once a production runtime exists — parked behind R0.
- **Track opened (Tier-2 design opening authored by Opus):** [`docs/production_paths/runtime_0080_0_opening_spec.md`](production_paths/runtime_0080_0_opening_spec.md). **First rung R0** (recipient Cursor/Codex5.5max after the opening lands): run the R6C 100-tick loop with state GPU-resident across ticks for the **single galactic tier**, dispatching the measured shapes via the accepted **ATLAS-0080-0** residency + GPU-EXEC/AccumulatorOp paths, asserting CPU-oracle parity (incl. R6C checksum `1bba891c779190a4`); convert R6C whole-run posture to `GPU-measured` only if actually measured. **Stop-lines:** stays single resident theater (no `request_atlas_batching` — gated until a §11 M-4 PR), no M-4A masking-at-scale, no new substrate primitive / semantic WGSL / op / invariant / pinned-number change / scenario reopen → any such need escalates to Opus as a distinct rung. Report: `docs/tests/runtime_0080_0_r0_results.md`.
- **GPU posture preserved:** residency remains the target; CPU oracle remains the determinism reference; `GPU-measured` claimed only if actually run. Docs only; no code/invariant change. Track §12.5 trailer, mapping, and this worklog updated.

# 2026-06-04 - SCENARIO-0080-2-COMPLETE-0: close 0080-2 (with caveats) + open GPU-MEASURE-0080-0 (design authority)

- **Ruling: A — close `SCENARIO-0080-2` and open the next named track.** R6C (PR #526) was reviewed against its opening spec at report and source level: one mutable world ticked 100x with write-back, per-substep fresh field reads + REENROLL, combat gated on R5 `moved_cells` (movement-produced co-location, not hand-placed), R4 sparse-field tie-breaker genuinely **dropped** (`tie_breaker_gradient_magnitude_bits=0`, field built from a non-degenerate composite), conservation rows, deterministic replay checksum `1bba891c779190a4`, no planner/WGSL/new-op/invariant. **No spec violation → no reopen.**
- **Closed with caveats recorded as findings (not defects):** Terran patrols never crossed the movement threshold (one-sided, pirate-driven contest); production/attrition race unresolved in 100 ticks (Terran 3→7, Pirate 10→12, Terran stockpile 356 converting to few ships); front/standoff + self-sustaining pirate loop only *partially emerged*; CPU-oracle parity is structural (oracle re-runs the same deterministic model). These feed a **future** `SCENARIO-0080-3 — richer multi-hotspot emergence run`, deliberately **not** opened now (anti-ceremony: do not extend the rehearsal because some behavior did not emerge).
- **Next track opened: `GPU-MEASURE-0080-0` — GPU execution measurement pass** (recipient Cursor/Codex5.5max). One rung: measure the rehearsal's row/mask/threshold/emission-band shapes (R1/R2/R6/R6B integer reductions bit-exact; R4 f32 magnitude verified-approximate) executing on the discrete GPU via the **already-accepted** generic AccumulatorOp / ATLAS STORE-GPU / Candidate-F paths, asserting parity vs the CPU oracle — converting `GPU-conformant; GPU execution not yet measured` into `GPU-measured` per shape. No new semantic WGSL, no new op, no new invariant, no pinned-number change, no reopen. Opening handoff: [`docs/handoffs/gpu_measure_0080_0_opening.md`](handoffs/gpu_measure_0080_0_opening.md); report path `docs/tests/gpu_measure_0080_0_results.md`. GPU posture preserved: residency is the target, GPU-shaped is positive conformance, CPU oracle is the determinism reference, GPU execution claimed only if measured.
- **Docs only; no code/invariant change.** Track §12.5 trailer, mapping guidance, and this worklog updated.
...
- **Claim boundary:** "loop" downgraded to "slice/chain"; single galactic tier, opt-in/default-off, CPU-oracle primary; pathfinding NOT solved (greedy local FIELD_POLICY steps); combat is a Resource-Flow proof, not production-ready.
...
- **Numeric-only reconciled:** FrontierV1 "FIELD_POLICY route" + mapping first-slice heatmap → consumption-proven via R1/R4/R5. GPU: only ATLAS STORE-GPU carries discrete-GPU evidence; rehearsal rungs are GPU-*shaped*, not kernels.
...
- **Verified first-hand (re-ran, did not trust the reports):** R2 `13/0`, R3 `13/0`, R4 `16/0` on current `master`; PR #517 confirmed merged to `master`. R2 disburse-down reaches real deficit systems (Terran 3 starports ×2 disbursed from a non-empty stockpile, Pirate system-10 ×2), not decorative ledger motion. R2 blockade/divert stays an owner-column flip (`structural_parent` unchanged, no occupant moved, no `BoundaryRequest`). R3 masks down by owner-column with co-located cell-284 Terran/Pirate entries receiving *distinct* effective modifiers and real signal mutation (−15→−18, 20→25, 10→11); combat bonus is data-only (0 HP/Damage). R4 genuinely consumes R1/R2/R3 via cross-checked checksums, runs the real `GradientXY` operator (CPU oracle) → exact Q16 `mag2` → Candidate-F `sqrt_cr_f_bits` for the authoritative gate, with raw f32 magnitude diagnostic-only; both sit-still and step-opportunity evidence present; no relocation/REENROLL/movement.
- **One honest note (not a blocker):** R4's composite field adds a small deterministic spatial bias (`cell_index*0.01 + x*0.001`) to keep `GradientXY` non-degenerate in the sparse canonical field. It is ~0.01–0.2 per neighbour-step against composite values in the ~100s; canonical movers are selected by **max gradient magnitude per faction**, so they ride real disruption/economy/disposition structure and the bias is a negligible tie-breaker. **R7's emergence narrative must not attribute movement to this bias** — when the field is richened (R5 movement begins to vary it), the bias should be dropped or proven dominated. Logged for R5/R7, not gating R5.

# 2026-06-04 - SCENARIO-0080-2-R4-IMPL-0: FIELD_POLICY field-consumption + exact sqrt implemented/pass

- **Implemented R4** as an opt-in/default-off fixture helper: `crates/simthing-driver/src/dress_rehearsal_r4_field_policy_consumption.rs` + `crates/simthing-driver/tests/dress_rehearsal_r4_field_policy_consumption.rs`. It consumes accepted R1 (`17de0080304b3da7`), implemented/pass R2 (`4fe0590589ddd975`), and implemented/pass R3 (`28afb4a204d101d2`), then emits `docs/tests/scenario_0080_2_r4_field_policy_consumption_report.md`.
- **FIELD_POLICY field-consumption landed:** Pirate/Patrol movers read parent-grid composite fields at their own cells (disruption, location-status, economy, R3 masked disposition), compute `GradientXY` via the `simthing-gpu` CPU oracle, derive exact Q16 pre-sqrt `mag2`, apply Candidate-F `sqrt_cr_f_bits` for commitment magnitude, and threshold-gate `SitStill` vs `StepOpportunity` without relocation or `BoundaryRequest`.
- **Verified:** R4 test battery `16/0`; R3 regression `13/0`; R2 regression `13/0`; R1 regression `34/0`; ATLAS STORE CPU `11/0`; ATLAS STORE-GPU `10/0`; `phase_m_jit_sqrt_mag2_0_fixed_exact` `7/0`; `phase_m_jit_sqrt_mag0_f_exact_magnitude` `12/0`; `simthing-gpu` `gradient_xy_cpu_oracle` `2/0`; `cargo check --workspace` PASS with existing warning noise only. CPU oracle parity true; artifact checksum `f0acbe2ccb98badb`. No R5/R6/R7, movement, REENROLL, reparenting, combat, new semantic WGSL, default session wiring, CPU planner, hard currency/markets/trade/`ai_budget`, ClauseThing, UI, or invariant edit.
...
- **Verified:** R3 test battery `13/0`; R2 regression `13/0`; R1 regression `34/0`; ATLAS STORE CPU `11/0`; ATLAS STORE-GPU `10/0`; `cargo check --workspace` PASS with existing warning noise only. CPU oracle parity true; artifact checksum `28afb4a204d101d2`. No R4/R5/R6/R7, movement, reparenting, `BoundaryRequest`, FIELD_POLICY action, GradientXY consumption, combat resolution, new op, shader/WGSL/GPU requirement, default session wiring, CPU planner, hard currency/markets/trade/`ai_budget`, ClauseThing, UI/realtime, or invariant edit.
...
- **Stop conditions enforced:** R1 forbids FIELD_POLICY movement (EC2/R4), GradientXY consumption, exact sqrt Candidate F, REENROLL, the disruption≥100 blockade/divert gate (R1/R2 coupling), R2–R6, OWNER runtime beyond cell-contribution routing, and any default-session-pass change (R1 is opt-in/default-off). If implementation needs any, the agent stops and returns to Opus.
...
- **Inventory of Intel-run GPU tests** (for Codex's NVIDIA re-run) recorded in `docs/tests/gpu_intel_run_inventory_2026_06_03.md`: essentially the **entire GPU-dependent test surface** (~70+ targets across simthing-gpu/driver/sim/feeder). Grouped by risk:
  ...
  - Remainder (FIELD_POLICY obs/event/act/pipe, eml gadgets, e11/e11b, mobility kernel fixtures, s-series, feeder) re-run via `cargo test --workspace`.
...
- **Honest closure confirmed:** production doc records EC-A2a PASS; **EC-A2b (batched GPU dispatch + bit-exact parity) deferred to `ATLAS-BATCH-0-PACK-GPU`, explicitly NOT proven**; STORE unimplemented; M-4A + REENROLL parked; no economy/disruption/FIELD_POLICY.
...
# 2026-06-03 - HARNESS-ANCHORS-1: rebalance §12.0 links to anchor FIELD_POLICY/Accumulator/emergence + R7 layman report (design-authority directive)

- **Rebalanced the §12.0 harness link packet** so each Codex handoff is anchored to the three things that must not drift. Review found it was OWNER-heavy (2 top-level OWNER links) and left **FIELD_POLICY principles** and the **GPU-resident Accumulator primitive** only transitive. Demoted both OWNER links to reachable-via-§12.4 and promoted two direct anchors. New 6: (1) constitution §0; (2) `invariants.md` (Scenario Proof + AccumulatorOp v2 + Resource Flow Substrate + FIELD_POLICY closure-posture); (3) this file §12–§12.5 (§12.4 → OWNER); (4) scenario spec (+ anticipated emergence §8.1); (5) **`crates/simthing-core/src/accumulator_op.rs`** — the GPU-resident Accumulator primitive (SourceSpec/CombineFn/GateSpec/ScaleSpec/ConsumeMode); (6) **`workshop/field_policy_track.md`** — the FIELD_POLICY charter. Added an explicit anchor map: FIELD_POLICY → 1/2/6; Accumulator resource flow → 1/2/5; emergence → 4 (§8.1). Stays within §0.5 (6 links).
...
- **FrontierV1-ACCEPT-0** (`docs/tests/phase_m_frontier_v1_acceptance_review_results.md`): non-destructive dated banner — the "FIELD_POLICY route" (`validate_field_policy_v1_consumed`) only asserts two descriptors are **registered**; FIELD_POLICY never consumed the field to act; consumption-proof pending **R4/R7**. Active status rows (Frontier V1 + FIELD_POLICY-V1) in `mapping_current_guidance.md` flagged accordingly; Candidate F named as the R4 consumer.
...
- **Base SimThing principle checklist (6 lines, every track/rung):** (1) everything is a SimThing (no subsystem outside the tree, no runtime `match kind`); (2) all conflict/opportunity/ambition/extraction is resource flow (accumulate→reduce→mask→threshold); (3) recursive allocation, emergent settling depth; (4) GPU threshold crossings — FIELD_POLICY, not a CPU planner; (5) semantic-free `simthing-sim` + CPU-oracle bit-exact parity; (6) proven only through a real reduction, opt-in/default-off. If a change can't fit 1–6 → escalate to design authority, don't special-case. "Six lines on purpose: a low-context agent holds six and drifts past sixty." Cites §12.0 as the worked example. Docs-only.
...
# 2026-06-03 - DRESS-REHEARSAL-PROVISIONAL-FINDINGS-0: log heatmap/FIELD_POLICY audit gaps + exit criteria (design-authority directive)

- Recorded **provisional findings** from the 2026-06-03 design-authority audit of prior mapping/FIELD_POLICY/0080 work as **§12.1** of the production track (`design_0_0_8_0_consumer_pulled_production_track.md`) and a tracking **§5.1** in the constitution (`design_0_0_8_0.md`). Marked PROVISIONAL — to be firmed up before the dress rehearsal opens as a gate. Work continues tomorrow.
- **Gap findings:** F1 — 0080 modeled no spatial structure (1-D scalar line; "heatmap" appears nowhere in code). F2 — the mapping track built real 2-D field machinery (10×10, diffusion, gradient, 100-cell→parent reduction, GPU/CPU bit-exact) but it was hand-seeded, never demoed/exported as a heatmap, and never run through SimThing cells (flat slot-range, not children of a starmap SimThing); the gate measured numeric-pipeline correctness, not a heatmap deliverable. F3 — engine and consumer never met (mapping closed "proven, unconsumed"; 0080 bypassed it with the 1-D toy). F4 — FIELD_POLICY never consumed a heatmap for pathing/critical-path (FIELD_POLICY-OBS scores an entity's own overlays; FrontierV1-4 `validate_field_policy_v1_consumed()` only asserts two kernel descriptors are registered; 0080 pathing was a scalar target-score over 2 nodes). F5 — the loop field → diffuse → gradient → FIELD_POLICY-reads-local-cell → action was never wired; prior passes satisfied the two ends, never the connection.
- **Provisional design resolutions** logged: falloff = stencil diffusion (not arena enrollment); two-column `disruption` source / `location_status` sink (strict-sink); neighbors not enrolled; sparse arenas + dense diffusion; diffusion horizon = FIELD_POLICY sight radius; recursion = multi-resolution escape from local optima; grid-of-simthings needs contiguous row-major slot allocation; ownership = decaying D=2 overlay (not D=3 node).
- **Proposed hard exit criteria:** EC1 — starmap SimThing holds a non-trivial reduced disruption heatmap over its 100 child gridcell SimThings, produced by pirate/patrol presence (not hand-seeded), vs CPU oracle, emitted as an inspectable artifact. EC2 — a mover's FIELD_POLICY action is a function of the diffused heatmap gradient read at its own cell, vs CPU oracle (not a hand-seeded or registration-only stand-in); the field→gradient→FIELD_POLICY→action loop closed through real SimThings. Both would have failed every prior "pass." Docs-only.
...
- Added **§0.0 "Purpose — the unitary vision (why §0 is transient)"** as the first clause of the transient constitution in `design_0_0_8_0.md`. Records the WHY behind maximal SimThing conformance: it is the mechanism by which **conflict, opportunity, ambition, and extraction collapse into a single generic, GPU-resident SimThing** (conflict = combat HP/Damage + disruption; opportunity = desirability fields/gradients; ambition = faction drives/fight-or-flight as threshold-gated value decisions; extraction = extraction/raiding/economy) — all one *accumulate → reduce → mask → threshold* loop, no combat/economy/AI engine.
- The payoff stated: **resolution lives as GPU automata in a FIELD_POLICY model** — decisions emerge as GPU-resident threshold crossings over the resolved masked field, not a CPU planner. The rejected **D=3 ownership-node** is recorded as the canonical conformance violation (ownership smuggled back as a bespoke tree shape instead of a decaying owner overlay); any structural special-case leaves the generic substrate and breaks the unitary vision. Conformance is the **precondition** for the whole sim being one GPU-resident FIELD_POLICY automaton, which is why it is transient/carry-forward and non-negotiable.
- The D=2-vs-D=3 worked example (ownership-as-decaying-overlay; combat/movement/engage-withdraw in one FIELD_POLICY pass) is flagged as a central concept to be **tested in the dress-rehearsal design** (production track §12); §0.0 captures only the constitutional WHY. Docs-only.
...
- **Kept (B1, B2):** the Tier-1/Tier-2 lane machinery + proven-capability stop rule (the anti-loop antidote) and the JIT/FIELD_POLICY closure-posture rows stay. **Tier C untouched** — semantic-free `simthing-sim`, CPU-oracle bit-exact parity, AI-is-a-SimThing/no-CPU-planner (FIELD_POLICY), bounded-feedback for recurrent EML gadgets, and the Property/Registry/Evaluation/State-Authority/AccumulatorOp structural floor are the SimThing soul and were explicitly preserved.
...
- **Note:** this lands the dual-output kernel in the f32 dense-grid stencil substrate (where gradients live). Rung 4 consumes it over the rung-2 compound desirability field; bridging the integer sparse-node 0080-2 field to the gradient is part of rung 4 (gradient-follow FIELD_POLICY movement + 20-tick schedule — Opus authors the movement-decision contract). **Pre-existing unrelated breakage observed (not touched by this PR): `simthing-sim` C8 transfer/emission accumulator tests fail to compile — `TransferRegistration` missing `order_band` field (42 errors on clean master).**
...
- The slice is structurally realized (sparse-residency nested `session → starmap → 10 starsystems → planet submap`), economically contended (faction-index ECON, Pirate as full economy faction, integer clearing under CPU parity oracle), runnable (FIELD_POLICY-sourced live movement), controllable through bounded admission (parameters only), observable (read-only export), and reproducibly exportable (headless demo, No CLI binary). Movement remains GPU-resident FIELD_POLICY-sourced (`Threshold`+`EmitEvent`→`BoundaryRequest`); no CPU planner entered the path. Identity + owner overlays preserved; membership updates without reparenting; capture-as-reparenting rejected; I8 parity holds.
...
- The demo applies `Control0081CommandBatch::canonical_run()` via `admit_control_0080_1` and runs the existing `CONTROL-0080-1 → DEFAULT-SCHEDULE-0080-1 → GAMEPLAY-0080-1` path. The report includes atlas residency, faction-index ECON, owner-overlay/up-aggregation, FIELD_POLICY movement trace, Terran/Pirate movement rows, command transcript, and a deterministic FNV-64 replay checksum.
- No direct movement command, external `BoundaryRequest`, FIELD_POLICY bypass, CPU planner, player command loop, UI framework, real-time loop, global default schedule, semantic/raw WGSL, new shader/GPU kernel, hard currency/markets/trade/`ai_budget`, nested Resource Flow, ClauseThing dependency, `simthing-spec` alteration, invariant edit, passive proof wrapper, or general command/demo framework was added.
...
- The admission layer writes only `DefaultSchedule0081Input.step_count`, `DefaultSchedule0081Input.movement_threshold`, and bounded Nested Starmap control config. It never directly moves Terran/Pirate ships, emits an external `BoundaryRequest`, bypasses FIELD_POLICY, adds a CPU planner/urgency/commitment, player command loop, UI, real-time loop, demo packaging, global default schedule, semantic/raw WGSL, shader/kernel, hard currency/markets/trade/`ai_budget`, nested Resource Flow, ClauseThing, `simthing-spec` alteration, invariant edit, passive proof wrapper, or general command system.
...
- **Key narrowing:** commands **admit parameters**, they do not control ships — movement still emerges from the implemented GPU-resident `Threshold + EmitEvent → BoundaryRequest` schedule. A command never moves a Terran/Pirate ship, never emits a `BoundaryRequest`, never bypasses FIELD_POLICY, and adds no CPU planner/urgency/commitment. The "control" name authorizes admission only; direct movement control, player command bus/loop, UI, real-time loop, demo packaging, and global default schedule remain a separate **CLOSED** concern (stop-and-escalate). Same proven posture as `CONTROL-0080-0`.
- Option B (remediation) rejected: schedule + observation implemented/pass with green regressions, the schedule input exposes exactly the bounded fields the vocabulary targets, docs consistent; no blockers. 20 command-admission tests named, **none implemented**. **Docs-only: no control implementation, no command input, no direct movement command, no external `BoundaryRequest`, no FIELD_POLICY bypass, no CPU planner, no player command loop, no UI, no real-time loop, no global default schedule, no semantic/raw WGSL, no new shader/GPU kernel, no hard currency/markets/trade/`ai_budget`, no nested Resource Flow, no ClauseThing implementation, no `simthing-spec` alteration, no invariant edit, no passive proof wrapper, no code change.** All `0080-1` gates remain IMPLEMENTED/PASS. Spec: [`gameplay/control_0080_1_opening_spec.md`](gameplay/control_0080_1_opening_spec.md); review: [`tests/phase_control_0080_1_opening_review_results.md`](tests/phase_control_0080_1_opening_review_results.md).
...
- Export/report includes starmap shape, active/resident theaters, fixed Terran/Pirate faction ECON, Pirate full-economy participation, contended ECON, owner-overlay inheritance, ownership up-aggregation, FIELD_POLICY movement trace, Terran and Pirate movement rows, no-mover rows, and replay checksum. No control/command input, demo packaging, player command loop, UI, real-time loop, global default schedule, direct movement command, external `BoundaryRequest`, CPU planner/urgency/commitment, semantic/raw WGSL, new shader/kernel, hard currency/markets/trade/`ai_budget`, nested Resource Flow, ClauseThing, invariant edit, passive proof wrapper, or general gameplay framework was added.
...
# 2026-06-02 - DEFAULT-SCHEDULE-0080-1-IMPL-0: Nested Starmap FIELD_POLICY-sourced schedule/movement PASS

- Implemented `DEFAULT-SCHEDULE-0080-1` as a scenario-scoped, opt-in/default-off Nested Starmap schedule/movement slice in `simthing-driver`: new `default_schedule_0080_1` module, exported runner/replay/report types, and focused regression tests. The schedule consumes `run_production_path_0080_1`, validates the production path is admitted/pass, uses read-only FIELD_POLICY composite-gap terms as deterministic threshold input, emits events, materializes `BoundaryRequest` records, and routes accepted moves through the existing mobility/transfer substrate posture.
...
- **Key narrowing:** commands **admit parameters**, they do not control movers — movement still emerges from the accepted GPU-resident `Threshold`+`EmitEvent`→`BoundaryRequest` path. A command never moves a patrol/pirate, never emits a `BoundaryRequest`, never bypasses FIELD_POLICY, and adds no CPU planner/urgency/commitment. The "control" name authorizes admission only; direct movement control, player command bus/loop, UI framework, real-time loop, and global default schedule remain a separate **CLOSED** concern (stop-and-escalate).
...
- **Design-authority enrichment (per the FIELD_POLICY/Ownership/Flow instinct):** the packet exercised Ownership (owner overlay continuity) and Flow (local economy reassociation) but not FIELD_POLICY — the patrol move was an undefined `move_request`. Accepted with the relocate/patrol decision **sourced from the accepted GPU-resident FIELD_POLICY posture** (`Threshold`+`EmitEvent`→`BoundaryRequest`), not a CPU planner and not an externally-scripted request. This makes the first consumer a genuine **FIELD_POLICY + Ownership + Flow** testbed. It pulls **no new substrate** (FIELD_POLICY V1 is an accepted decision mechanism; mobility/transfer remains the only substrate the production-path gate wires) and reinforces the no-CPU-planner stop condition.
...
- Scanned README, agents.md, design_v7_8.md §6, mapping_current_guidance.md, field_policy_track.md for live v7.9/design_v7_9 navigation refs. README and agents.md had none. field_policy_track.md had none. design_v7_8.md §6 production-track pointer still implied the track was open/in-progress — updated to "0.0.7.9 COMPLETE and PARKED / no open gate / 0.0.8.0 is next phase." No filename changes (path stays stable per closure handoff). No code, no invariant edits.
...
- **Collapsed the JIT/FIELD_POLICY hygiene-loop accretion:** ~14 near-identical FIELD_POLICY/observer/event/proposal descriptor rows (each restating "exact only under fixed-point; authorizes no CPU planner/scheduler/cache/default-wiring/bridge") → **3 consolidated rules** (exact-authority-requires-pinned-fixed-point; GPU-atomic-compaction = exact-count+unordered-membership-not-ordering; FIELD_POLICY/JIT closure posture). Kept the field agent-routing rule and the full sqrt exact-authority chain (Candidate F artifact, proof-gated) intact; trimmed its redundant tail.
...
- FrontierV2-0 landed the first default-off multi-tick closed-loop field agent consumer fixture, consuming FrontierV1-5 feedback candidates across two ticks without implementing ClauseThing or declaring phase closure. Replay fingerprint `0238c18ce3b559da`. Report: [`phase_m_frontier_v2_0_closed_loop_consumer_results.md`](tests/phase_m_frontier_v2_0_closed_loop_consumer_results.md).

# 2026-05-30 — FrontierV1-5 design-authority ruling: ACCEPT; next gate FrontierV2-0 (Opus)

- Reviewed FrontierV1-5 against §10 — **code-verified** (not just the report): `run_pipe0_gpu`/`run_act2_chain_gpu` build real compute pipelines + `dispatch_workgroups` (score→threshold→compact, bucket→reduce→propose→consume→admit) with readback, and resource dispatch routes through the `FlatStarResourceFlow` allocator. The live score→threshold→proposal→dispatch loop is **genuinely GPU-resident**, satisfying §10 for the bounded single-tick route. **ACCEPT (Opus design authority).** Field agent resource-dispatch loop = `GpuVerified` (bounded single-tick); structural/movement honestly `ReplayAccepted`; feedback candidate `FixtureOnly`; no ladder reopening; hard guardrails intact; no implementer closure declared. **Next gate = `FrontierV2-0`** (multi-tick closed-loop consumer that proves structural/movement within it; ClauseThing is its eventual authoring front-end; not gated on standalone structural/movement proofs per the relax-toward-the-ClauseThing-horizon doctrine). Superseded the stale implementer "Pause (F) / FrontierV1-5 not authorized" disposition. Ruling: `field_policy_track.md` §10.

# 2026-05-30 — FrontierV1-5 live GPU field agent resource route toward FrontierV2 (Tier-2, landed)

- FrontierV1-5 executed the required live GPU-resident integrated field agent resource route inside default-off FrontierV1 and shaped the output as a fixture-only feedback candidate for the named FrontierV2 closed-loop consumer, without reopening the FIELD_POLICY ladder or declaring phase closure. Replay fingerprint `1653b84847be2dd2`. Report: [`phase_m_frontier_v1_5_live_field_agent_route_results.md`](tests/phase_m_frontier_v1_5_live_field_agent_route_results.md).
...
- Product feedback: the guardrails were deliberately relaxed because codex **stalls on "no consumer" and loops on hygiene handoffs without a forward scenario in its horizon** (ClauseThing parked); self-acceptance is how it escapes. A standalone `FrontierV1-5` proof gate would re-create that stall. **Amended ruling:** withdraw `FrontierV1-5` as a hygiene gate; keep the honest `ReplayAccepted` label on the field agent loop; fold its live GPU run into the **first consumer, `FrontierV2`** (multi-tick closed-loop: field-derived proposals drive movement/dispatch that feeds back into field/economy — the ClauseThing stepping-stone). Substrate closure stands ("move on"). `FrontierV2` is the forward horizon for codex; build toward it, not hygiene passes. `field_policy_track.md` §10 amendment.

# 2026-05-30 — FrontierV1 design-authority review: substrate accepted, field agent loop pending FrontierV1-5 (Opus)

- Reviewed FrontierV1-0..4 + the implementer-authored `FrontierV1-ACCEPT-0`/`POSTACCEPT-0`. **Substrate accepted by design authority:** first-slice mapping + flat-star Resource Flow are GPU-verified with oracle parity, routing/hard guardrails clean, FIELD_POLICY ladder consolidated/closed → **Phase E closes at flat-star.** **Two oversight skips corrected:** (1) `FrontierV1-ACCEPT-0` is implementer self-acceptance — Tier-2 reserves M/E closure to design-authority + product, so it is not closure; (2) the FIELD_POLICY **field agent loop** (score→threshold→proposal→route) is `ReplayAccepted`, not GPU-run inside Frontier (only field+urgency and RF are GPU), and the implementer wrongly reclassified the live run as "optional/cosmetic" and declared "No FrontierV1-5." **Overridden:** `FrontierV1-5` (one live GPU-resident integrated field agent route run) is **required** before the loop is production-proven — non-blocking for moving on, binding before ClauseThing/real scenarios lean on it. Ruling: `field_policy_track.md` §10; status row corrected from "M/E closed" to "substrate accepted; field agent loop pending FrontierV1-5."

# 2026-05-30 — FrontierV1-POSTACCEPT-0 post-acceptance roadmap reset (Tier-2, landed)

- FrontierV1-POSTACCEPT-0 reset the roadmap after FrontierV1 M/E closure, preserving accepted evidence, stopping further Frontier/FIELD_POLICY ladder expansion, and requiring the next implementation to begin from a separately named production gate.
...
# 2026-05-30 — FrontierV1-4 FIELD_POLICY V1 route replay acceptance (Tier-2, landed)

- FrontierV1-4 integrated FIELD_POLICY V1 route replay into the default-off FrontierV1 fixture, preserving accepted routing substrates and all guardrails.
...
- FrontierV1-1 wired the default-off FrontierV1 fixture through first-slice mapping, flat-star Resource Flow, and FIELD_POLICY Field agent routing in driver/spec space; no default runtime wiring added.

# 2026-05-30 — FrontierV1-0 opt-in scenario skeleton and admission contract (Tier-2, landed)

- FrontierV1-0 defined the default-off FrontierV1 scenario skeleton and admission envelope for M/E closure: first-slice mapping, flat-star Resource Flow, and FIELD_POLICY Field agent routing through accepted substrates; no runtime wiring added.

# 2026-05-30 — FIELD_POLICY-V1-CONSOLIDATE-0 FIELD_POLICY Field agent Proposal Pipeline V1 (Tier-2, docs-only)

- FIELD_POLICY-V1-CONSOLIDATE-0 ingested `field_policy_track.md`, consolidated OBS/EVENT/PIPE/ACT evidence into FIELD_POLICY Field agent Proposal Pipeline V1, retained ACT-3/ACT-4 as supporting Economic V1 fixture evidence, stopped ACT-N expansion, and set FrontierV1 opt-in integration as the next M/E closure step.

# 2026-05-29 — FIELD_POLICY-ACT-4 Economic V1-style fixture validation corpus (Tier-2, landed)

- FIELD_POLICY-ACT-4 added default-off authorable validation corpus `field_policy_act4_economic_fixture_validation_corpus_v1` (18 rows) over ACT-3 numeric fixture records; CPU oracle exact; stable fingerprint; no new WGSL, descriptor, or runtime wiring.

# 2026-05-29 — FIELD_POLICY-ACT-3 Economic V1-style fixture substrate records (Tier-2, landed)

- FIELD_POLICY-ACT-3 added default-off `m_jit_field_policy_act3_economic_fixture_records` fixture; ACT-2 admission_record → Economic V1-style fixture_record under fixed integer lookup/overflow contracts; record_code/priority/tier/flags exact; 6-pass ACT-2/full-chain smokes and 34k/warm benchmarks recorded; no CPU planner or production wiring added.

# 2026-05-30 — FIELD_POLICY field agent track charter + Frontier V1 closing scenario + M-JIT acceptance (design authority, Opus)

- Chartered the FIELD_POLICY field agent pipeline (OBS→EVENT→PIPE→ACT) as a track with a **V1 closure boundary** (stop the ACT-N ladder; consolidate into "FIELD_POLICY Field agent Proposal Pipeline V1") and a binding **proposal→Resource-Flow routing guardrail** (proposals dispatch via the real flat-star allocator + Threshold/EmitEvent, never a parallel fixture economy; no CPU planner). Named **Frontier V1** — the single-theater strategic vertical that **closes Phase M and Phase E** on already-accepted substrates (first-slice mapping single grid + FlatStarResourceFlow depth-2 + FIELD_POLICY field agent + exact F sqrt). **Accepted M-JIT-PROD-0 closure** (PASS WITH CONDITIONS). **Product-authorized bounded relaxation** (this session): one opt-in, default-off economy↔field + proposal→action integration for Frontier V1 only — all hard guardrails (no semantic WGSL / no CPU planner / `simthing-sim` semantic-free / defaults Disabled / atlas+nested+perception deferred) intact. E closes at flat-star; nested E-11B / D-2a stay deferred until a bigger named scenario. Frontier V1 is the first ClauseThing target (implementation still parked). Charter + closure plan: `docs/workshop/field_policy_track.md`.

# 2026-05-29 — FIELD_POLICY-ACT-2 fixture-local proposal admission records (Tier-2, landed)

- FIELD_POLICY-ACT-2 added default-off `m_jit_field_policy_act2_proposal_admission_records` fixture; ACT-1 proposal_summary → fixture-local admission_record under fixed integer threshold/overflow contracts; admission_code/flags/rejection reasons exact; 5-pass ACT-1/full-chain smokes and 34k/warm benchmarks recorded; no CPU planner or production wiring added.

# 2026-05-29 — FIELD_POLICY-ACT-1 Phase E-style numeric proposal consumer (Tier-2, landed)

- FIELD_POLICY-ACT-1 added default-off `m_jit_field_policy_act1_phase_e_proposal_consumer` fixture; ACT-0 proposal records → numeric proposal_summary under fixed admitted-code table and overflow contracts; accepted/ignored/invalid/summary/max exact; order-invariant summary; 34k/warm benchmarks and ACT-0/full-chain smokes recorded; no CPU planner or production wiring added.

# 2026-05-29 — FIELD_POLICY-ACT-0 GPU-resident numeric action proposals (Tier-2, landed)

- FIELD_POLICY-ACT-0 added default-off `m_jit_field_policy_act0_numeric_proposals` probe; EVENT-2 reductions → bounded numeric proposal records under fixed integer rule/capacity contracts; proposal count/membership/overflow exact when capacity sufficient; order UnspecifiedAtomicOrder; 34k/warm benchmarks and EVENT-2/PIPE smokes recorded; no CPU planner or production wiring added.

# 2026-05-29 — FIELD_POLICY-EVENT-2 GPU-resident per-bucket reductions (Tier-2, landed)

- FIELD_POLICY-EVENT-2 added default-off `m_jit_field_policy_event2_bucket_reductions` probe; per-code count/sum/min/max with i64 sum overflow flag and empty-bucket contract; order-invariant reductions over unordered buckets; 34k/warm benchmarks and integrated smokes recorded; no CPU planner or production wiring added.

# 2026-05-29 — FIELD_POLICY-EVENT-1 GPU-resident event-code bucketing (Tier-2, landed)

- FIELD_POLICY-EVENT-1 added default-off `m_jit_field_policy_event1_code_bucketing` probe; compact unordered event records bucket by numeric event code with exact per-code counts/membership under capacity; invalid-code accounting and overflow exact; ordering UnspecifiedAtomicOrder; 34k distribution/warm benchmarks and integrated PIPE-0→bucket smoke recorded; no CPU planner or production wiring added.

# 2026-05-29 — FIELD_POLICY-PIPE-0 integrated GPU observer-event pipeline (Tier-2, landed)

- FIELD_POLICY-PIPE-0 added default-off `m_jit_field_policy_pipe0_observer_event_pipeline` two-pass GPU fixture; OBS-4 threshold rows feed EVENT-0 compaction without CPU filtering; count/membership exact under capacity; ordering UnspecifiedAtomicOrder; 34k integrated/warm benchmarks recorded; no CPU planner or production wiring added.

# 2026-05-30 — FIELD_POLICY-EVENT-0 GPU-resident event compaction from threshold event rows (Tier-2, landed)

- FIELD_POLICY-EVENT-0 added default-off `m_jit_field_policy_event0_compaction` probe; atomic GPU compaction of exact OBS-4 event codes with exact count/membership under capacity contract; ordering UnspecifiedAtomicOrder; 34k density benchmarks recorded; no CPU planner or production wiring added.

# 2026-05-30 — FIELD_POLICY-OBS-4 GPU-resident threshold event emission from exact observer scores (Tier-2, landed)

- FIELD_POLICY-OBS-4 added default-off `m_jit_field_policy_obs4_threshold_event` probe; exact Q16.16 score + threshold/hysteresis → deterministic state/event codes; warm 34k benchmark recorded; no CPU planner or production wiring added.

# 2026-05-30 — FIELD_POLICY-OBS-3 fixed-point aggregate score for multi-layer observer overlay (Tier-2, landed)

- FIELD_POLICY-OBS-3 added default-off `m_jit_field_policy_obs3_multilayer_fixed_score` descriptor with `ExactQ16WeightedSum` Q16.16 score accumulation; per-layer mag exact; OBS-2 f32 score unchanged; 34k benchmarks recorded; no production wiring added.

# 2026-05-30 — FIELD_POLICY-OBS-2 multi-layer GPU-resident observer overlay score (Tier-2, landed)

- FIELD_POLICY-OBS-2 added default-off 4-layer multilayer overlay score fixture and `m_jit_field_policy_obs2_multilayer_overlay_score` descriptor; per-layer mag exact, score ApproximateDiagnosticF32; 34k benchmarks recorded; no production wiring added.

# 2026-05-30 — FIELD_POLICY-OBS-1 descriptor/admission for mobile observer overlay score (Tier-2, landed)

- FIELD_POLICY-OBS-1 added default-off `m_jit_field_policy_obs0_overlay_score` descriptor/admission with exact mag2/mag and ApproximateDiagnosticF32 score; warm 34k benchmark recorded; no production wiring added.

# 2026-05-30 — FIELD_POLICY-OBS-0 GPU-resident mobile observer overlay score probe (Tier-2, landed)

- FIELD_POLICY-OBS-0 added default-off/test-only GPU overlay score fixture (Q16.16 mag2 + F exact mag, f32 diagnostic score); 34k benchmark and 50k-row dense corpus recorded; no production wiring added.
...
# 2026-05-30 — SQRT-MAG-0 F-backed exact Euclidean magnitude FIELD_POLICY hot-path probe (Tier-2, landed)
...
- **ACCEPTED — PASS WITH CONDITIONS.** The chain (abstract boundary doctrine → Daily Economy Fixture V1 → Resource Economy Authoring Ergonomics V1 → Economy + FIELD_POLICY Product Fixture V1) is accepted as a fixture-level product proof. Memo: [`reviews/phase_m_product_fixture_chain_acceptance_opus_review.md`](reviews/phase_m_product_fixture_chain_acceptance_opus_review.md).
- Verified in code: the economy→FIELD_POLICY link lives only in `tests/support/economy_field_policy_product_fixture.rs` (not exported). The CPU's sole step is `eml_weights_from_treasury_stress` selecting between two pre-authored weight profiles by resolved treasury; urgency and the FIELD_POLICY commitment stay GPU-resident (field→reduction→`field_urgency` EvalEML→Threshold+EmitEvent; `reduction_stencil_readbacks == 0`). Designer/importer-layer guardrail real: `resource_economy_admission` rejects malformed specs at the spec stage.
- Re-ran on GPU: economy+FIELD_POLICY 6/6, authoring ergonomics 4/4, spec authoring preview 8/8.
- Verdicts: chain ACCEPT WITH CONDITIONS · fixture-orchestration boundary ACCEPT · FIELD_POLICY discipline PASS · boundary/economy doctrine PASS · non-authorizations kept binding.
- Conditions: economy→FIELD_POLICY link stays fixture-only (no production bridge without a separate gated decision); CPU may select authored profiles but never compute urgency / emit commitments; guardrails stay at the designer/importer/scenario-admission layer, not sim/boundary.
- **Next implementation step: authoring ergonomics R2, or another tiny non-map-substrate + FIELD_POLICY product fixture — not a generic boundary-output packet (D), not the M-4 atlas packer (E).**
...
- Chain parked for review: abstract tick/boundary doctrine → Daily Economy Fixture V1 → Resource Economy Authoring Ergonomics V1 → Economy + FIELD_POLICY Product Fixture V1.
...
# 2026-05-29 — Phase M Economy + FIELD_POLICY Product Fixture V1

- Phase M Economy + FIELD_POLICY Product Fixture V1 landed.
...
- Surplus (treasury 107): low stress → `(0.2, 0.1)` weights → urgency below threshold → 0 FIELD_POLICY events.
- Deficit (treasury 94): high stress → `(0.9, 0.1)` weights → urgency crosses threshold → 1 FIELD_POLICY event (`0x53454144`) via Threshold+EmitEvent.
- CPU reads economy storage and selects weight profiles only; CPU does not compute urgency or emit FIELD_POLICY commitments.
- This remains a product/acceptance fixture — not a production economy→mapping runtime bridge or general scenario engine.
- Tests: `phase_m_economy_field_policy_product_fixture` (6/6); regressions green.
- Test report: [`tests/phase_m_economy_field_policy_product_fixture_test_results.md`](tests/phase_m_economy_field_policy_product_fixture_test_results.md).
...
- **Next implementation handoff:** resource-economy authoring ergonomics, or an economy+FIELD_POLICY product fixture; a generic boundary-output packet only if tightly bounded (never `DailyResolutionBoundary` by another name); **not** the M-4 atlas packer.
...
- **FIELD_POLICY discipline: PASS.** **Boundary discipline: ACCEPT WITH WATCHLIST** (added W-1: the `*_fixture` commitment methods on the production session must not accrete general runtime responsibilities; promote to a bounded API or keep behind a clearer boundary before any non-test caller).
...
- Phase M CommitmentSpec fixture landed. It moves the first-slice commitment threshold/event binding into a designer/spec-facing RON-admitted configuration while preserving the existing GPU-resident FIELD_POLICY path: field propagation -> parent reduction -> field_urgency EvalEML -> Threshold + EmitEvent.
...
- Phase M product commitment fixture landed. It extends the product-facing first-slice fixture by using the existing threshold/event substrate over parent field_urgency, proving the FIELD_POLICY commitment path: GPU-resident field propagation -> parent reduction -> EvalEML urgency -> threshold event.
...
- Evaluated PR #226 (`bf8c189`): **consistent** with V7.7, Mapping ADR, FIELD_POLICY principles. No code remedial.
...
- Docs cleanup: superseded mapping/FIELD_POLICY workshop preserves, candidate notes, revert reports, and full logs moved to `docs/workshop/archive/` and `docs/tests/archive/`. Active pointer: [`mapping_current_guidance.md`](workshop/mapping_current_guidance.md).
...
## 2026-05-19 — FIELD_POLICY tensor/stencil WGSL refinement probe (reverted to parked state)

- Ran fifth FIELD_POLICY feasibility sandbox (PR #208): refinement probe for long-horizon stability, ping-pong correctness, directed-compatible setup, source injection policies, column-aware parent EML, active mask, and cost scaling. 12/12 PASS (`--test-threads=1`). Overall verdict **PARTIAL**.
...
- Preserved at [`field_policy_tensor_stencil_refinement_sandbox_code_preserve.rs`](workshop/archive/field_policy/tensor_stencil_refinement_sandbox_code_preserve.rs), [`field_policy_tensor_stencil_refinement_prototype.wgsl`](workshop/archive/field_policy/tensor_stencil_refinement_prototype.wgsl), [`field_policy_tensor_stencil_refinement_sandbox_test_results.md`](tests/field_policy_tensor_stencil_refinement_sandbox_test_results.md).

**Verification:** [`revert_field_policy_tensor_stencil_refinement_sandbox_to_parked_state_test_results.md`](tests/archive/reverts/field_policy_tensor_stencil_refinement_sandbox_to_parked_state_test_results.md)

---

## 2026-05-19 — FIELD_POLICY tensor/stencil WGSL prototype probe (reverted to parked state)

- Ran fourth FIELD_POLICY feasibility sandbox (PR #206): prototype WGSL structured 2D stencil kernel vs per-edge AccumulatorOp. 10/10 PASS (`--test-threads=1`). Overall verdict **PARTIAL**.
...
- Preserved at [`field_policy_tensor_stencil_wgsl_sandbox_code_preserve.rs`](workshop/archive/field_policy/tensor_stencil_wgsl_sandbox_code_preserve.rs), [`field_policy_tensor_stencil_prototype.wgsl`](workshop/archive/field_policy/tensor_stencil_prototype.wgsl), [`field_policy_tensor_stencil_wgsl_sandbox_test_results.md`](tests/field_policy_tensor_stencil_wgsl_sandbox_test_results.md).

**Verification:** [`revert_field_policy_tensor_stencil_wgsl_sandbox_to_parked_state_test_results.md`](tests/archive/reverts/field_policy_tensor_stencil_wgsl_sandbox_to_parked_state_test_results.md)

---

## 2026-05-19 — FIELD_POLICY operator toolkit probe (reverted to parked state)

- Ran third FIELD_POLICY feasibility sandbox (PR #204): stabilized propagation operators, dirty/frontier skip, cadence, whitelist admission, hierarchy-first awareness, hybrid model, PF/dirty comparison, cost projection. 11/11 PASS (`--test-threads=1`). Overall verdict **PARTIAL**.
...
- Preserved at [`field_policy_operator_toolkit_sandbox_code_preserve.rs`](workshop/archive/field_policy/operator_toolkit_sandbox_code_preserve.rs) and [`field_policy_operator_toolkit_sandbox_test_results.md`](tests/field_policy_operator_toolkit_sandbox_test_results.md).

**Verification:** [`revert_field_policy_operator_toolkit_sandbox_to_parked_state_test_results.md`](tests/archive/reverts/field_policy_operator_toolkit_sandbox_to_parked_state_test_results.md)

---

## 2026-05-19 — FIELD_POLICY strategic horizon / velocity / PF-skip probe (reverted to parked state)

- Ran second FIELD_POLICY feasibility sandbox (PR #202): strategic horizon sweep, multi-cadence, explicit-column velocity, PF convergence/skip simulation. 11/11 PASS (`--test-threads=1`). Overall verdict **PARTIAL**.
...
- Preserved at [`field_policy_strategic_horizon_sandbox_code_preserve.rs`](workshop/archive/field_policy/strategic_horizon_sandbox_code_preserve.rs) and [`field_policy_strategic_horizon_sandbox_test_results.md`](tests/field_policy_strategic_horizon_sandbox_test_results.md).

**Verification:** [`revert_field_policy_strategic_horizon_sandbox_to_parked_state_test_results.md`](tests/archive/reverts/field_policy_strategic_horizon_sandbox_to_parked_state_test_results.md)

---

## 2026-05-19 — FIELD_POLICY field-intelligence feasibility probe (reverted to parked state)

- Ran staged FIELD_POLICY / sparse RegionCell field-intelligence feasibility probe (PR #200). 13/13 sandbox tests PASS; overall decision-gate verdict **PARTIAL**.
...
- Reverted production test file to parked state; preserved source at [`field_policy_sandbox_code_preserve.rs`](workshop/archive/field_policy/sandbox_code_preserve.rs) and results at [`field_policy_intelligence_sandbox_test_results.md`](tests/field_policy_intelligence_sandbox_test_results.md).
...
**Verification:** [`revert_field_policy_sandbox_to_parked_state_test_results.md`](tests/archive/reverts/field_policy_sandbox_to_parked_state_test_results.md)
...
- `simthing-driver::session`: `react_to_fission_clones(&BoundaryOutcome)`
  ...
  - Synthesize threshold registrations selection `cloned_root_id`.
...
- **6 acceptance tests** in `session_integration.rs`: matching-owner install,
  multi-owner clone with distinct `OverlayId`s, scenario-listed selection,
