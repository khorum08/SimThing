...
## STUDIO-FRAMERATE-REGRESSION-OPUS-AUDIT-0 — post-Scenario-ladder framerate regression audit

After the Scenario Runtime + Save/Load ladder, Studio framerate regressed from the previously reported 30–60 FPS range to approximately 4 FPS. Existing telemetry shows the measured render-loop systems are sub-millisecond while the egui/UI pass is measured near total frame time. This audit reviews the Scenario ladder, egui pass placement, CPU-side Scenario evaluation paths, telemetry overhead, and Bevy/egui schedule attribution to identify the next targeted remediation. The audit does not reopen DA-approved Scenario authority, save/load, RF/Accumulator, spatial hierarchy, or capability-tree evidence.

**Finding (executive DA):** Root cause is **egui layout/tessellation/paint** of the ladder-enlarged, always-expanded UI in a **debug** build (classification B/C+E), with a measurement artifact (D): per-panel timers wrap only the widget-queue closure, so egui's paint cost is misattributed to "Left panel: 226 ms" (while the Settings closure reads 0.14 ms). **Root cause A is ruled out** — the per-frame `refresh_runtime_saveload_status_if_needed(false)` passes `authority_digest = None` and returns `UseCache` (no serialization, no proof recomputation); every expensive Scenario/proof/IO call is event-gated (load/save/reopen-adopt/Refresh button). Next remediation: **STUDIO-EGUI-PAINT-ISOLATION-0** — add a true minimal-egui isolation mode, add an `egui_paint_ms` timer around the egui run, collapse diagnostic panels by default, and compare a release build. Full audit: `docs/tests/studio_framerate_regression_opus_audit_0_results.md`; durable guide: `docs/simthing-bevy-performance.md`.
