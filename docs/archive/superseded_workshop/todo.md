...
**Phase M Economy + FIELD_POLICY Product Fixture V1:** **Done (product/acceptance fixture, Option A orchestration)** — discrete economy boundary treasury stress maps to authored EML weight profiles; opt-in first-slice FIELD_POLICY commitment emerges via GPU field/reduction/EML/Threshold+EmitEvent only. Not a production economy→mapping bridge. See [`tests/phase_m_economy_field_policy_product_fixture_test_results.md`](tests/phase_m_economy_field_policy_product_fixture_test_results.md).

**Phase M product-fixture chain:** **ACCEPTED (Opus/product 2026-05-29, PASS WITH CONDITIONS)** — abstract boundary → daily economy → authoring preview → economy+FIELD_POLICY chain accepted as a fixture-level product proof. The economy→FIELD_POLICY link stays in `tests/support` orchestration (CPU selects authored EML weight profiles; never computes urgency or emits commitments; both stay GPU-resident). Binding row added to [`invariants.md`](invariants.md) Mapping section (economy→mapping influence is fixture-only; no production bridge without a separate gated decision). See [`reviews/phase_m_product_fixture_chain_acceptance_opus_review.md`](reviews/phase_m_product_fixture_chain_acceptance_opus_review.md).

**Phase M EML Gadget Library:** **EML-GADGET-1 ACCEPTED (Opus/product 2026-05-29, PASS WITH CONDITIONS)** — [`reviews/phase_m_eml_gadget_tier1_acceptance_opus_review.md`](reviews/phase_m_eml_gadget_tier1_acceptance_opus_review.md). Tier-1 stateless gadgets (`FieldSampler`, `WeightedAccumulator`, algebraic `SoftStep`) accepted; R1 composition (per-gadget executable; multi-gadget `PerGadgetOnly`; preview ≠ runtime) + R2 node-cap (per executable tree) accepted; binding rows in [`invariants.md`](invariants.md) ("EML Gadget Library"). Conditions: preview ≠ runtime; `PerGadgetOnly` only until intermediate wiring gated; oracle-per-gadget binding. *(The #262 parking packet was reverted off master; acceptance memo is authoritative.)* **EML-GADGET-2 temporal-memory design ACCEPTED as a gate (Opus/product 2026-05-29, PASS WITH CONDITIONS)** — [`reviews/phase_m_eml_gadget_tier2_design_acceptance_opus_review.md`](reviews/phase_m_eml_gadget_tier2_design_acceptance_opus_review.md). Explicit-column temporal memory; Layer-3 default; authored snapshot/copy bands (`Identity`+`ResetTarget`, no new opcode); bounded-feedback admission contract (default `0 ≤ decay < 1`; clamp required feeding a hard threshold; admission-checkable bound; stateful-sequence oracle parity). Candidates: `VelocityMonitor`/`Decay`-EMA/`BoundedFeedback` accepted; `Hysteresis` conditional (2D); `Acceleration` + dense per-cell deferred. **Approved ladder: 2A (snapshot/copy fixture proof) → 2B (VelocityMonitor + Decay/EMA) → 2C (BoundedFeedback) → 2D (Hysteresis).** Implementation unauthorized until 2A. Tests: [`tests/phase_m_eml_gadget_tier1_test_results.md`](tests/phase_m_eml_gadget_tier1_test_results.md), [`tests/phase_m_eml_gadget_tier1_r1_hygiene_test_results.md`](tests/phase_m_eml_gadget_tier1_r1_hygiene_test_results.md), [`tests/phase_m_eml_gadget_tier1_r2_node_cap_test_results.md`](tests/phase_m_eml_gadget_tier1_r2_node_cap_test_results.md), [`tests/phase_m_eml_gadget_tier2_design_review_test_results.md`](tests/phase_m_eml_gadget_tier2_design_review_test_results.md).

**Phase M Resource Economy Authoring Ergonomics R2:** **Unblocked — sequenced AFTER EML-GADGET-1 (landed).** R2 should expose/leverage the gadget library rather than re-invent composition. Begin after EML-GADGET-2 or in parallel with gadget-library integration only if product agrees.

**Phase M Resource Economy Authoring Ergonomics V1:** **Done (spec/admission preview)** — `ResourceEconomyAuthoringPreview` / `ResourceEconomyPreviewReport` summarize transfers, recipes, threshold emits, order bands, bindings, Resource Flow posture, and simple static transfer-only nets; admission diagnostics reuse existing compile rejections. No runtime economy changes. See [`tests/phase_m_resource_economy_authoring_ergonomics_test_results.md`](tests/phase_m_resource_economy_authoring_ergonomics_test_results.md).

**Phase M boundary-resolution (tick / boundary / day) + example economy:** **ACCEPTED (Opus/product 2026-05-29, PASS WITH CONDITIONS)** — boundary doctrine accepted; Daily Economy Fixture V1 accepted as example/product fixture only; `ResourceEconomySpec` (discrete banking) vs Resource Flow E-11 (continuous, default-off) distinction accepted; guardrails made **binding** in [`invariants.md`](invariants.md) ("Boundary resolution (tick / boundary / day)"). **Naming preference (product 2026-05-29):** `tick`/`boundary`/`day`/`day_index`/`ticks_per_day` are the preferred legible names — keep them; the R1/R2 abstract-naming pull is reversed. Guardrail is on Clausewitz/calendar *semantics* (calendar arithmetic / `Calendar`/`Pause` types / `DailyResolutionBoundary`), **not** the legible names. See [`reviews/phase_m_boundary_resolution_and_example_economy_acceptance_opus_review.md`](reviews/phase_m_boundary_resolution_and_example_economy_acceptance_opus_review.md).
