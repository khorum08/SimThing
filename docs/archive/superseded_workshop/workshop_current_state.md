...
**Next action:** **JIT track closed at default-off production registry shell; Opus review surface compacted.** Explicit registered exact ProductionCandidatePreview cohort execution remains test-invoked only; default wiring/scheduler/cache/economy bridge remain gated; ClauseThing proposal-only. E11 stalled/restart evidence is retained in [`e11_implementation_handoff.md`](e11_implementation_handoff.md), [`e11_readiness_review.md`](e11_readiness_review.md), and [`e11_hierarchical_allocation_design.md`](e11_hierarchical_allocation_design.md).
...
| **Next FIELD_POLICY proof** | Threshold event over first-slice urgency | **Done** — landed as an opt-in commitment fixture; no CPU-side planner |
| **Next authoring proof** | Designer-facing commitment threshold binding | **Done** — landed as RON-admitted spec structure; no default wiring |
...
**Phase M Boundary Resolution Doctrine R2:** **Done (docs terminology correction)** — restores legible tick/boundary/day_index/ticks_per_day vocabulary while preserving host/spec day interpretation guardrail. See [`../tests/phase_m_boundary_resolution_doctrine_r2_terminology_test_results.md`](../tests/phase_m_boundary_resolution_doctrine_r2_terminology_test_results.md).

**Phase M abstract boundary-resolution + example economy review packet:** **Done (parked for review)** — Opus/product parking packet distinguishing abstract tick/boundary doctrine from game-level daily interpretation; Daily Economy Fixture V1 framed as example only. See [`../reviews/phase_m_boundary_resolution_and_example_economy_review_packet.md`](../reviews/phase_m_boundary_resolution_and_example_economy_review_packet.md).
...
**Phase M first-slice vertical proof:** **ACCEPTED (Opus/product 2026-05-28, PASS WITH CONDITIONS)** — review packet summarizes complete landed chain from scenario RON through GPU-resident mapping and FIELD_POLICY commitment. See [`phase_m_first_slice_vertical_proof_review_packet.md`](../reviews/phase_m_first_slice_vertical_proof_review_packet.md).
...
**FIELD_POLICY tensor/stencil WGSL refinement sandbox:** **Reverted** — PR #208 probe merged then reverted to parked state. FIELD_POLICY tensor/stencil WGSL refinement sandbox completed and was reverted to parked state. The sandbox source, WGSL prototype(s), notes, and decision-gate results are preserved in `docs/workshop` and `docs/tests`. No sandbox test/prototype remains in the production runtime/test suite. Overall probe verdict: **PARTIAL**. Mapping/location architecture remains provisional. Implementation remains parked until the mapping doc is ready or product names a concrete non-mapping scenario. No mapping runtime, Scatter/Gather, wavefront propagation, dynamic nested enrollment, D-2a, E-11B-5, production WGSL, new AccumulatorRole variants, CPU fallback, slot compaction, or simthing-sim arena awareness landed. FlatStarResourceFlow remains the accepted bounded production Resource Flow posture. Global `PipelineFlags::default().use_accumulator_resource_flow` remains false. Presence of `ResourceFlowSpec` alone does not enable GPU execution.

**Mapping optimization remedial sandbox:** **Reverted** — PR #215. Workshop preserve under `docs/workshop/archive/mapping/`; superseded test logs removed in M-JIT doc closeout. Decision-gate summary: [`mapping_optimization_remedial_sandbox_test_results.md`](../tests/mapping_optimization_remedial_sandbox_test_results.md). Verdict **PARTIAL+** — incorporated into approved Mapping ADR.

**Mapping optimization toolkit sandbox:** **Reverted** — PR #213. Workshop preserve under `docs/workshop/archive/mapping/`; superseded test logs removed in M-JIT doc closeout. Decision-gate summary: [`mapping_optimization_toolkit_sandbox_test_results.md`](../tests/mapping_optimization_toolkit_sandbox_test_results.md). Verdict **PARTIAL** — incorporated into approved Mapping ADR.
...
**Next gates:** **Paused (F)** — gather product requirements per selection review. Re-open exactly one track (A–E) only after product names a scenario:

- **A** — scenario-driven vertical slice (FlatStarResourceFlow / Phase T primitives)
- **B** — narrow D-2a (named hard-currency ordering)
- **C** — narrow E-11B-5 (named nested dynamic Resource Flow)
- **D** — simthing-spec/RON/Designer rebuild (authoring track opens)
