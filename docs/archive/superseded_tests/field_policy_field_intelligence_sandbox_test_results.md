# FIELD_POLICY field-intelligence feasibility probe — test results
...
| Command | Result |
|---------|--------|
| `cargo test -p simthing-driver --test field_policy_intelligence_sandbox -- --nocapture` | **PASS** — 13/13 |
| `git rev-parse HEAD` | PASS — `50dfcc0` (pre-sandbox) |

**Full log:** [`archive/field_policy/field_policy_intelligence_sandbox_full.log`](archive/field_policy/field_policy_intelligence_sandbox_full.log)
...
```text
Production use requires: either map FIELD_POLICY formulas to whitelisted legacy class aliases (e.g. conversion_rate) or extend whitelist/policy for field-intelligence formula classes; direct C-8 register_formula works in sandbox but production authoring path must be verified separately.
```
...
- Source: [`docs/workshop/archive/field_policy/field_policy_sandbox_code_preserve.rs`](../workshop/archive/field_policy/field_policy_sandbox_code_preserve.rs)
- This report + full log (this directory)

Production test file `crates/simthing-driver/tests/field_policy_intelligence_sandbox.rs` is removed on revert to parked state.
