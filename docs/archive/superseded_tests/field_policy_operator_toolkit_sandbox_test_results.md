# FIELD_POLICY operator toolkit probe — test results

**Date/time:** 2026-05-19  
**Base HEAD (before sandbox branch):** `7e52c94` — FIELD_POLICY strategic horizon revert HEAD doc update  
...
| Command | Result |
|---------|--------|
| `cargo test -p simthing-driver --test field_policy_operator_toolkit_sandbox -- --nocapture --test-threads=1` | **PASS** — 11/11 |

**Full log:** [`archive/field_policy/field_policy_operator_toolkit_sandbox_full.log`](archive/field_policy/field_policy_operator_toolkit_sandbox_full.log)
...
recommended_hierarchy_strategy = SlotRange Sum cell→faction + EvalEML urgency at parent for empire-scale awareness; lateral FIELD_POLICY for local tactical gradient
...
- Source: [`docs/workshop/archive/field_policy/field_policy_operator_toolkit_sandbox_code_preserve.rs`](../workshop/archive/field_policy/field_policy_operator_toolkit_sandbox_code_preserve.rs)
