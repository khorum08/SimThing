# FIELD_POLICY-OBS-3 — Fixed-Point Aggregate Score for Multi-Layer Observer Overlay Results

## Base HEAD

`8d5d3e0` (post-FIELD_POLICY-OBS-2 merge)

## Files changed

| File | Change |
|---|---|
| `crates/simthing-driver/tests/phase_m_field_policy_obs3_fixed_point_score.rs` | **New** — 6 tests: edge rows, dense corpus, 34k perf, warm 32× dispatch, score authority, no wiring |
| `crates/simthing-spec/src/compile/jit_exact_sqrt_artifact_admission.rs` | `ExactQ16WeightedSum`, `m_jit_field_policy_obs3_multilayer_fixed_score` descriptor + admission |
| `crates/simthing-spec/tests/field_policy_obs0_overlay_score_admission.rs` | FIELD_POLICY-OBS-3 admission tests (+2) |
| `docs/workshop/mapping_current_guidance.md` | FIELD_POLICY-OBS-3 row |
| `docs/accumulator_op_v2_production_plan.md` | FIELD_POLICY-OBS-3 section |
| `docs/invariants.md` | Exact aggregate score invariant |
| `docs/worklog.md` | Append-only milestone |

## Pre-edit evaluation summary

1. **Per-layer magnitude authority (FIELD_POLICY-OBS-2):** Q16.16 gx/gy → exact integer mag2 → artifact-backed Candidate F → exact `layerN_mag_bits` (`max_ulp = 0`).
...
4. **f32 score leak prevention:** Distinct descriptor `m_jit_field_policy_obs3_multilayer_fixed_score` with `ExactQ16WeightedSum`; OBS-2 `m_jit_field_policy_obs2_multilayer_overlay_score` unchanged (`ApproximateDiagnosticF32`); admission rejects exact f32 score and score_bits under fixed contract.
...
**Landed:** `m_jit_field_policy_obs3_multilayer_fixed_score` — default_off, `ExactQ16WeightedSum`, per-layer mag exact, `score_fixed` exact.  
**Preserved:** `m_jit_field_policy_obs2_multilayer_overlay_score` — `ApproximateDiagnosticF32` unchanged.
...
## Comparison to FIELD_POLICY-OBS-2

| Path | per_dispatch_ms (warm) | score authority |
|---|---:|---|
| FIELD_POLICY-OBS-2 f32 | ~0.238 | ApproximateDiagnosticF32 |
| FIELD_POLICY-OBS-3 fixed | ~0.278 | ExactQ16WeightedSum |
...
cargo test -p simthing-driver --test phase_m_field_policy_obs2_multilayer_overlay_score -- --nocapture  # 6 passed
cargo test -p simthing-spec --test field_policy_obs0_overlay_score_admission -- --nocapture  # 10 passed
cargo test -p simthing-spec --test jit_exact_sqrt_artifact_admission -- --nocapture  # 10 passed
cargo check --workspace  # ok
rg "field_policy_obs3|ExactQ16WeightedSum|m_jit_field_policy_obs3" crates docs  # OBS-3 present
rg "score_bits.*ExactAuthoritative" crates docs  # rejects only; no authorized f32 exact score
rg "ApproximateDiagnosticF32|m_jit_field_policy_obs2" crates docs  # OBS-2 f32 remains diagnostic
```
