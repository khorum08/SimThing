# FrontierV2-0 — Multi-Tick Closed-Loop Field agent Consumer Fixture Results
...
| `docs/workshop/field_policy_track.md` | §10 FrontierV2-0 addendum |
...
| Question | Answer |
|---|---|
| **1. What did FrontierV1-5 prove?** | One live GPU-resident single-tick score→threshold→proposal→dispatch resource route; Resource Flow allocator dispatch; fixture-only `FrontierV1LiveFieldAgentFeedbackCandidate`; fingerprint `1653b84847be2dd2`. |
| **2. What does the Forward Horizon require for FrontierV2?** | Multi-tick closed-loop where field-derived FIELD_POLICY proposals drive movement/dispatch feeding back into field/economy next tick; consumer proof not hygiene; no implementer phase closure. |
...
FrontierV2-0 implements the named Forward Horizon consumer as a bounded two-tick fixture. It does not implement ClauseThing, declare phase closure, or reopen the FIELD_POLICY ladder.
...
| Field agent hash | stable | stable | no (same live pipe inputs from EML) |
...
Combined fingerprint XORs tick0/tick1 mapping hashes, field agent hashes, feedback hash, closed-loop delta hash, and overflow flags.
...
|---|---|
| GpuVerified | tick0/tick1 resource routes, resource flow, mapping+EML+FIELD_POLICY live pipe |
...
1. **How does FrontierV2-0 consume FrontierV1-5?** Reuses live GPU PIPE-0 + ACT-2 + Resource Flow route and applies `FrontierV1LiveFieldAgentFeedbackCandidate` as tick1 seed input.
