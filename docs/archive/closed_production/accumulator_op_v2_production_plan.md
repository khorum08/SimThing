...
> (M-1..M-3) all landed. **Phase M closed** at the accepted Frontier substrate + FIELD_POLICY Field agent
> Proposal Pipeline V1 (see `workshop/field_policy_track.md`); **Phase E closed** at
...
**FIELD_POLICY field-intelligence sandbox status:** **Reverted** — PR #200 probe merged then reverted to parked state (PR #201). FIELD_POLICY field-intelligence sandbox completed and was reverted to parked state. The sandbox preserved its source at [`docs/workshop/archive/field_policy/field_policy_sandbox_code_preserve.rs`](workshop/archive/field_policy/field_policy_sandbox_code_preserve.rs) and its decision-gate results at [`docs/tests/field_policy_intelligence_sandbox_test_results.md`](tests/field_policy_intelligence_sandbox_test_results.md). No sandbox test/prototype remains in the production test suite. Overall probe verdict: **PARTIAL** — P1 later-band propagation, P2 GPU EvalEML urgency, P3 ScaleTarget dissipation substrate-real; velocity DEFERRED; corridor gradient PARTIAL. Mapping/location architecture remains provisional. Implementation remains parked until the mapping doc is ready or product names a concrete non-mapping scenario. No mapping runtime, Scatter/Gather, wavefront propagation, dynamic nested enrollment, D-2a, E-11B-5, new WGSL, new AccumulatorRole variants, CPU fallback, slot compaction, or simthing-sim arena awareness landed. FlatStarResourceFlow remains the accepted bounded production Resource Flow posture. Global `PipelineFlags::default().use_accumulator_resource_flow` remains false. Presence of `ResourceFlowSpec` alone does not enable GPU execution.

**FIELD_POLICY strategic horizon / velocity / PF-skip sandbox status:** **Reverted** — PR #202 probe merged then reverted to parked state. FIELD_POLICY strategic horizon / velocity / PF-skip feasibility sandbox completed and was reverted to parked state. The sandbox source and decision-gate results are preserved at [`docs/workshop/archive/field_policy/field_policy_strategic_horizon_sandbox_code_preserve.rs`](workshop/archive/field_policy/field_policy_strategic_horizon_sandbox_code_preserve.rs) and [`docs/tests/field_policy_strategic_horizon_sandbox_test_results.md`](tests/field_policy_strategic_horizon_sandbox_test_results.md). No sandbox test/prototype remains in the production test suite. Overall probe verdict: **PARTIAL** — strategic horizon directional at H=8; explicit-column velocity PASS; PF skip PARTIAL. Mapping/location architecture remains provisional. Implementation remains parked until the mapping doc is ready or product names a concrete non-mapping scenario. FlatStarResourceFlow remains the accepted bounded production Resource Flow posture. Global `PipelineFlags::default().use_accumulator_resource_flow` remains false. Presence of `ResourceFlowSpec` alone does not enable GPU execution.

**FIELD_POLICY operator toolkit sandbox status:** **Reverted** — PR #204 probe merged then reverted to parked state. FIELD_POLICY operator toolkit sandbox completed and was reverted to parked state. The sandbox source and decision-gate results are preserved in [`docs/workshop/archive/field_policy/field_policy_operator_toolkit_sandbox_code_preserve.rs`](workshop/archive/field_policy/field_policy_operator_toolkit_sandbox_code_preserve.rs) and [`docs/tests/field_policy_operator_toolkit_sandbox_test_results.md`](tests/field_policy_operator_toolkit_sandbox_test_results.md). No sandbox test/prototype remains in the production test suite. Overall probe verdict: **PARTIAL** — directed_decayed + hierarchy-first toolkit substrate-real; frontier cadence direction and scale cost remain constraints. Mapping/location architecture remains provisional. Implementation remains parked until the mapping doc is ready or product names a concrete non-mapping scenario. No mapping runtime, Scatter/Gather, wavefront propagation, dynamic nested enrollment, D-2a, E-11B-5, new WGSL, new AccumulatorRole variants, CPU fallback, slot compaction, or simthing-sim arena awareness landed. FlatStarResourceFlow remains the accepted bounded production Resource Flow posture. Global `PipelineFlags::default().use_accumulator_resource_flow` remains false. Presence of `ResourceFlowSpec` alone does not enable GPU execution.

**FIELD_POLICY tensor/stencil WGSL prototype sandbox status:** **Reverted** — PR #206 probe merged then reverted to parked state. FIELD_POLICY tensor/stencil WGSL prototype sandbox completed and was reverted to parked state. The sandbox source, WGSL prototype(s), notes, and decision-gate results are preserved in [`docs/workshop/archive/field_policy/field_policy_tensor_stencil_wgsl_sandbox_code_preserve.rs`](workshop/archive/field_policy/field_policy_tensor_stencil_wgsl_sandbox_code_preserve.rs), [`docs/workshop/archive/field_policy/field_policy_tensor_stencil_prototype.wgsl`](workshop/archive/field_policy/field_policy_tensor_stencil_prototype.wgsl), and [`docs/tests/field_policy_tensor_stencil_wgsl_sandbox_test_results.md`](tests/field_policy_tensor_stencil_wgsl_sandbox_test_results.md). No sandbox test/prototype remains in the production runtime/test suite. Overall probe verdict: **PARTIAL** — general StructuredFieldStencilOp candidate confirmed; ~11× projected 30k speedup; normalized_stencil H=8 directional. Mapping/location architecture remains provisional. Implementation remains parked until the mapping doc is ready or product names a concrete non-mapping scenario. No mapping runtime, Scatter/Gather, wavefront propagation, dynamic nested enrollment, D-2a, E-11B-5, production WGSL, new AccumulatorRole variants, CPU fallback, slot compaction, or simthing-sim arena awareness landed. FlatStarResourceFlow remains the accepted bounded production Resource Flow posture. Global `PipelineFlags::default().use_accumulator_resource_flow` remains false. Presence of `ResourceFlowSpec` alone does not enable GPU execution.

**FIELD_POLICY tensor/stencil WGSL refinement sandbox status:** **Reverted** — PR #208 probe merged then reverted to parked state. Workshop artifacts preserved; findings informed V7.6 promotion below.
...
(single grid, no atlas), and the full first-slice vertical FIELD_POLICY slice is now
...
**Status:** **Landed** — narrow product-facing FIELD_POLICY commitment fixture over the accepted
...
substrate over parent field_urgency, proving the FIELD_POLICY commitment path: GPU-resident field
...
RON-admitted configuration while preserving the existing GPU-resident FIELD_POLICY path: field
...
authoring ergonomics, or an economy+FIELD_POLICY product fixture — a generic boundary-output packet only if
...
### PR M-economy-field-policy-product-fixture-v1 — Economy + FIELD_POLICY product fixture — **Done**

**Status:** **Done (PASS)** — Phase M Economy + FIELD_POLICY Product Fixture V1 landed.
It demonstrates that an accepted discrete resource-economy boundary result can influence an opt-in first-slice FIELD_POLICY commitment fixture without adding runtime semantic coupling: economy remains `ResourceEconomySpec`/discrete/opt-in, FIELD_POLICY remains GPU-resident field/reduction/EML/Threshold+EmitEvent, and the CPU does not compute urgency or emit commitments.
...
**Test:** [`phase_m_economy_field_policy_product_fixture_test_results.md`](tests/phase_m_economy_field_policy_product_fixture_test_results.md) — PASS.

### PR M-product-fixture-chain-parking — Product-fixture chain review/parking packet — **Done; ACCEPTED 2026-05-29**

**Acceptance:** **ACCEPTED (Opus/product 2026-05-29, PASS WITH CONDITIONS)** — [`reviews/phase_m_product_fixture_chain_acceptance_opus_review.md`](reviews/phase_m_product_fixture_chain_acceptance_opus_review.md). Accepted as a fixture-level product proof; the economy→FIELD_POLICY link stays in `tests/support` orchestration (CPU selects authored EML weight profiles, never computes urgency or emits commitments; both stay GPU-resident). Binding row added to `docs/invariants.md` Mapping section. Next implementation step: authoring ergonomics R2 or another tiny non-map-substrate + FIELD_POLICY product fixture — not a generic boundary-output packet (D), not the M-4 atlas packer (E). No production economy→mapping bridge authorized.

**Status:** **Done (PASS)** — Phase M product-fixture chain parking packet landed.
The repo now has a reviewed chain from abstract tick/boundary doctrine through discrete `ResourceEconomySpec` boundary banking, resource-economy authoring preview, and an Economy + FIELD_POLICY product fixture where resolved treasury stress selects authored EML weights and the existing GPU-resident first-slice path emits or does not emit a FIELD_POLICY commitment through Threshold + EmitEvent.
...
landed, and the **full first-slice vertical FIELD_POLICY slice is now ACCEPTED (Opus/product
...
- **SQRT-EXACT-1D-R1 (PASS, verbatim WGSL artifact harness):** Candidate D now lives as test artifact `crates/simthing-driver/tests/wgsl/sqrt_cr_d_candidate.wgsl` and is consumed in the battery via `include_str!` (`SQRT_CR_D_WGSL`) rather than Rust string regeneration; artifact hash is recorded in test/report for identity tracking. Report: [`docs/tests/phase_m_jit_sqrt_exact1d_r1_verbatim_wgsl_results.md`](tests/phase_m_jit_sqrt_exact1d_r1_verbatim_wgsl_results.md).
- **SQRT-EXACT-2E (PASS, integer-only contingency probe):** Candidate E now lives as `crates/simthing-driver/tests/wgsl/sqrt_cr_e_candidate.wgsl` and is consumed via `include_str!` (`SQRT_CR_E_WGSL`) with authoritative `u32` bit-pattern IO (`sqrt_cr_e_bits`). E removes D-style subnormal readback flush but remains `RejectedDeferred` on dense normal accuracy (`max_ulp=119`). Report: [`docs/tests/phase_m_jit_sqrt_exact2e_integer_candidate_results.md`](tests/phase_m_jit_sqrt_exact2e_integer_candidate_results.md).
- **SQRT-EXACT-3E (PASS, integer mantissa core remediation):** Candidate E artifact was upgraded to a correctly-rounded integer mantissa core (u32 limb-pair arithmetic + exact nearest-even rounding) while preserving verbatim WGSL and `u32` bit IO. Current battery sweeps hit `max_ulp=0` on edge/dense/subnormal corpora and `flush_count=0`, so E is now `ExactCandidatePendingExhaustiveSweep` only. Report: [`docs/tests/phase_m_jit_sqrt_exact3e_integer_core_results.md`](tests/phase_m_jit_sqrt_exact3e_integer_core_results.md). Ignored exhaustive sweep added and still required for promotion.
- **SQRT-EXACT-4E (PASS, exhaustive proof gate):** Ran the ignored full finite non-negative exhaustive sweep for Candidate E (`0x0000_0000..=0x7F7F_FFFF`) with zero ULP and zero flush (`exact_bits=2,139,095,040`). E classification is now `ExactDeterministicCandidate`; production descriptor/admission promotion remains a separate explicit slice. Candidate F remains a documented performance alternative and was not built. Report: [`docs/tests/phase_m_jit_sqrt_exact4e_exhaustive_sweep_results.md`](tests/phase_m_jit_sqrt_exact4e_exhaustive_sweep_results.md).
- **SQRT-EXACT-4F (PASS, verbatim hot-path probe):** Candidate F (`CorrectlyRoundedHwBitmaskNormalized`) now lives as standalone verbatim artifact `crates/simthing-driver/tests/wgsl/sqrt_cr_f_candidate.wgsl`, consumed via `include_str!` (`SQRT_CR_F_WGSL`) with authoritative `u32` bit IO (`sqrt_cr_f_bits`). Edge/dense/subnormal sweeps and F-vs-E3 comparison are all zero ULP on current corpora, contraction-risk probe is clean (`native_mismatch=129`, `F_mismatch=0`), and 34,000-row smoke timing shows F faster than E3 in the harness. Classification is `ExactCandidatePendingExhaustiveSweep` until F's own ignored full exhaustive sweep is executed green; no descriptor/admission promotion in this slice. Report: [`docs/tests/phase_m_jit_sqrt_exact4f_candidate_f_results.md`](tests/phase_m_jit_sqrt_exact4f_candidate_f_results.md).
- **SQRT-EXACT-5F (PASS, exhaustive proof gate for hot-path F):** Ran the ignored full finite non-negative exhaustive sweep for Candidate F (`0x0000_0000..=0x7F7F_FFFF`) with exact parity across the full domain (`tested=2,139,095,040`, `exact_bits=2,139,095,040`, `max_ulp=0`, `flush_count=0`, `worst=none`). F classification is now `ExactDeterministicCandidate`. E3 remains exact deterministic fallback/reference. Report: [`docs/tests/phase_m_jit_sqrt_exact5f_exhaustive_sweep_results.md`](tests/phase_m_jit_sqrt_exact5f_exhaustive_sweep_results.md).
- **SQRT-PROMOTE-0 — artifact-backed Candidate F exact descriptor/admission (PASS):** Exact authority is tied to F artifact hash `e2e9e27601ee2e13`, entrypoint `sqrt_cr_f_bits`, `u32` bit IO, proof report `phase_m_jit_sqrt_exact5f_exhaustive_sweep_results.md`, landed descriptor `m_jit_sqrt_f_exact`. Raw native sqrt remains `ApproximateJitOnly`. `mag2` remains blocked unless routed through exact F. No default runtime wiring/scheduler/cache/bridge. Report: [`docs/tests/phase_m_jit_sqrt_promote0_descriptor_admission_results.md`](tests/phase_m_jit_sqrt_promote0_descriptor_admission_results.md).
- **SQRT-MAG-0 — F-backed exact Euclidean magnitude for FIELD_POLICY hot path (PASS):** Exact magnitude authority via `m_jit_mag_f_from_exact_mag2` when pre-sqrt mag2 is exact-authoritative; raw dx/dy probe via `m_jit_mag_f_from_dxdy_probe`; native sqrt remains approximate; diagnostic `mag2` remains blocked as exact final output; 34k mobile-simthing benchmark recorded (~5.4 ms, 1 dispatch, readback included). No default runtime wiring/scheduler/cache/bridge. Report: [`docs/tests/phase_m_jit_sqrt_mag0_f_exact_magnitude_results.md`](tests/phase_m_jit_sqrt_mag0_f_exact_magnitude_results.md).
- **SQRT-MAG2-0 — exact pre-sqrt mag2 construction for FIELD_POLICY hot path (PASS):** Fixed-point dx/dy mag2 is exact-authoritative under bounded Q16.16 range/scale; raw f32 dx/dy mag2 remains approximate/probe; F-backed sqrt consumes exact mag2; 34k benchmark recorded (~21.5 ms, 1 dispatch, readback included). No default runtime wiring/scheduler/cache/bridge. Report: [`docs/tests/phase_m_jit_sqrt_mag2_0_fixed_exact_results.md`](tests/phase_m_jit_sqrt_mag2_0_fixed_exact_results.md).
- **SQRT-MAG2-PERF-0 — exact mag2 + F sqrt performance decomposition (PASS):** 34k readback/baseline ~2.5 ms; mag2-only ~1.5 ms; F-only ~1.8 ms; combined Q16.16 ~1.7–2.5 ms; Q16.16 single-kernel retained as exact hot-path candidate; Q12.12 probe-only; lo-only conversion rejected; split kernels rejected. Report: [`docs/tests/phase_m_jit_sqrt_mag2_perf0_fixed_hotpath_results.md`](tests/phase_m_jit_sqrt_mag2_perf0_fixed_hotpath_results.md).
- **FIELD_POLICY-OBS-0 — GPU-resident mobile observer overlay score probe (PASS):** Q16.16 mag2 + F sqrt exact mag path; f32 weighted score (approximate/diagnostic); 34k single-dispatch benchmark recorded; dense corpus 50,176/50,176 mag2 exact, max_ulp=0; descriptor deferred (driver fixture). No scheduler/cache/default wiring/bridge. Report: [`docs/tests/phase_m_field_policy_obs0_mobile_overlay_score_results.md`](tests/phase_m_field_policy_obs0_mobile_overlay_score_results.md).
- **FIELD_POLICY-OBS-1 — descriptor/admission for mobile observer overlay score (PASS):** `m_jit_field_policy_obs0_overlay_score` default-off descriptor; mag2_bits/mag_bits exact, score_bits ApproximateDiagnosticF32; warm 34k 32× dispatch ~0.129 ms/dispatch; exact score over f32 rejected. No scheduler/cache/default wiring/bridge. Report: [`docs/tests/phase_m_field_policy_obs1_overlay_score_admission_results.md`](tests/phase_m_field_policy_obs1_overlay_score_admission_results.md).
- **FIELD_POLICY-OBS-2 — multi-layer GPU-resident observer overlay score (PASS):** `m_jit_field_policy_obs2_multilayer_overlay_score`; 4 Q16.16+F layers/row; per-layer mag exact; score ApproximateDiagnosticF32; 34k ~7 ms single dispatch; warm 32× ~0.238 ms/dispatch. Report: [`docs/tests/phase_m_field_policy_obs2_multilayer_overlay_score_results.md`](tests/phase_m_field_policy_obs2_multilayer_overlay_score_results.md).
- **FIELD_POLICY-OBS-3 — fixed-point aggregate score contract for multi-layer observer overlay (PASS):** `m_jit_field_policy_obs3_multilayer_fixed_score`; Q16.16 weighted sum (`ExactQ16WeightedSum`); per-layer mag exact; score_fixed exact; OBS-2 f32 score unchanged; 34k ~7.8 ms; warm 32× ~0.278 ms/dispatch. Report: [`docs/tests/phase_m_field_policy_obs3_fixed_point_score_results.md`](tests/phase_m_field_policy_obs3_fixed_point_score_results.md).
- **FIELD_POLICY-OBS-4 — GPU-resident threshold event emission from exact observer scores (PASS):** `m_jit_field_policy_obs4_threshold_event`; Q16.16 threshold/hysteresis; state/event ExactDeterministicEventFlag; warm 32× ~0.231 ms/dispatch; no CPU planner/bridge. Report: [`docs/tests/phase_m_field_policy_obs4_threshold_event_results.md`](tests/phase_m_field_policy_obs4_threshold_event_results.md).
- **FIELD_POLICY-EVENT-0 — GPU-resident compaction for threshold event rows (PASS):** `m_jit_field_policy_event0_compaction`; atomic compaction; ExactAuthoritativeUnordered membership; UnspecifiedAtomicOrder; warm 32× ~0.202 ms/dispatch (50% density); no CPU planner/bridge. Report: [`docs/tests/phase_m_field_policy_event0_compaction_results.md`](tests/phase_m_field_policy_event0_compaction_results.md).
- **FIELD_POLICY-PIPE-0 — integrated GPU observer-event pipeline (PASS):** `m_jit_field_policy_pipe0_observer_event_pipeline`; two-dispatch layout (OBS-4 threshold_pass + EVENT-0 compact_pass); score/state/event exact on intermediate rows; event_count/overflow ExactAuthoritative; membership ExactAuthoritativeUnordered under capacity; order UnspecifiedAtomicOrder; 34k integrated ~0.855 ms (2 dispatches + readback); warm 32× ~0.326 ms/pipeline; no scheduler/cache/default wiring/bridge. Report: [`docs/tests/phase_m_field_policy_pipe0_observer_event_pipeline_results.md`](tests/phase_m_field_policy_pipe0_observer_event_pipeline_results.md).
- **FIELD_POLICY-EVENT-1 — GPU-resident event-code bucketing (PASS):** `m_jit_field_policy_event1_code_bucketing`; per-code atomic buckets (CODE_COUNT=4); bucket_counts/overflow/invalid ExactAuthoritative; membership ExactAuthoritativeUnordered under per-code capacity; order UnspecifiedAtomicOrder; 34k balanced warm ~0.284 ms/dispatch; integrated PIPE-0→bucket 3-pass smoke; no scheduler/cache/default wiring/bridge. Report: [`docs/tests/phase_m_field_policy_event1_code_bucketing_results.md`](tests/phase_m_field_policy_event1_code_bucketing_results.md).
- **FIELD_POLICY-EVENT-2 — GPU-resident per-bucket reductions (PASS):** `m_jit_field_policy_event2_bucket_reductions`; per-code count/sum_lo/sum_hi/min/max + overflow/empty flags; i64 sum with explicit overflow flag; order-invariant reductions; 34k balanced warm ~0.52 ms/dispatch; EVENT-1→reduce and PIPE-0→bucket→reduce smokes; no scheduler/cache/default wiring/bridge. Report: [`docs/tests/phase_m_field_policy_event2_bucket_reductions_results.md`](tests/phase_m_field_policy_event2_bucket_reductions_results.md).
- **FIELD_POLICY-ACT-0 — numeric action proposal substrate (PASS):** `m_jit_field_policy_act0_numeric_proposals`; consumes EVENT-2 reductions; fixed integer max-score and optional sum thresholds; proposal_count/overflow ExactAuthoritative; membership ExactAuthoritativeUnordered under capacity; order UnspecifiedAtomicOrder; 34k bucket→reduce→propose warm ~1.24 ms/pipeline; no CPU planner/scheduler/cache/default wiring/bridge. Report: [`docs/tests/phase_m_field_policy_act0_numeric_proposals_results.md`](tests/phase_m_field_policy_act0_numeric_proposals_results.md).

**FIELD_POLICY-ACT-0 — numeric action proposal substrate**

FIELD_POLICY-ACT-0 extends the default-off FIELD_POLICY observer/event proof stack from event observability to bounded numeric proposal records. It is supporting infrastructure for future Phase E / FIELD_POLICY-economics consumers, not a new production phase. It does not add runtime planner behavior, scheduler/cache/default wiring, semantic WGSL, simthing-sim awareness, or production economy bridge wiring.

Further FIELD_POLICY work remains allowed only when it advances a usable FIELD_POLICY toolset or directly supports M/E closure. Each FIELD_POLICY slice must update the production plan with M/E closure relevance.

- **FIELD_POLICY-ACT-1 — Phase E-style numeric proposal consumer (PASS):** `m_jit_field_policy_act1_phase_e_proposal_consumer`; consumes ACT-0 proposal records; fixed integer admitted-code table; accepted/ignored/invalid/summary/max ExactAuthoritative under overflow contract; summary order-invariant; 34k bucket→reduce→propose→consume warm ~1.35 ms/pipeline; no CPU planner/scheduler/cache/default wiring/bridge. Report: [`docs/tests/phase_m_field_policy_act1_phase_e_proposal_consumer_results.md`](tests/phase_m_field_policy_act1_phase_e_proposal_consumer_results.md).
- **FIELD_POLICY-ACT-2 — fixture-local proposal admission records (PASS):** `m_jit_field_policy_act2_proposal_admission_records`; consumes ACT-1 proposal_summary; fixed integer admission thresholds; admission record ExactAuthoritative under rule/overflow contract; 34k bucket→reduce→propose→consume→admit warm ~1.59 ms/pipeline; no CPU planner/scheduler/cache/default wiring/bridge. Report: [`docs/tests/phase_m_field_policy_act2_proposal_admission_records_results.md`](tests/phase_m_field_policy_act2_proposal_admission_records_results.md).
- **FIELD_POLICY-ACT-3 — Economic V1-style fixture substrate records (PASS):** `m_jit_field_policy_act3_economic_fixture_records`; consumes ACT-2 admission_record; fixed integer lookup table; fixture record ExactAuthoritative under mapping/overflow contract; 34k bucket→reduce→propose→consume→admit→fixture warm ~1.19 ms/pipeline; no CPU planner/scheduler/cache/default wiring/bridge. Report: [`docs/tests/phase_m_field_policy_act3_economic_fixture_records_results.md`](tests/phase_m_field_policy_act3_economic_fixture_records_results.md).
- **FIELD_POLICY-ACT-4 — Economic V1-style fixture validation corpus (PASS):** `field_policy_act4_economic_fixture_validation_corpus_v1`; validates ACT-3 fixture records against 18-row authorable expected-output corpus via CPU oracle; no new WGSL/descriptor/runtime gate. Report: [`docs/tests/phase_m_field_policy_act4_economic_fixture_validation_corpus_results.md`](tests/phase_m_field_policy_act4_economic_fixture_validation_corpus_results.md).
- **FIELD_POLICY-V1-CONSOLIDATE-0 — FIELD_POLICY Field agent Proposal Pipeline V1 (PASS, docs-only):** consolidated OBS-0..4 + EVENT-0..2 + PIPE-0 + ACT-0..2 as V1 core; ACT-3/ACT-4 supporting evidence; ladder closed; FrontierV1-0 next. Report: [`docs/tests/phase_m_field_policy_v1_consolidation_results.md`](tests/phase_m_field_policy_v1_consolidation_results.md).
...
- **FrontierV1-1 — opt-in end-to-end fixture wiring (PASS):** wires first-slice mapping + flat-star Resource Flow + FIELD_POLICY V1 routing through CPU-oracle fixture; replay fingerprint `49d4c94ce1f52be5`; no default wiring. Report: [`docs/tests/phase_m_frontier_v1_1_opt_in_fixture_results.md`](tests/phase_m_frontier_v1_1_opt_in_fixture_results.md).
...
- **FrontierV1-4 — FIELD_POLICY V1 route replay acceptance (PASS):** integrated FIELD_POLICY V1 route replay after GPU mapping+RF; replay fingerprint `4382ec7ef93c9174`; route/PIPE-0 replay_accepted. Report: [`docs/tests/phase_m_frontier_v1_4_field_policy_route_replay_results.md`](tests/phase_m_frontier_v1_4_field_policy_route_replay_results.md).
- **FrontierV1-5 — live GPU field agent route toward FrontierV2 (PASS):** one live GPU-resident integrated score→threshold→proposal→dispatch resource route; replay fingerprint `1653b84847be2dd2`; feedback candidate FixtureOnly; FrontierV2 NotImplemented. Report: [`docs/tests/phase_m_frontier_v1_5_live_field_agent_route_results.md`](tests/phase_m_frontier_v1_5_live_field_agent_route_results.md).
- **FrontierV2-0 — multi-tick closed-loop consumer fixture (PASS):** two-tick default-off FrontierV2 closed loop consuming V1-5 feedback; replay fingerprint `0238c18ce3b559da`; ClauseThing NotImplemented. Report: [`docs/tests/phase_m_frontier_v2_0_closed_loop_consumer_results.md`](tests/phase_m_frontier_v2_0_closed_loop_consumer_results.md).
...
- **Design-authority correction (Opus, 2026-05-30):** the line above is implementer **self-acceptance** and is superseded by the design-authority ruling (`docs/workshop/field_policy_track.md` §10). Of record: **substrate (mapping + flat-star RF) closed → Phase E closes at flat-star**; the field agent **resource-dispatch** loop is `GpuVerified` (bounded single-tick) per the now-**ACCEPTED FrontierV1-5**; structural/movement remain `ReplayAccepted`. The earlier "Pause (F) / FrontierV1-5 not authorized" disposition is **void**; **next named gate is `FrontierV2-0`** (multi-tick closed-loop consumer; ClauseThing is its eventual authoring front-end).
- **FrontierV1-POSTACCEPT-0 — post-acceptance roadmap reset (PASS):** roadmap aligned after M/E closure; next gate remains Pause (F). Report: [`docs/tests/phase_m_frontier_v1_post_acceptance_roadmap_results.md`](tests/phase_m_frontier_v1_post_acceptance_roadmap_results.md).

**FIELD_POLICY-ACT-1 — Phase E-style numeric proposal consumer**

FIELD_POLICY-ACT-1 verifies that numeric proposal records from the FIELD_POLICY actionability stack can be consumed by a default-off Phase E-style fixture as generic numeric candidate inputs. This supports future Economic V1 / FIELD_POLICY-economics integration without adding production runtime wiring, semantic WGSL, CPU planner behavior, scheduler/cache, simthing-sim awareness, or economy bridge wiring.

**FIELD_POLICY-ACT-2 — fixture-local proposal admission records**

FIELD_POLICY-ACT-2 verifies that Phase E-style numeric proposal summaries can be converted into fixture-local admission records under fixed integer contracts. This supports future Economic V1 / FIELD_POLICY-economics authoring by providing an admitted-candidate artifact shape, but it does not add production runtime wiring, semantic WGSL, CPU planner behavior, scheduler/cache, simthing-sim awareness, or economy bridge wiring.

**FIELD_POLICY-ACT-3 — Economic V1-style fixture substrate records**

FIELD_POLICY-ACT-3 verifies that fixture-local proposal admission records can be converted into Economic V1-style numeric substrate records under fixed integer mapping contracts. This supports future Economic V1 / FIELD_POLICY-economics authoring by providing a reusable numeric fixture artifact shape, but it does not add production runtime wiring, semantic WGSL, CPU planner behavior, scheduler/cache, simthing-sim awareness, or economy bridge wiring.

**FIELD_POLICY-ACT-4 — Economic V1-style fixture validation corpus**

FIELD_POLICY-ACT-4 validates already-landed ACT-3 numeric fixture records against a stable authorable expected-output corpus. This is fixture-authoring evidence for future Phase E/Economic V1 work, not runtime implementation. No production wiring, semantic WGSL, scheduler/cache/default SimSession behavior, simthing-sim awareness, or economy bridge wiring is authorized. ACT-4 does not open a new runtime implementation gate.

**FIELD_POLICY-V1-CONSOLIDATE-0 — FIELD_POLICY Field agent Proposal Pipeline V1**

The FIELD_POLICY field agent fixture ladder is consolidated at Proposal Pipeline V1 per [`docs/workshop/field_policy_track.md`](workshop/field_policy_track.md). OBS-0..4, EVENT-0..2, PIPE-0, and ACT-0..2 form the accepted V1 core. ACT-3 and ACT-4 are retained as supporting Economic V1 fixture evidence: ACT-3 provides fixture-local numeric substrate records; ACT-4 provides an authorable validation corpus over those records. No ACT-5, EVENT-3, OBS-5, or PIPE-1 is authorized.

The next implementation step is FrontierV1-0: a bounded opt-in/default-off scenario combining first-slice mapping, flat-star Resource Flow, and FIELD_POLICY field agent proposal routing through accepted substrates. M and E close when FrontierV1 integration is green and accepted.

M-JIT-PROD-0 is accepted by the FIELD_POLICY field agent charter. Atlas, active mask, perception, source identity, nested E-11B, E-11B-5, D-2a, hard-currency ordering, and ClauseThing implementation remain deferred.

**FrontierV1-0 — opt-in scenario skeleton and admission contract**

FrontierV1-0 defines the default-off FrontierV1 scenario skeleton and validates the admission envelope for the named M/E closing vertical: one ≤32×32 first-slice RegionCell theater, flat-star depth-2 Resource Flow economy, and FIELD_POLICY Field agent Proposal Pipeline V1 routing through accepted substrates. It adds no runtime wiring, no semantic WGSL, no scheduler/cache/default SimSession behavior, no simthing-sim semantic awareness, and no generic production economy→mapping bridge. Next step is FrontierV1-1 end-to-end opt-in fixture wiring.

**FrontierV1-1 — opt-in end-to-end fixture wiring**

FrontierV1-1 wires the default-off FrontierV1 fixture through the accepted first-slice Mapping, flat-star Resource Flow, and FIELD_POLICY Field agent Proposal Pipeline V1 substrates. It proves explicit opt-in fixture integration and routing through accepted substrates without default SimSession behavior, scheduler/cache, semantic WGSL, simthing-sim awareness, CPU planner/urgency/commitment emission, or a generic production economy→mapping bridge. Next step is FrontierV1-2 GPU-resident execution + replay acceptance, if FrontierV1-1 passes.
...
FrontierV1-3 verifies flat-star Resource Flow allocation on GPU inside the default-off FrontierV1 fixture and matches the CPU oracle exactly. Resource dispatch remains routed through the accepted Resource Flow allocator path; no parallel fixture economy, shared-pool tick writes, nested E-11B, E-11B-5, D-2a, default SimSession behavior, scheduler/cache, semantic WGSL, simthing-sim awareness, or CPU planner/urgency/commitment emission is added. Next step is FrontierV1 acceptance review if mapping, Resource Flow, and FIELD_POLICY routing are all GPU-verified; otherwise FrontierV1-4 must close any explicitly remaining GPU coverage gap.

**FrontierV1-4 — FIELD_POLICY V1 route replay acceptance**

FrontierV1-4 integrates accepted FIELD_POLICY Field agent Proposal Pipeline V1 route replay into the default-off FrontierV1 fixture after GPU mapping and GPU flat-star Resource Flow verification. It confirms resource dispatch routes through Resource Flow allocator, structural commitments through Threshold+EmitEvent→BoundaryRequest, and movement through own-column-only routing. It does not add ACT-5, EVENT-3, OBS-5, PIPE-1, semantic WGSL, default SimSession behavior, scheduler/cache, simthing-sim awareness, CPU planner/urgency/commitment emission, or a generic economy→mapping bridge. Next step is FrontierV1 acceptance review if no GPU coverage gaps remain, otherwise FrontierV1-5 must close only the explicitly remaining gap.

**FrontierV1-ACCEPT-0 — M/E closure acceptance**

FrontierV1-ACCEPT-0 accepts FrontierV1 as the named M/E closing vertical. Mapping first-slice and flat-star Resource Flow are GPU-verified; FIELD_POLICY Field agent Proposal Pipeline V1 and route contracts are replay-accepted against accepted evidence; replay fingerprints are stable; all hard guardrails remain intact. Phase M and Phase E are closed for FrontierV1 scope. Deferred items remain non-blocking: atlas, active mask, perception/fog, source identity, nested E-11B/E-11B-5, D-2a, ClauseThing, and any broader production integration outside FrontierV1.

**FrontierV1-POSTACCEPT-0 — post-acceptance roadmap reset**

FrontierV1 is accepted as the named M/E closing vertical; Phase M and Phase E are closed for FrontierV1 scope. FrontierV1-5, ACT-5, EVENT-3, OBS-5, and PIPE-1 are not authorized. Full PIPE-0 GPU re-run inside FrontierV1 remains optional only and requires a separately named product need. Deferred items remain non-blocking: atlas, active mask, perception/fog, source identity, nested E-11B/E-11B-5, D-2a, ClauseThing, and broader production integration outside FrontierV1. Next work must begin from a separately named production gate and preserve V7.7 / Mapping ADR / Resource Flow ADR / FIELD_POLICY charter guardrails. Next named gate: **Pause implementation (F)** per [`product_priority_vertical_slice_selection.md`](reviews/product_priority_vertical_slice_selection.md).

**FrontierV1-5 — live GPU-resident field agent route run toward FrontierV2**

FrontierV1-5 responds to the Opus design-authority ruling in `docs/workshop/field_policy_track.md` §10 and the Forward Horizon in `docs/workshop/mapping_current_guidance.md` by executing one live GPU-resident integrated field agent route inside the default-off FrontierV1 fixture: score→threshold→proposal→dispatch. It preserves accepted mapping and flat-star Resource Flow GPU verification, routes resource dispatch through the Resource Flow allocator, records CPU oracle parity and replay reproducibility, and emits a fixture-only feedback candidate shape for the named FrontierV2 multi-tick closed-loop consumer. It does not implement FrontierV2, reopen the FIELD_POLICY ACT/EVENT/OBS/PIPE ladder, declare phase closure, or add default SimSession behavior, scheduler/cache, semantic WGSL, CPU planner/urgency/commitment emission, simthing-sim semantic awareness, parallel fixture economy, shared-pool tick writes, or a generic economy→mapping bridge.

The FrontierV1 field agent resource-dispatch loop is now GpuVerified for the bounded single-tick smoke route. FrontierV2 remains the next named consumer for multi-tick closed-loop behavior. Structural/movement routes remain ReplayAccepted unless separately proven.

**FrontierV2-0 — multi-tick closed-loop field agent consumer fixture**

FrontierV2-0 implements the named Forward Horizon consumer from `docs/workshop/mapping_current_guidance.md`: a default-off, bounded multi-tick closed-loop fixture where field-derived FIELD_POLICY field agent proposals drive resource dispatch and fixture-only feedback into the next tick's field/economy input. It consumes FrontierV1-5's live GPU field agent route proof and feedback candidate shape, runs at least two ticks, records CPU oracle parity and replay reproducibility, and does not implement ClauseThing, reopen the FIELD_POLICY ACT/EVENT/OBS/PIPE ladder, declare phase closure, or add default SimSession behavior, scheduler/cache, semantic WGSL, CPU planner/urgency/commitment emission, simthing-sim semantic awareness, parallel fixture economy, shared-pool tick writes, or a generic economy→mapping bridge.
...
**FrontierV2-3 — structural BoundaryRequest feedback application**
...
[`docs/workshop/field_policy_track.md`](workshop/field_policy_track.md) §11.

Further FIELD_POLICY actionability work must either: (1) produce a reusable numeric substrate consumed by Phase E fixtures, (2) directly close a Phase M/E blocker, or (3) be paused until a named product scenario requires runtime integration.
...
mapping wiring, CPU planner/urgency, or economy→mapping bridge. FIELD_POLICY commitment stays GPU-resident.
