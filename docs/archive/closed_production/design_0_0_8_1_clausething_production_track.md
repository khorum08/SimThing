...
pathfinding/movement/route/predecessor/border/frontline semantics, Euclidean authority in output, or
FIELD-MOVIE-DATASET-0 export. **DA-APPROVED (2026-06-14, #678).** See [`mapgenerator_cli_pr4_emitter_results.md`](tests/mapgenerator_cli_pr4_emitter_results.md) (CURRENT_EVIDENCE).

**0.0.8.2.5 MapGen addendum (2026-06-14, lowerer child-id amendment).** Closed lowerer bugfix: scope initializer payload child IDs per system (`{system.id}_{initializer.id}_planet/_deposit`) so multiple systems sharing one initializer hydrate without duplicate global node IDs. **Not a reopening** of 0.0.8.2.5 scope. **DA-APPROVED & MERGED (#680).** See [`mapgen_lowerer_child_id_amendment_results.md`](tests/mapgen_lowerer_child_id_amendment_results.md) (CURRENT_EVIDENCE). Sign-off record: #681.

**0.0.8.6 MapGeneratorCLI addendum (2026-06-14, PR5).** **0.0.8.2.5 MapGen remains closed.** MapGeneratorCLI PR5 proves PR4 `static_galaxy_scenario` text lowers through `parse_mapgen_neutral_document` → `generate_mapgen_lattice_hierarchy` on the **amended** closed lowerer. It adds only integration tests plus `simthing-mapgenerator` as a **dev-dependency** on `simthing-clausething`; **zero** `crates/simthing-clausething/src/` changes. Decoupled sequence: **#680** = DA-authorized 0.0.8.2.5 lowerer child-id amendment; **#681** = record correction restoring lost DA approval; **#682** = cleaned PR5 (producer + test + docs only). No topology, links, field operators, RF, Movement-Front, PALMA, runtime, GPU, simthing-sim, new `SimThingKind`, pathfinding/movement/route/predecessor/border/frontline semantics, Euclidean authority in output, or FIELD-MOVIE-DATASET-0 export. **DA-APPROVED & MERGED (#682, superseding auto-closed #679).** See [`mapgenerator_cli_pr5_lowering_results.md`](tests/mapgenerator_cli_pr5_lowering_results.md) (CURRENT_EVIDENCE).

**0.0.8.6 MapGeneratorCLI addendum (2026-06-14, PR6).** **0.0.8.2.5 MapGen remains closed.** MapGeneratorCLI PR6 adds bounded producer-side hyperlane edge selection and `static_galaxy_scenario` `add_hyperlane` emission only — **special routes (wormhole-pair / gateway long-range lane couplings) remain outstanding as PR6b.** Generated output parses and lowers through existing closed MapGen neutral-AST + lattice/link surfaces without front-end widening. **Zero** `crates/simthing-clausething/src/` changes. No route/path/predecessor/movement/border/frontline semantics, field operators, RF, Movement-Front, PALMA, runtime, GPU, simthing-sim, new `SimThingKind`, Euclidean authority in output, or FIELD-MOVIE-DATASET-0 export. **DA-APPROVED & MERGED (#684).** See [`mapgenerator_cli_pr6_hyperlane_results.md`](tests/mapgenerator_cli_pr6_hyperlane_results.md) (CURRENT_EVIDENCE). **PR6b next:** bounded special-route emission; **PR7 after PR6b:** partition/bridge structural producer + clustering — no route/path/predecessor semantics and no GPU.

**0.0.8.6 MapGeneratorCLI addendum (2026-06-14, PR6R).** **0.0.8.2.5 MapGen remains closed.** Remedial PR before PR6b/PR7: corrects PR6 production-doc evidence status and adds fail-closed validation for public `HyperlaneOptions` (`InvalidEdgeCounts`, `InvalidFanoutCap`, `UnsatisfiedMinEdgeCount`). **Zero** closed `src/` changes. No route/path/predecessor/movement/border/frontline semantics, field operators, RF, Movement-Front, PALMA, runtime, GPU, simthing-sim, or FIELD-MOVIE-DATASET-0 export. **DA-APPROVED & MERGED (#685).** See [`mapgenerator_cli_pr6r_hardening_results.md`](tests/mapgenerator_cli_pr6r_hardening_results.md) (CURRENT_EVIDENCE).

**0.0.8.6 MapGeneratorCLI addendum (2026-06-14, PR6b).** **0.0.8.2.5 MapGen remains closed.** MapGeneratorCLI PR6b adds bounded producer-side wormhole-pair and gateway special-route endpoint selection represented **only** as `static_galaxy_scenario` `add_hyperlane` declarations that lower through existing closed `mapgen_links` surfaces as bounded lane couplings. No new grammar, no front-end widening, no route/path/predecessor/movement/border/frontline semantics, field operators, RF, Movement-Front, PALMA, runtime, GPU, simthing-sim, new `SimThingKind`, Euclidean authority in output, or FIELD-MOVIE-DATASET-0 export. **DA-APPROVED & MERGED (#686).** See [`mapgenerator_cli_special_routes_results.md`](tests/mapgenerator_cli_special_routes_results.md) (CURRENT_EVIDENCE).

**0.0.8.6 MapGeneratorCLI addendum (2026-06-14, PR7).** **0.0.8.2.5 MapGen remains closed.** MapGeneratorCLI PR7 adds bounded producer-side partition/cluster assignment and bridge endpoint selection represented **only** as `static_galaxy_scenario` `add_hyperlane` declarations. Generated output parses and lowers through existing closed MapGen lattice/link surfaces without front-end widening. No new grammar, no route/path/predecessor/movement/border/frontline semantics, field operators, RF, Movement-Front, PALMA, runtime, GPU, simthing-sim, new `SimThingKind`, Euclidean authority, or FIELD-MOVIE-DATASET-0 work. **DA-APPROVED & MERGED (#687).** See [`mapgenerator_cli_pr7_partition_bridge_results.md`](tests/mapgenerator_cli_pr7_partition_bridge_results.md) (CURRENT_EVIDENCE).

**0.0.8.6 MapGeneratorCLI addendum (2026-06-14, PR8).** **0.0.8.2.5 MapGen remains closed.** MapGeneratorCLI PR8 completes remaining vanilla shape registry descriptors and single-sources executable strategy dispatch/names via `ShapeStrategyEntry` rows in `strategies/registry.rs`. Adding shapes now touches registry entries and tests, not producer core or closed lowerer. Procedural `static`/`arbitrary_static` mode-gate asymmetry fixed (procedural mode rejects both without explicit cells). No topology, field operators, RF, Movement-Front, PALMA, runtime, GPU, simthing-sim, new `SimThingKind`, route/path/predecessor/movement/border/frontline semantics, Euclidean authority, or FIELD-MOVIE-DATASET-0 work. **DA-APPROVED & MERGED (#688).** See [`mapgenerator_cli_pr8_shape_registry_results.md`](tests/mapgenerator_cli_pr8_shape_registry_results.md) (CURRENT_EVIDENCE).

**0.0.8.6 MapGeneratorCLI addendum (2026-06-14, PR9).** **0.0.8.2.5 MapGen remains closed.** PR9 adds bounded producer-side nebula field declarations, initializer bucket emission, and inert metadata passthrough/deferred metadata notes. Generated output uses only already-accepted `static_galaxy_scenario` and closed `nebula = { name radius }` surfaces plus existing Movement-Front `RegionFieldSpec` lowering — parses through the neutral-AST parser and lowers through closed MapGen lattice/field surfaces without front-end widening. No new grammar, no route/path/predecessor/movement/border/frontline semantics, no runtime/GPU/simthing-sim, no new `SimThingKind`, no semantic WGSL, no field execution, no Euclidean authority, and no FIELD-MOVIE-DATASET-0 work. Inert gameplay metadata (`num_empires`, crisis levers, etc.) is captured in producer dry-run reports only until a closed surface admits it. **DA-APPROVED & MERGED (#689).** See [`mapgenerator_cli_pr9_field_operator_results.md`](tests/mapgenerator_cli_pr9_field_operator_results.md) (CURRENT_EVIDENCE). **PR10 now proceeds** to generated scenario admit/install + GPU compact evidence.

**0.0.8.6 MapGeneratorCLI addendum (2026-06-15, PR10).** **0.0.8.2.5 MapGen remains closed.** PR10 proves a generated MapGeneratorCLI scenario can parse, lower, admit/install, and produce GPU-resident compact evidence using existing generic SimThing/MapGen GPU paths. **DA-APPROVED & MERGED (#690).** See [`mapgenerator_cli_pr10_gpu_compact_evidence_results.md`](tests/mapgenerator_cli_pr10_gpu_compact_evidence_results.md) (CURRENT_EVIDENCE). **PR11 now proceeds** to 1000-star scale envelope + closeout evidence.

**0.0.8.6 MapGeneratorCLI addendum (2026-06-15, PR11).** **0.0.8.2.5 MapGen remains closed.** PR11 proves the MapGeneratorCLI scale envelope for 1000-star producer generation and records which lowered/admitted/GPU paths are proven under existing closed caps. Producer hardening: u64 lattice capacity fail-closed, occupancy free-placeable list (no per-insert full lattice scan), bounded pair enumeration (`PRODUCER_PAIR_CANDIDATE_CAP` = 65536), spatial hyperlane candidate windows; DA min-heap remediation closed O(N²·cap) time cliff (43s→1s). Integration proof: 1000-star elliptical scenario emits, parses, lowers lattice (1000 gridcells), exposes bounded hyperlane feedstock; RF/admit/install at 1000 stars **blocked** by closed lowerer caps (no widening). Full 1000-star admission/install is deferred pending DA-authorized 0.0.8.2+ closed-lowerer capacity amendment. PR10 real-adapter GPU compact evidence harness remains live for the tiny fixture. **DA-APPROVED & MERGED (#692).** See [`mapgenerator_cli_pr11_scale_envelope_results.md`](tests/mapgenerator_cli_pr11_scale_envelope_results.md) (CURRENT_EVIDENCE). **PR12 next:** docs-only closeout.

**0.0.8.6 MapGeneratorCLI addendum (2026-06-15, PR12 closeout).** **0.0.8.6 MapGeneratorCLI is CLOSED after PR12 DA approval.** It proved producer-side generation from high-level galaxy levers to `static_galaxy_scenario` neutral-AST output, including shapes, hyperlanes, special routes, partition/cluster bridges, nebula declarations, initializer buckets, metadata deferral, tiny generated admit/install + real-adapter GPU compact evidence, and a 1000-star producer scale envelope. The runtime remains semantic-free. The producer has no runtime dependencies. 0.0.8.2.5 MapGen remains closed. Full 1000-star RF/admit/install/GPU is deferred to a DA-authorized 0.0.8.2+ closed-lowerer capacity amendment. **FIELD-MOVIE-DATASET-0** is the next track unless DA reorders. Docs-only PR; no code, tests, runtime, GPU, lowerer, producer behavior, grammar, or artifact deletions. **PASS pending DA review.** See [`mapgenerator_cli_pr12_closeout_results.md`](tests/mapgenerator_cli_pr12_closeout_results.md) (PROBATION).
