...
| Writes per new test | 2 tables by hand (inventory + boundary) | 1 write path (boundary ledger retired) |
...
| 3 | `HU-INVENTORY-ONEWRITE-0` | **Boundary audit ledger retired; one table.** `test_lifecycle_boundary_rows.tsv` (+ parked_boundary pen + unwired `test_lifecycle_boundary_check.sh`) deleted. Agents add/remove a test by editing `test_inventory.tsv` only. Policy doctrine stays in `test_lifecycle_boundaries.tsv`. Drift FAIL unchanged. Closeout tolerates absent boundary table. | PROOF-PRESENT — evidence [`hu_inventory_onewrite_0_results.md`](tests/hu_inventory_onewrite_0_results.md) | T2 |
| 4 | `HU-FIXTURE-LIFECYCLE-0` | **Harness eats its own dog food.** Mint `harness-fixture` birth_track (open, fused semantics documented); rebirth the 0.0.8.4.6 seal-proof fixture cohort onto it with a documented necessity note per fixture *family* (not per file); then **close `0.0.8.4.6-ci-scaffolding` via `track_closeout.sh`** (build-manifest → check-eval → apply). Fixture-count snapshot recorded; unused families flagged to the pen. No fixture deleted without its scan surface named. | **PROOF-PRESENT** — evidence [`hu_fixture_lifecycle_0_results.md`](tests/hu_fixture_lifecycle_0_results.md) | T2 |
