...
| 2b | `OH-COLD-START-0` (after 2) | `orient.sh` + `ORIENT-RECEIPT` emission; receipt validation in `relay_lint.sh` | **PROBATION / proof-present / DA-review-pending** — PR pending @ head; `orient.sh` + receipt validation + `/orient` receipt output; DA clearance required (gate-wiring) |
