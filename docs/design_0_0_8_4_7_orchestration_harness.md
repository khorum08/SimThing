...
| 1R | `OH-CLEARANCE-ROUTER-0R` | empty-diff precision + local PR-number resolution in `clearance_check.sh` | **PROBATION / proof-pending** — branch `oh-clearance-router-0r-empty-diff`; empty requested diff → `DA-RESERVE(harness-error)`; bare PR resolves or hard-errors with `--range` remedy; evidence [`oh_clearance_router_0r_results.md`](tests/oh_clearance_router_0r_results.md); DA clearance required (gate-wiring) |
