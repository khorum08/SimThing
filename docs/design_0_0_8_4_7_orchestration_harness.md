...
|---|---|---|---|
| 0 | `OH-CLEARANCE-ROUTER-0` | `clearance_check.sh` + `precedented_classes.tsv` + `binding_conditions.tsv` (backfill: PALMA→6.2 conditions as the first rows) + `clearance_ledger.tsv` emission + selftest fixtures (clearable / reserve / fail-closed / self-application-refusal / suspended-class cases) | **PROBATION / proof-pending** — branch `oh-clearance-router-0`; router + TSVs + nine fixtures + `/clearance` wired on `doctrine-exec-commands.yml`; owner-local selftest PASS (9/9); evidence [`oh_clearance_router_0_results.md`](tests/oh_clearance_router_0_results.md); DA clearance required (gate-wiring) |
