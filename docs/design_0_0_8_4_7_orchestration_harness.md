...
| 1 | `OH-RELAY-LINT-0` | `relay_lint.sh` + schema doc block; advisory mode wired to the comment surface | **PROBATION / proof-pending** — branch `oh-relay-lint-0`; linter + six fixtures + `/relay-lint` wired; selftest PASS (6/6); evidence [`oh_relay_lint_0_results.md`](tests/oh_relay_lint_0_results.md); DA clearance required (gate-wiring) |
