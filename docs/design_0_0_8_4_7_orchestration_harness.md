...
| 1R | `OH-CLEARANCE-ROUTER-0R` | empty-diff precision + local PR-number resolution in `clearance_check.sh` | **PROBATION / proof-present / DA-review-pending** — PR [#1164](https://github.com/khorum08/SimThing/pull/1164) @ `faba7981`; empty requested diff → `DA-RESERVE(harness-error)`; bare PR resolves or hard-errors with `--range` remedy; Doctrine Scan PASS; evidence [`oh_clearance_router_0r_results.md`](tests/oh_clearance_router_0r_results.md); DA clearance required (gate-wiring) |
