...
| 2 | `OH-ORIENTATION-DIGEST-0` | `gen_orientation.sh` + generated `docs/orchestrator_orientation.md` + CI freshness gate + `/orient` | **PROBATION / proof-present / DA-review-pending** — PR pending @ head; generated `orchestrator_orientation.md` + freshness gate + `/orient` wired; DA clearance required (gate-wiring) |
