...
| 2cR | `OH-SELFTEST-DECOUPLE-0` (remedial, DA-found at #1167 closure) | Decouple `relay_lint.sh`/`orient.sh` receipt selftests from the **live** orientation digest — validate each fixture against a **fixture-local orientation snapshot** (self-contained per test doctrine), so routine digest-source edits (rung-status flips, etc.) no longer rot them; then CI-gate the two selftests in `doctrine-scan.yml` | `relay_lint --selftest` + `orient --selftest` green **and** invariant under an unrelated design-doc edit (regenerate digest → selftests still PASS); both wired into CI. Motivation: fixtures were `FAIL(stale-orient-receipt)` on master `131cf858a3`, merging red because ungated — the §2 SHA-treadmill inside the harness's own selftests |
