# FIELD_POLICY Field agent Track — Charter, Closing Scenario, and M/E Closure Plan

> **Status:** Active design-authority decision (2026-05-30).
> **Design authority:** Opus 4.8 (delegated Mapping/FIELD_POLICY track authority).
...
> **Purpose:** (1) charter the FIELD_POLICY field agent pipeline as a real track with a **closure
...
Since the sqrt release, the FIELD_POLICY work grew into a 12-fixture ladder
(`OBS-0..4 → EVENT-0..2 → PIPE-0 → ACT-0..2`) building, GPU-resident and guardrail-clean,
the **field-as-policy field agent**: a mobile SimThing reads its local FIELD_POLICY field through a
...
## 2. FIELD_POLICY field agent track — charter
...
single **"FIELD_POLICY Field agent Proposal Pipeline V1"** acceptance packet and accept it as one vertical.
...
The field agent must not grow a parallel economy in fixtures. Therefore:

- **A FIELD_POLICY proposal that dispatches resources MUST route through the accepted Resource Flow
  ...
  the field agent does not get to reintroduce it.)
- **A FIELD_POLICY proposal that commits structurally** (reinforce / withdraw / spawn) fires via the
...
- **A FIELD_POLICY proposal that moves a unit** updates the unit SimThing's own property columns
  (velocity/position) on GPU; it never writes another entity's authoritative state.
- `simthing-sim` never learns "proposal", "observer", or "FIELD_POLICY" — the pipeline lives in
  `simthing-spec`/driver and compiles to flat AccumulatorOp / overlay / threshold registrations.

The FIELD_POLICY-ACT-1 "Phase-E-style proposal consumer" naming is hereby **corrected in intent**: the
...
- **Mobile cohorts** (SimThings) that read the theater's threat/supply field through a
  ...
  reinforce/withdraw when parent `field_urgency` crosses an authored threshold. → FIELD_POLICY field agent
...
| FIELD_POLICY field agent ladder open-ended | Coherent but unbounded | Charter + V1 boundary (§2) — consolidate & accept |
...
**M closes when:** M-JIT-PROD-0 accepted (done), FIELD_POLICY field agent V1 consolidated+accepted, and
Frontier V1 mapping+field agent integration runs end-to-end opt-in/default-off. No new GPU math is
...
| D-2a hard-currency ordering | **B-0 ACCEPTED — Line B/T CLOSED at narrow smoke level (no B-1)** | B-0 does not alter FIELD_POLICY field agent, ClauseThing/L3, Line A, or Line C runtime posture |
| E-11B nested static RF (A-0) | A-0 evidence landed (pending review) | A-0 does not alter FIELD_POLICY field agent, ClauseThing/L3, hard-currency B posture, or Line C map batching closure |
...
1. A `FrontierV1` scenario profile (default Disabled) that opens map + flat-star economy +
   FIELD_POLICY field agent together.
...
- "FIELD_POLICY proposals are fixture-local only" → **FIELD_POLICY proposals may drive SimThing actions within
...
pattern as the accepted first-slice vertical proof, now spanning mapping + economy + field agent.
...
**ACCEPTED (Opus, Mapping/FIELD_POLICY delegation, 2026-05-30) — PASS WITH CONDITIONS.** The Phase M-JIT
...
1. FIELD_POLICY field agent is a chartered track with a **V1 closure boundary**; the fixture ladder stops and consolidates.
2. FIELD_POLICY proposals route through the **real Resource Flow allocator** + Threshold/EmitEvent — no parallel fixture economy, no CPU planner.
...
movement→own-columns); all hard guardrails preserved; the FIELD_POLICY ladder is consolidated/closed; the
...
2. **Softened closure bar.** The charter's criterion (§4) is "runs end-to-end **GPU-resident**."
   The FIELD_POLICY **field agent loop** (observer score → threshold event → compaction → proposal → route) is
   **`ReplayAccepted`, not run** inside Frontier — prior FIELD_POLICY descriptors are *consumed* and route
   *codes* are oracle-checked; only field+urgency and RF allocation are GPU-resident. The implementer
   ...
   entitled to make. The live field agent loop is the **entire field-as-policy thesis**; it is not cosmetic.

**Ruling:**
- **Substrate (mapping + flat-star RF): CLOSED, accepted.** Phase E closed at flat-star.
- **FIELD_POLICY field agent loop: keep the honest label** — `ReplayAccepted`, not `GpuVerified`, inside
...
- **`FrontierV1-5` is withdrawn as a standalone hygiene gate.** The live GPU run of the field agent loop
...
- **Forward horizon (give codex a target, not a loop): `FrontierV2` — a multi-tick *closed-loop*
  run** where field-derived field agent proposals drive unit movement/dispatch that feeds back into the
  field/economy on the next tick, observed for emergent behavior. `FrontierV2` *requires* the field agent
...
- Until `FrontierV2`, the field agent loop stays honestly `ReplayAccepted` and **M/E "substrate closed,
  move on" stands.** This is not a re-block — it is replacing a hygiene gate with a consumer.

### FrontierV1-5 result

FrontierV1-5 executed one live GPU-resident integrated field agent resource-dispatch route inside the default-off FrontierV1 fixture, satisfying the §10 requirement for a live score→threshold→proposal→dispatch run for the bounded single-tick smoke route. It also produced a fixture-only feedback candidate shape for the named FrontierV2 closed-loop consumer. This does not reopen the FIELD_POLICY ladder, does not implement FrontierV2, does not declare phase closure, and does not authorize default runtime wiring. Structural and movement routes remain ReplayAccepted unless separately proven.
...
**Status change (design authority):** the FrontierV1 field agent **resource-dispatch** loop is
...
FrontierV2-0 implemented the first bounded default-off multi-tick closed-loop consumer fixture. It consumed the FrontierV1-5 feedback candidate shape, ran at least two ticks, and verified that field-derived FIELD_POLICY field agent proposal output can feed back into the next tick's field/economy fixture input. This remains fixture/test support only: no ClauseThing implementation, no default runtime wiring, no phase closure, and no FIELD_POLICY ladder reopening.

### FrontierV2-1 result

FrontierV2-1 extended the default-off FrontierV2 closed-loop consumer by deriving fixture-only movement and structural candidates across ticks from the live field agent feedback path. Candidate outputs remain fixture/test support only: no ClauseThing implementation, no default runtime wiring, no phase closure, and no FIELD_POLICY ladder reopening.

### FrontierV2-2 result

FrontierV2-2 extended the default-off FrontierV2 closed-loop consumer by applying fixture-only movement candidates to an own-column shadow state across ticks. The updated shadow position feeds the next tick's source placement without writing another entity's authoritative state. This remains fixture/test support only: no ClauseThing implementation, no default runtime wiring, no phase closure, and no FIELD_POLICY ladder reopening.

### FrontierV2-3 result

FrontierV2-3 extended the default-off FrontierV2 closed-loop consumer by applying fixture-only structural candidates to a BoundaryRequest shadow queue across ticks. The structural shadow context feeds the next tick without production commitment emission or simthing-sim semantic state. This remains fixture/test support only: no ClauseThing implementation, no default runtime wiring, no phase closure, and no FIELD_POLICY ladder reopening.

### FrontierV2-4 result

FrontierV2-4 combined fixture-only movement and structural feedback paths in the default-off FrontierV2 closed-loop consumer. Own-column movement shadow and BoundaryRequest structural shadow both feed downstream ticks without cross-entity writes, production commitment emission, simthing-sim semantic state, or FIELD_POLICY ladder reopening. This remains fixture/test support only: no ClauseThing implementation, no default runtime wiring, and no phase closure.
...
- **AO-WGSL-0** is generic AO GPU performance work, **ACCEPTED (AO-WGSL-0-ACCEPT, 2026-05-30)** as a
  generic GPU performance option. It does not alter FIELD_POLICY field agent posture, ClauseThing/L3, or designer
...
- **A-0 is ACCEPTED (A-0-ACCEPT-0, 2026-05-30)** — static nested Resource Flow first slice; Line A/E
  ...
  closed for their current named scenarios. This does not open E-11B-5 dynamic enrollment, alter FIELD_POLICY
  field agent posture, ClauseThing/L3, or designer semantic-WGSL rejection; `simthing-sim` stays
...
Frontier, FIELD_POLICY, CLAUSE-SPEC, or ClauseThing implementation. L0 remains accepted, L1 remains active,
...
object; `simthing-sim` carries no FrontierV2/FIELD_POLICY/ClauseThing awareness; the L1 diagnostic nit is
...
C-0 remains mapping-line work under v7.8; it does not open ClauseThing/L3 or alter FIELD_POLICY field agent
