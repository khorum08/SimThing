...
**Next action:** **M-JIT-SQRT-0 native WGSL `sqrt` candidate battery landed (test-only)** — local battery classification is `ApproximateJitOnly` (direct scalar corpus shows max ULP=1; Euclidean/gradient magnitude corpora are exact on this platform); baseline runtime remains `sqrt`-free and existing interpreter/JIT-0 fixtures remain green. Deterministic production `sqrt` admission and cross-backend/cross-vendor evidence remain separately gated.
