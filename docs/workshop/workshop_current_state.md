...
**Next action:** **M-5D-gradient landed** — frame/scenario-level admission rejects gradient output columns used as same-frame diffusion `source_col`; M-5B/M-5C valid-sink fixtures green. Dual-output `GradientXY`, `sqrt`, L1 coupling, dense per-cell temporal memory, atlas/M-4A, source-mask (`M-5`), and production economy→mapping bridge remain Tier-2/deferred.
