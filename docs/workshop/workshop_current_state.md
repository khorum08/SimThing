...
**Next action:** **M-JIT-SQRT-0 R1 oracle-order correction landed (test-only remedial pass)** — vector magnitude batteries now compare GPU to shader-text-order CPU oracle as primary; FMA/`mul_add` is diagnostic-only; overall classification remains `ApproximateJitOnly` (direct scalar max ULP=1); baseline runtime remains `sqrt`-free. Deterministic production `sqrt` admission and cross-backend/cross-vendor evidence remain separately gated.
