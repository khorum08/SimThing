...
**Next action:** **M-JIT-GRAD-0 GPU-resident batched spatial observer proof landed (test-only)** — 10,000 observers in one dispatch; semantic-free WGSL computes `dx`/`dy`/descent/`mag2` without `sqrt`; CPU/GPU oracle parity on deterministic subsets; production observer scheduling/caching and JIT cohort dispatch remain separately gated.
