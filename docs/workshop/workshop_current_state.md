...
**Next action:** **M-5D R1 landed** — `compile_region_field_frame_preview` is the safe grouped RegionField admission default (strict-sink validation then per-field compile). Dual-output `GradientXY`, `sqrt`, L1 coupling, dense per-cell temporal memory, atlas/M-4A, source-mask (`M-5`), and production economy→mapping bridge remain Tier-2/deferred.
