...
**Next action (as of post-2E):** EML-GADGET-2E explicit velocity-column Acceleration landed in `simthing-spec` spec/admission/compiler/oracle only. Tier-2 ladder complete through 2E for explicit-column velocity acceleration; position-history acceleration and dense per-cell temporal memory remain separately gated. No runtime gadget execution, chained scheduling, atlas/M-4A, or production economy→mapping bridge.
