...
**Next action:** **M-4A Atlas Readiness Gate completed — DEFER.** No named multi-theater product scenario requires atlas batching; M-5-gradient substrate remains sufficient for current paths. M-4A waits for named scenario + VRAM budget + §11 gate. Dual-output `GradientXY`, `sqrt`, L1 coupling, source-mask (`M-5`), and production economy→mapping bridge remain Tier-2/deferred.
