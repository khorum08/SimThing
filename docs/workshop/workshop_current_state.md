...
**Next action:** **M-JIT-GRAD-1 observer+exact-formula fusion proof landed (test-only)** — one dispatch fuses field sampling, exact-authoritative `dx`/`dy`/descent, and exact-subset score; `mag2` excluded from score; production observer scheduling/caching and JIT cohort dispatch remain separately gated.
