...
**Next action:** **M-JIT-GRAD-0 R1 observer output classification landed (test-only remedial)** — `dx`/`dy`/descent remain exact-authoritative; `mag2` classified `ApproximateJitOnly` on batch corpus (diagnostic/ranking hint only); 10,000-observer one-dispatch proof unchanged; production observer scheduling/caching remains separately gated.
