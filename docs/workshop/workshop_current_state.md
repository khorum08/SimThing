...
**Next action:** **M-5A-gradient landed** — single-target Gradient axis + per-direction stencil weights in `simthing-spec`/`simthing-gpu`. **Next implementation: M-5B-gradient** (L3 Strategic Pressure Composition Pattern RON fixture). Dual-output `GradientXY`, `sqrt`, L1 coupling, dense per-cell temporal memory, atlas/M-4A, source-mask (`M-5`), and production economy→mapping bridge remain Tier-2/deferred.
