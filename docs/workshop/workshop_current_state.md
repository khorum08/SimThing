...
**Next action:** **M-JIT-0 generic EvalEML WGSL emission prototype landed (test-only)** — admitted WeightedAccumulator/Ema gadget graphs lower to deterministic semantic-free straight-line WGSL, compile through wgpu, and match `eval_eml_postfix`/`eval_eml_cpu`/named oracles bit-exactly; the existing EvalEML interpreter runtime fixture stays green. Production JIT caching/cohort dispatch, chained gadget-stack scheduling, automatic snapshot/copy scheduling, and production multi-gadget runtime wiring remain separately gated.
