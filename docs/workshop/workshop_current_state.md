...
**Next action:** **EML Gadget Library is the next implementation track** ([`eml_gadget_library_design_note.md`](eml_gadget_library_design_note.md)) — approved by Opus 2026-05-29 and **sequenced BEFORE Phase M Resource Economy Authoring Ergonomics R2**, so R2's designer-facing authoring can expose/leverage it. Gadgets are RON-authored EML node-template macros over the existing `EvalEML` opcodes (no new WGSL) in `simthing-spec`: EML-GADGET-1 (Tier-1 stateless: `FieldSampler`, `WeightedAccumulator`, `SoftStep`, mandatory CPU-oracle parity), then EML-GADGET-2 (Tier-2 temporal-memory slice: `VelocityMonitor`, `Decay`/EMA, hysteresis + bounded-feedback admission guardrail). The product-fixture chain is ACCEPTED ([`../reviews/phase_m_product_fixture_chain_acceptance_opus_review.md`](../reviews/phase_m_product_fixture_chain_acceptance_opus_review.md)). Still **not** a generic boundary-output packet (D), **not** the M-4 atlas packer (E). Not default session wiring; `MappingExecutionProfile` default remains `Disabled`.
