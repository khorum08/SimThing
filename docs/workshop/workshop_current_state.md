...
**Next action:** **M-JIT-DESC-0 kernel descriptor/admission manifest landed (test-only)** — exact vs approximate output authority explicit across landed JIT/observer proofs; production descriptor registry and observer/JIT scheduling remain separately gated.
