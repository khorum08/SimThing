...
**Next action:** **M-6A Single-Grid Active Mask Readiness Gate completed — DEFER.** GPU active-mask hook is generic but `ActiveOnlyExperimentalNoHalo` is constitutionally blocked pending halo contract + CPU/GPU parity. M-4A atlas and M-6 active mask both deferred. Dual-output `GradientXY`, `sqrt`, L1 coupling, source-mask (`M-5`), and production economy→mapping bridge remain Tier-2/deferred.
