//! FIELD_POLICY tensor/stencil WGSL prototype probe — structured field kernel vs per-edge AccumulatorOp.
...
fn guard_no_production_pipeline_integration() {
    ...
    assert!(!passes.contains("field_policy_tensor_stencil"));
}
...
fn test7_wgsl_generality_constitutional_review() {
    let wgsl = include_str!("../../simthing-gpu/src/shaders/field_policy_tensor_stencil_prototype.wgsl");
