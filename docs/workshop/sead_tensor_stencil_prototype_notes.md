# SEAD tensor/stencil WGSL prototype notes
...
Single WGSL entry point `stencil_step` in `sead_tensor_stencil_prototype.wgsl`.
...
|---|---|---|---|---|
| `sead_tensor_stencil_raw_additive_prototype.wgsl` | 0 | 1.0 | 0.8 | blowup baseline |
| `sead_tensor_stencil_normalized_prototype.wgsl` | 1 | 1.0 | 0.8 | **recommended** |
| `sead_tensor_stencil_decayed_prototype.wgsl` | 4 | 0.8 | 0.2 | stable but weak horizon |
| `sead_tensor_stencil_clamped_prototype.wgsl` | 3 | 1.0 | 0.8 | cap=10000, saturates |

Directed variant (2) requires directed setup seeding; not equivalent to NSEW one-hop setup used in horizon tests.

## Harness

Rust prototype: `crates/simthing-gpu/src/sead_tensor_stencil_prototype.rs`  
Integration test: `crates/simthing-driver/tests/sead_tensor_stencil_wgsl_sandbox.rs`
...
All production paths under `crates/simthing-gpu/src/shaders/sead_tensor_stencil_prototype.wgsl`, `sead_tensor_stencil_prototype.rs`, and driver test are removed on revert. Preserved copies remain under `docs/workshop/`.
