...
The proposal is therefore narrow and constitutionally clean:
...
It is **not** "run Stellaris on SimThing." We adopt the *grammar* and the
*economically/spatially meaningful subset* of the language, compiled to SimThing's
substrate. We do **not** implement the Stellaris standard library of thousands of
named effects/triggers, the binary save format, or any engine-specific behavior.
The honest scope is §8 and the non-goals are §10.
...
>1 GB/s parser. Binary save format support exists in `jomini` but is **out of scope**
(§10) — ClauseThing ingests authoring `.txt`, not save games.
...
## 8. Honest limits & hard problems

The textbook is candid that ClauseScript is *"recklessly flexible."* We are equally
candid about what does **not** translate cleanly:

- **The Stellaris standard library does not come along.** Thousands of named effects
  (`set_owner`, `create_fleet`, …), triggers, and the 3.8 MB of generated modifier
  keys are *engine* semantics, not language semantics. We support the **grammar** and
  a **declared, bounded vocabulary** the designer maps to SimThing columns/arenas. An
  unknown effect/trigger is a hard admission error with a diagnostic, never a silent
  no-op.
...
- **Not** a Stellaris emulator; no engine-specific effect/trigger/modifier library.
- **Not** a binary save-format reader (`jomini` can, we will not).
- **Not** a runtime authorization — no default wiring, no new primitive, no WGSL.
- **Not** a CPU AI planner or semantic sidecar; `ai_will_do` is GPU-read data.
- **Not** a relaxation of any invariant; `invariants.md` stays binding and any change
  to it is Tier-2.
- **Not** a replacement for RON authoring; ClauseScript is a *second* front-end into
  the same `simthing-spec` contract. RON remains valid and authoritative.
- **Not** localization/presentation tooling (`custom_tooltip`, `defined_text`,
  portrait/name lists are dropped before runtime).
- **Not** an expanding language surface.** Admitting a new ClauseScript construct
  (effect, trigger variant, math form) beyond what is declared in the accepted tier
  is itself a **gated decision** — Tier-1 if strictly within accepted design and
  generic, Tier-2 otherwise. The non-goals list does not enforce itself; fidelity
  creep is real and requires active gate discipline on every PR that touches the
  compiler.
...
*ClauseThing is a designer-facing dialect spoken at SimThing's existing spec/admission
layer. It changes how designers write, not how the simulation runs. The runtime stays
flat, deterministic, GPU-resident, replayable, and semantic-free — and never learns
that it is, in part, speaking Clausewitz.*
