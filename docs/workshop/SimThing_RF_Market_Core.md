...
The upward stroke receives market pressure. The downward stroke disburses the continuously resolved
flow. Exact settlement is the terminal continuous-to-discrete stage of that same program.
...
## 0.1 Candidate identity sentence

> **The GPU-resident RF market endpoint is the terminal exact-settlement stage of the Field Triad’s
> ordinary reduce/disburse cycle. Every StemThing intrinsically receives descendant market
> pressure, disburses Triad-resolved constrained flow, and settles that flow into exact grants for
> its children. Every child carries the same germ, so a settled grant becomes that child’s lawful
> constrained supply for its descendants. The endpoint does not independently value, route, or
> model claims.**

## 0.2 The intended deletion
...
This is “receiving” in the market sense. It is not receipt of possession; it is receipt of
legible downstream demand and constraint pressure.
...
## 1.3 Downward stroke — disburse continuous flow
...
## 1.4 Edge — settle exact possession
...
## 1.5 Re-enter — grant becomes child supply
...
- `N_c` is lawful need surfaced by child `c`;
...
intrinsic / accumulated world-level supply
        +
recursively reduced subtree demand
        ↓
...
invoke a route solver, or author another model.
...
> **No new persistent per-claim state is admitted merely to make settlement field-aware.**

`NativeClearingPotential`, `FieldShare`, and `ClaimFieldView` remain explanatory terms until the
surface-reuse archaeology proves that no existing surface carries the required meaning.
...
```text
children's IntrinsicFlow + AllocatorWeight
                ↓ upsweep
parent intrinsic-flow sum + weight sum
                ↓ broadcast
...
production meaning, units, sign, bounds, scope, and ordering are sufficient for constrained exact
settlement.

## 4.5 StemThing-B market vocabulary
...
collapse unit price, hard precedence, continuous share, and exact grant into one scalar.

## 4.6 Exact clearing oracle
...
## 4.7 Grant lifecycle and holding accounts
...
## 4.8 Generation and async seams
...
The intended continuous result is claimant-local allocated flow after lawful need, policy, route
impedance, competition, and channel capacity have already interacted.
...
It may not re-run the continuous market.
...
continuous allocation
    RF/Triad-resolved soft share within a precedence class
```
...
# 8. Urgency, persistence, and market transparency
...
Therefore innate first-order urgency persistence must not be claimed until archaeology proves the
Current→Next behavior of the ordinary `Need`/`Balance`/STEAD lanes.
...
## 8.3 Candidate native persistence law

If archaeology finds first-order persistence missing, the Owner may amend the baseline:

> Every unresolved lawful quantity remains or re-enters the claimant’s STEAD pressure in the next
> generation through the ordinary bounded Current→Next field recurrence. Authored EML may deform
> decay, saturation, escalation, or policy response, but does not create first-order persistence.

The update must remain bounded. No unbounded positive recurrence and no per-claim age-counter service is
admitted by this candidate law.

## 8.4 Market transparency law
...
Current N sealed
    ↓
reduce need and pressure upward
    ↓
...
| Continuous weight input | `AllocatorWeight` | Existing RF role | **REUSE WITH BINDING** | Prove Triad/overlay state already feeds or can lawfully feed it without clearing-private projection |
...
| Continuous child share | `child_share_formula` + `AllocatedFlow` | Strong presumptive existing shape | **REUSE WITH BINDING** | Prove units, sign, request/supply bounds, clearing-home scope, and post-Triad ordering |
...
| STEAD stakes/pressure | existing resident pressure/Need fields | Born field output | **REUSE AS-IS** | Prove which sealed Current lane informs the continuous market |
...
| Urgency persistence | `Balance`/Need/STEAD recurrence; authored persistence path | Authority unresolved | **MISSING — STOP FOR OWNER RULING** | Classify persistence vs accumulation vs authored-only consequence |
...
Add the following witnesses:

```text
root → child → grandchild recursive grant-to-supply re-entry
...
next-generation persistence/urgency disposition
```

Prove:

- zero additional field dispatch;
- zero new persistent clearing potential/share state;
- zero CPU preweight or route/flux lookup;
...
```text
host-built weight table
CPU PALMA query
CPU congestion/Gu-Yang query
clearing-owned field cache
...
## 13.2 Terminal Exact Settlement Law
...
## 13.3 Receive-Not-Recompute Law

> The settlement stage receives already-resolved resident RF/Triad outputs. It may not privately
> reduce STEAD, relax PALMA, solve Gu-Yang, author urgency, query host state, or materialize a
> clearing-owned field/weight/share representation.

## 13.4 Recursive Grant-to-Supply Law
...
## 13.5 Continuous/Discrete Authority Law

> The Field Triad owns continuous value, route impedance, and realizable conservative flow. Exact
> settlement owns only exact quantity conservation, discrete residue, commitment-state transition,
> provenance, and replay.

## 13.6 Gu-Yang Authority Law

> Gu-Yang remains capacity and realization authority. No weight, policy, hard-precedence class, or
> settlement residue may manufacture flow the conservative field forbids.

## 13.7 Hard-Precedence/Soft-Share Law

> Hard precedence is explicit admitted policy. Continuous RF/Triad allocation governs soft sharing
> within a precedence class. Arbitrary floating-point field differences may not silently become strict
> exact-score winner-take-all precedence.

## 13.8 Market Transparency Law
...
## 13.10 Native Persistence Law — conditional
...
> EML deforms persistence; it does not create the baseline.

This law must not be adopted if existing `Balance`/Need/STEAD recurrence already supplies it.
...
2. Adds a persistent per-claim potential/share/urgency plane before proving the existing surface
   insufficient.
3. Runs a private PALMA, Gu-Yang, or STEAD computation in settlement.
4. Requires a host preweight, route lookup, congestion lookup, or weight upload before resident
   settlement.
5. Uses continuous float differences as strict exact score bands without explicit authored hard
   precedence.
6. Lets atomic append, upload order, physical row, workgroup schedule, or dispatch partition decide a
   grant.
7. Cannot trace parent grant → child supply → grandchild disbursement through the same germ.
8. Creates a second demand vocabulary with independent authority beside RF Need/Balance.
9. Conflates `U` with CostBand remainder `R`.
10. Conflates pre-settlement impairment, contention shortfall, and post-grant delivery shortfall.
11. Grants immediately executable flow beyond Gu-Yang serviceability.
12. Forbids lawful entitlement-first `in_flight` commitment merely because current delivery is blocked.
13. Hides demand, impairment, or unmet quantity from the governing scope without an authored policy.
14. Builds same-generation reweight/re-clear convergence.
15. Requires synchronous ancestor RPC for unrelated local subtree generations.
16. Creates a second schedule/history or permits state commit without its exact schedule rows.
17. Uses a root-only or leaf-only special market path instead of the same degenerate germ.
18. Preserves CPU collection shape merely because it is the oracle, rather than extracting the exact
    settlement law.
...
## 15.2 Which existing surface owns hard precedence?
...
## 15.3 Does unresolved lawful demand persist or accumulate natively?

Trace the exact Current→Next path for `Need`, `Balance`, STEAD pressure, and `U`.

Do not infer authority from the existence of a `field_urgency` formula whose runtime may be preview or
proof-only.

## 15.4 Where exactly does settlement land in the existing band layout?
...
## 15.5 What is the exact continuous-to-integer contract?
...
## 15.6 How does the CPU oracle generalize?

The CPU oracle must remain an exact referee for:

```text
neutral proportional case
field-resolved continuous-share case
...
> **Every StemThing is an RF market germ. Descendant need and deficit reduce upward; the RF Field
> Triad resolves the continuous market; allocated flow disburses downward; exact settlement turns
> that resolved flow into discrete identity-bearing possession; the child then repeats the same
> cycle with its own descendants. Receiving, clearing, and disbursement are not peer facilities but
> stages of one recursive market filter. The exact endpoint receives rather than recomputes: it owns
> no route, flux, urgency, weight, or field model—only exact conservation, integer residue,
...
program while preserving the exact settlement oracle as proof.
