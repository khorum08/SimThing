...
| **Shader/software deterministic sqrt** | **T2** | **design-open** | Active design note: [`sqrt_candidates.md`](sqrt_candidates.md). Native sqrt remains `ApproximateJitOnly`; exact sqrt authority requires exhaustive bit-exact proof (`max_ulp == 0` vs CPU `f32::sqrt`) before admission may permit exact sqrt outputs. Follow-on track — not a blocker to M-JIT closure. |
