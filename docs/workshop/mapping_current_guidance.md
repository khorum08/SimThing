...
- **Next consumer (`RUNTIME-0080-RR-OPEN-0`, OPEN): recursive galaxy→system→planet-surface rehearsal.** Builds the dress-rehearsal spec with no tier collapse: nested galaxy→system(10×10)→planet→surface(10×10) residency (generalize atlas_0080_0) + pop/factory/labor planet-surface economy + proven R2 galactic loop, 100 ticks vs recursive CPU oracle. Ladder RR-0→RR-4; Scope Ledger per rung. Opening: [`../production_paths/runtime_0080_recursive_rehearsal_opening.md`](../production_paths/runtime_0080_recursive_rehearsal_opening.md). **SUPERSEDES** the mis-scoped `RUNTIME-0080-M4A-OPEN-0` (parallel theaters); M-4A residency consumed here as the nested mechanism.
