...
> open Line C implementation gate.** **B-0 (hard-currency ordering) is ACCEPTED (B-0-ACCEPT-0) — Line B is CLOSED at the narrow smoke level; no B-1.** **A-0 (nested RF) implementation evidence landed — pending Opus review:** static nested arena materialization and D=3/D=4 GPU/CPU oracle parity; no default-on Resource Flow or dynamic enrollment. **L3 — ClauseThing / ClauseScript — is parked pending separate product authorization;
