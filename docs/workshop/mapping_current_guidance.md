...
| **Shader/software deterministic sqrt** | **T2** | **SQRT-EXACT-1D landed (Candidate D probe)** | Design note: [`sqrt_candidates.md`](sqrt_candidates.md). SQRT-EXACT-0: [`phase_m_jit_sqrt_exact_candidate_battery_results.md`](../tests/phase_m_jit_sqrt_exact_candidate_battery_results.md). SQRT-EXACT-1D: [`phase_m_jit_sqrt_exact1d_candidate_d_results.md`](../tests/phase_m_jit_sqrt_exact1d_candidate_d_results.md). D (`CorrectlyRoundedHwBitmask`) test-only; corrections fire on DX12; subnormal FTZ unresolved; C/f64 rejected; A legacy dead; B fallback. Native sqrt remains `ApproximateJitOnly`. M-JIT closure unchanged. |
