...
| **M-JIT-GRAD-0** (GPU-resident batched spatial field observer) | **T2** | **landed (test-only)** | semantic-free observer WGSL; 10,000 observers in 1 dispatch; central-difference `dx`/`dy`, descent, `mag2` (no `sqrt`); CPU/GPU oracle parity on small grid + sampled batch subset; production observer scheduling/caching remains separately gated; [`phase_m_jit_grad0_spatial_observer_test_results.md`](../tests/phase_m_jit_grad0_spatial_observer_test_results.md) |

**Next authorized step:** **M-JIT-GRAD-0 batched spatial observer proof landed (test-only).** Production observer scheduling/caching, production JIT cohort dispatch, production exact `sqrt` admission, chained gadget-stack scheduling, automatic snapshot/copy scheduling, and production multi-gadget runtime wiring remain separately gated. Atlas/M-4A, active mask/M-6A, source-mask, dual-output `GradientXY`, and production economy→mapping bridge remain Tier-2/deferred.
