...
**Next authorized step:** **M-JIT-GRAD-1 observer+exact-formula fusion proof landed (test-only).** Production observer scheduling/caching and production JIT cohort dispatch remain separately gated. `mag2` remains diagnostic-only (GRAD-0 R1). Production exact `sqrt` admission, chained gadget-stack scheduling, automatic snapshot/copy scheduling, and production multi-gadget runtime wiring remain separately gated. Atlas/M-4A, active mask/M-6A, source-mask, dual-output `GradientXY`, and production economy→mapping bridge remain Tier-2/deferred.
