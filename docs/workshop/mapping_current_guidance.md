...
| **M-5D-gradient** (gradient strict-sink / no within-frame feedback admission) | **T1** | **landed** | [`phase_m_m5d_gradient_sink_admission_test_results.md`](../tests/phase_m_m5d_gradient_sink_admission_test_results.md) |
...
**Next authorized step:** **M-5A/B/C/D-gradient landed.** M-5D enforces frame-level gradient strict-sink admission (rejects same-frame derivative feedback into diffusion `source_col`). Dual-output `GradientXY`, `sqrt`, L1 coupling, dense per-cell temporal memory, atlas/M-4A, source-mask (`M-5`), and production economy→mapping bridge remain Tier-2/deferred.
