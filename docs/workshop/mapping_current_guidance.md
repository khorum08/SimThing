...
| **Frontier V1 — named closing scenario (M + E)** | **T2** | **FrontierV1-0 skeleton landed** | Single-theater strategic vertical that closes Phase M and Phase E on accepted substrates: one ≤32×32 grid (first-slice mapping, no atlas/mask), two flat-star depth-2 faction economies (FlatStarResourceFlow, no nested E-11B), mobile cohorts self-directing on the field via exact F-magnitude + SEAD self-AI, commitments via Threshold/EmitEvent. One opt-in `FrontierV1` profile, **default Disabled**. **FrontierV1-0:** default-off FrontierV1 scenario skeleton and admission contract validates one ≤32×32 first-slice RegionCell theater + flat-star Resource Flow + SEAD Self-AI Proposal Pipeline V1 routing. No runtime wiring yet. Next: FrontierV1-1 opt-in end-to-end fixture. Report: [`phase_m_frontier_v1_0_scenario_skeleton_results.md`](../tests/phase_m_frontier_v1_0_scenario_skeleton_results.md). Charter: [`sead_self_ai_track.md`](sead_self_ai_track.md). |
