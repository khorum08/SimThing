...
| **M-JIT-SQRT-0** (native WGSL `sqrt` candidate battery) | **T2** | **landed (test-only, approximate)** | direct scalar `sqrt` shows non-bit-exact local behavior (max ULP=1) while magnitude batteries are exact on this platform; classified `ApproximateJitOnly`; deterministic production `sqrt` remains blocked and requires separate cross-backend/cross-vendor gate; [`phase_m_jit_sqrt_candidate_battery_test_results.md`](../tests/phase_m_jit_sqrt_candidate_battery_test_results.md) |

**Next authorized step:** **M-5E-gradient + EML-GADGET runtime execution gate fixture + M-JIT-0 landed; M-JIT-SQRT-0 candidate battery classified `ApproximateJitOnly`.** Production exact `sqrt` admission, production JIT caching/cohort dispatch, chained gadget-stack scheduling, automatic snapshot/copy scheduling, and production multi-gadget runtime wiring remain separately gated. Atlas/M-4A, active mask/M-6A, source-mask, dual-output `GradientXY`, and production economy→mapping bridge remain Tier-2/deferred.
