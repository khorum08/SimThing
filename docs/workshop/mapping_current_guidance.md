...
| **Shader/software deterministic sqrt** | **T2** | **SQRT-MAG-0 landed** | Design note: [`sqrt_candidates.md`](sqrt_candidates.md). SQRT-PROMOTE-0: [`phase_m_jit_sqrt_promote0_descriptor_admission_results.md`](../tests/phase_m_jit_sqrt_promote0_descriptor_admission_results.md). SQRT-MAG-0: [`phase_m_jit_sqrt_mag0_f_exact_magnitude_results.md`](../tests/phase_m_jit_sqrt_mag0_f_exact_magnitude_results.md). **SQRT-MAG-0 landed:** artifact-backed F exact sqrt now supports exact Euclidean magnitude in a default-off/test-only JIT path (`m_jit_mag_f_exact`); native sqrt and diagnostic `mag2` remain non-exact unless routed through exact F; 34k mobile-simthing benchmark recorded. F artifact hash `e2e9e27601ee2e13`. No default runtime wiring/scheduler/cache/bridge. |
