...
> open Line C implementation gate.** **B-0 (hard-currency ordering) is ACCEPTED (B-0-ACCEPT-0) — Line B is CLOSED at the narrow smoke level; no B-1.** **A-0 (nested RF) implementation evidence landed — pending Opus review:** static nested arena materialization and D=3/D=4 GPU/CPU oracle parity; no default-on Resource Flow or dynamic enrollment. **A-0-R1:** verification remediation only — stale E-11B no-new-WGSL whitelist now recognizes accepted C-0 atlas WGSL while preserving the guard that A-0 adds no Resource Flow WGSL. **AO-WGSL-0:** generic semantic-free AccumulatorOp WGSL performance path. Authorized by named gate after WGSL-GUARD-0/R1; designer-authored semantic WGSL remains rejected. **L3 — ClauseThing / ClauseScript — is parked pending separate product authorization;
