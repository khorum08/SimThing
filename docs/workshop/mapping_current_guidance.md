...
- **`RUNTIME-0080-0 R1a` IMPLEMENTED / PARTIAL (REMEDIAL) — fake PASS removed:** Audit found R1a-IMPL-0 did **not** earn the claim: the CPU recomputed the full Tier-A next-state and injected it into the GPU each tick (`COL_JOURNAL_DELTA`), while the GPU "tick" was three `Identity` copies + swap. The remedial code/report now deletes that producer, leaves all R1a per-column authority/parity flags false, measures inter-tick Tier-A uploads/readbacks/dispatches/oracle writes-after-seed as `0`, and names the exact remaining gap: integrated `WorldGpuState`/`Pipelines` Tier-A transition registration must compute `state_N+1` on GPU before the negative control can be earned. Report: [`../tests/runtime_0080_0_r1a_next_tick_authority_results.md`](../tests/runtime_0080_0_r1a_next_tick_authority_results.md). **Opcode/WGSL-gate clarification (Opus, 2026-06-05):** a generic, semantic-free EML opcode / `AccumulatorOp` combine fn / kernel is a Tier-2 gate (handoff §4a), not a ban; semantic ops stay banned and anti-faking is unchanged. Handoff: [`../handoffs/runtime_0080_0_r1a_remedial_opening.md`](../handoffs/runtime_0080_0_r1a_remedial_opening.md); spec §14/§14.1: [`../production_paths/runtime_0080_0_r1_next_tick_authority_spec.md`](../production_paths/runtime_0080_0_r1_next_tick_authority_spec.md). **R1b/R1c remain parked:** R1b (`RESIDENT-EVENTLOG-0`) → R1c (`RESIDENT-REENROLL-0`).
