...
> territory (no implementation gate):** [`mobility_and_transfer_allocation.md`](mobility_and_transfer_allocation.md)
...
> MOBILITY-SCENARIO-0 first slice; ALLOC is only the next candidate ladder and remains unopened.
...
| **MOBILITY-SCENARIO-0 — v7.9 mobility/transfer scenario admission packet** | **T2** | **ACCEPTED (MOBILITY-SCENARIO-0-ACCEPT-0, 2026-06-01); MOBILITY-AUDIT-0 PASS** | Bounded first-slice scenario (depth 4, k≤4, 48 cells, 34k soak, narrowed adversarial routing) accepted as the named v7.9 scenario; guardrail rejections wired/tested. `owner_band_budget_audit` requires 13 of current 16 OrderBands, so no narrowing or depth expansion scenario is needed. ALLOC is the next candidate ladder only; ALLOC/REENROLL/IDROUTE/ECON/OWNER stay parked; no runtime gate. Reports: [`phase_mobility_scenario0_acceptance_review_results.md`](../tests/phase_mobility_scenario0_acceptance_review_results.md), [`phase_mobility_owner_band_budget_audit_results.md`](../tests/phase_mobility_owner_band_budget_audit_results.md). |
