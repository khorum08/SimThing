...
- **`RUNTIME-0080-0 R1c-f` PASS:** GPU-decided `ZeroCohort` from resident `num_ships` (`RESIDENT-ZERO-COHORT-GPU-DECIDE-0`). Per-tick combat-attrition probe + threshold/emission-band emits ZeroCohort journal rows; CPU witness no longer decides that class; boundary pass applies GPU rows only. `structural_decisions_gpu_emitted_zero_cohort = true`; umbrella `structural_decisions_gpu_emitted` remains false. M-4A/multi-atlas remains parked. Report: [`../tests/runtime_0080_0_r1c_f_resident_zero_cohort_decision_results.md`](../tests/runtime_0080_0_r1c_f_resident_zero_cohort_decision_results.md).
