...
> open Line C implementation gate.** **B-0 (hard-currency ordering) is ACCEPTED (B-0-ACCEPT-0) — Line B is CLOSED at the narrow smoke level; no B-1.** Authored `order_band` wired through deterministic boundary scheduling to existing AccumulatorOp OrderBand execution with exact CPU-oracle parity; no new WGSL, no Resource Flow substitution, no global scheduler. **A-0 (nested RF) is now the ONLY accepted, queued M/E/T line not yet implemented** — opening it is a product decision. **L3 — ClauseThing / ClauseScript — is parked pending separate product authorization;
