...
2B VelocityMonitor + Decay/EMA, 2C BoundedFeedback, and 2D Hysteresis (2D R1 exact CMP/SELECT compiler parity) have landed in simthing-spec. Acceleration and dense per-cell temporal memory remain deferred. No runtime gadget execution, chained scheduling, new opcode/WGSL, simthing-sim semantics, production economy→mapping bridge, default mapping wiring, or atlas/M-4A landed.
