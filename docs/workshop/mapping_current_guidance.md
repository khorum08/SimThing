...
**Next authorized step:** **M-JIT-GRAD-0 R1 observer output classification complete (test-only).** Exact-authoritative observer outputs are `dx`/`dy`/descent; `mag2` is bounded diagnostic only on batch corpus. Production observer scheduling/caching, production JIT cohort dispatch, production exact `sqrt` admission, chained gadget-stack scheduling, automatic snapshot/copy scheduling, and production multi-gadget runtime wiring remain separately gated. Atlas/M-4A, active mask/M-6A, source-mask, dual-output `GradientXY`, and production economy→mapping bridge remain Tier-2/deferred.
