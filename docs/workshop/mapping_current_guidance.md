...
- **`RUNTIME-0080-0 R1a` PASS:** Tier-A GPU-side next-tick source-of-truth hardened against oracle-fed replay; per-tick inputs derived from `R1aBoundaryWitness` + resident GPU readback (no R6C per-tick answer tables); exact-bit proofs + disabled-transform parity check earned; Tier-B structural residency remains future work. Report: [`../tests/runtime_0080_0_r1a_next_tick_authority_results.md`](../tests/runtime_0080_0_r1a_next_tick_authority_results.md). Remaining horizons: R1c-a free-list mark-only, resident REENROLL/scatter/compact, M-4A, recursion, multi-faction ECON.
