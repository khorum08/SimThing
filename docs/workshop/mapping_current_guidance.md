...
**Next authorized step:** **M-5A-gradient landed.** Single-target `Gradient { axis, output_col }` + generic per-direction stencil weights in `simthing-spec`/`simthing-gpu`. **Next: M-5B-gradient** (L3 composition RON fixture). Dual-output `GradientXY`, `sqrt`, L1 coupling, dense per-cell temporal memory, atlas/M-4A, source-mask (`M-5`), and production economy→mapping bridge remain Tier-2/deferred.
