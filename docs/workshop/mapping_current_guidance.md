...
- **`RUNTIME-0080-0-R0` R0A corrected: PARTIAL CPU-authoritative dispatch** (report: [`../tests/runtime_0080_0_r0_results.md`](../tests/runtime_0080_0_r0_results.md)). Persistent GPU session + per-tick shape dispatches preserved; CPU remains tick authority; checksum `1bba891c779190a4`. **Next step:** GPU-resident next-tick authority substrate primitive (Tier-2 rung) before whole-run `GPU-measured` claim. **Parked horizon:** multi-atlas/M-4A (§11 gate), richer emergence (`SCENARIO-0080-3`), multi-faction ECON, recursion.
