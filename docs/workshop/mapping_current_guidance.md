...
| **Frontier V1 — named closing scenario (M + E)** | **T2** | **Substrate ACCEPTED (Opus) — M/E "substrate closed, move on"; self-AI loop `ReplayAccepted`, live GPU proof folded into first consumer (FrontierV2)** | Single-theater strategic vertical that closes Phase M and Phase E on accepted substrates: one ≤32×32 grid (first-slice mapping, no atlas/mask), two flat-star depth-2 faction economies (FlatStarResourceFlow, no nested E-11B), mobile cohorts self-directing on the field via exact F-magnitude + SEAD self-AI, commitments via Threshold/EmitEvent. One opt-in `FrontierV1` profile, **default Disabled**. **Design-authority review (Opus, 2026-05-30 — `sead_self_ai_track.md` §10):** the implementer's `FrontierV1-ACCEPT-0` / `FrontierV1-POSTACCEPT-0` are *self-acceptance* and do not constitute M/E closure (Tier-2 reserves acceptance to design-authority + product). **Substrate (mapping + flat-star RF) is GPU-verified and ACCEPTED → Phase E closes at flat-star.** But the **SEAD self-AI loop is `ReplayAccepted`, not GPU-run** inside Frontier (route codes oracle-checked + prior descriptors consumed; only field+urgency and RF run on GPU). **Amendment (after product feedback):** the standalone `FrontierV1-5` proof gate is **withdrawn** — a consumer-less "prove the loop on GPU" gate re-creates the "no consumer" stall that makes codex loop on hygiene. Instead the live GPU run of the self-AI loop is **folded into the first consumer scenario, `FrontierV2`** (a multi-tick closed-loop run where field-derived proposals drive unit movement/dispatch that feeds back into the field/economy — the natural ClauseThing stepping-stone). That scenario *consumes* the loop, so the proof comes for free. Until `FrontierV2` the loop stays honestly `ReplayAccepted` and **"substrate closed, move on" stands.** ACT/EVENT/OBS/PIPE ladder stays closed. **`FrontierV2` is the forward horizon — build toward it, not hygiene passes.** Reports: [`phase_m_frontier_v1_0_scenario_skeleton_results.md`](../tests/phase_m_frontier_v1_0_scenario_skeleton_results.md), [`phase_m_frontier_v1_1_opt_in_fixture_results.md`](../tests/phase_m_frontier_v1_1_opt_in_fixture_results.md), [`phase_m_frontier_v1_2_gpu_replay_acceptance_results.md`](../tests/phase_m_frontier_v1_2_gpu_replay_acceptance_results.md), [`phase_m_frontier_v1_3_gpu_resource_flow_results.md`](../tests/phase_m_frontier_v1_3_gpu_resource_flow_results.md), [`phase_m_frontier_v1_4_sead_route_replay_results.md`](../tests/phase_m_frontier_v1_4_sead_route_replay_results.md), [`phase_m_frontier_v1_acceptance_review_results.md`](../tests/phase_m_frontier_v1_acceptance_review_results.md), [`phase_m_frontier_v1_post_acceptance_roadmap_results.md`](../tests/phase_m_frontier_v1_post_acceptance_roadmap_results.md). Charter: [`sead_self_ai_track.md`](sead_self_ai_track.md). |
