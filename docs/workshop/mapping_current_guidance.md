...
| **Frontier V2 — multi-tick closed-loop consumer** | **T2** | **FrontierV2-0 landed (fixture/test support only; no phase closure; ClauseThing NotImplemented)** | **FrontierV2-0:** first default-off multi-tick closed-loop consumer fixture. Consumes FrontierV1-5 feedback candidate; runs two ticks; field-derived SEAD proposals drive resource dispatch and fixture-only next-tick feedback (seed deltas). Movement/structural candidates are FixtureCandidate. ClauseThing remains unimplemented; no phase closure declared. Report: [`phase_m_frontier_v2_0_closed_loop_consumer_results.md`](../tests/phase_m_frontier_v2_0_closed_loop_consumer_results.md). |
