...
> (substrates + RUNTIME-0 green; RUNTIME-1A CPU-only production fixture authorized; RUNTIME-1B GPU pass-graph closed). (Charter:
...
| **MOBILITY-SCENARIO-0 — v7.9 mobility/transfer scenario admission packet** | **T2** | **ACCEPTED; substrate ladder complete through MOBILITY-RUNTIME-0** | ALLOC, REENROLL, IDROUTE(+R1), ECON-0, OWNER-0(+R1), and RUNTIME-0 composition harness reports are green. Real production `SimSession`/GPU wiring remains closed. |
