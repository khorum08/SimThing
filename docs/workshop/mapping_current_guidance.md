...
- `SCENARIO-0080-2` **COMPLETE (`SCENARIO-0080-2-COMPLETE-0`, 2026-06-04, design authority)** — closed *with caveats recorded as findings*: R6C ran the assembled R1→R6B stack over one mutable 100-tick session with write-back (detector table, race curve, checksum `1bba891c779190a4`), R7 reclosed with that evidence, and the mechanism chain holds. Findings (not defects): Terran patrols never crossed the movement threshold (one-sided contest), the production/attrition race is unresolved in 100 ticks, and front/self-sustaining-loop behaviors only partially emerged — these feed a *future* `SCENARIO-0080-3`, not opened now. Run is GPU-conformant + CPU-oracle verified; R6C whole-run GPU execution remains unmeasured. R6C report: [`../tests/scenario_0080_2_r6c_integrated_run_report.md`](../tests/scenario_0080_2_r6c_integrated_run_report.md); closeout: [`../tests/scenario_0080_2_r7_closeout_report.md`](../tests/scenario_0080_2_r7_closeout_report.md).
- **`GPU-MEASURE-0080-0` IMPLEMENTED / PASS** (report: [`../tests/gpu_measure_0080_0_results.md`](../tests/gpu_measure_0080_0_results.md)). The accepted rehearsal row/mask/threshold/emission-band shapes were measured on the discrete GPU: R1, R2, R6, and R6B are `GPU-measured (integer bit-exact)`; R4 is `GPU-measured (verified-approximate, within accepted f32 bound)`. No new semantic WGSL, no new op, no invariant edit, no pinned-number change, no default SimSession wiring, and no reopen of 0080-2. R6C whole-run posture remains exactly `GPU-conformant; GPU execution not yet measured`.
- **Next named track: `RUNTIME-0080-0` — production runtime tick scheduler over sparse-resident GPU shapes** (`GPU-MEASURE-0080-0-COMPLETE-0` ruling C; design opening: [`../production_paths/runtime_0080_0_opening_spec.md`](../production_paths/runtime_0080_0_opening_spec.md); implementation recipient Cursor/Codex5.5max). Realizes the parked M-4A sparse-residency scheduler / Atlas production runtime and **subsumes the R6C whole-run GPU gap** (a runtime-substrate gap, not a measurement gap). First rung R0: run the R6C 100-tick loop with state GPU-resident across ticks for the single galactic tier, dispatching the measured per-tick shapes via ATLAS-0080-0 residency + GPU-EXEC/AccumulatorOp paths, asserting CPU-oracle parity (incl. checksum `1bba891c779190a4`). Stays single resident theater (no `request_atlas_batching` — gated until a §11 M-4 PR); no new substrate/WGSL/op/invariant. Report: `docs/tests/runtime_0080_0_r0_results.md`. Candidates A (richer emergence / `SCENARIO-0080-3`), D (multi-faction ECON), E (recursion) parked behind R0.
