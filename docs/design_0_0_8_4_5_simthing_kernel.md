...
| 6 | `KERNEL-CRATE-EXTRACT-0` | §4 — semantic-free core as a dependency-graph fact | **The keystone.** Carve the already-identified semantic-free core (the `AccumulatorOp` passes behind `SimulationFabric` + the sealed column buffer) into **`simthing-kernel`**: minimal deps (ideally only `simthing-eml` + the fabric/index types), `#![forbid(unsafe_code)]`, owns the only mutable column buffer; every other crate depends on its **read-only view**. The seals (3–5) become dependency-enforced, not convention. **May split.** | The semantic-free *source scans* → narrowed to shader-text only; the kernel's dep graph is now the firewall. `compile_fail`/dep-check: a consumer crate reaching kernel-internal mutation. | Cursor/Grok | **HELD — DA re-review 2026-06-29 (remediation required).** Crate, dep direction (`core→kernel→gpu/sim`, no cycle, minimal justified deps), `#![forbid(unsafe_code)]`, ZST `ResolvedWriteAuthority` move, and perf (0.2145→0.1888 ms, inlining-by-construction) are all correct. **But the extraction REOPENED the emission forge vector:** the readback bridges are `pub fn threshold_events_from_gpu(gpu: &[ThresholdEventGpu]) -> Vec<ThresholdEvent>` and the `ThresholdEventGpu` POD has all-`pub` fields — so any crate depending on `simthing-kernel` can fabricate a POD and launder it into a forged sealed event (the `pub(crate)→pub` widening across the new crate seam). Remediation: gate the readback bridges with a kernel-minted `ReadbackAuthority` ZST (same pattern as `ResolvedWriteAuthority`) **or** move the readback in-crate over a sealed buffer; add a `compile_fail` on the bridge-launder vector. (`docs/tests/kernel_crate_extract_0_results.md`) |
