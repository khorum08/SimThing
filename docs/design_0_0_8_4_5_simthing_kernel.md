...
| 6 | `KERNEL-CRATE-EXTRACT-0` | §4 — semantic-free core as a dependency-graph fact | **The keystone.** Carve the already-identified semantic-free core (the `AccumulatorOp` passes behind `SimulationFabric` + the sealed column buffer) into **`simthing-kernel`**: minimal deps (ideally only `simthing-eml` + the fabric/index types), `#![forbid(unsafe_code)]`, owns the only mutable column buffer; every other crate depends on its **read-only view**. The seals (3–5) become dependency-enforced, not convention. **May split.** | The semantic-free *source scans* → narrowed to shader-text only; the kernel's dep graph is now the firewall. `compile_fail`/dep-check: a consumer crate reaching kernel-internal mutation. | Cursor/Grok | **HELD — DA re-review 2026-06-29 (0R insufficient).** The 0R `ReadbackAuthority` ZST seals the struct-literal, but its minter **`for_kernel_readback()` is `pub`** (`#[doc(hidden)]` ≠ private; gpu calls it cross-crate, proving it must be) — so any crate mints the authority and forges via the bridge. **Convention, not enforcement** (the deny.toml lesson). Rust has no friend-crate visibility, so a token minter cannot be gpu-only — the DA's prior token guidance does not work across this seam. **Genuine fix: move the threshold/emission readback-minting + its source output buffers INTO the kernel** (map kernel-private buffers, cast, mint; expose only sealed `Vec<…>`; no public POD bridge, no public minter) — the readback portion of the deferred orchestration slice — **OR** an explicit owner/DA-approved Deviation that this one seam is convention-protected pending a named follow-on `KERNEL-READBACK-INCRATE-0`. The `compile_fail` must test the REAL attack (mint-then-forge, which currently compiles). (`docs/tests/kernel_crate_extract_0_results.md`) |
