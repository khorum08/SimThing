...
> **Status: CLOSED — DA-APPROVED (2026-06-29).** All rungs (1–6, F0, F) DA-APPROVED; `simthing-kernel` is the
> dependency-enforced SimThing authority. The load-bearing doctrine (kernel authority + the cross-crate seal
> law + residue-as-tripwire) is landed permanently in [`simthing_core_design.md`](simthing_core_design.md)
> **§1.2.1** and the constitution **§0.9** (carry-forward), with the spine in [`handoff_template.md`](handoff_template.md).
> Next track: **0.0.8.5 Terran-Pirate** (its live-STEAD-decision phase now has the write+emission seals it
> required). Sits *beneath* the permanent
...
| 6 | `KERNEL-CRATE-EXTRACT-0` | §4 — semantic-free core as a dependency-graph fact | **The keystone.** Carve the already-identified semantic-free core (the `AccumulatorOp` passes behind `SimulationFabric` + the sealed column buffer) into **`simthing-kernel`**: minimal deps (ideally only `simthing-eml` + the fabric/index types), `#![forbid(unsafe_code)]`, owns the only mutable column buffer; every other crate depends on its **read-only view**. The seals (3–5) become dependency-enforced, not convention. **May split.** | The semantic-free *source scans* → narrowed to shader-text only; the kernel's dep graph is now the firewall. `compile_fail`/dep-check: a consumer crate reaching kernel-internal mutation. | Cursor/Grok | **DONE — DA-APPROVED** (2026-06-29, after 0R/0R2/0R3). **B3 class-seal clean:** zero pub `&Buffer`/buffer fields escape the kernel — all 9 `WorldGpuState` authoritative buffers (state/deltas/`threshold_registry`/program/topology) + session/EML/input-list accessors are `pub(crate)`/private; bridges, `ReadbackAuthority`, `for_boundary_install` removed; `#![forbid(unsafe_code)]` kept; `compile_fail` proves external `queue.write_buffer(&threshold_registry,…)` fails. Dispatch/encode/readback are in-crate; the kernel is now the **dependency-enforced authority** (core→kernel→gpu/sim, no cycle). Perf multi-run 0.1869 ms (within band). **Per-item residue (declared tripwires):** `IndexedScatterOp::dispatch` — inert without authoritative handles; standing tripwires `cpu_oracle_threshold_events` + WGSL text. Candidate-F write sealed by **F0** below. (`docs/tests/kernel_crate_extract_0_results.md`) |
| F0 | `KERNEL-CANDIDATE-F-INCRATE-0` | scoped final seal — candidate-F write in-crate | Retire cross-crate `write_max_candidate_f_magnitude_bits` B5 residue. External crates supply `CandidateFMagnitudeRequest` (gradients + slot/col); kernel writes sealed session values via `apply_candidate_f_exact_magnitude`. `max_candidate_f_magnitude_bits` stays public as inert caller-buffer probe only. | Cross-crate candidate-F resolved-state write helper. | Cursor/Grok | **DONE — DA-APPROVED** (2026-06-29, PR #1011) — verified: `write_max_candidate_f_magnitude_bits` → `pub(crate)`/unexported; `CandidateFMagnitudeRequest` carries **no `&Buffer`**; `apply_candidate_f_exact_magnitude` is a sanctioned in-crate door (passes the kernel's own `values_buffer()`, exact-sqrt-derived, no handle crosses — the `dispatch_tick` shape); free `max_candidate_f_magnitude_bits` confirmed inert (oracle probe); retired-API `compile_fail`s present. Last write residue retired; B5 gone. (`docs/tests/kernel_candidate_f_incrate_0_results.md`) |
| F | `KERNEL-CLOSEOUT-0` | — | Scope Ledger + perf-parity ledger; consolidate sub-rung docs (one ledger each); **the four mandatory documentation landings below (§2A)**. **Prerequisite:** `KERNEL-CANDIDATE-F-INCRATE-0` (F0) landed. | — | Opus/Owner (DA) | **DONE — DA-APPROVED** (2026-06-29, owner-greenlit). **Four landings complete:** (1) Core Design **§1.2.1** — kernel authority + cross-crate seal law + residue-as-tripwire (the load-bearing, governing home); (2) Constitution **§0.9** — carry-forward clause (propagates verbatim, amended only by addition); (3) handoff template spine — the cross-crate seal law + residue-tripwire, with "one authoritative path" now stated as type-fact; (4) STEAD-completeness recorded. **0.0.8.4.5 TRACK CLOSED — rungs 1–6 + F0 + F all DA-APPROVED; `simthing-kernel` is the dependency-enforced SimThing authority.** |
...
4b. **The residue-as-tripwire model (§5.1 + the §5.2 bypass-state catalogue B1–B8)** documented at the same three altitudes — Core Design Doc, Constitution §0, handoff template — so every future agent knows the residue is a *red-flag scan*, not a passive gap, and that routing through it is a declared, scanned, deliberate act.
