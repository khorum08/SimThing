...
| 6 | `KERNEL-CRATE-EXTRACT-0` | §4 — semantic-free core as a dependency-graph fact | **The keystone.** Carve the already-identified semantic-free core (the `AccumulatorOp` passes behind `SimulationFabric` + the sealed column buffer) into **`simthing-kernel`**: minimal deps (ideally only `simthing-eml` + the fabric/index types), `#![forbid(unsafe_code)]`, owns the only mutable column buffer; every other crate depends on its **read-only view**. The seals (3–5) become dependency-enforced, not convention. **May split.** | The semantic-free *source scans* → narrowed to shader-text only; the kernel's dep graph is now the firewall. `compile_fail`/dep-check: a consumer crate reaching kernel-internal mutation. | Cursor/Grok | **HELD — DA re-review 2026-06-29 (0R2: emission bridge closed, but a deeper seam open).** 0R2 correctly deleted `ReadbackAuthority` + the POD bridges and moved minting in-crate — the POD-bridge forge is gone. **But the kernel exposes `pub fn values()/output_vectors()/records_binding() -> &Buffer` on its authoritative buffers, and `WorldGpuState.ctx` (queue) is public** — so any crate does `queue.write_buffer(state.resolved().values(), …)` to forge resolved state (re-opens the WRITE-seal), or writes the emission *output* buffer via `records_binding()` so the kernel's own in-crate mint seals forged data (re-opens emission through the buffer). **ROOT (architectural):** gpu does the dispatch, so it needs `&Buffer` cross-crate, and cross-crate `&Buffer` = write capability for everyone (no friend visibility). **A sealed kernel requires the GPU dispatch to live IN the kernel** — the deferred orchestration slice *is* the seal. **OWNER DECISION (escalated):** (1) move the GPU dispatch + encode into the kernel so `&Buffer`/`ctx` never cross the boundary (the real, complete seal; large), or (2) graduate as an honest PARTIAL — sealed type definitions + dependency direction are real wins, but the buffer-binding seam stays convention-protected and the spine 'one authoritative path' remains a DIRECTIVE for write/emission until `KERNEL-DISPATCH-INCRATE-0` lands — via an explicit owner-approved Deviation. (`docs/tests/kernel_crate_extract_0_results.md`) |
