...
| 6 | `KERNEL-CRATE-EXTRACT-0` | §4 — semantic-free core as a dependency-graph fact | **The keystone.** Carve the already-identified semantic-free core (the `AccumulatorOp` passes behind `SimulationFabric` + the sealed column buffer) into **`simthing-kernel`**: minimal deps (ideally only `simthing-eml` + the fabric/index types), `#![forbid(unsafe_code)]`, owns the only mutable column buffer; every other crate depends on its **read-only view**. The seals (3–5) become dependency-enforced, not convention. **May split.** | The semantic-free *source scans* → narrowed to shader-text only; the kernel's dep graph is now the firewall. `compile_fail`/dep-check: a consumer crate reaching kernel-internal mutation. | Cursor/Grok | **PROBATION — 0R2 landed 2026-06-29.** 0R failed: public `ReadbackAuthority::for_kernel_readback()` was a cross-crate forge door (`#[doc(hidden)]` ≠ private). **0R2 fix:** kernel owns readback output buffers (`EmissionRecordReadback`, `ThresholdEmissionReadback`, `ThresholdEventCandidatesReadback`); maps/casts/mints in-crate; public API returns sealed `Vec<…>` only; all public POD bridges and `ReadbackAuthority` removed; CPU oracle mint paths moved in-crate. `compile_fail` tests mint-then-launder attack. DA re-review pending. (`docs/tests/kernel_crate_extract_0_results.md`) |
