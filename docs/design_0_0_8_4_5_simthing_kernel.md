...
| 6 | `KERNEL-CRATE-EXTRACT-0` | §4 — semantic-free core as a dependency-graph fact | **The keystone.** Carve the already-identified semantic-free core (the `AccumulatorOp` passes behind `SimulationFabric` + the sealed column buffer) into **`simthing-kernel`**: minimal deps (ideally only `simthing-eml` + the fabric/index types), `#![forbid(unsafe_code)]`, owns the only mutable column buffer; every other crate depends on its **read-only view**. The seals (3–5) become dependency-enforced, not convention. **May split.** | The semantic-free *source scans* → narrowed to shader-text only; the kernel's dep graph is now the firewall. `compile_fail`/dep-check: a consumer crate reaching kernel-internal mutation. | Cursor/Grok | **HELD → owner chose the COMPLETE SEAL (2026-06-29).** The 0R2 closed the POD-bridge forge and moved minting in-crate (real wins, kept), but the `pub &Buffer` accessors + public `ctx` re-opened write/emission through the buffers — proving a seal can't hold while gpu (a separate crate) does the dispatch. **Owner directive: complete the seal regardless of size — see §1.1 standing ruling.** Deliverable now = **move the GPU dispatch + encode + readback (and the buffers they bind) INTO `simthing-kernel`** so `&Buffer`/`Queue`/`Device` `ctx`/raw write paths **never cross the kernel boundary**; consumers get only `dispatch_tick()` / `read_*() -> Vec<Sealed>` + a read-only view. Then the write/emission seals are dependency-enforced and the spine line upgrades directive→type-fact. Sub-ladder under `KERNEL-DISPATCH-INCRATE-*`. Rung stays HELD until no authoritative `&Buffer`/`ctx`/raw-write crosses the boundary (proven by a boundary scan + compile_fail). (`docs/tests/kernel_crate_extract_0_results.md`) |
