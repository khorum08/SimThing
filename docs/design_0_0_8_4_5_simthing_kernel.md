...
| 6 | `KERNEL-CRATE-EXTRACT-0` | §4 — semantic-free core as a dependency-graph fact | **The keystone.** Carve the already-identified semantic-free core (the `AccumulatorOp` passes behind `SimulationFabric` + the sealed column buffer) into **`simthing-kernel`**: minimal deps (ideally only `simthing-eml` + the fabric/index types), `#![forbid(unsafe_code)]`, owns the only mutable column buffer; every other crate depends on its **read-only view**. The seals (3–5) become dependency-enforced, not convention. **May split.** | The semantic-free *source scans* → narrowed to shader-text only; the kernel's dep graph is now the firewall. `compile_fail`/dep-check: a consumer crate reaching kernel-internal mutation. | Cursor/Grok | **HELD — DA re-review 2026-06-29 (DISPATCH-INCRATE-0R: named leaks sealed, but B3 not comprehensively clean).** The 4 named accessors are sealed (`values_buffer`/EML/input-list → `pub(crate)`), perf multi-run confirms no regression. **But the scan finds 9 PUBLIC buffer *fields* on `WorldGpuState`** (`overlay_deltas`, `intent_deltas`, `threshold_registry`, `governed_pairs`, `column_rules`, `child_starts`, `child_indices`, `depth_slots`, `slot_delta_ranges`) waved through as 'non-authoritative tick-upload' — but `threshold_registry` is the **decision-gate config** (inject a threshold → forge decisions at the input, reopening what the emission seal closed at the output), `overlay_deltas` is the **universal modifier**, the rest are the **program/topology**. So B6 Option B ('no authoritative handle escapes') is FALSE. **This is whack-a-mole — 3 rounds of naming specific leaks. Remediation must apply the PRINCIPLE to the CLASS:** every kernel buffer field/accessor that participates in authoritative computation (state, deltas, registry, program, topology) → `pub(crate)` (the uploader is in-crate now; nothing external needs them); any genuine exception is a PER-ITEM justified residue (provably inert + ctx-unpairable), never a category wave-through. **Gate = §5.2 B1–B8 scan returns only `pub(crate)` for authoritative handles + a per-item (not categorical) residue list.** (`docs/tests/kernel_crate_extract_0_results.md`) |
