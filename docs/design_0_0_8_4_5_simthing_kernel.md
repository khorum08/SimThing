...
| 6 | `KERNEL-CRATE-EXTRACT-0` | §4 — semantic-free core as a dependency-graph fact | **The keystone.** Carve the already-identified semantic-free core (the `AccumulatorOp` passes behind `SimulationFabric` + the sealed column buffer) into **`simthing-kernel`**: minimal deps (ideally only `simthing-eml` + the fabric/index types), `#![forbid(unsafe_code)]`, owns the only mutable column buffer; every other crate depends on its **read-only view**. The seals (3–5) become dependency-enforced, not convention. **May split.** | The semantic-free *source scans* → narrowed to shader-text only; the kernel's dep graph is now the firewall. `compile_fail`/dep-check: a consumer crate reaching kernel-internal mutation. | Cursor/Grok | **PROBATION** — real `simthing-kernel` crate owns sealed write/emission/participation surfaces + `ResolvedGpuBuffers`; gpu/sim depend on kernel (no cycle); 7 kernel + 10 gpu `compile_fail` doctests; `c1_threshold_perf` release smoke green (0.2145→0.1888 ms readback, within noise). Session/pipeline orchestration remains in `simthing-gpu` (deferred slice). (`docs/tests/kernel_crate_extract_0_results.md`) |
