...
| F0 | `KERNEL-CANDIDATE-F-INCRATE-0` | scoped final seal — candidate-F write in-crate | Retire cross-crate `write_max_candidate_f_magnitude_bits` B5 residue. External crates supply `CandidateFMagnitudeRequest` (gradients + slot/col); kernel writes sealed session values via `apply_candidate_f_exact_magnitude`. `max_candidate_f_magnitude_bits` stays public as inert caller-buffer probe only. | Cross-crate candidate-F resolved-state write helper. | Cursor/Grok | **PROBATION** — `docs/tests/kernel_candidate_f_incrate_0_results.md` |
