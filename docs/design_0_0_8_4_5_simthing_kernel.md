...
| 6 | `KERNEL-CRATE-EXTRACT-0` | §4 — semantic-free core as a dependency-graph fact | **The keystone.** Carve the already-identified semantic-free core (the `AccumulatorOp` passes behind `SimulationFabric` + the sealed column buffer) into **`simthing-kernel`**: minimal deps (ideally only `simthing-eml` + the fabric/index types), `#![forbid(unsafe_code)]`, owns the only mutable column buffer; every other crate depends on its **read-only view**. The seals (3–5) become dependency-enforced, not convention. **May split.** | The semantic-free *source scans* → narrowed to shader-text only; the kernel's dep graph is now the firewall. `compile_fail`/dep-check: a consumer crate reaching kernel-internal mutation. | Cursor/Grok | **HELD — DA re-review 2026-06-29 (dispatch moved in-crate; boundary scan NOT clean).** Real progress: dispatch/encode/readback + resolved/readback buffers moved into the kernel, `resolved.rs` accessors `pub(crate)`, POD bridges/ReadbackAuthority/for_boundary_install removed, `#![forbid(unsafe_code)]` kept. **But the boundary scan (B3) still finds PUB `&Buffer` escaping:** `AccumulatorOpSession::values_buffer()` (authoritative resolved values), `EmlProgramTable::node_buffer()/range_buffer()` (the EML opcode program — behavior injection), `InputListTable::buffer()` + a `pub buffer: Buffer` field; with `pub device`/`pub queue` on `ctx`, any crate does `ctx.queue.write_buffer(session.values_buffer(), …)`. Codex's 'no authoritative &Buffer escapes' is contradicted by the tree. Remediation: **seal these to `pub(crate)`** (dispatch is in-crate now → kernel binds them internally; no pub accessor needed), or prove provably-non-authoritative + ctx-unpairable. **Graduation gate = the §5.2 B1–B8 scan clean** (every authoritative buffer/program sealed). Also: perf +7.2% single-run needs a multi-run confirm (the move *removes* a boundary — should not add cost); `pollster` dep acknowledged (B8, minimal). (`docs/tests/kernel_crate_extract_0_results.md`) |
