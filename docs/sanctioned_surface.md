...
| scripts/ci/allow/kernel_surface.txt | 297 | afb3ee46c0b9ffcfaec40ab4fe2b09a86b01d1b722da8acd534315981bfc8c0e |
...
| PowerLawGadget | authority-export | EML-LN-PRIMITIVE-0 power-law consumer gadget over guarded LN and EXP call sites | retire when kernel export set is closed by type-boundary admission | kernel_surface.txt |
| EmlOperatorGadget | authority-export | EML-LN-PRIMITIVE-0 elementary eml(x,y) consumer gadget | retire when kernel export set is closed by type-boundary admission | kernel_surface.txt |
| EntropyTermGadget | authority-export | EML-LN-PRIMITIVE-0 single entropy term consumer gadget with authored p>0 handling | retire when kernel export set is closed by type-boundary admission | kernel_surface.txt |
| LogAccumulateMapGadget | authority-export | EML-LN-PRIMITIVE-0 Sum-lane log-domain map half (not a Product substitute) | retire when kernel export set is closed by type-boundary admission | kernel_surface.txt |
...
| admit_ln_call_sites | authority-export | EML-LN-PRIMITIVE-0 tree-scan wiring of the 5.10 call-site shapes for the admitted LN primitive | retire if exact primitive call-site admission becomes wholly kernel-internal | kernel_surface.txt |
...
| eml_ln_qualification | authority-export | EML-LN-PRIMITIVE-0 pinned exhaustive-qualification artifacts and certified toolchain roster | retire if qualification pinning becomes wholly kernel-internal | kernel_surface.txt |
...
| ln_primitive_domain | authority-export | EML-LN-PRIMITIVE-0 canonical admitted-domain LN interval constructor | retire if exact primitive call-site admission becomes wholly kernel-internal | kernel_surface.txt |
...
| WORKSHOP-HOMING-DETECTION | HEURISTIC | ci_screening_surface §12 + WORKSHOP-HOMING-DETECTION-0; HEURISTIC tripwire for net-new scenario vocabulary homing into sealed production crates while simthing-workshop remains the sandbox | crates/simthing-{core,kernel,gpu,feeder,sim,driver,spec,clausething,mapgenerator,mapeditor,tools}/{src,tests}/**/*.rs | faction\|combat\|terran\|pirate\|diplomacy | compile_fail;^\\s*//!;^\\s*///;^\\s*//;SimThingKind:: | retire when game semantics are spec-boundary typed only | scans.tsv |
