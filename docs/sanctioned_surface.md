...
| scripts/ci/scans.tsv | 21 | f6633e364ba9bf66b702a7a0a0188412fc9dba9d17249ea5d8fb19b162426ded |
...
| CELL-STORAGE-POLYMORPHISM | HEURISTIC | design 0.0.8.7 §2 P0(e) fence (i) CELL-STORAGE-POLYMORPHISM; HEURISTIC reach detector for tagged/templated/heterogeneous matrix-cell storage | crates/simthing-{kernel,core,gpu,sim,driver}/src/** | enum\\s+\\w*(?:Matrix\|Lane)?Cell\\w*\\s*\\{[\\s\\S]{0,400}(?:Box\\s*<\|dyn\\s+\|tagged.?union\|Any\\b)\|heterogeneous\\s+(?:matrix\\s+)?cell\|CellStorage\\s*[<=]\|type\\s+\\w*CellValue\\s*=\\s*enum | compile_fail;^\\s*//!;^\\s*///;^\\s*// | retire when homogeneous-lane storage is type-admitted and heterogeneous cell reaches cannot compile | scans.tsv |
| BESPOKE-PATHFINDER | HEURISTIC | design 0.0.8.7 §4 TRIAD DOORS / P5 PALMA BESPOKE-PATHFINDER; HEURISTIC reach detector for production A*/Dijkstra/priority-queue graph search outside PALMA | crates/simthing-{kernel,core,gpu,sim,driver}/src/** | BinaryHeap[\\s\\S]{0,1200}(?:came_from\|g_score\|open_set)\|(?:came_from\|g_score\|open_set)[\\s\\S]{0,1200}BinaryHeap | compile_fail;^\\s*//!;^\\s*///;^\\s*// | retire when pathing is exclusively the PALMA door (type-admitted) and bespoke graph-search cannot compile in production crates | scans.tsv |
| BORDER-SERVICE | HEURISTIC | design 0.0.8.7 §4 TRIAD DOORS / P5 Gu-Yang BORDER-SERVICE; HEURISTIC reach detector for contour/frontline/border-service reaches outside Gu-Yang (mapeditor presentation paths excluded by target-glob) | crates/simthing-{kernel,core,gpu,sim,driver}/src/** | marching_squares\|contour_extract(?:ion)?\|border_service\|FrontlineTracer\|trace_border(?:line)?\\s*\\(\|BorderPolylineEngine | compile_fail;^\\s*//!;^\\s*///;^\\s*// | retire when border observation is exclusively the Gu-Yang door and bespoke border services cannot compile in production crates | scans.tsv |
