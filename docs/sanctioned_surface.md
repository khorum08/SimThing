...
| OVERLAY-PEER-AUTHORITY | HEURISTIC | stemthing_intrinsic_overlay_capability.md sections 17-19 + core design section 9 detector 14; HEURISTIC tripwire for peer overlay/action authority minted between the Owner adjudication and the 7.7/7.8 type landings | crates/simthing-{core,kernel,gpu,sim,driver,spec,clausething,feeder,mapgenerator,mapeditor,tools}/src/** | struct\\s+(?:ActionThing\|OverlayManager\|OverlayService\|SaturationListener\|OverlayHistory\|MovementPlanner)\\b\|enum\\s+(?:ActionKind\|OverlayCommandKind)\\b\|fn\\s+\\w*overlay_manager\\w*\\( | compile_fail;^\\s*//!;^\\s*///;^\\s*// | retire when 7.7 GPU-OVERLAY-LIFECYCLE-EXTRACTION-0 and 7.8 ACTIONBAND-OVERLAY-ACTUATION-0 make peer actuation authority uncompilable at the type boundary | scans.tsv |
