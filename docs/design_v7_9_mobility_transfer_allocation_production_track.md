...
| RUNTIME-1B | Non-scheduled GPU pass-graph node registration (opt-in, non-default) | RUNTIME-1A green | **PASS (MOBILITY-RUNTIME-1B, non-scheduled GPU pass-graph node registration in driver test/support)** | Registration floor green; no dispatch/WGSL |
| GPU-EXEC-0 | Semantic-free generic GPU execution readiness (opt-in, non-default) | — (generic prerequisite for future dispatch) | **PASS (GPU-EXEC-0, driver test/support identity-buffer pass; CPU/GPU parity or honest unsupported)** | Readiness floor green; no mobility shader/dispatch |
| RUNTIME-1B-DISPATCH-0 | Semantic-free GPU-EXEC identity-probe dispatch through registered node (opt-in, test/support) | RUNTIME-1B + GPU-EXEC-0 green | **PASS (RUNTIME-1B-DISPATCH-0, driver test/support; GPU-EXEC probe via registered node; no mobility shader)** | Dispatch floor green; registration non-executing until invoked |
| MOBILITY-GPU-KERNEL-0 | Semantic-free mobility-shaped column-transform GPU kernel (opt-in, test/support) | GPU-EXEC-0 green | **PASS (MOBILITY-GPU-KERNEL-0, driver test/support built-in parent-select kernel; generic columns only)** | Kernel floor green; no live-slot compaction/allocator/atomics |
| MOBILITY-GPU-KERNEL-1 | Registered-node dispatch of KERNEL-0 column-transform (opt-in, test/support) | MOBILITY-GPU-KERNEL-0 + RUNTIME-1B green | **PASS (MOBILITY-GPU-KERNEL-1, driver test/support; KERNEL-0 via registered node; no new shader)** | Dispatch floor green; registration non-executing until invoked |
| MOBILITY-GPU-KERNEL-2 | 34k deterministic registered-node column dispatch soak (opt-in, test/support) | MOBILITY-GPU-KERNEL-1 green | **PASS (MOBILITY-GPU-KERNEL-2, driver test/support; 34k column soak via KERNEL-1; no new shader)** | Scale floor green; v7.9 scenario row count |
| MOBILITY-GPU-KERNEL-3 | Project accepted RUNTIME composition outputs into generic GPU column buffers + registered-node dispatch (opt-in, test/support) | MOBILITY-GPU-KERNEL-2 green | **PASS (MOBILITY-GPU-KERNEL-3, driver test/support; composition→generic columns via KERNEL-1; no new shader)** | Composition projection green; synthetic-only column input superseded for fixture path |
| MOBILITY-GPU-KERNEL-4 | 34k composition-derived projection + registered-node GPU column dispatch (opt-in, test/support) | MOBILITY-GPU-KERNEL-3 green | **PASS (MOBILITY-GPU-KERNEL-4, driver test/support; 34k composition-derived projection via KERNEL-3/KERNEL-1; no new shader)** | Scale + composition projection green; default schedule/gameplay still closed |
| MOBILITY-GPU-KERNEL-5 | Second semantic-free GPU kernel over the KERNEL-4 34k composition-derived projection (opt-in, test/support) | MOBILITY-GPU-KERNEL-4 green | **PASS (MOBILITY-GPU-KERNEL-5, driver test/support; row digest + move-weight outputs via registered node)** | Second kernel parity green; default schedule/gameplay still closed |
| MOBILITY-GPU-KERNEL-6 | Ordered KERNEL-0 -> KERNEL-5 semantic-free multi-kernel chain over the 34k composition-derived projection (opt-in, test/support) | MOBILITY-GPU-KERNEL-5 green | **PASS (MOBILITY-GPU-KERNEL-6, driver test/support; whole-chain CPU oracle + GPU checksum classification; no new shader)** | Ordered multi-kernel chain green; default schedule/gameplay still closed |
| MOBILITY-GPU-KERNEL-7 | Deterministic multi-dispatch replay soak over the KERNEL-6 chain and KERNEL-4 34k projection (opt-in, test/support) | MOBILITY-GPU-KERNEL-6 green | **PASS (MOBILITY-GPU-KERNEL-7, driver test/support; 8-iteration replay stability + per-iteration checksum classification; no new shader)** | Replay stability green; default schedule/gameplay still closed |
| MOBILITY-GPU-KERNEL-8 | Varied-input projection-batch replay soak over the KERNEL-6 chain (opt-in, test/support) | MOBILITY-GPU-KERNEL-7 green | **PASS (MOBILITY-GPU-KERNEL-8, driver test/support; 4 projection variants × 2 replays; per-variant checksum classification; no new shader)** | Varied-input batch replay green; default schedule/gameplay still closed |
| MOBILITY-GPU-KERNEL-9 | Multi-frame projection-variant stream soak over the KERNEL-6 chain (opt-in, test/support) | MOBILITY-GPU-KERNEL-8 green | **PASS (MOBILITY-GPU-KERNEL-9, driver test/support; 4 frames × 4 variants × 2 replays; per-frame checksum classification; no new shader)** | Multi-frame stream replay green; default schedule/gameplay still closed |
| MOBILITY-GPU-KERNEL-10 | Deterministic throughput/accounting summary over the KERNEL-9 frame stream (opt-in, test/support) | MOBILITY-GPU-KERNEL-9 green | **PASS (MOBILITY-GPU-KERNEL-10, driver test/support; integer dispatch/row counters + aggregate stream checksums; no new shader; no wall-clock timing)** | Stream accounting green; default schedule/gameplay still closed |
| MOBILITY-GPU-KERNEL-11 | Deterministic budget-envelope assertions over KERNEL-10 stream accounting (opt-in, test/support) | MOBILITY-GPU-KERNEL-10 green | **PASS (MOBILITY-GPU-KERNEL-11, driver test/support; integer budget envelopes + deterministic over-budget diagnostics; no new shader; no wall-clock timing)** | Budget envelope green; default schedule/gameplay still closed |
| RUNTIME-1B-DISPATCH | Semantic-free registered-node mobility GPU dispatch path (opt-in, test/support; no default schedule) | GPU-EXEC-0 + RUNTIME-1B-DISPATCH-0 + MOBILITY-GPU-KERNEL-0 green | **PASS / satisfied (closed-gate blocker -- no generic GPU exec path -- resolved by GPU-EXEC-0 + semantic-free KERNEL-0 path)** | Default schedule/default `SimSession`/gameplay remain separate closed gates |
...
registration were untouched at the spec layer. No default schedule, no gameplay-facing integration.
(RUNTIME-1A-RUNTIME-FIXTURE and RUNTIME-1B have since landed — see ladder index.) See
...
default-path surface**, a default production schedule, gameplay-facing integration, and any
semantic/default mobility shader. **RUNTIME-1B-DISPATCH is green only for the semantic-free
registered-node dispatch path**; the old closed-gate blocker -- no generic GPU exec path -- is
resolved by GPU-EXEC-0 + the semantic-free KERNEL-0 path.

**GPU-EXEC-0 — PASS (GPU-EXEC-0, 2026-06-02).** Semantic-free generic GPU execution readiness in
`simthing-driver` test/support: opt-in/default-off identity-buffer pass, CPU oracle + GPU checksum,
exact parity or honest `GpuUnavailable`. No mobility shader, no semantic/raw WGSL, no designer
shader input, no default schedule, no gameplay. See
[`phase_gpu_exec0_results.md`](tests/phase_gpu_exec0_results.md).

**RUNTIME-1B-DISPATCH-0 — PASS (RUNTIME-1B-DISPATCH-0, 2026-06-02).** Opt-in/default-off
test/support dispatch of the GPU-EXEC-0 semantic-free identity-buffer probe through the green
RUNTIME-1B registered pass-graph node. Registration non-executing until dispatch invoked; no mobility
shader, no default schedule, no gameplay. See
[`phase_mobility_runtime1b_dispatch_results.md`](tests/phase_mobility_runtime1b_dispatch_results.md).

**MOBILITY-GPU-KERNEL-0 — PASS (MOBILITY-GPU-KERNEL-0, 2026-06-02).** Opt-in/default-off
test/support semantic-free mobility-shaped column-transform GPU kernel over generic columns
(`src_parent`, `dst_parent`, `entity_id`, `move_mask` → `out_parent`, `out_changed`). Built-in WGSL
only; CPU oracle + GPU checksum; no live-slot compaction, no allocator, no nondeterministic atomics.
See [`phase_mobility_gpu_kernel0_results.md`](tests/phase_mobility_gpu_kernel0_results.md).

**MOBILITY-GPU-KERNEL-1 — PASS (MOBILITY-GPU-KERNEL-1, 2026-06-02).** Opt-in/default-off
test/support dispatch of MOBILITY-GPU-KERNEL-0 column-transform through the green RUNTIME-1B
registered pass-graph node. Registration non-executing until dispatch invoked; reuses KERNEL-0
built-in WGSL; no default schedule, no gameplay. See
[`phase_mobility_gpu_kernel1_results.md`](tests/phase_mobility_gpu_kernel1_results.md).

**MOBILITY-GPU-KERNEL-2 — PASS (MOBILITY-GPU-KERNEL-2, 2026-06-02).** Opt-in/default-off
test/support 34k-row deterministic column soak through MOBILITY-GPU-KERNEL-1 registered-node
dispatch. Edge/sparse/dense move-mask clusters; CPU oracle complete; reuses KERNEL-0 WGSL; no new
shader text. See [`phase_mobility_gpu_kernel2_results.md`](tests/phase_mobility_gpu_kernel2_results.md).

**MOBILITY-GPU-KERNEL-3 — PASS (MOBILITY-GPU-KERNEL-3, 2026-06-02).** Opt-in/default-off
test/support projection of accepted RUNTIME-0 composition outputs into generic GPU column buffers
and dispatch through MOBILITY-GPU-KERNEL-1 registered-node path. Moved + unmoved entities; owner/econ
present in composition fixture but not encoded as shader semantics; deterministic row order; reuses
KERNEL-0 WGSL; no new shader text. See
[`phase_mobility_gpu_kernel3_results.md`](tests/phase_mobility_gpu_kernel3_results.md).

**MOBILITY-GPU-KERNEL-4 - PASS (MOBILITY-GPU-KERNEL-4, 2026-06-02).** Opt-in/default-off
test/support 34k-row composition-derived projection soak through the green MOBILITY-GPU-KERNEL-3 ->
MOBILITY-GPU-KERNEL-1 -> MOBILITY-GPU-KERNEL-0 path. Sparse/dense masks, edge rows, repeated
destination parents, deterministic row order, CPU oracle, and exact-parity-or-honest-unavailable
classification are covered; owner/econ context stays out of shader semantics; no new shader text.
See [`phase_mobility_gpu_kernel4_results.md`](tests/phase_mobility_gpu_kernel4_results.md).

**MOBILITY-GPU-KERNEL-5 - PASS (MOBILITY-GPU-KERNEL-5, 2026-06-02).** Opt-in/default-off
test/support second semantic-free GPU kernel over the MOBILITY-GPU-KERNEL-4 34k
composition-derived projection. It dispatches only through the registered-node path, emits row
digest + move-weight outputs, uses a complete CPU oracle, and reports exact parity, honest
unavailable, or actual GPU execution failure distinctly. No designer/semantic WGSL, default
schedule, gameplay, default `SimSession` path, live-slot compaction, GPU allocator,
nondeterministic atomics, or CPU planner/urgency/commitment. See
[`phase_mobility_gpu_kernel5_results.md`](tests/phase_mobility_gpu_kernel5_results.md).

**MOBILITY-GPU-KERNEL-6 - PASS (MOBILITY-GPU-KERNEL-6, 2026-06-02).** Opt-in/default-off test/support ordered KERNEL-0 -> KERNEL-5 semantic-free multi-kernel chain over the KERNEL-4 34k composition-derived projection. It reuses the existing built-in generic shaders, dispatches through the registered-node path, covers the whole chain with a CPU oracle, and reports exact parity, honest unavailable, or actual GPU execution failure distinctly. No new shader text, designer/semantic WGSL, default schedule, gameplay, default `SimSession` path, live-slot compaction, GPU allocator, nondeterministic atomics, or CPU planner/urgency/commitment. See [`phase_mobility_gpu_kernel6_results.md`](tests/phase_mobility_gpu_kernel6_results.md).

**MOBILITY-GPU-KERNEL-7 - PASS (MOBILITY-GPU-KERNEL-7, 2026-06-02).** Opt-in/default-off test/support deterministic replay soak over the existing KERNEL-6 chain and KERNEL-4 34k composition-derived projection. It runs 8 explicit dispatch iterations, reuses the registered-node KERNEL-6 chain, records per-iteration CPU/GPU/projection checksums and parity classification, verifies CPU oracle and GPU checksum stability, preserves the source projection, and confirms the replay oracle is stable under deterministic input permutation. Local GPU execution classified as exact parity. No new shader text, designer/semantic WGSL, default schedule, gameplay, default `SimSession` path, live-slot compaction, GPU allocator, nondeterministic atomics, or CPU planner/urgency/commitment. See [`phase_mobility_gpu_kernel7_results.md`](tests/phase_mobility_gpu_kernel7_results.md).

**MOBILITY-GPU-KERNEL-8 - PASS (MOBILITY-GPU-KERNEL-8, 2026-06-02).** Opt-in/default-off test/support varied-input projection-batch replay soak over the KERNEL-6 semantic-free chain. Four deterministic generic-column variants (baseline, sparse-delta, dense-bulk, parent-key offset), two replays each, per-variant CPU/GPU/projection checksums, replay stability, distinct variant checksums, and source projection immutability. Reuses KERNEL-6 chain; no new shader text. See [`phase_mobility_gpu_kernel8_results.md`](tests/phase_mobility_gpu_kernel8_results.md).

**MOBILITY-GPU-KERNEL-9 - PASS (MOBILITY-GPU-KERNEL-9, 2026-06-02).** Opt-in/default-off test/support multi-frame projection-variant stream soak over the KERNEL-6 semantic-free chain. Four explicit frames with canonical, reversed, repeated, and alternate variant orderings; two replays per variant; per-frame aggregate CPU/GPU checksums; repeated-frame identity and distinct-frame separation; source projection immutability. Reuses KERNEL-8 variants and KERNEL-6 chain; no new shader text. See [`phase_mobility_gpu_kernel9_results.md`](tests/phase_mobility_gpu_kernel9_results.md).

**MOBILITY-GPU-KERNEL-10 - PASS (MOBILITY-GPU-KERNEL-10, 2026-06-02).** Opt-in/default-off test/support deterministic throughput/accounting summary over the KERNEL-9 semantic-free frame stream. Integer-only frame/variant/replay/dispatch/row counters, aggregate CPU/GPU stream checksums, repeated-run identity, and KERNEL-9 parity/checksum preservation without wall-clock timing. Reuses KERNEL-9 stream unchanged; no new shader text. See [`phase_mobility_gpu_kernel10_results.md`](tests/phase_mobility_gpu_kernel10_results.md).

**MOBILITY-GPU-KERNEL-11 - PASS (MOBILITY-GPU-KERNEL-11, 2026-06-02).** Opt-in/default-off test/support deterministic budget-envelope assertions over the KERNEL-10 stream accounting summary. Integer-only active-stream and zero-cost envelopes, deterministic over-budget fake-input diagnostics, KERNEL-10 checksum preservation, and no wall-clock timing. Reuses KERNEL-10 accounting unchanged; no new shader text. See [`phase_mobility_gpu_kernel11_results.md`](tests/phase_mobility_gpu_kernel11_results.md).

**MOBILITY-RUNTIME-1B — PASS (MOBILITY-RUNTIME-1B, 2026-06-02).** Non-scheduled GPU pass-graph
node registration in `simthing-driver` test/support, opt-in/default-off, delegating to the RUNTIME-1A
CPU driver fixture. No scheduled GPU dispatch, no WGSL/shader, no default schedule, no gameplay. See
[`phase_mobility_runtime1b_results.md`](tests/phase_mobility_runtime1b_results.md).
...
`runtime1b_non_scheduled_registration_no_gpu_dispatch`,
...
`runtime1b_34k_gpu_fixture_soak_or_precise_blocker` (CPU delegate soak; real GPU soak deferred to
RUNTIME-1B-DISPATCH).
...
| MOBILITY-RUNTIME-1B | Non-scheduled GPU pass-graph node registration in driver test/support; delegates to RUNTIME-1A CPU fixture; no dispatch/WGSL | **PASS / non-scheduled GPU pass-graph node registration** | [`phase_mobility_runtime1b_results.md`](tests/phase_mobility_runtime1b_results.md) |
| MOBILITY-RUNTIME-1B-OPEN-0 | Tier-2 opening review for GPU pass-graph registration | **OPEN WITH NARROWING / docs-only authorization** → non-scheduled GPU pass-graph node registration (driver test/support, opt-in/default-off, delegates to RUNTIME-1A CPU fixture; no scheduled dispatch, no new WGSL); the later RUNTIME-1B-DISPATCH semantic-free registered-node path is now satisfied by GPU-EXEC-0 + KERNEL-0..7 | [`phase_mobility_runtime1b_opening_review_results.md`](tests/phase_mobility_runtime1b_opening_review_results.md) |
| GPU-EXEC-0 | Semantic-free generic GPU execution readiness in driver test/support; built-in identity-buffer pass; CPU/GPU parity or honest unsupported | **PASS / generic GPU execution readiness** | [`phase_gpu_exec0_results.md`](tests/phase_gpu_exec0_results.md) |
| RUNTIME-1B-DISPATCH-0 | Dispatch GPU-EXEC-0 semantic-free identity probe through RUNTIME-1B registered node; opt-in/default-off; no mobility shader | **PASS / semantic-free GPU-EXEC probe dispatch through registered node** | [`phase_mobility_runtime1b_dispatch_results.md`](tests/phase_mobility_runtime1b_dispatch_results.md) |
| MOBILITY-GPU-KERNEL-0 | Semantic-free mobility-shaped column-transform GPU kernel; built-in generic columns; opt-in/default-off | **PASS / semantic-free mobility column-transform kernel fixture** | [`phase_mobility_gpu_kernel0_results.md`](tests/phase_mobility_gpu_kernel0_results.md) |
| MOBILITY-GPU-KERNEL-1 | Registered-node dispatch of KERNEL-0 column-transform; opt-in/default-off; reuses KERNEL-0 WGSL | **PASS / registered-node dispatch of semantic-free mobility column-transform fixture** | [`phase_mobility_gpu_kernel1_results.md`](tests/phase_mobility_gpu_kernel1_results.md) |
| MOBILITY-GPU-KERNEL-2 | 34k deterministic registered-node column dispatch soak; opt-in/default-off; no new shader | **PASS / 34k registered-node column dispatch soak** | [`phase_mobility_gpu_kernel2_results.md`](tests/phase_mobility_gpu_kernel2_results.md) |
| MOBILITY-GPU-KERNEL-3 | Project RUNTIME composition outputs to generic GPU columns + registered-node dispatch; opt-in/default-off; no new shader | **PASS / composition-output-to-generic-GPU-column projection + registered-node dispatch** | [`phase_mobility_gpu_kernel3_results.md`](tests/phase_mobility_gpu_kernel3_results.md) |
| MOBILITY-GPU-KERNEL-4 | Scale composition-derived projection to 34k generic GPU columns + registered-node dispatch; opt-in/default-off; no new shader | **PASS / 34k composition-derived projection + registered-node GPU column dispatch** | [`phase_mobility_gpu_kernel4_results.md`](tests/phase_mobility_gpu_kernel4_results.md) |
| MOBILITY-GPU-KERNEL-5 | Add second semantic-free GPU kernel over the KERNEL-4 34k projection; opt-in/default-off; built-in generic columns only | **PASS / second semantic-free 34k composition-derived GPU kernel** | [`phase_mobility_gpu_kernel5_results.md`](tests/phase_mobility_gpu_kernel5_results.md) |
| MOBILITY-GPU-KERNEL-6 | Chain KERNEL-0 -> KERNEL-5 over the 34k composition-derived projection; opt-in/default-off; no new shader | **PASS / ordered semantic-free multi-kernel chain** | [`phase_mobility_gpu_kernel6_results.md`](tests/phase_mobility_gpu_kernel6_results.md) |
| MOBILITY-GPU-KERNEL-7 | Deterministic replay soak over the KERNEL-6 chain and KERNEL-4 34k projection; opt-in/default-off; no new shader | **PASS / deterministic multi-dispatch replay soak** | [`phase_mobility_gpu_kernel7_results.md`](tests/phase_mobility_gpu_kernel7_results.md) |
| MOBILITY-GPU-KERNEL-8 | Varied-input projection-batch replay soak over the KERNEL-6 chain; opt-in/default-off; no new shader | **PASS / varied-input projection-batch replay soak** | [`phase_mobility_gpu_kernel8_results.md`](tests/phase_mobility_gpu_kernel8_results.md) |
| MOBILITY-GPU-KERNEL-9 | Multi-frame projection-variant stream soak over the KERNEL-6 chain; opt-in/default-off; no new shader | **PASS / multi-frame projection-variant stream soak** | [`phase_mobility_gpu_kernel9_results.md`](tests/phase_mobility_gpu_kernel9_results.md) |
| RUNTIME-1B-DISPATCH | Reconcile semantic-free registered-node dispatch status after GPU-EXEC-0 + KERNEL-0 path | **PASS / satisfied for opt-in semantic-free registered-node dispatch; default schedule/default `SimSession`/gameplay remain closed** | [`phase_mobility_gpu_kernel7_results.md`](tests/phase_mobility_gpu_kernel7_results.md) |
