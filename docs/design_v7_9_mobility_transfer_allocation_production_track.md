...
| RUNTIME-1B-DISPATCH | Real scheduled GPU dispatch of mobility composition (needs mobility shader + scheduled wiring) | RUNTIME-1B-DISPATCH-0 green | **Closed (separate later gate; no mobility shader exists yet)** | Generic probe dispatch proven; mobility dispatch still closed |
