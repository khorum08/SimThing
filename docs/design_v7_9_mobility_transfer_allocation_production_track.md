...
| RUNTIME-1B-DISPATCH | Real scheduled GPU dispatch of mobility composition (needs mobility shader + scheduled wiring) | MOBILITY-GPU-KERNEL-1 green | **Closed (separate later gate; no default production schedule yet)** | Column kernel + registered-node dispatch proven; mobility scheduled dispatch still closed |
