...
| Rung | ID | Scope | Exit proof | Tier |
|---|---|---|---|---|
| 11.4 | `STUDIO-CLAUSE-LOADER-SIMPLIFY-0` | **Needs 11.1.** Scenario Library shows **only the ClauseScript loader**; the resolver/source field is removed from the load panel and **auto-populated off-screen** (sibling-file / canonical convention). The resolved source path + resolver state move to a **new "Scenario" dropdown in the Telemetry window**, alongside scenario telemetry (scenario id, star/owner counts, STEAD status, tick index, paused). Read-only; no Spec mutation. Wire production ingest `source_base`. | **PROBATION** [#1306](https://github.com/khorum08/SimThing/pull/1306) @ 8bfba01b — ClauseScript-only loader + source_base + Scenario telemetry; evidence [studio_clause_loader_simplify_0_results.md](tests/studio_clause_loader_simplify_0_results.md). Not graduated. | Tier-2 · Std |
| 11.5 | `STUDIO-FACTION-NAMEPLATES-0` | **Needs 11.2 + 11.3.** Star + planet nameplates render the **star/system name** (from 11.3) in the **owning faction's `color_rgb`** (from 11.2); unowned = neutral. Presentation only over existing `GalaxyStarNameplate` path. | NOT STARTED | Tier-2 · Std |
...
| Active open rung | `STUDIO-CLAUSE-LOADER-SIMPLIFY-0` (Phase 11; 11.1–11.3 graduated) |
| Debug baseline | `cargo build -p simthing-mapeditor --bin simthing-studio` |
| Clause load baseline | Canonical `scenarios/terran_pirate_galaxy.clause` via `hydrate_scenario_with_source_base`; 11.4 must wire production ingest `source_base` |

**Park instruction for agents:** Phase 9 complete; Phase 10 parked. Phase 11 active at `STUDIO-CLAUSE-LOADER-SIMPLIFY-0` (11.4, Std; includes Studio `source_base` wire residual). Do not reopen 0.0.8.5.
