...
| REMEDIAL | `STUDIO-STAR-NAMING-REPAIR-0` | **Needs 11.3 + canonical TP data.** Persist deterministic, seed-stable star display names into `scenarios/terran_pirate_galaxy.base_disc.json` so every canonical system resolves through `star_system_display_name()`. Data/generator/spec authority only; no mapeditor UI/render changes. | NOT STARTED | DA-reserve · **Frontier** |
