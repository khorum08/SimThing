...
| RF-2 | `RF-EXECUTE-RECURSIVE-DEFAULT-0` | Flip `resource_flow_execution_profile` default to recursive executed source; wire executed reduce-up + disburse-down + `runtime_local_allocation` writeback in ordinary `step_once`; retire legacy default tick source. RF-1 oracle proves it. **Needs RF-1.** | DISPATCHED 2026-07-17 (HD-RECEIPT 6a2771cb341f; manual progression) | DA-reserve |
...
| Active open rung | `RF-EXECUTE-RECURSIVE-DEFAULT-0` — RF-2 DISPATCHED 2026-07-17 (HD-RECEIPT 6a2771cb341f, base 14c714f2); MANUAL PROGRESSION (Owner drives coder+orchestrator; orchestrator delegated merge). The load-bearing RF execution flip; RF-1 oracle is the judge. Queue after: RF-3 → RF-4 (=12.9). |
