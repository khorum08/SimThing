...
| 12.9 | `STUDIO-FIELD-SESSION-ELEVATE-0` | **Needs 12.8.** The Studio live bridge opens the **field-bearing session path** (`open_from_spec` + authored profile — elevating the TP-LIVE-RUN-0 workshop residue to production) so the authored fields accumulate under live ticks: disruption accretes from authored emitters, production/need accrete from authored buildings and policy overlays, decisions fire only as threshold crossings (sealed ingress per OC-K-DECISION-INGRESS-0). **No bespoke economy code in the tick** — generic RF/STEAD pipeline only; the structural-shell path remains available as fallback. Replaces the property-strip posture for field-bearing scenarios. **[OVL]** — ops-telemetry rows: session path (structural-shell vs field-bearing), per-tick field accretion samples; Owner screenshot verifies live accretion. | **OWNER OVL PASS / orchestration-review-complete / DA-relay-ready / RF-5 SPLIT APPROVED** — Owner/orchestration ruling [#1413 comment `5012654906`](https://github.com/khorum08/SimThing/pull/1413#issuecomment-5012654906) closes replacement [OVL] on tested implementation `4334acda` and executable SHA-256 `4f2fa46b3152974e7957117c99ef50190614a319ed3ba8fecda4a89b688ff4c9`. Final policy is high-performance/nonfallback, exact RTX 4080 Laptop / NVIDIA `0x10de` / discrete fail-closed, supported non-DX12 Bevy backends, and truthful actual-backend telemetry. DX12-only and backend-all crash evidence remains superseded history. Owner accepted Vulkan proof for 1,500 systems / 2,714 links and the field-bearing recursive transition on real `Owner terran / 3 siblings`, loaded/live `0/15`; headless bite remains `15/10`, marginal `5`, governed-Balance disconnect → `ResidualNotIntegrated`. RF-5 split unchanged; screenshot C remains out of RF-4. HD-RECEIPT `a8e70c897f36`. | DA-reserve · **Frontier** |
| 12.10 | `TP-EMERGENT-TENSION-PROOF-0` | **Needs 12.9.** Falsification battery for **emergence, not scripting**: (a) multi-tick canonical TP session — Terran production fields accrete and construction thresholds fire from field state; (b) Pirate presence accretes disruption that suppresses local flows **through field coupling** (an authored coupling term, never a code branch); (c) **policy-sensitivity proof** — changing *only* the clause-authored owner overlay weights (e.g. Pirate disruption-need up, Terran manufacturing-need down) produces materially different macro outcomes with **zero code change**; (d) R6C oracle cited as reference behavior where comparable, never as implementation. **[OVL]** — ops-telemetry rows: per-owner macro gauges (production accreted, disruption accreted, construction crossings fired) under the two policy-weight authorings; Owner screenshot pair verifies the divergence. | **DA-GRADUATED / merged [#1418](https://github.com/khorum08/SimThing/pull/1418) @ `ced8ce2d`** — 2026-07-20 (DA graduation comment `5022399857`; Owner OVL PASS/CLOSED `5018879166`; Fork A fixture tip retained). Emergence + policy-sensitivity falsifiers green; freeze SHA-256 `66aa9ca8…e99c0`. Evidence: [tp_emergent_tension_proof_0_results.md](tests/tp_emergent_tension_proof_0_results.md). | DA-reserve · **Frontier** |
...
| RF-4 | (12.9 resumes) `STUDIO-FIELD-SESSION-ELEVATE-0` | Studio field-bearing consumes the now-executing RF; telemetry binds to the real ancestor aggregate; Owner OVL closes. Need/weight-profile install seam resolves here or splits to RF-5. **Needs RF-2.** | **DA-GRADUATED / merged [#1413](https://github.com/khorum08/SimThing/pull/1413) @ `99fa9d06`** — 2026-07-18. Recursive Arena RF consumed by Studio field-bearing; Owner OVL closed; RF-1 and exact RTX policy green; need transport split to RF-5. HD-RECEIPT `a8e70c897f36`. | DA-reserve |
| RF-5 | `RF-NEED-WEIGHT-PROFILE-TRANSPORT-0` | **Bounded need/`weight_profile` transport.** Generic binding of hydrated need/weight EML to an Arena participant AllocatorWeight cell through ordinary open; authored weights change live need and sealed threshold outcome. **Needs RF-4 + RF-5A.** | **DA-GRADUATED / merged [#1414](https://github.com/khorum08/SimThing/pull/1414) @ `da8f7035`** — 2026-07-19. DA ruling `5017421913`; exact final head `59d76baf`; accepted implementation `71005e59`; frozen executable source `85056ee1`, SHA-256 `81d34679…5a71`; Owner screenshot-C OVL closure `5017017028` remains authoritative. | DA-reserve |
| RF-5A | `RF-NEED-BINDING-AUTHORING-0` | **Prerequisite authoring surface for RF-5.** Generic scenario-agnostic ClauseScript `need_binding`: participant named entity; input/weight as `(entity, property, role)` identities; threshold/event; profile-id join. Admission resolves semantic identity to full live `(slot, column)` via entity uniqueness + role pathway. Existing EvalEML / AllocatorWeight / sealed threshold only. **Needs RF-4. Unblocks RF-5.** | **DA-GRADUATED / merged [#1416](https://github.com/khorum08/SimThing/pull/1416) @ `3ad5add3`** — 2026-07-19; DA graduation authority comment `5016080151`; candidate `acbf6a26`; implementation `d53d5253`; evidence `79e5ed4c`. | DA-reserve |
...
| Active open rung | `STUDIO-FLEET-ICONS-0` — **12.5 PROBATION / DRAFT** (HD-RECEIPT `c88f057a19fc`; Owner icon-base rider binding; [OVL] open). 12.3 is **DA-GRADUATED / merged [#1420](https://github.com/khorum08/SimThing/pull/1420) @ `61abf63b`** (DA ruling `5066605782`; Owner OVL CLOSED). `STUDIO-OWNER-CLOSURE-0` remains DEFERRED / Owner-gated. |
...
**Park instruction for agents:** Phase 9 complete; Phase 10 parked; Phase 11 complete (2026-07-12); Phase 12 **ACTIVE**; 12.3 graduated; active open rung is `STUDIO-FLEET-ICONS-0` (12.5). Track closeout lives only in `STUDIO-OWNER-CLOSURE-0` (§5b) and is deferred until explicit Owner authorization. Do not reopen 0.0.8.5.
