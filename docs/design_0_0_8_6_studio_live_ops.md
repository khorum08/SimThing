...
| Rung | ID | Scope | Exit proof | Tier |
|---|---|---|---|---|
| 12.6 | `TP-FIELD-ECONOMY-GRAMMAR-0` | **Generic ClauseScript grammar + hydration** for field-enrolled economics: production buildings (factory/starport chains), stockpile silos, field-enrolled resource quantities, disruption-emitting presence, and **owner policy overlay blocks** — need/opportunity **weight profiles** (expansion-need, disruption-need, manufacturing-need) lowering onto existing overlay/RF/FIELD_POLICY surfaces (`OverlaySpec`, `ResourceEconomySpec`, EML weight profiles per TP-COMMITMENTS-0). Grammar is scenario-agnostic — **zero TP tokens in clausething**; spatial enrollment obeys STEAD §5 (Location participants carry `StructuralGridPlacement`). Falsifiers: adversarial paired fixtures + a second synthetic scenario through the same grammar. | **PROBATION / proof-present / DA-review-pending** — implemented generic `field_economy` ClauseScript hydration in clausething; lowers supported production buildings, stockpile current transfers, field resource quantities, disruption presence, owner policy overlays, and EML weight profiles onto `ResourceEconomySpec`, `OverlaySpec`, and `EmlGadgetStackSpec`. Follow-up remand narrowing: production output coefficients and stockpile capacity/clamp authoring are rejected as spanned admission errors because no existing execution-bearing recipe-yield or storage-capacity surface exists; seam marked `HORIZON-ENTRY(2026-07-16)` in the lowerer for 12.7+ instead of manufacturing success via emissions/overlays. STEAD placement validation uses `StructuralGridPlacement`. Focused `field_economy_grammar_0` tests PASS and `cargo check -p simthing-clausething` PASS on 2026-07-16. Grep fence note: full-crate TP-token grep still reports pre-existing legacy TP surfaces; this rung added no scenario-specific literals in new clausething code or tests. | DA-reserve · **Frontier** |
...
| Active open rung | `TP-FIELD-ECONOMY-GRAMMAR-0` (12.6; PROBATION 2026-07-16, proof-present, DA-review-pending, receipt 1fb80d5713d5, coder Codex-CLI; orchestrator-led verification. Queue after: 12.7→12.8→12.9) |
