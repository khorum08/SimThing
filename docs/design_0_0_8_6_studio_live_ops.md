...
| 9.6 | `STUDIO-SCENARIO-LIBRARY-CREATE-0` | Create-new / blank or template from library (scope from readiness). | **PROBATION / proof-present** — Create builds a neutral one-cell ScenarioSpec shell, hydrates through `StudioSession::from_loaded_scenario`, preserves pause/no-autoplay, and roundtrips through existing scenario I/O; no TP hardcodes; evidence [`studio_scenario_library_create_0_results.md`](tests/studio_scenario_library_create_0_results.md). | Tier-2 |
