...
| 9.4 | `STUDIO-LIVE-OBSERVE-0` | **Observation surfaces:** tick, pause, optional tree-local summaries already available — no CPU planner. | **PROBATION** — pure `StudioLiveObservationReadout` + UI section over clock/bridge/session; 10 headless proofs; evidence [`studio_live_observe_0_results.md`](tests/studio_live_observe_0_results.md). Not complete / not graduated. | Tier-1 |
