...
| RF-5 | `RF-NEED-WEIGHT-PROFILE-TRANSPORT-0` | **Bounded need/`weight_profile` transport.** Generic binding of hydrated need/weight EML to an Arena participant AllocatorWeight cell through ordinary open; authored weights change live need and sealed threshold outcome. **Needs RF-4 + RF-5A.** | **ACTIVE / RESUMED / DRAFT** — PR [#1414](https://github.com/khorum08/SimThing/pull/1414); resume dispatch `5016103467`; exact RF-5A substrate merge `3ad5add3`. Canonical TP data, generic production open/step, read-only Studio telemetry, paired-weight threshold proof, and neutral second scenario are in scope. Freeze/Owner OVL remain blocked until orchestration accepts code/proofs. | DA-reserve |
...
| Active open rung | `RF-NEED-WEIGHT-PROFILE-TRANSPORT-0` — **RF-5 ACTIVE / RESUMED / DRAFT** (PR [#1414](https://github.com/khorum08/SimThing/pull/1414); resume dispatch `5016103467`; exact substrate merge `3ad5add3`). RF-5A is **DA-GRADUATED / merged [#1416](https://github.com/khorum08/SimThing/pull/1416) @ `3ad5add3`**. Code/proof/governance precede executable freeze and Owner screenshot C OVL; no self-graduation, readiness mutation, or merge. |
