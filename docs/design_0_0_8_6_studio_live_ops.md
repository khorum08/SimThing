...
| RF-2 | `RF-EXECUTE-RECURSIVE-DEFAULT-0` | Flip `resource_flow_execution_profile` default to recursive executed source; wire executed reduce-up + disburse-down + `runtime_local_allocation` writeback in ordinary `step_once`; retire legacy default tick source. RF-1 oracle proves it. **Needs RF-1.** | **PROBATION (proof-present, orchestration/DA-review-pending)** — 2026-07-17 @ tested code `61ab842f`. Ordinary default-profile `open_from_spec` + `step_once` executes admitted D=3 Arena RF in 8 OrderBands; named child marginal `5.5` reaches Owner aggregate (`5.5` with / `0` without) and local allocation (`17.5` with / `12` without), bit-exact replay; `economy_execution_deferred=false`; unchanged RF-1 oracle green. Evidence: `docs/tests/rf_execute_recursive_default_0_results.md`. | DA-reserve |
...
| Active open rung | `RF-EXECUTE-RECURSIVE-DEFAULT-0` — **RF-2 PROBATION** 2026-07-17 @ tested code `61ab842f` (HD-RECEIPT 6a2771cb341f; manual progression); default D=3 execution + named-child differential + RF-1 judge green, orchestration/DA review pending. Queue after graduation: RF-3 → RF-4 (=12.9). |
