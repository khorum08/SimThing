...
| Rung | ID | Scope | Exit proof | Tier |
|---|---|---|---|---|
| RF-1 | `RF-CONSERVATION-ORACLE-0` | **Oracle-first.** Independent closed-form conservation oracle (ADR three invariants); validated live vs current flat-opt-in RF; canonical TP reduce-up golden (RF-4 target) in workshop. Does NOT touch execution. | **PROBATION** — 2026-07-17 (coder=Grok-CLI). Independent `rf_conservation_oracle` in simthing-driver (derives ADR three invariants; no import of `owner_silo_recursive_rf_source` / recursive `runtime_rf_tick_source`); bite tests (non-conservative FAIL / conservative PASS); live validate vs FlatStarOptIn; TP reduce-up golden in workshop. Execution untouched (RF-2). | DA-reserve |
...
| Active open rung | `RF-CONSERVATION-ORACLE-0` — RF-1 **PROBATION** 2026-07-17 (HD-RECEIPT 9772abd8fcac; coder=Grok-CLI). Independent conservation oracle + TP golden landed; execution flip remains RF-2. 12.9 STUDIO-FIELD-SESSION-ELEVATE-0 is [OVL] FAIL, BLOCKED→RF-2 (resumes as RF-4). Sub-track queue: RF-1(PROBATION) → RF-2 → RF-3 → RF-4(=12.9). |
