...
| 9.2 | `STUDIO-SIM-CLOCK-UI-0` | **Transport UI:** Pause, Play, 2×, 4×, TPS selector; readout (tick index, effective rate, paused). | **PROBATION / PR pending** — transport UI + headless hooks over `StudioSimClock`; evidence [`studio_sim_clock_ui_0_results.md`](tests/studio_sim_clock_ui_0_results.md). **Next:** `STUDIO-LIVE-SESSION-BRIDGE-0` (∥ observe/library). | Tier-2 |
