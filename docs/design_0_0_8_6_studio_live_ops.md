...
| 9.8 | `STUDIO-LIVE-OPS-HARDENING-0` | Polish: cancel modal, double-open, rapid rate change, save-while-paused, no tick on modal. | **PROBATION** — shared bridge reset for JSON/Clause/create/reopen replacement; cancel purges pending actions and retains Pause; 13-test edge regression battery. | Tier-1 |
