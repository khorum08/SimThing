...
| REMEDIAL | `STUDIO-STAR-NAMING-REPAIR-0` | **Needs 11.3 + canonical TP data.** Preserve deterministic, seed-stable canonical star display names through embedded ClauseScript hydration so every loaded canonical system resolves through `star_system_display_name()`. Clausething authority metadata only; no mapeditor UI/render changes. | **PROBATION / [transport PR #1317](https://github.com/khorum08/SimThing/pull/1317) @ `847e21be` / DA-RESERVE** — transports the DA-passed #1316 repair onto master; carries all 1,500 canonical names through production clause load; canonical JSON was already byte-current; determinism, structure, ownership, loader, 11.2 identity, and render-source boundaries proven; evidence [studio_star_naming_repair_0_results.md](tests/studio_star_naming_repair_0_results.md). | DA-reserve · **Frontier** |
...
| Active open rung | `STUDIO-STAR-NAMING-REPAIR-0` (Phase 11 remedial; 11.7 DA-graduated) |
...
**Park instruction for agents:** Phase 9 complete; Phase 10 parked. Phase 11 active at `STUDIO-STAR-NAMING-REPAIR-0` (remedial, Frontier, DA-reserve). Do not reopen 0.0.8.5.
