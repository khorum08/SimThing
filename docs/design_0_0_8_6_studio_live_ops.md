...
| 12.7 | `WORKSHOP-HOMING-DETECTION-0` | **Gates 12.8+ scenario authoring — mechanical block, not just attestation.** The `workshop-candidate-homing` anchor-ack forces every production-crate PR to *acknowledge* §12, but acknowledgement is attestation, not detection: the existing `SEMANTIC-WORDS` / `SPEC-LOWERER-KIND-READ` scans cover only sim/kernel/spec/clausething and **exclude test code**, so a scenario-specific test inside a sealed crate lands unscanned today. Add a **RELIABLE `scans.tsv` tripwire** matching the game-semantic vocabulary (faction / combat / terran / pirate / diplomacy + TP entity tokens) across **every production crate** (all workspace members except the `simthing-workshop` sandbox), in **both `src/**` and `tests/**`**. Net-new hits in a PR diff are a hard CI FAIL routing the code/test to `simthing-workshop`; the delta model suppresses pre-existing hits; comments and `SimThingKind::` spec-boundary refs stay excluded; neutral synthetic fixtures (e.g. `foundry_valley`) do **not** match, so generic-grammar falsifiers (as 12.6 authored) still pass in a sealed crate. Falsifiers: a scenario-named test in a sealed crate FAILs; the same test under `simthing-workshop` passes; a neutral-fixture generic test passes in a sealed crate. | TODO | DA-reserve · **Frontier** |
