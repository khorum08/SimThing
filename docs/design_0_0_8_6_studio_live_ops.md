...
| 11.5 | `STUDIO-FACTION-NAMEPLATES-0` | **Needs 11.2 + 11.3.** Star + planet nameplates render the **star/system name** (from 11.3) in the **owning faction's `color_rgb`** (from 11.2); unowned = neutral. Presentation only over existing `GalaxyStarNameplate` path. | **PROBATION** PR pending — faction-colored star nameplates from owner color_rgb; evidence studio_faction_nameplates_0_results.md. Not graduated. | Tier-2 · Std |
