...
| 9.6 | `STUDIO-SCENARIO-LIBRARY-CREATE-0` | Create-new / blank or template from library (scope from readiness). | **PROBATION / proof-present / PR [#1291](https://github.com/khorum08/SimThing/pull/1291)** — Create builds a neutral one-cell ScenarioSpec shell, hydrates through `StudioSession::from_loaded_scenario`, preserves pause/no-autoplay, and roundtrips through existing scenario I/O; no TP hardcodes; evidence [`studio_scenario_library_create_0_results.md`](tests/studio_scenario_library_create_0_results.md). | Tier-2 |
...
| Active open rung | `STUDIO-SCENARIO-LIBRARY-CREATE-0` (after 9.0–9.5 graduated) |
...
**Park instruction for agents:** 9.0–9.5 landed (clock + UI + live bridge + observation + scenario library modal). Next: `STUDIO-SCENARIO-LIBRARY-CREATE-0`; do not treat orientation pointer as closeout of 0.0.8.5.
