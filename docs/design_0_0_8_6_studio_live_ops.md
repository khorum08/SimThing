...
| 11.7 | `STUDIO-FROSTED-GLASS-0` | Window backgrounds get a real **frosted-glass** effect: slight darkening tint + backdrop **blur** of content behind the panel. **Performance is a hard exit criterion** — frame-budget-bound (e.g. one downsampled/low-radius separable blur target, not a full-res per-frame gaussian); record before/after frame-time. May add a presentation `*.wgsl` blur pass → **DA-reserve** (benign presentation shader, not kernel authority). | **PROBATION / [PR #1314](https://github.com/khorum08/SimThing/pull/1314) @ `7d49599a` / DA-RESERVE** - one shared eighth-resolution backdrop; 2 separable blur passes; dark tint retained; Settings, Telemetry, side panels, and Scenario Library covered; operator picker steers to portable `scenarios`; local frame timing `33.818 ms` baseline / `32.346 ms` frosted; 11 proofs + prior-rung regressions; evidence [studio_frosted_glass_0_results.md](tests/studio_frosted_glass_0_results.md). Phase 11 remains open pending DA graduation. | Tier-2 / DA-reserve if wgsl · **Frontier** |
...
| Active open rung | `STUDIO-FROSTED-GLASS-0` (Phase 11; 11.1–11.6 graduated) |
...
**Park instruction for agents:** Phase 9 complete; Phase 10 parked. Phase 11 active at `STUDIO-FROSTED-GLASS-0` (11.7, Frontier; DA-reserve if `*.wgsl`). Do not reopen 0.0.8.5.
