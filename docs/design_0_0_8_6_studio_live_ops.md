...
| 9.3 | `STUDIO-LIVE-SESSION-BRIDGE-0` | **Wire loaded StudioSession → live tick path** (elevate workshop live-run policy only as needed). Prefer production driver/session. Bounded theater policy from readiness if required. No new gameplay systems. | **PROBATION / PR #1285** — clock schedule → SimSession::step_once; JSON multi-tick; STEAD/session identity; no workshop import; evidence [`studio_live_session_bridge_0_results.md`](tests/studio_live_session_bridge_0_results.md). **Next:** 9.4 observe / 9.5 library. | Tier-2 |
