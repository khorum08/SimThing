...
| RF-3 | `RF-LEGACY-RETIRE-REANCHOR-0` | Repoint existing RF tests + `runtime_0080_*` rehearsal + recursive-source oracle framing to the new default + RF-1 oracle; retire dead legacy; re-anchor the ADR/anchors so recursive-executed-by-default is the doctrine future agents ingest. **Needs RF-2.** | **DISPATCHED** — repair ct_2a/ct_2c participant enrollment and replace their `NoAdapter` fail-open skips with live fail-closed proof; retire legacy framing while preserving RF-2 regressions. HD-RECEIPT `11035f9c5602`. | DA-reserve |
...
| Active open rung | `RF-LEGACY-RETIRE-REANCHOR-0` — **RF-3 DISPATCHED** (HD-RECEIPT `11035f9c5602`). RF-2A + RF-2 DA-graduated and merged in [#1411](https://github.com/khorum08/SimThing/pull/1411) @ `c206b0ef`; RF-3 owns legacy retirement/re-anchor plus ct_2a/ct_2c enrollment and fail-closed repair. Queue after graduation: RF-4 (=12.9). |
