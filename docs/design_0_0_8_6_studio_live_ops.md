...
| 9.5 | `STUDIO-SCENARIO-LIBRARY-UI-0` | **Toggled/hidden library window:** load / create / save (JSON authority + clause open reusing production ingest/picker). **Modal/visible ⇒ pause.** | **PROBATION / proof-present / PR [#1289](https://github.com/khorum08/SimThing/pull/1289)** — modal open pauses clock and freezes bridge; JSON authority + production ClauseScript I/O reused; close/I/O do not autoplay; Create deferred to 9.6; evidence [`studio_scenario_library_ui_0_results.md`](tests/studio_scenario_library_ui_0_results.md). | Tier-2 |
...
| Active open rung | `STUDIO-SCENARIO-LIBRARY-UI-0` (after 9.0–9.4 graduated) |
...
**Park instruction for agents:** 9.0–9.4 landed (clock + UI + live bridge + observation). Next: `STUDIO-SCENARIO-LIBRARY-UI-0`; do not treat orientation pointer as closeout of 0.0.8.5.
