...
| 9.4 | `STUDIO-LIVE-OBSERVE-0` | **Observation surfaces:** tick, pause, optional tree-local summaries already available — no CPU planner. | **PROBATION** — [#1287](https://github.com/khorum08/SimThing/pull/1287) @ `6bb75a1b`; pure `StudioLiveObservationReadout` + UI section over clock/bridge/session; 10 headless proofs; evidence [`studio_live_observe_0_results.md`](tests/studio_live_observe_0_results.md). Not complete / not graduated. | Tier-1 |
...
| Active open rung | `STUDIO-LIVE-OBSERVE-0` **PROBATION** (presentation observation landed; not graduated) → next `STUDIO-SCENARIO-LIBRARY-UI-0` |
...
**Park instruction for agents:** 9.0–9.3 landed (clock + UI + live session bridge). Next: `STUDIO-LIVE-OBSERVE-0`; do not treat orientation pointer as closeout of 0.0.8.5.
