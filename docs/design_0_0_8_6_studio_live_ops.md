...
| Rung | ID | Scope | Exit proof | Tier |
|---|---|---|---|---|
| 12.2 | `STUDIO-DISRUPTION-READOUT-0` | **Needs 12.8 for live values.** **Read-only per-system disruption snapshot from the live session.** Read-only accessor surface over the field-bearing session (12.8): max-disruption-accreted per star-system gridcell, snapshot-consistent per tick, `0.0` when the field is absent (fail-soft to neutral, fail-loud on readback error) — so it also holds over the structural-shell fallback. Wire through the bridge to a mapeditor-consumable map keyed by generated system id. **No writes to field state; no scheduling changes; no kernel/WGSL semantics.** | **PROBATION / proof-present / DA-review-pending.** Coder implementation on branch `coder/studio-disruption-readout-0`: spec-owned typed read-only disruption snapshot with private records behind `records()`, generated-system-id translation, fail-soft absent-field and structural-shell `0.0`, fail-loud typed readback error, mapeditor typed map via live bridge readout, `HORIZON-ENTRY(2026-07-16): 12.8 STUDIO-FIELD-SESSION-ELEVATE-0 wires live values` seam, and no field/kernel/WGSL/scheduling/Spec mutation surface. Focused proofs: `cargo test -p simthing-spec --test studio_disruption_readout_0`; `cargo test -p simthing-mapeditor --test studio_disruption_readout_0`; `cargo check -p simthing-spec`; `cargo check -p simthing-clausething`; `cargo check -p simthing-mapeditor`. Handoff receipt `21f4b66d4de1`; DA stamps graduation at merge. | DA-reserve · **Frontier** |
...
| Active open rung | `STUDIO-DISRUPTION-READOUT-0` (12.2; PROBATION / proof-present / DA-review-pending, receipt 21f4b66d4de1, branch `coder/studio-disruption-readout-0`. Queue after DA review: field-economy chain 12.6→12.7→12.8→12.9) |
