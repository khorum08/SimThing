...
| 12.9 | `STUDIO-FIELD-SESSION-ELEVATE-0` | **Needs 12.8.** The Studio live bridge opens the **field-bearing session path** (`open_from_spec` + authored profile — elevating the TP-LIVE-RUN-0 workshop residue to production) so the authored fields accumulate under live ticks: disruption accretes from authored emitters, production/need accrete from authored buildings and policy overlays, decisions fire only as threshold crossings (sealed ingress per OC-K-DECISION-INGRESS-0). **No bespoke economy code in the tick** — generic RF/STEAD pipeline only; the structural-shell path remains available as fallback. Replaces the property-strip posture for field-bearing scenarios. **[OVL]** — ops-telemetry rows: session path (structural-shell vs field-bearing), per-tick field accretion samples; Owner screenshot verifies live accretion. | **PROBATION / OWNER [OVL] PASS / RF-5 SPLIT APPROVED / DA-review-pending** — Owner evidence at tick 9 shows field-bearing ordinary `step_once`, recursive RF active, and the real Owner aggregate `0 → 15`; the headless child-disabled control remains `10` (exact marginal `5`) and governed-Balance disconnect yields `ResidualNotIntegrated`. Need/`weight_profile` transport is explicitly deferred to bounded RF-5; no Studio mirror was added. HD-RECEIPT `a8e70c897f36`. | DA-reserve · **Frontier** |
...
| RF-4 | (12.9 resumes) `STUDIO-FIELD-SESSION-ELEVATE-0` | Studio field-bearing consumes the now-executing RF; telemetry binds to the real ancestor aggregate; Owner OVL closes. Need/weight-profile install seam resolves here or splits to RF-5. **Needs RF-2.** | **PROBATION / OWNER [OVL] PASS / RF-5 SPLIT APPROVED / DA-review-pending** — recursive RF proof passes; Owner visual evidence binds the canonical tick-9 `0 → 15` real Owner aggregate; #1405 salvage is complete and closed unmerged. Need transport is deferred to bounded RF-5 at the missing admitted consumer contract. HD-RECEIPT `a8e70c897f36`. | DA-reserve |
...
| Active open rung | `STUDIO-FIELD-SESSION-ELEVATE-0` — **RF-4 / 12.9 PROBATION / OWNER [OVL] PASS / RF-5 SPLIT APPROVED / DA-review-pending** (HD-RECEIPT `a8e70c897f36`). RF-3 DA-graduated and merged in [#1412](https://github.com/khorum08/SimThing/pull/1412) @ `d42b9109`; RF-4 consumes ordinary recursive-default RF and proves the real Owner aggregate. [#1405](https://github.com/khorum08/SimThing/pull/1405) is closed unmerged after salvage completion; need/`weight_profile` transport is deferred to bounded RF-5. |
