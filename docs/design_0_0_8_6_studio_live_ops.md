...
| RF-5 | `RF-NEED-WEIGHT-PROFILE-TRANSPORT-0` | **Bounded need/`weight_profile` transport (approved RF-4 split).** One generic spec-owned binding from existing hydrated need/`weight_profile` EML data to an existing Arena participant role/cell; install through ordinary GameMode/session-open and existing EML/Accumulator machinery. Authored weights change live need state and FIELD_POLICY threshold outcome with zero code change; read-only telemetry only. Canonical + neutral second scenario; RF-4 regressions and exact RTX policy remain green. **[OVL]** screenshot C: scenario, tick, authored profile identity/value, live need value, threshold result from frozen executable. Owner alone closes OVL. **Needs RF-4.** | **DISPATCHED / DRAFT** — PR #1414; HD-RECEIPT `9132665903e6`. | DA-reserve |
...
| Active open rung | `RF-NEED-WEIGHT-PROFILE-TRANSPORT-0` — **RF-5 DISPATCHED / DRAFT** (HD-RECEIPT `9132665903e6`). RF-4/12.9 **DA-GRADUATED / merged [#1413](https://github.com/khorum08/SimThing/pull/1413) @ `99fa9d06`**; Owner OVL closed. Bounded need/`weight_profile` transport only — no 12.10 macro emergence; Owner alone closes RF-5 screenshot-C OVL. |
