...
| RF-2 | `RF-EXECUTE-RECURSIVE-DEFAULT-0` | Flip `resource_flow_execution_profile` default to recursive executed source; wire executed reduce-up + disburse-down + `runtime_local_allocation` writeback in ordinary `step_once`; retire legacy default tick source. RF-1 oracle proves it. **Needs RF-1.** | **PROBATION (remand-proof-present, orchestration-review-pending)** — 2026-07-17 @ tested code `6aa302b5`. Ordinary default-profile `open_from_spec` + `step_once` executes admitted D=3 Arena RF in 8 OrderBands. Fixed siblings retain aggregate `5.375`; selected marginal `5.5` raises it to `10.875`; downstream leaf allocations retain the marginal. Actual Owner Balance delta `-0.0000019073486` matches the non-zero allocator residual; unchanged RF-1 composite allocator + structural/no-orphan PASS (recipe vacuous); disconnecting only `governed_by` yields actual delta `0` and `ResidualNotIntegrated`. Explicit `DefaultDisabled` dispatches 0 RF bands and writes no aggregate/AllocatedFlow. Evidence: `docs/tests/rf_execute_recursive_default_0_results.md`. | DA-reserve |
...
| Active open rung | `RF-EXECUTE-RECURSIVE-DEFAULT-0` — **RF-2 PROBATION** 2026-07-17 @ tested code `6aa302b5` (HD-RECEIPT 6a2771cb341f; manual progression); sibling-aware D=3 execution, measured non-zero Balance residual, RF-1 composite judge, runtime disconnect falsifier, and explicit opt-out green; orchestration review pending. Queue after graduation: RF-3 → RF-4 (=12.9). |
