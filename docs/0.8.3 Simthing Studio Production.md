...
**Owner / RF channel doctrine:** Owners are GameSession children and resource-flow channel scopes, not spatial parents. Every SimThing may carry owner/channel metadata. Changing ownership updates metadata/properties/columns, not spatial parentage. RF resolves locally first; only surplus/deficit reduces upward to planet, star system, galaxy, session, and owner channel nodes.

- Owners are GameSession children, not overlays and not spatial parents.
- Spatial participants reference owners through properties/columns.
- GalaxyMap / WorldStateMap remains the root spatial Location SimThing.
...
- Planet gridcells and admitted non-grid children can now contribute owner/channel-scoped RF participant inputs using metadata/properties/columns, not spatial owner-parenting.
- Participant admission depends on the recursive local-grid doctrine from #790/#791/#792.
...
- Planet gridcell and non-grid child RF participants now reduce into scoped owner/resource/scope buckets.
...
- **simthing-spec** exports `evaluate_planet_child_rf_reduce_up` with per-bucket `surplus_total`, `deficit_total`, `net_surplus`, and `net_deficit`.
- **simthing-driver** compiles per-bucket AccumulatorOp participant-sum proof plans via `compile_planet_child_rf_reduce_up_gpu_proof_plan`.
...
- **simthing-spec** exports `owner_silo_writeback_inputs_from_planet_child_reduce_up` and `apply_owner_silo_runtime_writeback_cpu` with checked arithmetic, capacity clamp, and unmet deficit recording.
...
**Date / PR:** 2026-06-20 — PR TBD — OWNER-SILO-DISBURSE-DOWN-0 (merge TBD; builds on PR #797)
...
1. Allocation application to runtime local participant state remains next after disburse-down proof.
2. Runtime tick integration remains after writeback/disburse-down/application boundary proof.
3. Star-system local-grid GPU operators remain deferred.
4. Fleet movement/combat remains deferred.
5. Broader scenario corpus ingestion UX / batch reports.
6. UI affordances for planet/structural editing if command layers remain headless.
7. Cohort/population/resource overlays under admitted planets.
