...
- Scenario/model save/load UI is not wired yet (backend file IO exists; menu/button hooks deferred).
...
Scenario/model save/load UI, runtime vertical-test loading, platform-specific config directories, new-map flow, full editor mutation command surface beyond the proof helper, live SimThing simulation, RF execution arenas, heatmap rendering, pathfinding, route/predecessor semantics, movement-order logic, semantic WGSL, simulation GPU kernels, Clausewitz UI importer, CSS/WebView, exact typed property representation, and new SimThing kinds remain deferred.

## Next Production Rungs

1. Wire scenario/model save/load UI (Save Scenario / Load Scenario) to `scenario_io` helpers.
2. Wire ClauseThing imports and future runtime vertical-test loading into the scenario authority path.
3. Expand mutation/edit commands that update `SimThingScenarioSpec` first and refresh projections.
4. Advance heatmap/RF visualization only after canonical scenario inputs stay enforced.
5. Platform-specific config and scenario file directories.
