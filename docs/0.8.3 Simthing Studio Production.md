...
SimThingScenarioSpec
  root: Scenario
    -> GameSession
       -> Owner(s)
  structural_grid: transitional/canonical structural metadata until GalaxyMap child is enforced
  links: structural adjacency/link metadata
  sidecars: transitional serde mirrors only (scenario_id, provenance)
```

Owners are **GameSession children** — SimThings, not overlays and not spatial parents. Owner identity (`owner_id`, display name, archetype), stockpile/silo placeholders, policies, bonuses, penalties, and capability/talent/tech subtree placeholders live on Owner SimThings. Assets refer to Owners through owner references, properties, and columns — never through spatial reparenting. GalaxyMap / WorldStateMap remains deferred to **SESSION-GALAXYMAP-WORLDSTATE-0**. Resource-flow reduce-up/disburse-down and silos remain deferred to **SESSION-RESOURCE-FLOW-SILOS-0**.

World-root fixtures are **legacy compatibility only**. Terran Pirate is a **lower-layer golden fixture** only — not the canonical save-game tree shape. Generated/hydrated galaxies will eventually hang a GalaxyMap/WorldStateMap under GameSession; that enforcement is deferred to SESSION-GALAXYMAP-WORLDSTATE-0.

Historical STEAD layouts may still resolve a transitional `World` spatial subtree under GameSession (or legacy direct World under Scenario) for mapping validation only; World is not the running session root.
