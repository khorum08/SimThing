...
1. **SESSION-OWNER-ENTITIES-0** — Owner entities as GameSession sibling children.
2. **SESSION-GALAXYMAP-WORLDSTATE-0** — GalaxyMap/WorldStateMap under GameSession.
3. **GENERAL-SCENARIO-INGESTION-ADMISSION-0** — arbitrary scenario ingestion admission beyond minimal corpus.
4. **STUDIO-SCENARIO-LOAD-SAVE-DISPLAY-0** — Studio load/save/display over canonical Scenario -> GameSession authority.
5. GPU-resident execution/visualization only after canonical scenario/session/map authority remains enforced.
