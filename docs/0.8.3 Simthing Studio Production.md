...
**Date / PR:** 2026-06-19 — STUDIO-SCENARIO-LOAD-SAVE-DISPLAY-0
...
- **GENERAL-SCENARIO-INGESTION-ADMISSION-0** and **SESSION-RESOURCE-FLOW-SILOS-0** remain deferred.
...
1. **GENERAL-SCENARIO-INGESTION-ADMISSION-0** — arbitrary scenario ingestion admission beyond minimal corpus.
2. **SESSION-RESOURCE-FLOW-SILOS-0** — stockpile reduce-up/disburse-down execution under Owners.
3. GPU-resident execution/visualization only after canonical scenario/session/map authority remains enforced.
4. Studio structural placement edits and full editor mutation command surface beyond safe metadata roundtrip.
