...
- Fixed-point `i32` discipline: for each link `(a,b)`, `output[a] += input[b]` and `output[b] += input[a]` using `atomicAdd` in WGSL.
- CPU oracle `cpu_structural_link_accumulate_i32` compares against GPU readback; vertical seed `input=[10,20]` yields `output=[20,10]`.
