...
| HC-2 | `HC-GUARD-KABUKI-TRIPWIRE-0` | **Mechanize anti-kabuki rule 2 (defect 2).** Add a HEURISTIC scan (`scans.tsv` row + selftest) catching bespoke source-scanning guards: production `pub fn` taking `source: &str`/`&Path` that string-scans, and `include_str!("../src/` in tests. Routes to INSPECT + triage (not FAIL) — legitimate cases exist and must be justifiable, not silenced. Prose rule stays; the tripwire is its mechanized rung-3. | **PROBATION / proof-present / DA-review-pending** - `GUARD-KABUKI-TRIPWIRE` added as INSPECT-only HEURISTIC with sandbox-generated selftest controls and row-removal falsifier; prose rule demoted to scan pointer; existing live hits accounted in triage; evidence `docs/tests/hc_guard_kabuki_tripwire_0_results.md`. | Std |
