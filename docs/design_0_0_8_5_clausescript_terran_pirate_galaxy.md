...
| 0.2 | `TP-SCALE-ENVELOPE-0` | **PROBATION / INSPECT(1)** (branch evidence). Proves the **base 1500-star disc** generation, connectivity, lattice hierarchy, budgeted RF admission, link lowering, typed atlas deferral, and `install_atomic` at scale through the accepted widened capacity budget. It consumes `TP-RF-CAPACITY-AMENDMENT-0` as COMPLETE and does not reopen it. Evidence: [`docs/tests/tp_scale_envelope_0_results.md`](tests/tp_scale_envelope_0_results.md). | Headless proof added in `simthing-clausething/tests/tp_scale_envelope.rs`: deterministic 1500-star disc, connected topology, derived structural frame > standard field cap, 1500 participants per RF arena through budgeted caps, 7505-slot install footprint. Local real-adapter `SimSession::open_from_spec` attempts the scale pack but typed-skips after `initial_gpu_sync` reduction-topology upload rejects the scale shape; no compact mapping surfaces are fabricated. | Tier-1 over 0.1 |

### Phase 1 — Base galaxy production (mostly reuse; Studio-consistency proof)

**Next active rung remains gated by DA review of `TP-SCALE-ENVELOPE-0` (0.2).** The 0.1 capacity-amendment gate is DA-cleared (above); `TP-SCALE-ENVELOPE-0` is implemented as PROBATION / INSPECT(1), not COMPLETE. Phase 1+ content rungs remain gated until DA/Owner adjudicates the scale proof and the live-adapter reduction-topology inspect.
