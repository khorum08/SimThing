...
| 8.10r | `CLEARANCE-ADMITTED-SCOPE-GAP-0` | **Harness (gate-wiring).** Implement `DA-RESERVE(admitted-scope-router-gap)` in `clearance_check.sh` + orientation/docs; body fields `admitted_envelope` / `admitting_pr` / surfaces; selftests for true unknown vs admitted gap vs FAIL proofs vs novelty vs gate-wiring. | **OPEN — next production pointer.** Exit: selftests named in ruling results; not a Phase 8 product feature. | Tier-2 (gate-wiring) |
| 8.10h | `TP-STUDIO-CLAUSE-PICKER-CLASS-0` | **Harness adjacency.** Register a precedented class for admitted narrow UI picker shape (mapeditor picker module + UI/app glue + results + inventory + design/orient stamps) so #1239-shaped follow-ups are ORCHESTRATOR-CLEARABLE; reject TP defaults, dual parse/rebind, GameMode/RF/live-run/closeout, engine/runtime. | **OPEN — after or parallel to 8.10r.** Exit: clearance selftests clearable + reject fixtures. | Tier-2 (gate-wiring) |
