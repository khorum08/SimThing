...
**Next active rung: `TP-SCALE-ENVELOPE-0` (0.2).** The 0.1 capacity-amendment gate is DA-cleared (above); Phase 1+ content rungs remain gated behind `TP-SCALE-ENVELOPE-0` proving install-at-scale through the widened caps. This DA clearance is scoped to the RF capacity amendment only — it does not review or accept `TP-SCALE-ENVELOPE-0` or any Phase 1 content.
