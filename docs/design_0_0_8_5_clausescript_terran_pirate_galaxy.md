...
| 0.2 | `TP-SCALE-ENVELOPE-0` | **HELD** (orchestrator-tier adjudication 2026-07-01; PR #1073 open). Generate → lattice → RF-budget → link → `install_atomic` legs are proven at 1500-star scale. Terminal exit proof was false-green (swallowed `upload_reduction_topology` panic); repair tracked as `TP-SCALE-ENVELOPE-0R`. Evidence: [`docs/tests/tp_scale_envelope_0_results.md`](tests/tp_scale_envelope_0_results.md). | Real-adapter `mapping.is_none()` not accepted until orchestrator review after `TP-SCALE-ENVELOPE-0R`. | Tier-1 over 0.1 |
| 0.2r | `TP-SCALE-ENVELOPE-0R` | **PROBATION** (repair on PR #1073). Remove catch_unwind false-green path; fix `rebuild_for_slots` `column_rules` sizing when slot growth coincides with registry expansion. | Scale test no longer swallows panics; `upload_reduction_topology` no longer overruns 24-byte `column_rules`; orchestrator verifies full session open + `mapping.is_none()` on capable hardware. | Tier-2 (`seal-residue-risk`) |

### Phase 1 — Base galaxy production (mostly reuse; Studio-consistency proof)

**Phase 1+ remains blocked.** `TP-SCALE-ENVELOPE-0R` (repair) is the next active rung on PR #1073; orchestrator review required before Phase 1 opens.
