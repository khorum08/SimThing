...
| 8.7 | `TP-STUDIO-CLAUSE-API-ADMISSION-1` | **Owner/DA-gated.** Re-open production mapeditor ClauseScript API admission only with StructuralRebindReady proven and lowerer debt scoped. Default deny → keep workshop. | **OPEN — next production pointer.** Exit: DA/Owner decision record; still no UI picker until API admitted. | Tier-2 (substrate/API admission) |
