...
| 8.10 | `TP-STUDIO-CLAUSE-PICKER-0` | **Implement** admitted narrow UI `.clause` picker/menu: user selects path + explicit resolver entries; call production API only; surface unresolved-placeholder/errors; session hydrate PASS. | **DA-GRADUATED / merged [#1239](https://github.com/khorum08/SimThing/pull/1239) @ `0183883767`** (head `d9bd5c4054`) — mapeditor `clause_scenario_picker` + Studio **Open ClauseScript Scenario...**; production `clause_scenario_ingest` only; explicit `TOKEN=path` resolver; StructuralRebindReady session hydrate PASS; unresolved placeholders error-surfaced; no TP defaults / dual parse-rebind / GameMode-RF-live-run-closeout. Clearance `DA-RESERVE(unclassified-scope)` accepted as router friction (no picker class yet). Evidence: [`tp_studio_clause_picker_0_results.md`](tests/tp_studio_clause_picker_0_results.md). **Next:** optional `TP-STUDIO-CLAUSE-PICKER-CLASS-0`. | Tier-2 (implementation under admission) |
| 8.10h | `TP-STUDIO-CLAUSE-PICKER-CLASS-0` | **Harness adjacency (optional).** Register a precedented class for admitted narrow UI picker shape (mapeditor picker module + UI/app glue + results + inventory + design/orient stamps) so #1239-shaped follow-ups are ORCHESTRATOR-CLEARABLE; reject TP defaults, dual parse/rebind, GameMode/RF/live-run/closeout, engine/runtime. | **OPEN — next production pointer (optional gate-wiring).** Exit: clearance selftests clearable + reject fixtures; not required for Phase 8 product completeness. | Tier-2 (gate-wiring) |
