...
| 8.4 | `TP-STUDIO-CLAUSE-API-ADMISSION-0` | **Owner/DA-gated.** Admit a generic, non-TP-default `simthing-mapeditor` ClauseScript ingest API only if reusable and semantic-free (elevation of workshop candidate or reimplementation). Default deny → keep workshop. | **DONE — DA-DENIED / KEEP WORKSHOP (2026-07-09, Option B).** No production mapeditor ClauseScript ingest API admitted during 0.0.8.5. Stack audit: parse/hydrate/serde already production where clean; residual TP packaging, incomplete empty-placement projection, and lowerer owner-key heuristics are **not** yet elegant enough for future scenario widenings. Workshop #1222 candidate remains expirable evidence. Evidence: [`tp_studio_clause_api_admission_0_results.md`](tests/tp_studio_clause_api_admission_0_results.md). **Next:** STEAD rebind readiness, then re-open admission. | Tier-2 (substrate/API admission) |
| 8.5 | `TP-STUDIO-STEAD-REBIND-READINESS-0` | Evaluate the gap from candidate `SimThingScenarioSpec` (authority tree, empty placements/links) to full Studio session hydrate: placement/link rebind onto authority nodes, `map_container_id` / structural grid policy, Spec vs pack-only metadata, `GameModeSpec`/RF attach scope, and named projection modes for a future API admission. **No implementation.** | **DONE — DA-facing evaluation report present** in [`tp_studio_stead_rebind_readiness_0_results.md`](tests/tp_studio_stead_rebind_readiness_0_results.md). Candidate Spec is `AuthorityTreeCandidate` (empty placements/links/`map_container_id`); Studio session hydrate needs `StructuralRebindReady`. LIVE-RUN rebind remains `LiveRunWorkshopOnly`. No mapeditor `.clause` API; no UI picker. **Next:** `TP-STUDIO-STEAD-REBIND-0`. | Tier-1 (docs/evaluation only) |
| 8.6 | `TP-STUDIO-STEAD-REBIND-0` | Workshop-homed (default): project `AuthorityTreeCandidate` → `StructuralRebindReady` Spec (bind placements/links/`map_container_id` onto authority ids from embedded lattice). Prove `validate_stead_mapping_consistency` PASS; optional Studio session hydrate without production `.clause` API. | **DONE — DA-facing proof present** in [`tp_studio_stead_rebind_0_results.md`](tests/tp_studio_stead_rebind_0_results.md): workshop `tp_studio_stead_rebind`; map_container_id + placements bound to authority GalaxyMap node ids; STEAD validate PASS; authority serde roundtrip PASS; links remapped from embedded lattice. **No** production mapeditor `.clause` API. **Next:** `TP-STUDIO-CLAUSE-API-ADMISSION-1` (or session-hydrate proof). | Tier-1 (workshop candidate implementation) |
