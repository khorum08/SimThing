...
| 8.10 | `TP-STUDIO-CLAUSE-PICKER-0` | **Implement** admitted narrow UI `.clause` picker/menu: user selects path + explicit resolver entries; call production API only; surface unresolved-placeholder/errors; session hydrate PASS. | **OPEN — next production pointer.** Exit: picker invokes production API only; no dual parse/rebind; no TP defaults; error surface on unresolved placeholders; session hydrate PASS; no GameMode/RF/live-run/closeout. | Tier-2 (implementation under admission) |
