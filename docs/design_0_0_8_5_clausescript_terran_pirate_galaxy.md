...
| 8.10h | `TP-STUDIO-CLAUSE-PICKER-CLASS-0` | **Harness adjacency.** Register a precedented class for admitted narrow UI picker shape (mapeditor picker module + UI/app glue + results + inventory + design/orient stamps) so #1239-shaped follow-ups are ORCHESTRATOR-CLEARABLE; reject TP defaults, dual parse/rebind, GameMode/RF/live-run/closeout, engine/runtime. | **DONE** — class `tp-studio-clause-picker`; #1239-shaped clearable; reject fixtures for missing fields / API-only / TP defaults / dual parse-rebind / closeout / runtime; admitted-scope-gap + API non-regression. Evidence: [`tp_studio_clause_picker_class_0_results.md`](tests/tp_studio_clause_picker_class_0_results.md). **Next:** Owner/DA Phase 8 completeness decision (not closeout). | Tier-2 (gate-wiring) |
