...
**Design-ladder anti-drift (binding):** update each rung's exit-proof/status cell before declaring the rung ready — results docs and PR bodies do not substitute for the design ladder. **After a DA passed verdict** (graduate-merge, formal admission, or formal denial), the **DA** lands and merges the exit-proof stamp (design-ladder Exit proof + results COMPLETE + orientation regen) as part of that conclusion — not a deferred orchestrator chore (see `docs/agent_onboarding.md` DA section). If only an **ORCHESTRATOR-GRADUATED** self-clear applies and the design row cannot contain the final merge SHA before merge, the orchestrator must land an immediate docs-only status-stamp PR before opening the next rung (remedial example: `TP-COMMITMENTS-0-DESIGN-STATUS-STAMP` after #1155).
...
| 8.2 | `TP-STUDIO-INGEST-READINESS-0` | Evaluate the approved `terran_pirate_galaxy.clause` file and current transpile artifacts for SimThing Studio ingest/transpile/load/save readiness. Produce a report identifying every missing surface, format bridge, UI/driver boundary, save/load pathway, and test required for Studio to ingest the ClauseScript file, transpile it, load it, save it, and reload it without semantic drift. **No implementation.** | **DA-GRADUATED / merged [#1219](https://github.com/khorum08/SimThing/pull/1219) @ `54a5a5e445`** (head `d149e6cce1`) — evaluation report [`tp_studio_ingest_readiness_0_results.md`](tests/tp_studio_ingest_readiness_0_results.md): parse/hydrate/project available but not Studio-wired; JSON ScenarioSpec path proven in mapeditor; placement rebind / picker / session STEAD gaps named. **Next:** workshop candidate then API admission. | Tier-1 (docs/evaluation only) |
| 8.3 | `TP-STUDIO-CLAUSE-INGEST-0` | Workshop-homed TP candidate: `.clause` → parse/hydrate → ScenarioSpec candidate → production authority serde save/reload. **Not** a production mapeditor API. | **DA-GRADUATED / merged [#1222](https://github.com/khorum08/SimThing/pull/1222) @ `bcbc2f4389`** (head `ee0480c438`, 0R; status stamp [#1223](https://github.com/khorum08/SimThing/pull/1223) @ `5727c5aa23`) — workshop `tp_studio_clause_ingest`; zero mapeditor production surface; FULL-TRANSPILE projection shape; canonical digest parity; malformed status path. Evidence: [`tp_studio_clause_ingest_0_results.md`](tests/tp_studio_clause_ingest_0_results.md). **Next:** API admission. | Tier-1 (workshop candidate) |
| 8.4 | `TP-STUDIO-CLAUSE-API-ADMISSION-0` | **Owner/DA-gated.** Admit a generic, non-TP-default `simthing-mapeditor` ClauseScript ingest API only if reusable and semantic-free (elevation of workshop candidate or reimplementation). Default deny → keep workshop. | **DONE — DA-DENIED / KEEP WORKSHOP (2026-07-09, Option B) / status stamp [#1224](https://github.com/khorum08/SimThing/pull/1224) @ `72a29193cd`.** No production mapeditor ClauseScript ingest API admitted during 0.0.8.5. Stack audit: parse/hydrate/serde already production where clean; residual TP packaging, incomplete empty-placement projection, and lowerer owner-key heuristics are **not** yet elegant enough for future scenario widenings. Workshop #1222 candidate remains expirable evidence. Evidence: [`tp_studio_clause_api_admission_0_results.md`](tests/tp_studio_clause_api_admission_0_results.md). **Next:** STEAD rebind readiness, then re-open admission. | Tier-2 (substrate/API admission) |
| 8.5 | `TP-STUDIO-STEAD-REBIND-READINESS-0` | Evaluate the gap from candidate `SimThingScenarioSpec` (authority tree, empty placements/links) to full Studio session hydrate: placement/link rebind onto authority nodes, `map_container_id` / structural grid policy, Spec vs pack-only metadata, `GameModeSpec`/RF attach scope, and named projection modes for a future API admission. **No implementation.** | **DA-GRADUATED / merged [#1225](https://github.com/khorum08/SimThing/pull/1225) @ `8696f1f2ca`** (head `017db4c24c`) — evaluation report [`tp_studio_stead_rebind_readiness_0_results.md`](tests/tp_studio_stead_rebind_readiness_0_results.md). Candidate Spec is `AuthorityTreeCandidate` (empty placements/links/`map_container_id`); Studio session hydrate needs `StructuralRebindReady`. LIVE-RUN rebind remains `LiveRunWorkshopOnly`. No mapeditor `.clause` API; no UI picker. **Next:** `TP-STUDIO-STEAD-REBIND-0`. | Tier-1 (docs/evaluation only) |
| 8.6 | `TP-STUDIO-STEAD-REBIND-0` | Workshop-homed (default): project `AuthorityTreeCandidate` → `StructuralRebindReady` Spec (bind placements/links/`map_container_id` onto authority ids from embedded lattice). Prove `validate_stead_mapping_consistency` PASS; optional Studio session hydrate without production `.clause` API. | **DA-GRADUATED / merged [#1226](https://github.com/khorum08/SimThing/pull/1226) @ `03afe3d152`** (head `1c41c2372c`) — workshop `tp_studio_stead_rebind`; map_container_id + placements bound to authority GalaxyMap node ids; STEAD validate PASS; authority serde roundtrip PASS; links remapped from embedded lattice. Studio session hydrate **NOT_RUN_IN_WORKSHOP**. **No** production mapeditor `.clause` API. Evidence: [`tp_studio_stead_rebind_0_results.md`](tests/tp_studio_stead_rebind_0_results.md). **Next:** `TP-STUDIO-CLAUSE-API-ADMISSION-1`. | Tier-1 (workshop candidate implementation) |
| 8.6h | `TP-WORKSHOP-CANDIDATE-CLASS-0` | **Harness adjacency (clearance router).** Register precedented class `tp-workshop-candidate-proof` so workshop-homed TP candidate rungs (src/tests + results + inventory + 0.0.8.5 design/orientation stamps) are ORCHESTRATOR-CLEARABLE when requirements pass — closes repeated `unclassified-scope` DA friction. | **DA-GRADUATED / merged [#1227](https://github.com/khorum08/SimThing/pull/1227) @ `0141f03c62`** (head `597cf030a7`) — gate-wiring; selftest 34/34; clearable path + mapeditor/engine reject fixtures. **Not** API admission. | Tier-2 (gate-wiring) |
| 8.7 | `TP-STUDIO-CLAUSE-API-ADMISSION-1` | **Owner/DA-gated.** Re-open production mapeditor ClauseScript API admission only with StructuralRebindReady proven and lowerer debt scoped. Default deny → keep workshop. | **DONE — DA-ADMITTED (2026-07-09, Option A) / limited production surface.** StructuralRebindReady (#1226) closed the empty-STEAD blocker. Admit a **narrow, generic, non-TP-default** composition API for `.clause` → `StructuralRebindReady` Spec using existing clausething parse/hydrate + rebind policy + simthing-spec serde + STEAD validate. Session hydrate is an **implementation exit proof**, not a second admission gate (Studio `from_loaded_scenario` already consumes any STEAD-valid Spec). Evidence: [`tp_studio_clause_api_admission_1_results.md`](tests/tp_studio_clause_api_admission_1_results.md). **Next:** `TP-STUDIO-CLAUSE-API-1`. | Tier-2 (substrate/API admission) |
| 8.8 | `TP-STUDIO-CLAUSE-API-1` | **Implement** admitted production surface: generic mapeditor (or elevated clausething/spec) ClauseScript scenario ingest limited to `ProjectionMode::StructuralRebindReady`. Caller-supplied path/bytes + source/include resolver; no TP/fixture defaults; no UI picker; no GameMode/RF/combat attach. **Exit must prove** `StudioSession::from_loaded_scenario` (or `load_studio_session_from_scenario_path`) on the produced Spec. | **DA-GRADUATED / merged [#1230](https://github.com/khorum08/SimThing/pull/1230) @ `820a9a2ef6`** (head `b18d19ac13`) — mapeditor `clause_scenario_ingest` + clausething `clause_scenario_projection`; caller-supplied resolver only; StructuralRebindReady; STEAD + links validate; authority serde roundtrip; `StudioSession::from_loaded_scenario` PASS; no TP defaults; no UI picker. Evidence: [`tp_studio_clause_api_1_results.md`](tests/tp_studio_clause_api_1_results.md). **Next:** picker admission. | Tier-2 (implementation under admission) |
| 8.8h | `TP-ADMITTED-CLAUSE-API-CLASS-0` | **Harness adjacency.** Register `tp-admitted-clause-api-composition` so #1230-shaped admitted production composition is ORCHESTRATOR-CLEARABLE (no `no_engine_crate`); primary-selects over workshop-candidate class; requires `admitted_api` / `no_ui_picker` / `no_tp_defaults` / `session_hydrate`. | **DA-GRADUATED / merged [#1232](https://github.com/khorum08/SimThing/pull/1232) @ `5227f08000`** (head `2163667dab`) — gate-wiring; selftest 41/41; clearable admitted shape + picker/TP-default/session/runtime reserves. Evidence: [`tp_admitted_clause_api_class_0_results.md`](tests/tp_admitted_clause_api_class_0_results.md). | Tier-2 (gate-wiring) |
| 8.9 | `TP-STUDIO-CLAUSE-PICKER-ADMISSION-0` | **Owner/DA-gated.** Admit UI `.clause` file picker / menu surface only after API-1 production composition is proven. Default deny until explicit admission. | **DONE — DA-ADMITTED (2026-07-09, Option A) / narrow UI picker.** Production API + session hydrate proven (#1230); residual risk is UI/resolver error surface only (API already hard-errors unresolved placeholders). Admit minimal Studio/mapeditor affordance that selects `.clause` path + explicit resolver paths, calls production `clause_scenario_ingest`, hydrates via existing session path. No TP/fixture defaults; no API bypass; no GameMode/RF/live-run/closeout. Evidence: [`tp_studio_clause_picker_admission_0_results.md`](tests/tp_studio_clause_picker_admission_0_results.md). **Next:** `TP-STUDIO-CLAUSE-PICKER-0`. | Tier-2 (admission) |
| 8.10 | `TP-STUDIO-CLAUSE-PICKER-0` | **Implement** admitted narrow UI `.clause` picker/menu: user selects path + explicit resolver entries; call production API only; surface unresolved-placeholder/errors; session hydrate PASS. | **DA-GRADUATED / merged [#1239](https://github.com/khorum08/SimThing/pull/1239) @ `0183883767`** (head `d9bd5c4054`) — mapeditor `clause_scenario_picker` + Studio **Open ClauseScript Scenario...**; production `clause_scenario_ingest` only; explicit `TOKEN=path` resolver; StructuralRebindReady session hydrate PASS; unresolved placeholders error-surfaced; no TP defaults / dual parse-rebind / GameMode-RF-live-run-closeout. Clearance `DA-RESERVE(unclassified-scope)` accepted as router friction (no picker class yet). Evidence: [`tp_studio_clause_picker_0_results.md`](tests/tp_studio_clause_picker_0_results.md). **Next:** router gap class + picker class. | Tier-2 (implementation under admission) |
| 8.10g | `CLEARANCE-UNCLASSIFIED-SCOPE-REDUCTION-0` | **Owner/DA process ruling.** Reduce over-broad `DA-RESERVE(unclassified-scope)`: distinguish true novelty from admitted-scope router gaps so admitted proof-present implementations are not repeated DA design relays. | **DONE — DA-ADOPTED (2026-07-09, Option A).** Adopt `DA-RESERVE(admitted-scope-router-gap)` vocabulary + orchestration classification rule; router selftests deferred to implementation rung. Evidence: [`clearance_unclassified_scope_reduction_0_results.md`](tests/clearance_unclassified_scope_reduction_0_results.md). **Next:** `CLEARANCE-ADMITTED-SCOPE-GAP-0`. | Tier-2 (process/harness policy) |
| 8.10r | `CLEARANCE-ADMITTED-SCOPE-GAP-0` | **Harness (gate-wiring).** Implement `DA-RESERVE(admitted-scope-router-gap)` in `clearance_check.sh` + orientation/docs; body fields `admitted_envelope` / `admitting_pr` / surfaces; selftests for true unknown vs admitted gap vs FAIL proofs vs novelty vs gate-wiring. | **DA-GRADUATED / merged [#1243](https://github.com/khorum08/SimThing/pull/1243) @ `becb07fdd9`** (head `f7b184e994`) — machine `admitted-scope-router-gap`; 49/49 clearance selftests; 7/7 Option A matrix. Evidence: [`clearance_admitted_scope_gap_0_results.md`](tests/clearance_admitted_scope_gap_0_results.md). **Next:** `TP-STUDIO-CLAUSE-PICKER-CLASS-0`. | Tier-2 (gate-wiring) |
| 8.10h | `TP-STUDIO-CLAUSE-PICKER-CLASS-0` | **Harness adjacency.** Register a precedented class for admitted narrow UI picker shape (mapeditor picker module + UI/app glue + results + inventory + design/orient stamps) so #1239-shaped follow-ups are ORCHESTRATOR-CLEARABLE; reject TP defaults, dual parse/rebind, GameMode/RF/live-run/closeout, engine/runtime. | **DA-GRADUATED / merged [#1245](https://github.com/khorum08/SimThing/pull/1245) @ `bc041884b4`** (head `53d47a2151`) — class `tp-studio-clause-picker`; #1239-shaped ORCHESTRATOR-CLEARABLE; reject fixtures for missing fields / API-only NO / TP defaults / dual parse-rebind / closeout / runtime; API class + admitted-scope-gap non-regression; 59/59 clearance selftests. Evidence: [`tp_studio_clause_picker_class_0_results.md`](tests/tp_studio_clause_picker_class_0_results.md). **Next:** Owner/DA Phase 8 completeness decision (not closeout unless Owner declares workplan complete). | Tier-2 (gate-wiring) |
