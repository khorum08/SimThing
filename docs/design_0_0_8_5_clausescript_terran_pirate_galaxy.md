...
| 8.10h | `TP-STUDIO-CLAUSE-PICKER-CLASS-0` | **Harness adjacency.** Register a precedented class for admitted narrow UI picker shape (mapeditor picker module + UI/app glue + results + inventory + design/orient stamps) so #1239-shaped follow-ups are ORCHESTRATOR-CLEARABLE; reject TP defaults, dual parse/rebind, GameMode/RF/live-run/closeout, engine/runtime. | **OPEN — next production pointer.** Exit: clearance selftests clearable + reject fixtures. | Tier-2 (gate-wiring) |
