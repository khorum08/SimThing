...
| 8.10 | `TP-STUDIO-CLAUSE-PICKER-0` | **Implement** admitted narrow UI `.clause` picker/menu: user selects path + explicit resolver entries; call production API only; surface unresolved-placeholder/errors; session hydrate PASS. | **PROOF-PRESENT / PROBATION** — mapeditor `clause_scenario_picker` + Studio menu **Open ClauseScript Scenario...**; production `clause_scenario_ingest` only; explicit resolver text; StructuralRebindReady session hydrate PASS; unresolved placeholders error-surfaced; no TP defaults / dual parse-rebind / GameMode-RF-live-run-closeout. Evidence: [`tp_studio_clause_picker_0_results.md`](tests/tp_studio_clause_picker_0_results.md). **Next:** optional picker clearance class, else Owner/DA Phase 8 decision. | Tier-2 (implementation under admission) |
