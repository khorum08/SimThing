...
| 8.10r | `CLEARANCE-ADMITTED-SCOPE-GAP-0` | **Harness (gate-wiring).** Implement `DA-RESERVE(admitted-scope-router-gap)` in `clearance_check.sh` + orientation/docs; body fields `admitted_envelope` / `admitting_pr` / surfaces; selftests for true unknown vs admitted gap vs FAIL proofs vs novelty vs gate-wiring. | **DONE** — machine verdict + 7 DA-required selftests; empty-class split novelty → admitted-scope-router-gap → unclassified-scope; missing fields FAIL; forbidden surfaces not clearable. Evidence: [`clearance_admitted_scope_gap_0_results.md`](tests/clearance_admitted_scope_gap_0_results.md). **Next:** `TP-STUDIO-CLAUSE-PICKER-CLASS-0`. | Tier-2 (gate-wiring) |
