...
| 3.1 | `TP-FLEETS-SHIPS-0` | Author **fleets as mobile star-system-grid occupants** and **ships as cohort-style children** with HP/Damage/upkeep columns. Place 10 Terran fleets (200 ships) by the 60-40 rule and 10 Pirate fleets (400 ships) by the 80-20 rule, homed at the relevant border/interior/raid-posture systems. | **PROBATION / DA-OWNER REVIEW** — 0R applied; merge held pending DA re-review — [`tp_fleets_ships_0_results.md`](tests/tp_fleets_ships_0_results.md); `fleet_ship_payload` grammar; exact counts/distribution/parentage/owner refs/HP-Damage-upkeep metadata proven; **no kind-specific spec code landed** (Fleet/Cohort `planet_child_rf` special-case reverted); RF upkeep authored as ordinary owner/resource/deficit metadata; fleet-nested RF reduce-up **parked §0.6 Deviation** (generic child-depth admission candidate surfaced); reparent compatibility proven; not self-mergeable. | Tier-1 over 3.0 |
