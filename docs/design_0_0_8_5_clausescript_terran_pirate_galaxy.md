...
| 0.2 | `TP-SCALE-ENVELOPE-0` | **HELD** (orchestrator-tier adjudication 2026-07-01; PR #1073 open). Generate -> lattice -> RF-budget -> link -> `install_atomic` legs are proven at 1500-star scale. Terminal exit proof was false-green (swallowed `upload_reduction_topology` panic); repairs tracked through `TP-SCALE-ENVELOPE-0R2`. Evidence: [`docs/tests/tp_scale_envelope_0_results.md`](tests/tp_scale_envelope_0_results.md). | Real-adapter `mapping.is_none()` remains PROBATION until orchestrator review after `TP-SCALE-ENVELOPE-0R2`. | Tier-1 over 0.1 |
...
**Phase 1+ remains blocked.** `TP-SCALE-ENVELOPE-0R2` (repair) is the next active rung on PR #1073; orchestrator review required before Phase 1 opens.
