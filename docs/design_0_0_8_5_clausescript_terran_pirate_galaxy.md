...
| 0.2 | `TP-SCALE-ENVELOPE-0` — **HELD (DA-equivalent adjudication 2026-07-01, PR #1073 open, not merged).** The generate → lattice → RF-budget → link → `install_atomic` legs are real and proven (3000 participants / 2 arenas / 7505 slots through the accepted RF capacity budget). **But the ladder's terminal exit proof — real-adapter compact GPU readback `is_none()` — did not execute:** on a real adapter `SimSession::open_from_spec` panics in `initial_gpu_sync → WorldGpuState::upload_reduction_topology` (`Queue::write_buffer` would overrun a 24-byte destination buffer) and the test `catch_unwind`-swallows it and returns green before `assert!(session.mapping.is_none())`. A swallowed wgpu validation panic is a real scale-shape defect, not a typed deferral (contrast the Movement-Front leg's typed `is_atlas_deferral()`), so accepting it would land a false-green gate (§0.6.6). Adjudicated at the §1A orchestrator tier against the explicit contract — no metered DA turn burned. | Repair before promotion — see 0.2r. | Tier-1 over 0.1 |
| 0.2r | `TP-SCALE-ENVELOPE-0R` | **Repair the reduction-topology upload at scale.** Root-cause the 24-byte `upload_reduction_topology` destination-buffer overrun at 1500-star / 3000-participant / 7505-slot scale. Either **(a)** size the reduction-topology buffer from the resolved capacity budget / actual reduction-tree shape so a real-adapter `SimSession::open_from_spec` succeeds and yields `mapping.is_none()`, or **(b)** if reduction-topology at this dense shape is genuinely deferred, return a **typed** deferral `Err` from the session-open path (never a caught wgpu panic). Then make the scale-envelope test assert the real leg — no `catch_unwind`-swallow-and-return green path. | Headless real-adapter `SimSession::open_from_spec` at 1500-star scale either yields `mapping.is_none()` (compact readback) **or** a typed deferral result the test asserts; no swallowed panic is a passing path. | Tier-2 (kernel/driver GPU buffer sizing; `seal-residue-risk`) |

### Phase 1 — Base galaxy production (mostly reuse; Studio-consistency proof)

**Next active rung: `TP-SCALE-ENVELOPE-0R` (repair).** The 0.1 capacity-amendment gate is DA-cleared; the 0.2 scale-envelope gate (`TP-SCALE-ENVELOPE-0`, PR #1073) is **HELD** — its install-scale legs are proven, but the ladder's real-adapter compact-GPU-readback exit proof did not execute (a caught reduction-topology buffer-overrun panic; see the 0.2 row). PR #1073 stays open and unmerged; the scale-envelope proof is completed by `TP-SCALE-ENVELOPE-0R`. **Phase 1+ content rungs remain blocked** until a real-adapter session open at 1500-star scale is proven (or typed-deferred) by the repair. This adjudication was made at the §1A orchestrator tier against the explicit rung contract — deliberately **not** escalated to a metered DA/Owner turn, per the CI layer's token-discipline mission (a falsifiable contract miss needs no DA taste).
