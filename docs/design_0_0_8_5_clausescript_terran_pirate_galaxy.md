...
| 0.2 | `TP-SCALE-ENVELOPE-0` | **HELD** (DA-equivalent orchestrator adjudication 2026-07-01; PR #1073 open, not merged). The generate -> lattice -> RF-budget -> link -> `install_atomic` legs are real and proven (3000 participants / 2 arenas / 7505 slots through the accepted RF capacity budget). The original terminal exit proof was false-green because a real-adapter `SimSession::open_from_spec` panic in `initial_gpu_sync -> upload_reduction_topology` was caught and swallowed before `assert!(session.mapping.is_none())`. Repairs are tracked through `TP-SCALE-ENVELOPE-0R2`. Evidence: [`docs/tests/tp_scale_envelope_0_results.md`](tests/tp_scale_envelope_0_results.md). | Real-adapter `mapping.is_none()` remains PROBATION until orchestrator review after `TP-SCALE-ENVELOPE-0R2`. | Tier-1 over 0.1 |
| 0.2r | `TP-SCALE-ENVELOPE-0R` | **PROBATION** (repair on PR #1073). Remove catch_unwind false-green path; fix `rebuild_for_slots` `column_rules` sizing when slot growth coincides with registry expansion. | Scale test no longer swallows panics; `upload_reduction_topology` no longer overruns 24-byte `column_rules`; 0R2 continues the same PR to repair the later velocity upload surface. | Tier-2 (`seal-residue-risk`) |
| 0.2r2 | `TP-SCALE-ENVELOPE-0R2` | **PROBATION** (repair on PR #1073). Compact C-7 velocity upload from `n_slots * governed_pairs` materialized ops to one pair op expanded across slots by GPU dispatch; preserve RF/install/session proof without Phase 1 content. | Local real-adapter scale test reaches `SimSession::open_from_spec` success and asserts `session.mapping.is_none()`; orchestrator review still required before promoting `TP-SCALE-ENVELOPE-0`. | Tier-2 (`seal-residue-risk`) |

### Phase 1 - Base galaxy production (mostly reuse; Studio-consistency proof)

**Phase 1+ remains blocked.** `TP-SCALE-ENVELOPE-0R2` (repair) is the next active rung on PR #1073; orchestrator review required before Phase 1 opens. This adjudication remains at the orchestrator tier against the explicit rung contract; no metered DA/Owner turn is burned for the original falsifiable contract miss.
