...
| 3.1 | `TP-FLEETS-SHIPS-0` | Author **fleets as mobile star-system-grid occupants** and **ships as cohort-style children** with HP/Damage/upkeep columns. Place 10 Terran fleets (200 ships) by the 60-40 rule and 10 Pirate fleets (400 ships) by the 80-20 rule, homed at the relevant border/interior/raid-posture systems. | **PROBATION / DA-OWNER REVIEW** — implementation proof complete; orchestrator/DA review required before merge — [`tp_fleets_ships_0_results.md`](tests/tp_fleets_ships_0_results.md); `fleet_ship_payload` scenario-envelope grammar; fleets berth on mandated 1×1 surface gridcells; ships childed under fleets as `Cohort`; exact 10+10 fleets and 200+400 ships; 60-40 / 80-20 posture distribution from Chebyshev enemy-adjacency over ownership volumes; HP/Damage/upkeep authored columns + per-ship RF upkeep deficit; tree reparent compatibility proven (ships remain fleet children; RF admission survives); no combat/movement/diplomacy/AI runtime. Parked — awaiting DA/Owner clearance. | Tier-1 over 3.0 |
