...
- **SQRT-MAG-0 R1 — pre-sqrt exactness contract (PASS):** F-backed exact sqrt is admitted; Euclidean magnitude exactness requires exact pre-sqrt mag2. Raw dx/dy SEAD magnitude remains test-only/probe until mag2 construction is pinned/proven. Report: [`docs/tests/phase_m_jit_sqrt_mag0_r1_presqrt_contract_results.md`](tests/phase_m_jit_sqrt_mag0_r1_presqrt_contract_results.md).
