...
- **SQRT-MAG-0 — F-backed exact Euclidean magnitude for SEAD hot path (PASS):** Exact magnitude authority via `m_jit_mag_f_exact` routing `sqrt_cr_f_bits` over let-sequenced `dx²+dy²`; native sqrt remains approximate; diagnostic `mag2` remains blocked as exact final output; 34k mobile-simthing benchmark recorded (~5.4 ms, 1 dispatch, readback included). No default runtime wiring/scheduler/cache/bridge. Report: [`docs/tests/phase_m_jit_sqrt_mag0_f_exact_magnitude_results.md`](tests/phase_m_jit_sqrt_mag0_f_exact_magnitude_results.md).
