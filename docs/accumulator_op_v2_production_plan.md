...
**Next gate:** product-priority selection after continued flat-star soak checkpoint: narrow **D-2a** (named hard-currency scenario), **simthing-spec/RON rebuild**, narrow **E-11B-5** (named nested dynamic scenario), new scenario-driven vertical slice, or additional flat-star soak only if evidence gap remains. **E-11B remains paused.** Global default-on remains deferred.
