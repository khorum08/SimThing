...
- **SEAD-OBS-0 — GPU-resident mobile observer overlay score probe (PASS):** Q16.16 mag2 + F sqrt exact mag path; f32 weighted score (approximate/diagnostic); 34k single-dispatch benchmark recorded; dense corpus 50,176/50,176 mag2 exact, max_ulp=0; descriptor deferred (driver fixture). No scheduler/cache/default wiring/bridge. Report: [`docs/tests/phase_m_sead_obs0_mobile_overlay_score_results.md`](tests/phase_m_sead_obs0_mobile_overlay_score_results.md).
- **SEAD-OBS-1 — descriptor/admission for mobile observer overlay score (PASS):** `m_jit_sead_obs0_overlay_score` default-off descriptor; mag2_bits/mag_bits exact, score_bits ApproximateDiagnosticF32; warm 34k 32× dispatch ~0.129 ms/dispatch; exact score over f32 rejected. No scheduler/cache/default wiring/bridge. Report: [`docs/tests/phase_m_sead_obs1_overlay_score_admission_results.md`](tests/phase_m_sead_obs1_overlay_score_admission_results.md).
- **SEAD-OBS-2 — multi-layer GPU-resident observer overlay score (PASS):** `m_jit_sead_obs2_multilayer_overlay_score`; 4 Q16.16+F layers/row; per-layer mag exact; score ApproximateDiagnosticF32; 34k ~7 ms single dispatch; warm 32× ~0.238 ms/dispatch. Report: [`docs/tests/phase_m_sead_obs2_multilayer_overlay_score_results.md`](tests/phase_m_sead_obs2_multilayer_overlay_score_results.md).
- **SEAD-OBS-3 — fixed-point aggregate score contract for multi-layer observer overlay (PASS):** `m_jit_sead_obs3_multilayer_fixed_score`; Q16.16 weighted sum (`ExactQ16WeightedSum`); per-layer mag exact; score_fixed exact; OBS-2 f32 score unchanged; 34k ~7.8 ms; warm 32× ~0.278 ms/dispatch. Report: [`docs/tests/phase_m_sead_obs3_fixed_point_score_results.md`](tests/phase_m_sead_obs3_fixed_point_score_results.md).
- **SEAD-OBS-4 — GPU-resident threshold event emission from exact observer scores (PASS):** `m_jit_sead_obs4_threshold_event`; Q16.16 threshold/hysteresis; state/event ExactDeterministicEventFlag; warm 32× ~0.231 ms/dispatch; no CPU planner/bridge. Report: [`docs/tests/phase_m_sead_obs4_threshold_event_results.md`](tests/phase_m_sead_obs4_threshold_event_results.md).
- **SEAD-EVENT-0 — GPU-resident compaction for threshold event rows (PASS):** `m_jit_sead_event0_compaction`; atomic compaction; ExactAuthoritativeUnordered membership; UnspecifiedAtomicOrder; warm 32× ~0.202 ms/dispatch (50% density); no CPU planner/bridge. Report: [`docs/tests/phase_m_sead_event0_compaction_results.md`](tests/phase_m_sead_event0_compaction_results.md).
- **SEAD-PIPE-0 — integrated GPU observer-event pipeline (PASS):** `m_jit_sead_pipe0_observer_event_pipeline`; two-dispatch layout (OBS-4 threshold_pass + EVENT-0 compact_pass); score/state/event exact on intermediate rows; event_count/overflow ExactAuthoritative; membership ExactAuthoritativeUnordered under capacity; order UnspecifiedAtomicOrder; 34k integrated ~0.855 ms (2 dispatches + readback); warm 32× ~0.326 ms/pipeline; no scheduler/cache/default wiring/bridge. Report: [`docs/tests/phase_m_sead_pipe0_observer_event_pipeline_results.md`](tests/phase_m_sead_pipe0_observer_event_pipeline_results.md).
- **SEAD-EVENT-1 — GPU-resident event-code bucketing (PASS):** `m_jit_sead_event1_code_bucketing`; per-code atomic buckets (CODE_COUNT=4); bucket_counts/overflow/invalid ExactAuthoritative; membership ExactAuthoritativeUnordered under per-code capacity; order UnspecifiedAtomicOrder; 34k balanced warm ~0.284 ms/dispatch; integrated PIPE-0→bucket 3-pass smoke; no scheduler/cache/default wiring/bridge. Report: [`docs/tests/phase_m_sead_event1_code_bucketing_results.md`](tests/phase_m_sead_event1_code_bucketing_results.md).
- **SEAD-EVENT-2 — GPU-resident per-bucket reductions (PASS):** `m_jit_sead_event2_bucket_reductions`; per-code count/sum_lo/sum_hi/min/max + overflow/empty flags; i64 sum with explicit overflow flag; order-invariant reductions; 34k balanced warm ~0.52 ms/dispatch; EVENT-1→reduce and PIPE-0→bucket→reduce smokes; no scheduler/cache/default wiring/bridge. Report: [`docs/tests/phase_m_sead_event2_bucket_reductions_results.md`](tests/phase_m_sead_event2_bucket_reductions_results.md).
- **SEAD-ACT-0 — numeric action proposal substrate (PASS):** `m_jit_sead_act0_numeric_proposals`; consumes EVENT-2 reductions; fixed integer max-score and optional sum thresholds; proposal_count/overflow ExactAuthoritative; membership ExactAuthoritativeUnordered under capacity; order UnspecifiedAtomicOrder; 34k bucket→reduce→propose warm ~1.24 ms/pipeline; no CPU planner/scheduler/cache/default wiring/bridge. Report: [`docs/tests/phase_m_sead_act0_numeric_proposals_results.md`](tests/phase_m_sead_act0_numeric_proposals_results.md).
...
- **FrontierV1-5 — live GPU self-AI route toward FrontierV2 (PASS):** one live GPU-resident integrated score→threshold→proposal→dispatch resource route; replay fingerprint `1653b84847be2dd2`; feedback candidate FixtureOnly; FrontierV2 NotImplemented. Report: [`docs/tests/phase_m_frontier_v1_5_live_self_ai_route_results.md`](tests/phase_m_frontier_v1_5_live_self_ai_route_results.md).
...
- **Design-authority correction (Opus, 2026-05-30):** the line above is implementer **self-acceptance** and is superseded by the design-authority ruling (`docs/workshop/sead_self_ai_track.md` §10). Of record: **substrate (mapping + flat-star RF) closed → Phase E closes at flat-star**; the self-AI **resource-dispatch** loop is `GpuVerified` (bounded single-tick) per the now-**ACCEPTED FrontierV1-5**; structural/movement remain `ReplayAccepted`. The earlier "Pause (F) / FrontierV1-5 not authorized" disposition is **void**; **next named gate is `FrontierV2-0`** (multi-tick closed-loop consumer; ClauseThing is its eventual authoring front-end).
...
**SEAD-ACT-2 — fixture-local proposal admission records**

SEAD-ACT-2 verifies that Phase E-style numeric proposal summaries can be converted into fixture-local admission records under fixed integer contracts. This supports future Economic V1 / SEAD-economics authoring by providing an admitted-candidate artifact shape, but it does not add production runtime wiring, semantic WGSL, CPU planner behavior, scheduler/cache, simthing-sim awareness, or economy bridge wiring.
...
FrontierV1 is accepted as the named M/E closing vertical; Phase M and Phase E are closed for FrontierV1 scope. FrontierV1-5, ACT-5, EVENT-3, OBS-5, and PIPE-1 are not authorized. Full PIPE-0 GPU re-run inside FrontierV1 remains optional only and requires a separately named product need. Deferred items remain non-blocking: atlas, active mask, perception/fog, source identity, nested E-11B/E-11B-5, D-2a, ClauseThing, and broader production integration outside FrontierV1. Next work must begin from a separately named production gate and preserve V7.7 / Mapping ADR / Resource Flow ADR / SEAD charter guardrails. Next named gate: **Pause implementation (F)** per [`product_priority_vertical_slice_selection.md`](reviews/product_priority_vertical_slice_selection.md).

**FrontierV1-5 — live GPU-resident self-AI route run toward FrontierV2**

FrontierV1-5 responds to the Opus design-authority ruling in `docs/workshop/sead_self_ai_track.md` §10 and the Forward Horizon in `docs/workshop/mapping_current_guidance.md` by executing one live GPU-resident integrated self-AI route inside the default-off FrontierV1 fixture: score→threshold→proposal→dispatch. It preserves accepted mapping and flat-star Resource Flow GPU verification, routes resource dispatch through the Resource Flow allocator, records CPU oracle parity and replay reproducibility, and emits a fixture-only feedback candidate shape for the named FrontierV2 multi-tick closed-loop consumer. It does not implement FrontierV2, reopen the SEAD ACT/EVENT/OBS/PIPE ladder, declare phase closure, or add default SimSession behavior, scheduler/cache, semantic WGSL, CPU planner/urgency/commitment emission, simthing-sim semantic awareness, parallel fixture economy, shared-pool tick writes, or a generic economy→mapping bridge.

The FrontierV1 self-AI resource-dispatch loop is now GpuVerified for the bounded single-tick smoke route. FrontierV2 remains the next named consumer for multi-tick closed-loop behavior. Structural/movement routes remain ReplayAccepted unless separately proven.

**FrontierV2-0 — multi-tick closed-loop self-AI consumer fixture**

FrontierV2-0 implements the named Forward Horizon consumer from `docs/workshop/mapping_current_guidance.md`: a default-off, bounded multi-tick closed-loop fixture where field-derived SEAD self-AI proposals drive resource dispatch and fixture-only feedback into the next tick's field/economy input. It consumes FrontierV1-5's live GPU self-AI route proof and feedback candidate shape, runs at least two ticks, records CPU oracle parity and replay reproducibility, and does not implement ClauseThing, reopen the SEAD ACT/EVENT/OBS/PIPE ladder, declare phase closure, or add default SimSession behavior, scheduler/cache, semantic WGSL, CPU planner/urgency/commitment emission, simthing-sim semantic awareness, parallel fixture economy, shared-pool tick writes, or a generic economy→mapping bridge.
