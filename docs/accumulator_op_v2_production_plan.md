...
**V7.6 StructuredFieldStencilOp status:** **Live** — generic GPU primitive promoted from workshop evidence. See [`docs/design_v7_6.md`](design_v7_6.md). Live code: [`structured_field_stencil.rs`](../crates/simthing-gpu/src/structured_field_stencil.rs), [`structured_field_stencil.wgsl`](../crates/simthing-gpu/src/shaders/structured_field_stencil.wgsl). Opt-in library API only — not wired into default production pass graph. Default operator: normalized / source_capped_normalized; default horizon H≤8; ping-pong required for H>1; one-shot seed then zero source policy; column-aware SlotRange reduction → parent EML on later order band. EML whitelist extended for field_* classes at designer layer. Mapping/location architecture remains provisional. No mapping runtime landed. `PipelineFlags::default().use_accumulator_resource_flow` remains false.
