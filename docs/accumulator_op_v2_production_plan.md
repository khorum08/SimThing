...
**Next gate:** product-priority checkpoint — choose next implementation track: continued flat-star soak (default confidence), narrow **D-2a** (only after named hard-currency scenario), **simthing-spec/RON rebuild**, narrow **E-11B-5** nested dynamic enrollment (only after named nested dynamic Resource Flow scenario), or a new scenario-driven vertical slice. **E-11B is paused** (not abandoned). Global default-on remains deferred.
