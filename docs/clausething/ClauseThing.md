...
> **⚠ REFERENCE, NOT SPECIFICATION — read this box before using this book.**
> This textbook describes the **source language and a foreign engine** (Paradox's Clausewitz). It
> lives in this repo so implementation agents can look up ClauseScript semantics; it is **not** a
> design document for SimThing, and it is **subordinate to** the `CT-` track
> ([`../design_0_0_8_1_clausething_production_track.md`](../design_0_0_8_1_clausething_production_track.md)),
> `simthing_core_design.md`, and `invariants.md` everywhere they touch.
>
> **Drift warning (the known failure mode):** wherever this book describes *engine behavior* —
> lazy modifier invalidation and caching (§7.4), sequential imperative effect execution (§6.3),
> CPU-side evaluation, event-driven recalculation — that is what ClauseThing **compiles away
> from, not toward**. SimThing re-evaluates columns every tick on GPU, batches state changes at
> boundaries, and has no CPU planner. Implementing this book's engine model is the documented
> CPU-regression drift pattern; `simthing_core_design.md` §9 litmus tests apply.
>
> **Provenance caveat (binding):** this is an AI-authored synthesis of community
> reverse-engineering of a **closed-source engine** — its semantic claims are provisional until
> verified against the lab primary sources (track §6.1). Consult the sections your rung names;
> **do not derive scope from this book** — scope comes from the track ladder only.

> **ERRATA (verified against the lab primary sources, 2026-06-10):**
> **#1 — §10.5 granularity claim corrected.** This book states `modifier_category` "prevents
> leakage." The `modifiers.log` header states the opposite: categories are "used internally as a
> tag to *suggest* where they are intended to be used," and pop-scope modifiers "will generally
> work on e.g. planets and countries as well," applying to **all pops belonging to that object**
> (legal cross-level application with broadcast-down semantics).
> **#2 — §10.5 grammar attribution corrected.** "The vast majority of economic modifiers are
> auto-generated" is true of *economic* modifiers but misleading for the namespace: of 41,016
> keys, the economic grammar covers ~16% (6,426); the `shipsize_*` grammar covers ~69% (28,179).
> The namespace is generated by a **family** of grammars.
> **#3 — §4.3 scope count understated.** `scopes.log` documents **90** scope changes with
> Supported/Output rules, not the dozen-odd this book's examples suggest.
