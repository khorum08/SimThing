...
| **STEAD spatial contract** | The executable, normative STEAD contract (8 terms / 9 sections): Location≡gridcell, structural vs render coords, layout vs execution-profile admission, RF/Accumulator spatial binding, PALMA/Gu-Yang as fields. **Mandatory reading for any spatial task.** Enforced by `stead_spatial_contract_guards.rs` + `mapgen_rf_stead_binding.rs`. | [`../stead_spatial_contract.md`](../stead_spatial_contract.md), STEAD-CONTRACT-0 |
| **RF ⇄ STEAD binding** | RF/Accumulator stays generic, but an arena over gridcell `Location`s is **spatially bound** (`SpatialBindingMode::SpatiallyBoundToGridcellLocations`): every participant needs a structural `grid_metadata` placement (`validate_spatial_binding`); the arena records its `StructuralGridFrame` (`SpatialArenaBindingReport`). Spatially-neutral arenas need no grid. | `mapgen_resource_flow`, STEAD-CONTRACT-0 §5 |
