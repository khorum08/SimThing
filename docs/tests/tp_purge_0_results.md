...
- Board: `5133929991` · Remands `5134500978` / `5135691949` / `5136003644` / `5136696481` / `5137044659` / **`5137229638`** · Continuation `5136490881`
...
| B REPLACE | **CLOSED + Remand 6 JIT honesty** | `145 + 73 = 218`; `inline_unique_cases = 10`; two harnesses |
| C DETACH/DE-NAME/DELETE | ACCEPTED | ceiling **0/0/0**; admission preserved |

## Remand 6 repair (JIT only)

1. **`jit-artifact`** — same SoftStep+opts; red = test-local SoftStep compiler that emits ABS/LITERAL(1) in the wrong order **during node construction**
2. Does **not** call live `compile_eml_gadget` then reverse/edit returned nodes
3. Preserved accepted Remand 5: replay / overlay / canon / owner-silo / mobility-dispatch
4. Arithmetic `145+73=218` unchanged
