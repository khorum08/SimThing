...
## CC-SWEEP-simthing-clausething-MODULE-MARKERS-2
...
- Sweep type: module-marker ledger-only
- Candidate row source: `scripts/ci/test_inventory.tsv`
- Candidate file field: `crates/simthing-clausething/src/stellaris_names.rs`
- Candidate test_name: `cfg_test_mod::tests`
...
```text
simthing-clausething	crates/simthing-clausething/src/stellaris_names.rs	cfg_test_mod::tests	unit	deletion-candidate	B-T6-MODULE-MARKER-EXPANSION	AUDIT	deletion-candidate: cfg(test) mod marker captured for drift-gate completeness; not a runnable test; defer ledger-row removal to CC-SWEEP-simthing-clausething	ledger-only	pre-lifecycle	0
```
...
- #1196 added the `corpus-module-marker-sweep` route.
- #1197 changed drift behavior so `cfg_test_mod::*` markers are not mandatory discovered test rows.
- #1199, #1200, and #1201 proved production module-marker sweeps route `ORCHESTRATOR-CLEARABLE`.
- No source file is edited.
- No test file is edited or deleted.
- Inventory/lifecycle/drift checks prove consistency after removal.
...
| Metric | Before | After |
|---|---:|---:|
| total inventory rows | 970 | 969 |
| simthing-clausething cfg_test_mod deletion-candidate rows | 1 | 0 |
| source files edited | 0 | 0 |
| test files edited/deleted | 0 | 0 |
...
```text
simthing-clausething	crates/simthing-clausething/src/stellaris_names.rs	cfg_test_mod::tests	unit	unknown	B-T6-MODULE-MARKER-EXPANSION	TIER6_PROMOTION_REQUIRED	PROMOTION_REQUIRED			mapped_to_child_inventory:2 child rows already inventoried; remove or expand module marker in future inventory schema	medium	module-marker mapped to child inventory rows; not a generic audit blocker
```
...
LIFECYCLE-EXPIRY SCHEDULED CHECK
  closed tracks: pre-lifecycle
  expired candidates: 133
  survivor-set expired: 0
  justified-closed (audit): 0
LIFECYCLE-EXPIRY-VERDICT: INSPECT expired=133 audit=0 max_dsu_survivals=0 mode=scheduled
```
...
- No durable/KEEP row removed.
- Inventory row count decreases by exactly 1.
- `simthing-clausething` cfg marker deletion candidates decrease from 1 to 0.
- Scheduled lifecycle expired count decreases by exactly 1 because a matching boundary row was removed.
- Drift gate passes.
- Lifecycle schema gate passes.
- Doctrine scan has zero hard failures.
