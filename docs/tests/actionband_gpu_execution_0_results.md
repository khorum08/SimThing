...
- base_sha: `46f20673730c482a2ae39195fc1e9f4625fe1575`
...
Numerical state/projection readback is refused unless the existing explicit
debug-proof scope is enabled; it is not a production decision surface.
...
table. GPU-authorized structural rows select a pre-admitted `BoundaryRequest`,
which the CPU submits to the existing feeder boundary without inspecting or
re-evaluating numerical state.
...
deterministically from numeric program and destination shape only.
...
|---|---|
| `sparse_gpu_state_ping_pongs_and_matches_exact_eml_oracle` | Runs two real generations, checks current/next progression, exact GPU/CPU-referee EML parity, fixed structural output, boundary submission without CPU re-evaluation, and the repeated timestamp measurement. |
...
| `bucketing_is_numeric_deterministic_and_labels_are_semantic_shadow_only` | Same numeric product under different labels has the same fingerprint/buckets; equal program/binding shapes share a bucket and a distinct binding shape separates. |
...
- ActionBand depth-1 GPU compute: **9,856 ns**
- Existing sealed crossing GPU compute: **5,000 ns**
- Ratio: **1.971x**

The two-pass ActionBand work is below the planted 20x material-regression stop
threshold and shows no unexplained crossing-scale regression. This is a local
engineering comparison, not a broad hardware performance claim; the exact-head
run and hosted artifacts are carried in the relay.
