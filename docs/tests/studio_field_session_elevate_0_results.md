...
- Windows Studio renderer creation is constrained to DX12 and high-performance adapter selection with Bevy's fallback request disabled. Post-init validation fails closed unless the actual `RenderAdapterInfo` is exactly `NVIDIA GeForce RTX 4080 Laptop GPU`, NVIDIA vendor `0x10de`, `DiscreteGpu`, and `Dx12`.
...
Before the remedial, the Owner observed Vulkan validation/layer/version warnings, Vulkan-hidden adapters, and optional D3D12 debug-interface warning `0x887A002D`. After the DX12-only remedial, none of those warnings appeared. Validation remains enabled; only WGPU's optional `DEBUG` instance flag is omitted so missing Windows Graphics Tools does not create a false startup warning. WGPU still reports its unfiltered nonfatal DX12 downlevel-capability warning; Studio launches and renders on the required adapter, so that warning is recorded rather than broadly suppressed.

## Windows debug executable identity

The replacement executable was built from the frozen implementation/test SHA in a new SHA-named Cargo target directory. The tracked worktree was clean before and after the build; the pre-existing untracked generated scenario was left untouched. The executable is never committed:
...
Replacement OVL remains Owner-only and open. Use the exact executable identity above; do not reuse `target/rf4-ovl-clean` or either superseded image.

1. Capture `RF4_OVL_P_rebuilt_exe_provenance.png`: in Git Bash show the implementation SHA `b99fa6326b729cd6fb8a1e9adba364795015dccd`, current branch head, `sha256sum`, byte size, UTC modify time, and full path for `target/rf4-gpu-remedial-b99fa632/debug/simthing-studio.exe`.
2. Launch that executable from the repository root. Open Performance Telemetry, collapse Nameplate debug, and expand Render loop / GPU / VRAM. Capture `RF4_OVL_GPU_required_adapter.png` with all five exact live identity/policy rows above visible and no `unavailable` field.
