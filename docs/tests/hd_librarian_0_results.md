...
- Added `/librarian staleness|cull|catalog` parsing and doctrine-exec workflow plumbing; confirmed culls are owner-only and PR-branch-bound.
...
- `LIBRARIAN-CATALOG-VERDICT: PASS` for coding/orchestrator/da; report is 18 lines.
...
- Workflow `/librarian` replies have marker + one-line header plus explicit `reply-line-cap`, and update by originating comment marker.
...
- Remand-2 falsifiers: confirm preflight cap leaves guard bytes unchanged; failed confirms cannot satisfy the workflow commit condition; prune dry-run missing-log creates no file; catalog >10 payload markers are not sliced; owner orphan/expired staleness cannot claim aggregate PASS.
