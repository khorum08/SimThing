# FIELD_POLICY-OBS-4 — GPU-Resident Threshold Event Emission Results

## Base HEAD

`cb96202` (post-FIELD_POLICY-OBS-3 merge)

## Files changed

| File | Change |
|---|---|
| `crates/simthing-driver/tests/phase_m_field_policy_obs4_threshold_event.rs` | **New** — 7 tests: semantic-free WGSL, edge rows, dense corpus, 34k perf, warm 32× dispatch, event authority, no wiring |
| `crates/simthing-spec/src/compile/jit_exact_sqrt_artifact_admission.rs` | `m_jit_field_policy_obs4_threshold_event` descriptor + threshold/event contracts |
| `crates/simthing-spec/tests/field_policy_obs0_overlay_score_admission.rs` | FIELD_POLICY-OBS-4 admission tests (+2) |
| `docs/workshop/mapping_current_guidance.md` | FIELD_POLICY-OBS-4 row |
| `docs/accumulator_op_v2_production_plan.md` | FIELD_POLICY-OBS-4 section |
| `docs/invariants.md` | GPU threshold event invariant |
| `docs/worklog.md` | Append-only milestone |

## Pre-edit evaluation summary

1. **FIELD_POLICY-OBS-3 exact score:** `ExactQ16WeightedSum`; `score_fixed` ExactAuthoritative; per-layer mag exact under Q16.16 + F.
...
**Landed:** `m_jit_field_policy_obs4_threshold_event` — default_off, `ExactQ16WeightedSum` score, `ExactQ16Threshold` operands, `ExactDeterministicEventFlag` outputs.
...
## Comparison to FIELD_POLICY-OBS-3

| Path | warm per_dispatch_ms |
|---|---:|
| FIELD_POLICY-OBS-3 fixed score | ~0.278 |
| FIELD_POLICY-OBS-4 threshold event | ~0.231 |
...
cargo test -p simthing-spec --test field_policy_obs0_overlay_score_admission -- --nocapture  # 12 passed
