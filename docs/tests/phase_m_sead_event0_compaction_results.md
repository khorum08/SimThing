# SEAD-EVENT-0 — GPU-Resident Event Compaction Results

## Base HEAD

`9d86d87` (post-SEAD-OBS-4 merge)

## Files changed

| File | Change |
|---|---|
| `crates/simthing-driver/tests/phase_m_sead_event0_compaction.rs` | **New** — 7 tests: semantic-free WGSL, edge compaction, dense corpus, OBS-4 smoke, 34k density perf, warm 32× dispatch, no wiring |
| `crates/simthing-spec/src/compile/jit_exact_sqrt_artifact_admission.rs` | `m_jit_sead_event0_compaction` descriptor + compaction contracts |
| `crates/simthing-spec/tests/sead_obs0_overlay_score_admission.rs` | SEAD-EVENT-0 admission tests (+2) |
| `docs/workshop/mapping_current_guidance.md` | SEAD-EVENT-0 row |
| `docs/accumulator_op_v2_production_plan.md` | SEAD-EVENT-0 section |
| `docs/invariants.md` | Compacted GPU event invariant |
| `docs/worklog.md` | Append-only milestone |

## Pre-edit evaluation summary

1. **SEAD-OBS-4 event authority:** `state_u32` / `event_code_u32` ExactDeterministicEventFlag under Q16.16 score + threshold.
...
**Landed:** `m_jit_sead_event0_compaction` — default_off, reads OBS-4-style event inputs, writes `event_count`, `overflow_flag`, `event_record`.
...
```bash
cargo test -p simthing-driver --test phase_m_sead_event0_compaction -- --nocapture  # 7 passed
...
cargo test -p simthing-spec --test sead_obs0_overlay_score_admission -- --nocapture  # 14 passed
