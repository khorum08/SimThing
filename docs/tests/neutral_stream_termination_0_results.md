# NEUTRAL-STREAM-TERMINATION-0 — implementation proof return

Status: **PROBATION / proof-present / DA-review-pending / OPEN / UNMERGED**.

The official recursive-root referee now retains its lawful zero companion with full provenance while source 1000 remains G8/U2 and supplies the unchanged recursive chain. All three DA-admitted referee corrections are GREEN, the report-only zero-omission census is PASS, and the full workspace has ZERO RED. No production or E8 literal changed in this resume. Graduation, final scan triage, clearance and relay-lint remain orchestration/DA work.

## Binding state and preserved history


- Branch: codex/neutral-stream-termination-0.
- Amended-master comparison base: 5baa89d47830c98263617c93d8100c62984aab32 (PR #1990).
- HD-declared base: 369358733df35c1f556fe4c8844df20223dd776d.
- Original production base: 164c80158e224d8b9daa52176b232d435a3f640f.
- Owner neutral-departure law5561338557; current dispatch 5563523839; DA referee rulings 5563027436 and 5563496170.
...
- STOP6: 6a8dd658418423d7a07ea277a7542975dce1f75a / Board 5563341543; preserved prior master merge 5e50997fb59cde4089ebacc2b030ea399b339981.
- Current normal master merge: 4b7905926c711432e254ad7a605e66775290cdb2. No RED/STOP/merge rewritten, replanted, rebased or squashed.
- HD-RECEIPT: 7879c24f5332 (supersedes 252edc08cfb2).
...
## Recursive-root contract and complete parity GREEN

DA 5563496170 admits only the root assertion block in `three_recursive_edges_self_consume_exact_ts_and_u_recurs_once_at_n_plus_one`. The existing fixture remains requests [10,0], supply 8, generation 10. Both plan and product count are 2. The map requires source 1000 G8/U2 and source 1001 G0/U0. The zero claim explicitly has requested 0; its product matches the claim's semantic row, carries generation 10, root band 10 and STATUS_OK. Total grant is exactly 8, entirely from source 1000. The selected root chain product must equal source 1000's complete canonical product and remain G8/U2.

~~~text
source1000: semantic_row0 / G8/U2 / generation10 / band10 / STATUS_OK
source1001: requested0 / semantic_row1 / G0/U0 / generation10 / band10 / STATUS_OK
plan count2 / product count2 / total grant8 / chain product=source1000
RESIDENT-CLEARING-QUALIFICATION-FINGERPRINT: 64c82fb4de7690ac
resident_clearing_parity_terminal_referee: 1 passed / 0 failed
~~~

Source comparison with STOP6 proves the fixture, all earlier parity logic, and every line from the edge-2 comment onward are unchanged. The official test executes and passes the literal G8/U2 -> G6/U2 -> G4/U2 chain, N10->N11 authored 2 + prior U2 carry, repeat-mint refusal, and final scale/multitree/physical-invariance/qualification function with its tuple mutants. The earlier scratch diagnostic is preserved only as historical STOP evidence; the official whole referee is now GREEN.

## ZERO-OMISSION REFEREE CENSUS

ZERO-OMISSION REFEREE CENSUS: PASS — no remaining workshop referee asserts absence/reduced count for a lawful zero-request identity.

Report-only inspection searched every Rust file under `crates/simthing-workshop/tests`, including support modules. The untracked helper `.git/1511-root-census.py` records four reproducible `rg` queries and outputs: request/demand surfaces (274 lines), zero-fixture candidates (226 lines), omission/filter/count assertions (38 lines), and multiline request-array assignments (30 lines). The searches are evidence for manual semantic inspection, not a newly introduced gate or automated proof of absence.

~~~sh
rg -n -i 'request|demand\(' crates/simthing-workshop/tests -g '*.rs'
rg -n -i 'zero.?request|request.{0,50}(zero|0[,\];])|\[0[,;]|, 0[,\]]|request.*==.*0|request.*!=.*0' crates/simthing-workshop/tests -g '*.rs'
rg -n -i 'omit|absen|exclud|filter\(|claims\(\)\.len|products\.len|products\.is_empty|grants\.len|grants\.is_empty' crates/simthing-workshop/tests -g '*.rs'
rg -n -U 'requests\s*=\s*[^;]+;' crates/simthing-workshop/tests -g '*.rs'
~~~

| Referee / source locator | Inspected zero fixture or apparent hit | Disposition |
| --- | --- | --- |
| exact_cap_projection_0.rs:325, 348-397 | [0,5]/supply 3, case 325 | Both CPU/GPU vectors include the zero identity; full canonical digest and explicit row count/provenance bind it. Historical positive-request digest remains separately asserted. |
| exact_cap_projection_0.rs:305-345, 400-478 | Generated fixtures, zero supply/basis/precedence or expected grant | Requests are positive except case 325; a zero grant with positive unresolved remains in the vectors and digest. No omission/count assertion. |
| resident_clearing_apportionment_0.rs:425-468 | [0,5]/supply 3, historical label `zero-request-omission` | Label only. Both full CPU/GPU maps equal the ordinary CPU oracle, which retains source 0 G0/U0. No absence or reduced-count assertion. File remains byte-for-byte frozen and GREEN 7/7. |
| resident_clearing_parity_0.rs:1138-1206 | [0,1,1]/supply 2 | Plan/product count 3 and complete map explicitly retain the zero identity. Unchanged from STOP6. |
| resident_clearing_parity_0.rs:1475-1531 | [10,0]/supply 8 | Plan/product count 2 and complete map retain source 1001 with full provenance; only the admitted root block was changed. |
| resident_clearing_parity_0.rs:986-998 | `empty[0].grants.is_empty()` | Input claims are actually empty (`&[]`); no zero-request identity exists to omit. |
| resident_session_integration_conformance_0.rs:666-874 | Zero Draw authorization, established N1-N5, initial Draw [0,0]/[0,100] | Live G0/U0 remains in provenance. Only actual membership loss terminates; positive-minimum zero is a typed refusal. Both postures GREEN. |
| resident_clearing_score_and_bands_0.rs:1047-1096 | Omitted next vector / U=0 control | Missing next product is refused for an unresolved positive current claim. U=0 control retains one next identity. No zero-request omission. |
| persistence_deformation_port_0.rs:155-158, 207-229 | Authored zero, expiry and `vec![demand(0)]` | Decay/expiry outputs preserve authored-plus-U semantics; bounded-cap violation is a typed atomic refusal, not a reduced product/count assertion. |
| Remaining request hits | Positive scalar claims; GPU adapter/registration, action-band structural and consequence-door requests | Reviewed as distinct quantities/types. No lawful zero-request claimant is asserted absent. Zero basis, zero supply, source index 0 and generation 0 were not mistaken for requested 0. |

There is no remaining census hit requiring a seventh scope adjudication. No file or assertion outside the admitted root block was changed by this census.
...
## Earlier parity zero block GREEN
...
All following invalid-value, failed-dispatch recovery, signed-zero and overflow assertions in the negative matrix execute unchanged and pass before the now-corrected recursive-root block. The independent pin remains0x64c8_2fb4_de76_90ac. This earlier block is byte-for-byte unchanged from STOP6. The only additional parity change in this resume is the DA-admitted root assertion below; its entire later tail remains byte-for-byte unchanged.
...
The two literals were rolled together with the final planner edit at STOP5; no sealed-source edit followed. No fifth roll occurred for these test-only remands. Qualification ABI/child-share/planner/temporal mutation matrix4/4 passes again. The official complete parity referee now passes its unchanged final qualification phase and confirms that same independent record. No build.rs/component-list/comparator/record/ABI/dynamic-pin change.

## Validation and limits

- Full workspace/all-targets: **145 groups / 551 passed / 0 failed / 14 ignored**, ZERO RED. Command: `cargo test --workspace --all-targets --no-fail-fast -j 1 --quiet`; exit/elapsed: 0 (184.22s).
- Six touched packages/tests check PASS: core, kernel, GPU, Spec, driver, workshop.
- Selected nine-target corpus: **38 passed / 0 failed / 0 ignored**, 71.69 seconds. Exact-cap 3/3, session integration 10/10, frozen apportionment 7/7, generation-abort 3/3, recursion-axis 5/5, recursive RF 1/1, substrate-binding 4/4, execution-lifetime 4/4, complete terminal parity 1/1.
- Qualification mutation matrix 4/4; core execution-authority 3/3; private IntegrationSchedule compile-fail 1/1 (E0451).
- Structural 15/15 exit 0: inventory 1442/1442 missing 0/extra 0, drift prove, constitutional check/selftest, lifecycle schema/prove, sanctioned surface digest, detachability/selftest, anchor check/selftest, plan/observation/slot/overlay censuses. Selftests caught their expected planted failures. Existing anchor-coverage advisory remains INSPECT unanchored 40/59 with final PASS.
- Mechanical source audit: 15 changed paths relative to amended master, all in HD. This resume changes only the admitted parity root assertion block and this results packet. Production, both pins, root fixture, every later parity assertion, dual digest, earlier zero block, frozen apportionment, session matrices, inventory and reach rows remain unchanged. All 63 ACKs and RED/six-STOP ancestry remain intact.
- Generated test baseline output and generated Python bytecode are restored; no generated execution artifacts are part of the candidate diff.

Fresh committed-head Agent Scan, every scan-id-bearing INSPECT, and hosted Doctrine Scan/Doctrine Exec run IDs, conclusions and artifact evidence are bound to the final head in the PR/Board return after this source-state commit. Final-head TEST-BUDGET triage is deliberately left to orchestration before clearance. No clearance or graduation verdict is self-issued.

Logs: `.git/1511-root-handoff.txt`, `root-check.log`, `root-frozen.log`, `root-qualification.log`, `root-permit.log`, `root-compilefail.log`, `root-workspace.log`, `root-final-*.log`, `root-audit.log`, `root-census-*.log`, `root-agent-scan.log` (all root-prefixed names expand under `.git/1511-`). Preserved final-pin refusal: `.git/1511-plan-e8-refusal.log`.
...
| crates/simthing-workshop/tests/resident_clearing_parity_0.rs | Final independent pin plus admitted [0,1,1] zero block only; admitted recursive-root provenance/conservation block. |
| docs/tests/neutral_stream_termination_0_results.md | Consolidated GREEN proofs, report-only zero-omission census, 63 ACKs. |
