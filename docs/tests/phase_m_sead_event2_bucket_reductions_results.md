# SEAD-EVENT-2 — GPU-Resident Per-Bucket Reductions Results

## Base HEAD

`cb6b293` (post-SEAD-EVENT-1 merge, pre-EVENT-2)

## Files changed

| File | Change |
|---|---|
| `crates/simthing-driver/tests/phase_m_sead_event2_bucket_reductions.rs` | **New** — 8 tests: semantic-free WGSL, reduction edge, dense corpus, EVENT-1→reduce smoke, PIPE-0→bucket→reduce smoke, 34k perf, warm 32×, no wiring |
| `crates/simthing-spec/src/compile/jit_exact_sqrt_artifact_admission.rs` | `m_jit_sead_event2_bucket_reductions` descriptor + reduction authority enums |
| `crates/simthing-spec/src/compile/mod.rs`, `lib.rs`, `jit_kernel_descriptor_admission.rs` | exports + registry |
| `crates/simthing-spec/tests/sead_obs0_overlay_score_admission.rs` | SEAD-EVENT-2 admission tests (+2) |
| `docs/workshop/mapping_current_guidance.md` | SEAD-EVENT-2 row |
| `docs/accumulator_op_v2_production_plan.md` | SEAD-EVENT-2 section |
...
**Landed:** `m_jit_sead_event2_bucket_reductions` — default_off, reads bucket_counts + bucket_record, writes reduction outputs.
...
cargo test -p simthing-spec --test sead_obs0_overlay_score_admission -- --nocapture  → 20/20 PASS
