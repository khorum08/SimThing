...
(`PRODUCTION-PATH-0080-1`) → schedule SEAD-sourced movement (`DEFAULT-SCHEDULE-0080-1`) → observe/export
...
decision logic** — SEAD remains the sole mover-decision source. This is the natural "make the finished
...
- Direct movement control, externally-scripted boundary requests, SEAD bypass, CPU planner, player command
...
residency summary, faction-index ECON summary, owner-overlay/up-aggregation summary, SEAD movement trace,
...
no-CLI-binary, no-direct-move, no-external-boundary, no-SEAD-bypass, no-player-loop, no-UI/realtime,
...
`BoundaryRequest`, no SEAD bypass, no CPU planner/urgency/commitment, no player command loop, no UI, no
