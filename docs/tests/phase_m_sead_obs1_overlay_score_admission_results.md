# SEAD-OBS-1 — Descriptor/Admission for Mobile Observer Overlay Score Results

## Base HEAD

`8736adb` (post-SEAD-OBS-0 merge)

## Files changed

| File | Change |
|---|---|
| `crates/simthing-spec/src/compile/jit_exact_sqrt_artifact_admission.rs` | `m_jit_sead_obs0_overlay_score` descriptor, `ScoreAuthorityContract`, `InlineFixedPointMag2Sqrt`, admission validators |
| `crates/simthing-spec/src/compile/jit_kernel_descriptor_admission.rs` | `score_authority_contract` field, registry entry |
| `crates/simthing-spec/tests/sead_obs0_overlay_score_admission.rs` | **New** — 6 admission tests |
| `crates/simthing-driver/tests/phase_m_sead_obs0_mobile_overlay_score.rs` | Warm repeated-dispatch benchmark, descriptor wiring tests |
| `docs/workshop/mapping_current_guidance.md` | SEAD-OBS-1 row |
| `docs/accumulator_op_v2_production_plan.md` | SEAD-OBS-1 section |
| `docs/invariants.md` | Score descriptor authority invariant |
| `docs/worklog.md` | Append-only milestone |

## Pre-edit evaluation summary

1. **SEAD-OBS-0 exact outputs:** `mag2_bits`, `mag_bits` (Q16.16 integer mag2 + F sqrt, max_ulp=0).
...
| Field | Value |
|---|---|
| id | `m_jit_sead_obs0_overlay_score` |
| label | `SeadObserverOverlayScore` |
...
| Path | Timing |
|---|---|
| SEAD-OBS-0 cold 34k (1 dispatch + readback) | ~13–15.6 ms |
| SEAD-OBS-1 warm 34k (32 dispatches + readback) | ~0.129 ms/dispatch |
| SQRT-MAG2-PERF-0 combined Q16 warm | ~1.7 ms |

## Tests/scans run

```bash
cargo test -p simthing-spec --test sead_obs0_overlay_score_admission -- --nocapture  # 6 passed
