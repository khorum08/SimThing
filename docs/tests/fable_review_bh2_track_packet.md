...
| R1* `OnceLock` proof wrappers (removed) | former `runtime_0080_0_r1*.rs` integration binaries | **DELETE_PROOF_SCAFFOLD** | Bogged default workspace (60s+ per binary) | Purged R1-TEST-PURGE 2026-06-11 |
| `runtime_0080_0_r1_gate.rs` | driver tests | **KEEP_FAST_SENTINEL** | Default opt-in + no-authority sentinels | No GPU/report/checksum |
| `r1_fast_*` unit sentinels | `runtime_0080_0_r1*.rs` src modules | **KEEP_FAST_SENTINEL** | Tiny oracle fixtures on live production helpers | No GPU/report/checksum |
...
| R1 archived proof reports | `docs/archive/superseded_tests/runtime_0080_0_r1*` | DIAGNOSTIC_ONLY | Historical rung closure | Not default gate |
...
**R1-TEST-PURGE note:** Legacy R1* proof-ledger/report/checksum tests were deleted or quarantined from
the default workspace gate. Default workspace now retains only fast production-relevant R1* sentinels.
Historical proof batteries must not be reintroduced as default tests. BH track independent — see
...
| `cargo test -p simthing-driver --test runtime_0080_0_r1_gate` | PASS (0.00s) |
| `cargo test -p simthing-driver --lib r1_fast_` | PASS (6 unit sentinels) |
...
|---|---|---|
| `cargo test --workspace` | **PASS (~95s)** | No R1* 60s warnings; proof batteries removed |
