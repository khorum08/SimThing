# FIELD_POLICY-OBS-2 — Multi-Layer GPU-Resident Observer Overlay Score Results

## Base HEAD

`d6019c8` (post-FIELD_POLICY-OBS-1 merge)

## Files changed

| File | Change |
|---|---|
| `crates/simthing-driver/tests/phase_m_field_policy_obs2_multilayer_overlay_score.rs` | **New** — 6 tests: semantic-free WGSL, dense correctness, 34k perf, warm 32× dispatch, score authority, no wiring |
| `crates/simthing-spec/src/compile/jit_exact_sqrt_artifact_admission.rs` | `m_jit_field_policy_obs2_multilayer_overlay_score` descriptor + multilayer admission |
| `crates/simthing-spec/tests/field_policy_obs0_overlay_score_admission.rs` | FIELD_POLICY-OBS-2 admission tests |
| `docs/workshop/mapping_current_guidance.md` | FIELD_POLICY-OBS-2 row |
| `docs/accumulator_op_v2_production_plan.md` | FIELD_POLICY-OBS-2 section |
| `docs/invariants.md` | Multi-layer score invariant |
| `docs/worklog.md` | Append-only milestone |

## Pre-edit evaluation summary

1. **FIELD_POLICY-OBS-1 exactness:** mag2_bits/mag_bits ExactAuthoritative under Q16.16 + F; score ApproximateDiagnosticF32.
...
4. **Layer count:** 4 layers (fixed `FIELD_POLICY_OBS2_LAYER_COUNT`).
...
4 (`FIELD_POLICY_OBS2_LAYER_COUNT`)
...
**Landed:** `m_jit_field_policy_obs2_multilayer_overlay_score` — default_off, InlineFixedPointMag2Sqrt + Q16.16 mag2 contract, ApproximateDiagnosticF32 score.
...
## Comparison to FIELD_POLICY-OBS-1

| Path | per_dispatch_ms (approx) |
|---|---:|
| FIELD_POLICY-OBS-1 warm single-layer | ~0.129 |
| FIELD_POLICY-OBS-2 warm 4-layer | ~0.238 |
...
cargo test -p simthing-spec --test field_policy_obs0_overlay_score_admission -- --nocapture  # 8 passed
