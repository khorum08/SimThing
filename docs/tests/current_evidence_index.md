...
| **STEAD spatial contract** | `simthing-clausething/tests/stead_spatial_contract_guards.rs` | Active source/docs never reassert withdrawn drift doctrine; budget admission exported; no fixed edge cap; positions honored; MF typed atlas deferral; index has no stale pending rows. |
