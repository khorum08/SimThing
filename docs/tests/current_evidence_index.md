...
| **Editor prep (fail-closed params + JSON report)** | `simthing-mapgenerator/tests/editor_prep.rs` | `--shape-param` fail-closed parsing/validation; deterministic `--report-json` (`mapgenerator.report.v1`) with request, topology/connectivity/degree stats, artifact paths, and constitution flags for Bevy/editor consumption. |
...
| **MAPGENCLI-EDITOR-PREP-0** — fail-closed `--shape-param` validation + deterministic `--report-json` (`mapgenerator.report.v1`) for Bevy/editor consumption | #723 | `8102bacf` / merge `4584c65e` | PROBATION — sample report at `docs/tests/mapgenerator_cli_spiral2_dense_3000_editor_prep.report.json` |
