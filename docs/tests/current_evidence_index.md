...
| **Candidate F exact sqrt** | `simthing-driver/tests/phase_m_jit_sqrt_exact5f_exhaustive_sweep.rs` (+ artifact) | The exact-magnitude chain for decision gates (constitution §0.7). |
...
| **PR10** — generated admit/install + real-adapter GPU compact evidence | #690 | (merged) | CURRENT_EVIDENCE + LIVE GPU GUARDRAIL |
| **PR11** — 1000-star producer scale envelope (parse/lattice; heap-bounded enumeration) | #692 | (merged) | CURRENT_EVIDENCE |
...
| **STEAD-SCALE-1** — removed fixed structural edge cap; budget-based `admit_structural_grid`; typed atlas deferral; current-evidence index | (this change) | (pending) | CURRENT_EVIDENCE |
