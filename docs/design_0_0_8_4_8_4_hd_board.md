...
| HD-9 | `HD-PARK-REDIRECT-0` | **Corrective (Owner-directed 2026-07-15): parked-for-redirection state.** `--park` moves track-scoped TSV rows + live .hd objects into ONE machine block at absolute workplan EOF (receipt, staleness pass on re-park); `--unpark` restores rows/objects, deletes block, flips pointer; virgin `--open` unchanged; open rung PRs block park; closeout refuses while parked (unpark-first) and deletes blocks at close; 0.0.8.6 migrated live. | PROBATION / proof-present / DA-review-pending — `--park` and `--unpark` implemented with transactional rollback fixtures; closeout park refusal and stale-block deletion proven; 0.0.8.6 live rows migrated into its EOF block; receipt `99577dff1835`; branch `coder/hd-park-redirect-0`. | Std |
