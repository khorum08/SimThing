...
| HD-9 | `HD-PARK-REDIRECT-0` | **Corrective (Owner-directed 2026-07-15): parked-for-redirection state.** `--park` moves track-scoped TSV rows + live .hd objects into ONE machine block at absolute workplan EOF (receipt, staleness pass on re-park); `--unpark` restores rows/objects, deletes block, flips pointer; virgin `--open` unchanged; open rung PRs block park; closeout refuses while parked (unpark-first) and deletes blocks at close; 0.0.8.6 migrated live. | HANDOFF DISPATCHED as `handoffs/HD-PARK-REDIRECT-0.hd.md` (dispatched_at 2026-07-15T00:04Z; `HD-RECEIPT: 99577dff1835`; roles: orch=Codex coder=Owner-assigned da=Fable; mode=manual-progression). Awaiting PROBATION relay. | Std |
