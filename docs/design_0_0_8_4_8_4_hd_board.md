...
| HD-6 | `HD-POINTER-LIFECYCLE-GATE-0` | **Pointer lifecycle gate + board freshness.** `gen_orientation.sh --open` REFUSES the flip while the outgoing track doc status is not CLOSED/PARKED; `--force-owner "<directive>"` escape records an `owner_directives.tsv` row. Board sync also fires on push to master (kills the post-merge stale window); workflow-level fixture for board render with `current_handoff: none`. Fixtures: flip-from-OPEN FAILs; flip-from-PARKED/CLOSED pass; force path records the directive. | **DA-GRADUATED / merged [#1344](https://github.com/khorum08/SimThing/pull/1344)** — merged 2026-07-14T19:55Z (dispatched_at 17:40Z; roles: orch=Codex coder=Cursor-cloud da=Fable). DA deep pass on `70f9e892`: all 10 gate fixtures re-run (two-root bypass, invalid-status + duplicate-active-pair planned-bytes preflight, post-write rollback fault injection, force-directive recording, OPEN refusal, PARKED/CLOSED allow, board-sync + current_handoff none); clearance(99) + relay-lint(36) + dispatch battery; 4 remand loops all convergent (cross-root seam, atomic force, planned-bytes, clearance mirror). Known non-gating residue: pre-existing ingress-cap fixture brittleness on master (DA micro-remedial follows). Evidence `docs/tests/hd_pointer_lifecycle_gate_0_results.md`. (Dispatch history: receipt caab38a976d2; implementer PROBATION at 0410c1a3.) **Next:** `HD-CATALOG-LIBRARY-0` | Std |
| HD-7 | `HD-CATALOG-LIBRARY-0` | **Per-role library catalog (the original vision).** `librarian.sh --catalog` v2 enumerates the LIBRARY per role — anchors x trigger domains x role reach, always-on spine, payload sections — from `doctrine_anchors.tsv` + `anchor_triggers.tsv` + orientation, not current-handoff-bound; <=60 lines/role complete-or-fail. Staleness report gains a harness-fixture-count gauge line. | DA-GRADUATED / merged [#1347](https://github.com/khorum08/SimThing/pull/1347) — merged 2026-07-14T22:0xZ (dispatched_at 20:35Z; roles: orch=Codex coder=Owner-assigned-cold-start da=Fable; mode=manual-progression). DA deep pass on `80e7ef6a`: librarian selftest + full battery + clearance(99) on-branch; live proofs reproduced (3 role catalogs distinct at 23 lines each, library-wide 17 anchors x 38 domains, reach-log byte-identical, harness-fixture gauge 650); router debt discharged in graduation commit (librarian.sh added to GATE_WIRING_PATHS, class-harden per machine-split doctrine). Known residue for HD-8/HD-C: push-sync board renders current_handoff none (PR-event path resolves). Evidence `docs/tests/hd_catalog_library_0_results.md`. **Next:** `HD-OWNER-AUTHORING-GUIDE-0` | Std |
| HD-8 | `HD-OWNER-AUTHORING-GUIDE-0` | **Owner authoring guide + role-slot sweep (cap-budgeted).** Human-facing intro: authoring a new workplan (header, ladder cells lead with stamps, no escaped pipes), amendments (ruling 6), open/park/close lifecycle incl. the HD-6 gate, regenerating library + TSVs, browser-stack protocol (forward-by-pointer, orchestrator-authored coder blocks, verify-send) and the three coder surfaces (Cursor cloud; Grok CLI pinned `-m grok-4.5` + `--worktree`; Claude sonnet-class). Role-first heading sweep across onboarding + template (ruling 7). Net-prose accounting in results; any cap raise paired with compression. | DA-GRADUATED / merged [#1349](https://github.com/khorum08/SimThing/pull/1349) — merged 2026-07-14T23:5xZ (dispatched_at 22:35Z; roles: orch=Codex coder=Owner-assigned da=Fable; mode=manual-progression). DA deep pass on `d7fed2a2`: battery green; guide cold-read verified accurate against live HD-6/HD-7 machinery (39/40 cap at birth); zero vendor-conditioned steps; python-is-python3 caveat landed; net +96 adjudicated within track net-negative budget (HD-C cell compression owed). Evidence `docs/tests/hd_owner_authoring_guide_0_results.md`. **Next:** `HD-CLOSEOUT-0` | Std |
| HD-9 | `HD-PARK-REDIRECT-0` | **Corrective (Owner-directed 2026-07-15): parked-for-redirection state.** `--park` moves track-scoped TSV rows + live .hd objects into ONE machine block at absolute workplan EOF (receipt, staleness pass on re-park); `--unpark` restores rows/objects, deletes block, flips pointer; virgin `--open` unchanged; open rung PRs block park; closeout refuses while parked (unpark-first) and deletes blocks at close; 0.0.8.6 migrated live. | DA-GRADUATED / merged [#1351](https://github.com/khorum08/SimThing/pull/1351) — merged 2026-07-15T01:0xZ (dispatched_at 00:04Z; roles: orch=Codex coder=Owner-assigned da=Fable; mode=manual-progression). DA deep pass on `84132bfb`: 25 orientation fixtures (park round-trip byte-exact, hd fold/restore, open-PR blocks, re-park supersede, post-write rollbacks, tampered receipt, virgin parity); closeout --prove PASS incl. refuses-while-parked; live 0.0.8.6 migration verified (block at EOF, STUDIO-OWNER-CLOSURE-0 moved out of live TSV). Residue: active-directives-render selftest coupled to live table content exposed by correct migration — envelope-forbidden in-branch, DA micro-remedial follows. Evidence `docs/tests/hd_park_redirect_0_results.md`. **Next:** `HD-CLOSEOUT-0` (unpark-first invariant now live) | Std |
