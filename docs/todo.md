...
1. **Continued flat-star soak** — default confidence work; no new substrate semantics
2. **Narrow D-2a implementation** — only if a named multi-transaction hard-currency scenario requires sequential cross-band ordering
3. **Full simthing-spec/RON/Designer guardrail rebuild** — when the authoring track intentionally opens
4. **Narrow E-11B-5 nested dynamic enrollment** — only if product names a nested dynamic Resource Flow scenario (**E-11B track paused until then**)
5. **New scenario-driven vertical slice** — product-led, constitutionally bounded
