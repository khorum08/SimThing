...
**Phase M EML Gadget Library:** **EML-GADGET-1 Done (2026-05-19).** Tier-1 stateless gadgets (`FieldSampler`, `WeightedAccumulator`, algebraic `SoftStep`) compile in `simthing-spec` with mandatory CPU-oracle parity; no WGSL/GPU/runtime changes. **EML-GADGET-2** (temporal-memory slice) queued. Design note: [`workshop/eml_gadget_library_design_note.md`](workshop/eml_gadget_library_design_note.md). Test: [`tests/phase_m_eml_gadget_tier1_test_results.md`](tests/phase_m_eml_gadget_tier1_test_results.md).
