...
**Phase M EML Gadget Library:** **NEXT — approved track (Opus 2026-05-29); sequenced BEFORE Authoring Ergonomics R2.** Designer-facing, RON-authored EML gadgets = node-template macros over the existing `EvalEML` opcodes (no new WGSL) in `simthing-spec`. **EML-GADGET-1** (Tier-1 stateless: `FieldSampler`, `WeightedAccumulator`, `SoftStep` algebraic-sigmoid; mandatory CPU-oracle parity; default-off) → **EML-GADGET-2** (Tier-2 temporal-memory slice: snapshot/accumulate-band primitive, `VelocityMonitor`, `Decay`/EMA, acceleration, hysteresis; **bounded-feedback admission guardrail** — self-referential accumulator must declare decay<1 and/or clamp or admission rejects). `simthing-gpu` stays generic; `simthing-sim` map-free; temporal columns Layer-3 scoped (not dense per-cell). Design note: [`workshop/eml_gadget_library_design_note.md`](workshop/eml_gadget_library_design_note.md).

**Phase M Resource Economy Authoring Ergonomics R2:** **Sequenced AFTER the EML Gadget Library** — R2 should expose/leverage the gadget library rather than re-invent composition. Begin only after EML-GADGET-1/2 land.
