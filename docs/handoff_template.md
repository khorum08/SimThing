...
- Track closure, at any rung that declares a definitive closure point: `bash scripts/ci/track_closeout.sh --build-manifest <workplan|--track <id>>` → resolve dispositions → `--check-eval <manifest>` → `--apply <manifest>` (one batched pass: deletes + parks + elevations, closes the birth_track, size-first report). Never hand-delete inventory rows piecemeal — `--deletion-guard` blocks a removed row whose birth_track was not closed by closeout. See `docs/track_closeout_protocol.md`.
