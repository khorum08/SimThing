...
- **Clearance reserve classification** — do **not** open a DA design relay solely because clearance says `DA-RESERVE(unclassified-scope)`. Classify: unadmitted/novel → DA; already-admitted proof-present missing class → **router gap** (class-hardening; cite admitting PR/rung); hygiene → fix/`FAIL`. Policy: `docs/tests/clearance_unclassified_scope_reduction_0_results.md`.
