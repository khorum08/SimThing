...
| 0 | `CC-BASELINE-0` | Freeze the §2 baseline as an artifact; resolve every **`unknown`-class** row to a durable class or mark it a deletion candidate; no deletions yet | **NEXT** - 0 rows remain `unknown`-class; each reclassification cites its retention basis; drift gate PASS |
