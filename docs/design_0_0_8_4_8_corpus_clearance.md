...
| P | `CC-SWEEP-PREFLIGHT-0` (gate-wiring; DA-reviewed; **must land before the first sweep** — §4E) | Two anticipated failures, one preflight, both fixes via existing machinery: **(a) retire the one-shot class** — `CC-BASELINE-0` is graduated, so flip `corpus-baseline` → `status=retired` (router already skips retired classes) and register the *repeatable* **`corpus-sweep`** class scoped to the sweep shape (`crates/<crate>` test-file deletions + `scripts/ci/test_inventory.tsv` + `scripts/ci/test_lifecycle_boundary_rows.tsv` + `docs/tests/cc_sweep_*_results.md`; envelope `0.0.8.4.8-corpus-clearance`; requirements `tested_code_sha\|coverage_basis\|ci_green\|no_engine_src`), so a sweep PR matches exactly one class instead of colliding with `corpus-baseline` on the shared `test_inventory.tsv` glob → `harness-error`. Selftest fixture: a sweep-shaped diff routes to `corpus-sweep`, unambiguously. **(b) make the lifecycle schema gate blocking at merge** — 169 invalid `birth_track` rows accreted because `test_lifecycle_expiry_check.sh --schema` was operator-run only; wire it into `doctrine-scan.yml` as a blocking step **when the diff touches `test_inventory.tsv` or lifecycle TSVs** (necessity-scoped per §4B; this is a PR gate, not a scheduled cadence — §11's cadence fence is untouched). Fixture: an invalid `birth_track` row → schema FAIL blocks the PR | Router selftest proves a sweep-shaped diff → `corpus-sweep` (no harness-error, no multi-match); `corpus-baseline` reads `retired` and no longer matches anything; an invalid-birth_track fixture FAILs the schema gate in CI while a clean inventory diff passes; doctrine_scan green; scanner untouched → selftest not required |
| 1..N | `CC-SWEEP-<crate>` | Per-crate necessity-deletion waves over `pre-lifecycle` rows: for each, name the higher-rung owner (type/admission/scan/integration path) that makes the test redundant, delete it + its inventory row, prove production intact | **NOT STARTED** - crate compiles; remaining gates green; deleted rows leave no drift; **inventory row count decreases**; each deletion cites the superseding boundary |
...
validates it against the live rule stamp. The **only** sanctioned mid-session action is `orient.sh --since=<receipt>`
...
### 4D. The orientation receipt must not stale on prose churn (binding — anti-kabuki)
The receipt proves an agent oriented against the current governance **rules**, not against a byte-identical
copy of a page the DA keeps editing. Today it keys on the rendered digest sha, so a purely cosmetic regen
(a rung-table edit, a ledger tail, a clearance-mandate paragraph) stales every receipt and hard-blocks agents
whose work never touched a rule — the same SHA-match kabuki the Immutability Law (#1169) killed for proofs,
recurring on receipts. `CC-RECEIPT-REBIND-0` (rung R) rebinds freshness to the **rule sources only**
(`precedented_classes.tsv` + `binding_conditions.tsv` + `doctrine_anchors.tsv`). A stale receipt is then a
genuine "the rules moved" signal, resolved by the cheap `--since` delta — never a hard stop over
a prose diff. **Disposition for the in-flight `CC-BASELINE-0`:** its stale receipt was caused solely by this
session's prose regens, not any rule change — it is **not** a substantive block; refresh the receipt once and
proceed (the work — 137 `unknown` rows resolved — stands on its own proofs).

### 4E. Sweep preflight — anticipate the failures, don't discover them (binding)
Two defects were *predicted at the `CC-BASELINE-0` graduation* rather than discovered mid-sweep, and the
sweep does not start until `CC-SWEEP-PREFLIGHT-0` (rung P) forecloses them: (a) **class-glob collision** —
the one-shot `corpus-baseline` class holds `test_inventory.tsv` in scope, so every sweep PR (which also
edits that TSV) would multi-match → `DA-RESERVE(harness-error)`, re-creating the exact ambiguity #1193
fixed one layer down; retiring the completed class and registering the repeatable `corpus-sweep` class is
the whole fix (one status cell + one row — the router already skips `retired`). (b) **schema gate not
blocking** — 169 invalid `birth_track` rows accreted because the lifecycle schema check was operator-run
only; it becomes a blocking, necessity-scoped CI step on inventory-touching diffs, so hygiene debt cannot
re-accrete during the very track that exists to pay hygiene debt down. Rule of the section: when a
