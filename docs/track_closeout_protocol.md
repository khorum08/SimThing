...
| apply | `--apply <manifest>` | one batched mutation: deletes (both TSVs), class stamps, code/doc moves, doc leases, **parks (rows relocated to the pen)**; stamps the `birth_track` **closed**; if `scripts/ci/active_track.txt` still points at the closing source/doc scope, retires it to `docs/design_0_0_8_4_7_orchestration_harness.md` and regenerates orientation; runs the gate battery, including `cargo check -p <crate>` for code move targets; emits a compact, size-first report. |
...
committed result with `gen_orientation.sh --check`; choosing the next non-default mission remains DA/operator work.
