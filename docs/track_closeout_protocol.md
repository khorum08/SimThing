...
| apply | `--apply <manifest>` | one batched mutation: deletes (both TSVs), class stamps, code/doc moves, doc leases, **parks (rows relocated to the pen)**; **reaps the closing track's `discharged` `binding_conditions.tsv` rows** (HC-CLOSEOUT-BINDING-REAP-0; membership = ladder + binding-table rung IDs from the track source — same identity as `--park`; open/parked-track rows stay; active rows stay); stamps the `birth_track` **closed**; if `scripts/ci/active_track.txt` still points at the closing source/doc scope, retires it to `none` and regenerates orientation; runs the gate battery, including `cargo check -p <crate>` for code move targets; emits a compact, size-first report. Binding reaping is HD-6 transactional (preflight/staged/rollback) and is reported on the CLOSEOUT-RECEIPT line as `binding_reaped=<n> rungs=...`. |
...
| guard | `--deletion-guard <base> <head>` | a removed inventory row is lawful only if its `birth_track` was `closed` **at base**, or this PR is itself the closeout (closed at head **and** the diff carries both `docs/tests/<track>_closeout_report.md` and `docs/tests/<track>_closeout_manifest.tsv`), or an exact-identity ledger escape applies → otherwise FAIL. Hand-flipping `status=closed` in the same PR as the deletion is a bypass and is blocked. cfg-marker ledger sweeps exempt. **Rename escape:** `scripts/ci/authorized_renames.tsv` (exact old→new + successor inventory + `fn` presence). **Deletion escape:** `scripts/ci/authorized_deletions.tsv` (exact `crate+file+test_name+kind` + nonempty `authorizing_ruling`/`rung`/`hd_receipt` + inventory absent + source `fn` absent or file deleted). No note-based or glob authorization. |
