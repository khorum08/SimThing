...
- PALMA / min-plus traversal: **production GPU utility seated** (PATH-7) with **GPU-native W input / resident D output** (PATH-8) and **no public `tick()` scaffold** (PATH-8R). Use explicit `dispatch_gpu_resident` + `TraversalFieldGpuInput`; CPU shadow gather/readback only via named diagnostic/compatibility dispatch. PATH-0–5 proof/benchmark rungs; PATH-6 opt-in band scheduling (**default `SimSession` tick not wired**). **No pathfinding engine or movement policy.** Fable handoff: [`tests/palma_path_8r_remove_tick_scaffold_results.md`](tests/palma_path_8r_remove_tick_scaffold_results.md). Guide: [`design_0_0_8_1_palma_pathfinding_integration_guide.md`](design_0_0_8_1_palma_pathfinding_integration_guide.md)
