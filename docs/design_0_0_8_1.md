...
- PALMA / min-plus traversal: **production GPU utility seated** (PATH-7) — `MinPlusTraversalFieldOp` + opt-in `TraversalFieldBandSession`; **GPU-resident D is default**; CPU readback/shadow writeback is diagnostic-only. PATH-0–5 proof/benchmark rungs; PATH-6 opt-in band scheduling (**default `SimSession` tick not wired**). **No pathfinding engine or movement policy.** Fable greenfield handoff: [`tests/palma_path_7_gpu_traversal_utility_results.md`](tests/palma_path_7_gpu_traversal_utility_results.md). Guide: [`design_0_0_8_1_palma_pathfinding_integration_guide.md`](design_0_0_8_1_palma_pathfinding_integration_guide.md)
