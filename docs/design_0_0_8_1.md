...
- PALMA / min-plus traversal: **production GPU utility seated** (PATH-7) with **GPU-native W input / D output handles** (PATH-8) — `MinPlusTraversalFieldOp` + opt-in `TraversalFieldBandSession`; **GPU-resident D is default**; CPU shadow W gather and CPU D readback/shadow writeback are compatibility/diagnostic-only. PATH-0–5 proof/benchmark rungs; PATH-6 opt-in band scheduling (**default `SimSession` tick not wired**). **No pathfinding engine or movement policy.** Fable greenfield handoff: [`tests/palma_path_8_gpu_native_field_graph_results.md`](tests/palma_path_8_gpu_native_field_graph_results.md). Guide: [`design_0_0_8_1_palma_pathfinding_integration_guide.md`](design_0_0_8_1_palma_pathfinding_integration_guide.md)
