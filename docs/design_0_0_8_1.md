...
- PALMA ladder status: PATH-0 through **PATH-5** are proof/benchmark rungs; **PATH-6** adds an **opt-in test-profile** min-plus field band (`PalmaMinPlusFieldBandSession` + `FieldScheduler`) over admitted `w`/`d` columns — **default `SimSession` tick not wired** (see PATH-6 report). **PATH-4S** is measured performance only. **No production movement policy** has landed on this ladder.
