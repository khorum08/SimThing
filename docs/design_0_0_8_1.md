...
- PALMA ladder status: PATH-0 through **PATH-5** are proof/benchmark rungs over **numeric W/D fields** (see guide §15). **PATH-4S** is a measured 180×180 representative benchmark — **not** install/session property-column integration. **PATH-5** proves admitted Location/gridcell **`w`/`d` property columns** → GPU `MinPlusStencilOp` → D writeback and property sampling on the PATH-3R tree; **SimSession RegionField min-plus scheduling is not default-wired** (ledgered in PATH-5 report). **No production movement policy** has landed on this ladder.
