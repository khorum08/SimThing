...
stop_conditions: ["stale-orient-receipt or scope-widening", "any authoring or scenario mutation beyond the seven DA-authorized Unobserved{reason} edits", "zero or conflicting lawful candidates for any of the 18 post-conversion Anchored properties", "derivation requires kind/name/root fallback or a 5.3 semantic change"]
---
## BUILD
...
## EXIT-PROOF
- Canonical TP (topology unmodified) derives exactly one host for each of the 18 Anchored properties: 18 live loci, 18 GPU rows, 7 dark cells, inventory `18/7`, and census `exact=18 / zero=0 / conflict=0` over Anchored properties. The seven `Unobserved{reason}` edits are the only authoring; no other fixture mutation.
