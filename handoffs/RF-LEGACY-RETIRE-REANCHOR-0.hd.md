...
- Before implementation, perform a brief ladder hygiene pass in `docs/design_0_0_8_6_studio_live_ops.md`: audit the **Exit proof** cells for every recently orchestrator-merged rung and correct only stale `TODO` / `PROBATION` / missing-merge entries to the exact PR, merge SHA, and graduation status. At minimum reconcile RF-2A and RF-2 from `#1411`; also fix any other recent orchestrator merge still stale. Do not rewrite scope or tier columns. Regenerate orientation after the corrections.
