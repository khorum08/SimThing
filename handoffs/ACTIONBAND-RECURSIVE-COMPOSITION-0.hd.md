...
owner_approved: true
owner_notes: "DA 5233518871 graduated 7.2 (#1699 @ 6b2792b7; stamp #1702 @ afe6c7db) and cleared 7.3 to author. DA 5233617128 approved 7.3 with bounded A1: 7.3 explicitly owns the depth-2 common fast shape because depth-2 first exists with recursive composition; if depth-2 cannot reuse the depth-1 entry/shared inline descriptors+program IDs without violating generation pacing, STOP. Frozen 7.1/7.2 authority stands. The 7.2 carry baseline is 4096 ns / 32 bytes at cardinality 1; 7.3 owns the cardinality curve and §15.6 remedy only if scaling demonstrates need. `crates/simthing-core/src` is reach, not expected authored scope: consume existing CostBand/state machinery; any core diff must be explicitly justified in the return relay and an unexplained core edit is a review finding."
...
- Recursion stays physically flat/deterministic via admitted indices/spans and next-state bits, never pointer/append/atomic/physical-row semantic order. Depth-2 must preserve that same entry/law rather than mint a parallel executor.
...
## EXIT-PROOF
- Parent→child-next→later child execution→ordinary consequence→child collapse→later parent resolution is proven across generations, with the ordinary depth-2 case using the common fast shape rather than a new execution path.
...
- Report carry cardinality curve and any justified §15.6 remedy; return PROBATION with focused+inherited batteries and hosted artifacts, and explicitly justify any `crates/simthing-core/src` diff. Coding does not `/clearance`, merge, move pointer, or start successors.
