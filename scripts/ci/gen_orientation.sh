...
lines = [
    ...
    "",
    "**Sticky lag reads as PENDING, not a verdict (mechanized).** At `/clearance` run-start the sticky is",
    "overwritten with a head-bound `CLEARANCE-STATUS: PENDING` and NO `CLEARANCE-VERDICT` line until the router",
    "finishes. So a sticky lacking a `CLEARANCE-VERDICT` is *pending observability* -- never a mismatch. Route",
    "only on an emitted `CLEARANCE-VERDICT`; do not SHA-match a stale sticky head.",
    "",
