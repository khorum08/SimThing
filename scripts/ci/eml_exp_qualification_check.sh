...
#   1. the pinned CPU-twin sequence source (constants + eml_exp_pinned_f32)
#      hashes to the qualified pin -- any algorithm edit invalidates;
#   2. the two WGSL shader homes carry byte-identical eml_exp_pinned helpers
...
readonly REPO_ROOT="${EML_EXP_QUAL_ROOT:-$(cd "${SCRIPT_DIR}/../.." && pwd)}"
...
fail() {
  echo "EML-EXP-QUALIFICATION-CHECK: FAIL($1)"
  exit 1
}
...
check() {
  local core_twin="${REPO_ROOT}/crates/simthing-core/src/eml_exp.rs"
  local qual_module="${REPO_ROOT}/crates/simthing-kernel/src/eml_exp_qualification.rs"
  ...
  local results_doc="${REPO_ROOT}/docs/tests/eml_exp_primitive_0_results.md"
  ...
  grep -q "0x7875_a45b_a919_d588" "$qual_module" \
    || fail "qualification-module-digest-missing"
  grep -q "2_237_667_740" "$qual_module" || fail "qualification-module-domain-missing"
  ...
  echo "EML-EXP-QUALIFICATION-CHECK: PASS"
}
...
selftest() {
  ...
  cp "${REPO_ROOT}/crates/simthing-core/src/eml_exp.rs" "$tmp/crates/simthing-core/src/"
  cp "${REPO_ROOT}/crates/simthing-kernel/src/eml_exp_qualification.rs" \
  ...
  cp "${REPO_ROOT}/docs/tests/eml_exp_primitive_0_results.md" "$tmp/docs/tests/"
  cp "${REPO_ROOT}/Cargo.lock" "$tmp/"

  local verdict
  verdict="$(EML_EXP_QUAL_ROOT="$tmp" bash "${BASH_SOURCE[0]}" --check)" \
  ...
  sed -i 's/1\.442_695_04_f32/1.442_695_14_f32/' "$tmp/crates/simthing-core/src/eml_exp.rs"
  if EML_EXP_QUAL_ROOT="$tmp" bash "${BASH_SOURCE[0]}" --check >/dev/null 2>&1; then
  ...
  sed -i 's/0x3FB8AA3Bu/0x3FB8AA3Cu/' "$tmp/crates/simthing-kernel/src/shaders/field_sweep.wgsl"
  if EML_EXP_QUAL_ROOT="$tmp" bash "${BASH_SOURCE[0]}" --check >/dev/null 2>&1; then
  ...
  echo "EML-EXP-QUALIFICATION-SELFTEST: PASS"
}
...
case "${1:---check}" in
  ...
  --selftest) selftest ;;
  *)
    echo "usage: bash scripts/ci/eml_exp_qualification_check.sh [--check|--selftest]"
    exit 2
    ;;
esac
